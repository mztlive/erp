...
上位合同：[组织架构、数据范围与负责人查询执行合同](organization-data-scope-contract.md) v1.4，第 11 章及第 9.2—9.5 节阶段衔接要求。

后续执行约束：普通候选查询按[列表筛选候选独立查询执行合同](query-selector-decoupling-contract.md)分批迁移。本文第 3 章响应表和既有检查记录保留 S1 交付基线，不再要求新实现随列表返回 `owner_options`；独立候选专项尚未实施，不计入本文的已完成状态。
...
2. 人员候选通过独立业务人员查询接口获取，按人员资格与候选资源自身 DataScope 授权；不得从业务记录是否存在反推候选。具体切换按[候选独立查询合同](query-selector-decoupling-contract.md)执行，不调用系统账号管理接口获取负责人全集。
3. 候选以稳定 ID 为值，以姓名、登录账号和停用标记区分同名人员；停用人员在仍符合查询资格及人员目录 DataScope 时保留。业务行的负责人名称独立按业务展示授权返回，不依赖是否出现在人员候选中。
