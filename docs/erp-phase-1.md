...
| **供应商商品库** | 采购通过 Excel 或手工录入得到的供应商 SPU/SKU 全量目录；入库不等于进入公司商品池 |
| **供应商供给** | 一条“公司 SKU ↔ 供应商 SKU”的供货关系，按修订同时保存一件代发供给价、集采供给价、集采起订量、税率、费用、区域和有效期；不设置供给方式字段，所有供给天然支持一件代发与集采 |
| **公司商品池** | 公司 SKU 面向销售的选品条目，保存采购发布给销售的销售可见价和有效状态，不保存供应商成本 |
...
- 采购从供应商 SPU 页面逐个选择供应商 SKU：有同款公司 SKU 时直接关联；没有同款时，
  “加入公司商品池”对话框进入“新建公司商品/SKU”分支，把供应商商品与当前供应商 SKU
  ...
  和市场价后，系统原子创建公司商品/SKU、
  精确映射、供给修订和商品池条目；SPU 只作为页面容器和批量选择范围，不形成正式映射；
...
- 公司商品池只保存公司 SKU、销售可见价、有效期、供货区域、预计交期、允许履约方式和启用状态，不保存“参考供应商”或“参考成本”；
- 销售只从启用且在有效期内的公司商品池中选品并向客户报价；
- 销售查询和导出只返回公司商品池资料和销售可见价，不返回供应商、采购成本、进项税率或 MOQ；
...
| 类型         | 内容                                                                          |
| ------------ | ----------------------------------------------------------------------------- |
| **基础资料**   | 客户、供应商、供应商能力与资质、供应商商品库与供给、公司商品池、商品 SKU、商品分类、品牌、卡券类目、仓库 |
...
客户、供应商、公司商品池、商品 SKU、商品分类、品牌、卡券类目和仓库每次修改均保留完整历史版本；正式单据另外保存生效时的业务快照。
...
1. 客户、供应商、供应商能力与资质、供应商商品库与多供应商供给、合同、公司商品池、商品和卡券类目基础资料；
...
| 供应商商品库与供给 | 供应商 Excel | 采购 | 导入供应商 SPU/SKU 和来源报价；采购确认后才形成成本供给，不自动入公司商品池 |
| 公司商品池、商品 SKU、卡券类目 | Excel | 采购、运营 | 全部启用项目；商品池导入销售可见价，不导入参考供应商或参考成本 |
| 仓库 | Excel | 仓储 | 全部在用仓库 |
...
| **采购** | 公司商品池、供应商能力与资质、采购二次确认、采购单、电子交付、线下服务和代发 |
...
- 采购维护公司商品池、供应商能力与资质，并查看采购履约相关单据；
