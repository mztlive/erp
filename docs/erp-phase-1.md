...
| 类型         | 内容                                                                          |
| ------------ | ----------------------------------------------------------------------------- |
| **主数据**   | 客户、供应商、供应商能力与资质、可销售项目、商品 SKU、卡券类目、仓库          |
...
客户、供应商、可销售项目、商品 SKU、卡券类目和仓库每次修改均保留完整历史版本；正式单据另外保存生效时的业务快照。
...
1. 客户、供应商、供应商能力与资质、合同、可销售项目、商品和卡券类目主数据；
...
10. 卡券销售单的期初基线导入、定时轮询同步、主数据映射、同步版本、映射差异、只读展示和每日全量核对；
11. 统一待办中心与预警规则，覆盖采购二次确认、财务审核、票款复核、资质到期、履约超期和同步失败；
12. 期初数据迁移，含主数据、仓库、期初库存、卡券销售单期初基线和卡券应收派生。
...
- 迁移必须指定统一的上线基准日，主数据状态和期初库存均以该日为准；
...
| **运营** | 卡券类目主数据、卡券销售单的卡券类目映射与差异确认。卡号生产、实体卡印刷、绑定和激活在商城完成，ERP 不建单据 |
| **仓储** | 实物商品入库、仓发和库存调整                                       |
...
- 系统管理员处理同步技术异常和人工补拉，但无权修改卡券销售单商业字段或替代业务部门完成主数据映射；
...
| **页面与导航地图** `erp-page-map.md` | 定义两期共用的菜单、页面编号、角色入口和启用范围 |
| **页面布局与交互规范** `erp-interaction-spec.md` | 定义 ERP 工作台、列表、单据、核销、异常和分析页面的操作方式 |
...
本文和第二期文档是业务事实源。数据模型、商城映射、页面地图和交互规范负责实现这些规则，不得为了兼容旧商城表或简化页面而改变本文的业务边界。
