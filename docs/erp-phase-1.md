...
| **内容指纹** | （可选增强）对销售单商业事实字段计算的摘要，用于判定内容是否变化和防止重复应用；不启用时按更新时间、单号、状态和金额四元组比对 |
...
| **供应商商品库** | 采购通过 Excel 或手工录入得到的供应商 SPU/SKU 全量目录；入库不等于创建公司 SKU 或取得销售资格 |
| **供应商供给** | 一条“公司 SKU ↔ 供应商 SKU”的供货关系，按修订同时保存一件代发供给价、集采供给价、集采起订量、税率、费用、区域和有效期；不设置供给方式字段，所有供给天然支持一件代发与集采 |
...
切换时点 T 起，商城停止创建 B2B 销售单，全部 B2B 销售单统一由 ERP 服务，卡券的 ERP 侧协同能力另行约定，见第二期文档。
...
- 采购先通过供应商 Excel 或手工录入维护供应商商品库；Excel 的每一行和手工录入都形成供应商 SPU/SKU，不因来源不同使用第二套结构；
- 采购从供应商 SPU 页面逐个选择供应商 SKU：有同款公司 SKU 时只建立精确映射和该供应商
  供给；没有同款时，“加入公司商品池”是反向创建公司商品/SKU 的业务名称，对话框进入
  “新建公司商品/SKU”分支，把供应商商品与当前供应商 SKU
  中语义相同的名称、描述、来源商品类型、分类、品牌、单位、规格、条码和已归档图文全部预填为公司资料
  草稿，采购可二次修改。公司 `product_kind` 是决定商品作用的独立必填稳定属性，不得由分类
  派生；来源类型仅作预填，采购必须最终确认，且所选分类必须允许该类型。采购确认销售可见价
  和市场价后，将二者写入新公司 `sku_revision`，系统原子创建公司商品/SKU 及其修订、
  精确映射和供给修订；SPU 只作为页面容器和批量选择范围，不形成正式映射；
- W14 可从固定公司 SKU 正向发起 W21“添加供应商并登记成本”，使用公司 SKU 快照预填并
  创建供应商商品/SKU，再建立精确映射与供给；该正向链路属于一期，不得在后续来源变化时
  自动覆盖公司主档，也不得创建或修改公司 SKU 的销售价格；
- 一个公司 SKU 可以关联多个供应商 SKU，每家供应商分别形成供给修订，同时保存一件代发供给价（含税运）、集采供给价（含税）、集采起订量、税率、费用、区域、预计交期和有效期；供给不设置方式字段，默认同时支持一件代发与集采；
- 公司商品池仅是公司商品/SKU 的销售查询视图：销售可见价和市场价只写公司 `sku_revision`；
  启用 SKU 具有销售可见价且存在至少一条业务时点有效的 `supplier_offering_revision` 时，才在视图中具备销售资格。供应商、采购成本、进项税率、MOQ、供给区域和有效期仍只属于供给修订；
...
- 采购二次确认从有效供应商供给中选择实际供给，记录处理人、处理时间、供应商供给修订、最新成本、供货数量、预计交期、履约方式和处理结果；
...
| 类型         | 内容                                                                          |
| ------------ | ----------------------------------------------------------------------------- |
| **基础资料**   | 客户、供应商、供应商能力与资质、供应商商品库与供给、公司商品/SKU（含销售查询视图）、商品分类、品牌、卡券类目、仓库 |
...
- 第一期"ERP 不可编辑卡券销售单"是**主责系统为商城时的状态约束**，不是该单据的固有属性。切换时点 T 起商城停止创建 B2B 销售单，全部 B2B 销售单统一由 ERP 服务，同一条记录不迁移数据、不换单号、不换实体。

这一条是切换时点 T 后全部 B2B 销售单统一由 ERP 服务的前提。第一期若建成两套模型，T 后的统一服务会退化为一次带结构转换的数据搬迁。
...
1. 客户、供应商、供应商能力与资质、供应商商品库与多供应商供给、合同、公司商品/SKU（含销售查询视图）和卡券类目基础资料；
...
| ERP 销售单向商城下发卡券类目变更 | 不建设，切换时点 T 后全部 B2B 销售单统一由 ERP 服务时建设 |
| 商城自动发卡和消费同步         | 不建设，第一期只同步销售单             |
| 玩法规则纳入 ERP               | 不建设，限额限购等规则始终由商城自管    |
| 商城收付款数据同步             | 不建设，商城不持有该数据，由财务在 ERP 登记 |
| 对外消息与回调治理             | 不建设，第一期不做对外投递与回调接收；外部投递、webhook、inbox、供应商 API、通用接口错误中心与对账均在第二期建设 |
...
| 供应商商品库与供给 | 供应商 Excel | 采购 | 导入供应商 SPU/SKU 和来源报价；采购确认后才形成成本供给，不自动创建公司 SKU 或取得销售资格 |
...
- **四元组比对义务**：全量清单必须返回商城更新时间、单号、状态和金额合计，ERP 按这四项与当前数据逐单比对，发现差异后按单号补拉完整快照或转人工处理。内容指纹作为可选增强：启用时由 ERP 提供算法并与商城约定一致，用于发现"金额不变但内容变了"的情况，例如更换卡券类目、调整履约期限、面额与数量对调；不启用时按四元组比对，此类变化依赖增量同步的更新时间刷新兜底，差异由每日核对按单号补拉覆盖。
...
- ERP 按第 8.2 节列出的快照字段对比业务内容判断变化；内容指纹为可选增强，启用时只覆盖同一组字段；
- 获取到的快照业务内容与当前数据一致时，判定为无变化，只更新最近同步时间，不产生新版本；
- 不一致时，ERP 追加一个自增的同步版本，保留全部历史版本，并记录本次变化的字段；
- 幂等依据是"来源商城 + 来源销售单号 + 来源更新时间"，重复获取同一状态不产生重复版本；来源更新时间不同视为新的观测，同一来源单保留并追加新的同步版本；启用内容指纹时叠加作为防重依据；
- 迟到和乱序按商城更新时间判定，获取到的商城更新时间早于当前已记录的商城更新时间时丢弃该快照；
- 商城作废和关闭通过快照中的状态字段体现，不需要独立事件类型。

商城不提供来源版本号，因此第一期不建设来源版本缺口检测和补发协议。ERP 保存的是轮询实际观察到的完整快照版本：该机制能够保证最终当前态收敛，但不能还原两个轮询周期之间在商城连续发生、又被后续修改覆盖的中间状态。第二期保留这些已观察版本，不把它们表述为商城全部变更历史。
...
- **期初基线完成后，水位初值取基线拉取的开始时间，不取结束时间**。基线导入期间商城仍在正常变更，取结束时间会丢掉这段时间内的变化；取开始时间产生的重复拉取按"来源商城 + 来源销售单号 + 来源更新时间"幂等消化。
...
- 每日获取全量清单，按商城更新时间、单号、状态和金额合计逐单核对商城当前商业事实与 ERP 当前数据，差异按单号补拉或转人工处理；启用内容指纹时叠加比对业务内容，理由见第 8.2 节。
...
`commercial_status`）；采购确认、低毛利确认等中间环节属 `review_status`
（页面显示为"待二次确认"等审核中状态），"履约中 / 已关闭"是页面综合展示状态，
由履约进度与 `close_status` 推导，不占用主状态枚举，避免商城来源状态、履约和
票款互相覆盖。与 `erp-data-model.md` §7 口径一致，禁止在界面或服务端把复合态
写回主状态。

卡券销售单的履约期限一般为两年，因此这类销售单会长期处于"履约中"展示态，
属于正常状态，不作为积压处理。
...
| **采购** | 公司商品/SKU 销售视图、供应商能力与资质、采购二次确认、采购单、电子交付、线下服务和代发 |
...
| 同步 | 商城须承担更新时间刷新义务和全量清单四元组比对义务（内容指纹为可选增强） | 8.2 |
...
第二期不得隐式改变本文已经确认的第一期商业单据和履约事实。本次两期文档已共同明确卡券销售单恰好只有一条卡券明细；第二期另行显式约定切换时点 T（商城停止创建 B2B 销售单、全部 B2B 销售单统一由 ERP 服务）、稳定卡实例与关键事实回流、卡券实际成本和经营盈亏口径，以及供应商结算单对第一期应付和进项发票核销对象的扩展。
