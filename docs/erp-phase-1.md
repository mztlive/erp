...
- 第一期统一使用 `outbox_message` 记录正式事实提交后的**内部可靠事件**，仅供工作台、预警和经营分析等可重建投影消费；它不调用商城、供应商或其他外部系统；
...
13. `outbox_message` 内部可靠事件及其工作台、预警、经营分析投影消费；正式事实与事件同事务提交，事件不离开 ERP。
...
| 对外消息与回调治理             | 不建设，第一期 outbox 仅供 ERP 内部投影消费；外部投递、webhook、inbox、供应商 API、通用接口错误中心与对账均在第二期建设 |
...
- 幂等依据是"来源商城 + 来源销售单号 + 来源更新时间 + 内容指纹"，重复获取同一状态不产生重复版本；来源更新时间不同视为新的观测，同一来源单保留并追加新的同步版本；
...
- **期初基线完成后，水位初值取基线拉取的开始时间，不取结束时间**。基线导入期间商城仍在正常变更，取结束时间会丢掉这段时间内的变化；取开始时间产生的重复拉取按"来源商城 + 来源销售单号 + 来源更新时间 + 内容指纹"幂等消化。
...
| 同步 | 同步版本由 ERP 生成，幂等依据是来源商城加来源销售单号加来源更新时间加内容指纹，迟到快照按商城更新时间丢弃 | 8.3 |
