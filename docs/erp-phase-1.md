...
- 采购以供应商 SPU 为上下文 **入池**：系统先给出各供应商 SKU 的池内状态与匹配候选
  （条码/名称/规格/单位等证据，不自动合并）。有同款时 **关联入池**（精确映射 + 该供应商双价供给，
  不改公司销售价）；无同款时 **反向入池**（同构新建公司商品/SKU，预填可改，必填 `product_kind`、
  销售可见价与市场价，原子创建公司主档、映射与供给）。第二家供应商同款必须关联。SPU 只作页面容器，
  正式映射键始终是 `supplier_catalog_sku_id → sku_id`。入池确认即生效、不采集用户侧供给生效日；
  起订量取自目录 SKU；进项税率为整数百分比 UI。详见 `docs/ui-workspaces/w21-supplier-catalog.md`；
