...
- 付款工作台必须展示收款户名、开户行和脱敏账号。完整账号只能由当前任务责任人在具备敏感信息揭示权限时主动查看并记录审计；提交命令必须携带页面所见收款账户 ID 与版本，Service 必须在付款事务内执行 CAS 写锁，账户身份、版本或有效状态变化时必须阻断并要求重新核对；
- 供应商进项发票支持分多次登记；
- 付款和进项发票分别记录和核销；
- 每个付款任务固定绑定一笔采购应付，一张供应商付款单只能核销该任务绑定应付中的分录；一笔采购应付允许由多张付款单分次核销；
