...
| **实物与服务销售单** | 不经过商城消费，直接交付客户 | 慰问品、年节礼包、举办活动 | 销售生效后由采购按商品类型选择入仓、供应商直发、电子交付或服务履约 |
...
- 商品稳定主表的业务类型决定采购类型及允许路线：实物允许入仓或供应商直发，虚拟商品只允许电子交付，线下服务只允许服务履约；实际路线由采购建单时选择；
...
各履约路线**共用相同的前段和后段**，只有中间的履约段不同。因此第 7.3.1 节给出完整基准流程，第 7.3.2 和 7.3.3 节只画差异段。
...
- **选源与履约路线在本环节完成**：采购单创建时，采购从业务时点有效且当前可供的供应商供给中，为每条需要外部采购的销售明细选择精确供给、履约责任、供货数量和预计交付日，并记录供应商供给修订与最新成本。销售单审批期间不得选择供应商或履约路线，不得记录成本和采购预计交付日（第 4.4 节）；
- 销售明细的履约期限是对客户承诺的最晚日期；采购预计交付日必须由采购确认，且不得晚于该销售承诺期限；
- 同一销售明细允许按不同供应商或不同履约责任拆分数量；一次创建命令中该明细的拆分数量合计不得超过事务内最新剩余待采购数量，同一供应商供给跨销售明细或履约责任的数量合计不得超过最新可供量；
