...
| **采购二次确认**     | 实物与服务销售单审批链中的一个**审批节点**，审批人是采购。采购在该节点核实是否能履约（供应商可供、成本可接受、交期可满足），只作出通过或驳回决定。它不是独立单据、不是独立状态、不产生独立处理记录；具体使用现有库存还是外部采购，以及供应商、成本、数量、交期和履约路线，统一在销售单生效后的供给分配环节确认（第 7.4 节）。节点位置和审批人由流程管理员按 `approval-workflow-contract.md` §4.4.3 配置 |
| **履约期限**         | 公司对客户承诺完成交付或服务的最晚时间。**卡券销售单记在表头**，即卡券有效期，全单一个值；**实物与服务销售单记在明细**，各条可不同                                                                                                                                                                                                                                                                      |
| **配赠**             | 卡券面值超出成交金额的部分。配赠金额 = 面值小计 − 成交金额；同一卡券销售单只有一套配赠条件，由唯一明细的成交单价或配赠率表达                                                                                                                                                                                                                                                                            |
| **预占**             | 把指定仓库和 SKU 的库存数量锁定给唯一销售明细的状态。预占可以在供给分配确认时直接从现有库存建立，也可以在采购合格入库后沿采购销售分配建立；其他销售单不得占用                                                                                                                                                                                                                                           |
...
| **企业微信** | 组织、沟通和行政协作                                                                                       | 不承担 ERP 单据流转、采购审批或付款执行；ERP 上线后业务事实以 ERP 台账为准 |
| **ERP**      | 客户、合同、实物与服务销售单、采购、库存、票款、成本费用、实际经营盈亏和客户经营质量                       | 第一期核心建设对象；含卡券销售单的应收、回款和发票                         |
| **Excel**    | 数据导入、导出和临时分析                                                                                   | 不再作为正式业务台账                                                       |

商城不持有应收、回款、付款和发票数据。ERP 上线后，采购单最终审批提供付款授权，出纳在指定付款任务中登记实际付款；回款、付款和发票事实统一进入 ERP 台账。卡券销售单的应收由 ERP 按同步金额派生，回款单和销项发票由财务在 ERP 登记，与实物与服务销售单使用同一套票款能力。
...
| **实物与服务销售单** | 不经过商城消费，直接交付客户   | 慰问品、年节礼包、举办活动 | 销售生效后进入供给分配；实物可由现有库存或外部采购满足，虚拟商品和服务由外部采购满足 |
...
两个维度的区别必须清楚：**业务性质说的是卖什么以及允许进入哪类执行链路，主责系统说的是谁能改。** 具体供应商和履约路线不属于销售业务性质，由采购环节确认。切换时点 T 起商城停止创建 B2B 销售单，全部 B2B 销售单统一由 ERP 服务，卡券销售单仍叫卡券销售单。每张销售单必须记录当前主责系统。
...
- 实物与服务销售单的每条明细独立记录福利场景、销售标的、服务区域和履约期限；销售单及销售明细均不得记录供应商、采购类型、履约责任或执行路线；
- 卡券销售单的履约期限必须记在表头，全单卡券共享，明细不得单独设置（规则见第 4.3.1 节）；
- 福利场景包括年节礼包、餐补、慰问品、消费金和其他；
- 商品稳定主表的业务类型决定允许的供给与履约路线：实物允许现有库存仓发、采购入仓或供应商直发，虚拟商品只允许电子交付，线下服务只允许服务履约；实际路线由销售单生效后的供给分配环节确认；
...
履约期限表示公司对客户承诺完成交付或服务的最晚时间，不表示采购采用的具体履约路线。**两类销售单的记录粒度不同**：
...
- 销售只从上述具备销售资格的公司 SKU 中选品并向客户报价；选品册、客户公开选品及销售方案按 `docs/sales-selection.md` 执行。该能力上线后，Excel 导出仅作销售内部底稿，对客确认结果以客户提交的销售方案为准；
- 销售查询和导出只返回具备销售资格的公司商品/SKU 与销售可见价，不返回供应商、采购成本、进项税率或 MOQ；
- 客户确认意向后，销售在 ERP 创建销售单并**提交**，提交即启动该类型已发布审批定义的审批实例，销售单进入审批中，尚未生效；
- 采购确认可以是该审批链中的普通节点，由流程管理员决定是否配置。若配置了采购节点，采购在该节点核实需要外部采购的销售明细能否履约，并作出通过或驳回决定；未配置时销售单按已发布节点全部通过后生效；
- 采购在该节点只提交决定和原因，**不选择供给、不录入成本、不录入交期**；这些事实在销售单生效后的供给分配环节确认，见第 7.4 节；
- **最后一个审批节点通过后销售单生效**，系统据此生成供给分配任务；
- **任一节点驳回时销售单不生效**，审批人必须填写驳回原因，常见原因包括无法履约、成本上涨、交期不满足和供应商资质失效。

销售单生效闸门是已发布审批链的全部节点通过，不再强制包含采购确认用途节点。流程管理员按普通节点配置销售单审批，可增删、改名、排序和指定审批人。`services::approval` 发布时不再要求 `SALES_ORDER_PROCUREMENT_CONFIRMATION`；运行推进、决定 DTO、驳回和最终生效不得按用途增加专用分支。
...
| 照原条件承接 | 不撤回也不改单，在新一轮审批中说明理由，由各节点重新决定。**不需要销售上级确认** |
| 不做         | 作废销售单                                                                       |
...
- 毛利估算不改变单据状态、不参与审批决定、不创建待办；
- 供给分配确认后，采购缺口的实际成本以采购单事实为准，现有库存直配的实际成本以库存成本事实为准；缺少相应成本事实时不得把估算值展示为实际结论。
...
选品报价环节在 `docs/sales-selection.md` 规定的 P0 验收上线后由选品册承接；上线前仍按现有线下方式执行。销售方案只保存客户确认结果，不产生审批、应收、履约或库存预占。P0 不提供从方案自动创建销售单，后续销售开单仍按本章执行。本章对应按份采购方案；商城兑换方案不按 SKU 拆成实物与服务销售单。
...
各履约路线共用销售审批生效前段，以及履约完成后的客户验收、应收结清和销售关闭后段。销售单生效后必须先进入第 7.4 节的供给分配，再按每条销售明细的确认结果进入现有库存仓发、采购入仓、供应商直发、电子交付或服务履约。

现有库存仓发必须执行以下替换规则：供给分配确认后直接建立库存预占和仓发草稿，跳过采购单创建、采购审批、供应商付款和采购入库；同一销售明细只有剩余缺口进入采购分支。第 7.3.1 节展示外部采购入仓的完整基准流程，第 7.3.2 和 7.3.3 节只画外部采购履约段的差异。

三者共用的规则：

- 客户回款在销售单生效后的任一环节登记；销项发票须先提交开票申请并审批通过，再由财务登记；
- 全部明细履约完成且客户应收结清后销售单关闭。

#### 7.3.1 外部采购入仓后由公司仓库发货（完整基准流程）
...
- **供给分配是唯一选源环节**：销售单生效后生成供给分配任务。销售单和销售审批不得保存或决定供应商、采购类型、履约责任、执行路线、采购成本或采购预计交付日；
- 系统查询供给依据时必须自动生成推荐结果，但查询不得写库存、采购单或履约单。实物商品先按现有库存可覆盖量降序、仓库名称稳定排序分配库存，再对剩余缺口推荐采购；虚拟商品和线下服务不得分配现有库存，直接推荐采购；
- 采购推荐按可全量覆盖优先、含税成本低优先、预计交付日早优先、最大可供量大优先、供应商名称稳定排序。同一供给不足时允许保留未分配缺口，不得虚增库存或供应商可供量；
- 自动推荐必须允许操作人调整、取消或拆分。只有操作人确认才写入正式结果；页面加载、刷新和重新自动分配均不得产生库存预占或采购单；
- 前端一次确认最多调用一个后端供给分配方法。后端 Service 必须在一个事务内完成任务与权限复验、销售供给 guard 推进、库存余额原子预占、库存预占及流水、仓发草稿、采购缺口建单与提交审批、统一覆盖重算、任务同步和幂等收据；任一步失败时全部回滚；
- **现有库存直配**仅适用于实物商品。确认后必须原子执行 `available_quantity -= quantity`、`reserved_quantity += quantity`，建立唯一归属销售明细的库存预占及建立流水，并按销售单和仓库创建或补充仓发草稿；不得创建采购单，不得伪造供应商、采购引用、采购价格或进项税率；
- **采购缺口**必须从业务时点有效且当前可供的供应商供给中选择精确供给、履约责任、供货数量和预计交付日，并记录供应商供给修订与最新成本。采购单创建成功即提交审批，不得留下未提交草稿；
- 同一销售明细允许同时由一个或多个仓库的现有库存和一个或多个采购方案拆分满足。一次确认中该明细的库存与采购数量合计不得超过事务内最新剩余供给缺口；库存余额和供应商供给各自的分配合计不得超过事务内最新可供量；
- 统一供给覆盖量等于有效采购覆盖量加现有库存预占的当前预占量和已消耗量。采购入库形成的库存预占不得再次计入供给覆盖，已释放的现有库存预占不再计入；
- 现有库存足以覆盖全部任务范围时，允许本次创建零张采购单；事务必须完成库存预占、仓发草稿和供给分配任务。存在采购缺口时，只为确认的采购数量创建采购单；未分配缺口继续保留开放任务；
- 服务端必须在事务内复验库存与供应商可供量，并以原子条件写拒绝并发超分配。相同幂等键和相同载荷必须返回原库存预占及采购单结果；相同幂等键载荷不同必须拒绝；
- 库存成本属于库存核算事实，不得伪装成采购成本。供给分配预览中的采购含税估算只汇总推荐采购缺口，不把现有库存按零采购价计入采购金额；
- 销售明细的履约期限是对客户承诺的最晚日期；采购预计交付日必须由采购确认，且不得晚于该销售承诺期限；
- 客户端必须提交服务端下发的精确创建依据 `basis_id`；服务端不得仅凭供应商 ID 猜测采购类型或履约责任；
- 历史销售工作副本、提交、版本或改单文档中的 `fulfillment_mode` 为废弃冗余字段：运行时不得读取或据此决策，新写入不得再产生该字段；存量字段可离线清理，但不得阻断历史单据读取；
- 销售单生效前不存在已选定的供应商成本，因此生效前的毛利只能是第 4.4.1 节的估算值；采购单创建后以采购单事实为准；
- 选源结果与业务时点有效供给不一致（供给失效、停止可供或资质失效）时，采购单不得创建，按第 6.3 节走销售变更单；
- 一张销售单对应一张或多张采购单；
...
卡券票款复核功能已退役，当前执行约束见 [卡券票款复核退役约束](card-funds-review-retirement.md)。本文其他章节涉及逐单票款复核、复核待办或按已复核数据统计的旧规则停止执行。

- ERP 卡券销售单的创建、审批、生效、变更及已有协同能力必须保留。
- 销售生效后形成应收；回款、销项发票和核销继续使用统一客户票款能力。
- 新建应收不得生成期初复核任务；销售变更不得以旧复核状态作为生效前置条件。
- 不得删除或将历史未核实票款批量标记为已复核；历史数据处理与未来商城对接必须另行制定规则。
...
- 销售单生效只形成应收和可申请开票额度；申请人提交开票申请，按 `SalesInvoiceRequest` 已发布定义审批，末节点通过后生成财务开票任务；
- 一张开票申请固定关联一张销售单的一笔应收，允许分次申请和在批准额度内分次登记；实际登记发票后才计入已开票金额；
- 回款和销项发票分别记录和核销；
- 同一客户的一张回款单允许核销多张销售单应收，一张销售单应收允许由多张回款单核销，即同一往来主体内的多对多核销；
- 一次销项开票只允许分配至当前批准申请绑定的应收；不允许跨销售单合并消耗申请额度；
...
- 采购单最终审批通过后形成应付项目；
- 一张采购单支持分多次付款；
- 采购单最终审批通过即构成付款授权并形成指定到人的付款执行任务；出纳必须在该任务中核对供应商当前默认收款账户、登记银行回单和付款金额，确认后由服务端同一事务直接形成已过账供应商付款单、完成核销并更新任务进度，不再启动 `SupplierPayment` 审批；
- 付款工作台必须展示收款户名、开户行和脱敏账号。完整账号只能由当前任务责任人在具备敏感信息揭示权限时主动查看并记录审计；提交命令必须携带页面所见收款账户 ID 与版本，Service 必须在付款事务内执行 CAS 写锁，账户身份、版本或有效状态变化时必须阻断并要求重新核对；合并付款共用同一张回单和同一个收款账户版本；
- 供应商进项发票支持分多次登记；
- 付款和进项发票分别记录和核销；
- 每个付款任务固定绑定一笔采购应付；出纳可将同一供应商、同一当前收款账户下的多条开放付款任务合并为一次打款；一张供应商付款单允许核销这些已勾选任务对应应付中的分录；一笔采购应付允许由多张付款单分次核销；
- 合并付款必须由出纳在工作台明确发起，当前任务必须保留，允许在弹窗中取消部分任务；未勾选的任务保持开放，继续按单任务付款处理；
...
`APPROVED` 三值（页面显示为"未提交 / 审批中 / 已通过"），**禁止**再保存逐环节状态；当前处于哪个审批节点、由谁负责只取审批实例投影。"履约中 / 已关闭"是页面综合展示状态，
...
**映射说明**：本表是跨单据的抽象通用词，具体单据状态以 `erp-data-model.md` 为准。资金/库存类单据（回款、付款、库存调整等）的"已生效"对应数据模型 `POSTED`（界面表述为"已确认入账/已确认"），"已作废"对应 `REVERSED`（界面表述为"已冲正"）；销售单主状态"已生效/已作废"对应 `EFFECTIVE / VOIDED`。界面文案不使用"过账"表述（G5 决议）。
...
| **仓储**       | 实物商品入库、仓发和库存调整                                                                                                         |
| **财务**       | 财务总监审核采购单；出纳按付款任务办理供应商付款；财务登记客户回款、发票与核销，并负责卡券销售单票款逐单复核、成本费用和实际经营盈亏 |
...
组织成员、部门及团队管理范围、按负责人筛选、历史经营归属和分阶段实施执行 [组织架构、数据范围与负责人查询执行合同](organization-data-scope-contract.md)。该合同按阶段接入现有 RBAC 与 DataScope；组织管理可见权不授予审批、付款、开票或代办权限。
...
| 供给 | 销售单生效后进入供给分配；系统自动优先推荐现有库存，剩余缺口再推荐采购，操作人确认后由一个后端事务统一落地                                                 | 7.4                                                       |
| 供给 | 同一销售明细允许拆分为现有库存和采购；纯库存覆盖时不得创建采购单，必须建立库存预占和仓发草稿                                                               | 7.4                                                       |
| 采购 | 采购缺口按供应商、采购类型、付款条件和履约责任拆分采购单，并在供给分配确认时记录供给、成本、数量和交期                                                     | 7.4                                                       |
| 采购 | 销售单展示毛利估算值，采购单创建后以采购事实计算实际毛利                                                                                                   | 4.4.1                                                     |
...
| 票款 | 回款和进项发票支持同一往来主体内的多对多核销；销项开票按批准申请绑定单笔应收执行                                                                                                 | 9.1、9.2                                                  |
