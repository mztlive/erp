...
- 采购从供应商 SPU 页面逐个选择供应商 SKU，并分别关联已有公司 SKU。公司 SKU 不存在时，
  使用 W21「前往商品资料」进入 W14，再通过 `/master-data/products/new` 独立创建公司商品
  与 SKU；当前不使用供应商来源预填，也不自动返回或建立映射。取得稳定 `sku_id` 后，采购
  回到 W21 继续精确映射与入池；SPU 只作为页面容器和批量选择范围，不形成正式映射；
- W14 可从固定公司 SKU 反向发起 W21“添加供应商并登记成本”，使用公司 SKU 快照预填并
  创建供应商商品/SKU，再建立精确映射与供给；该反向链路属于一期，不得在后续来源变化时
  自动覆盖公司主档；
