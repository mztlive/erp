...
| 编号 | 必须交付 | 当前状态 |
| --- | --- | --- |
| S2-01 | M01 客户：统一范围、主责及协作、组织筛选、列表与详情、候选及导出 | 功能已接入、架构不符合合同：列表、详情、全量授权、导出、写命令与归属变更消费 v2；主责与协作分开；组织筛选按当前主负责人所属组织；跨页与 CSV 携带 scope_version；HTTP 输出 DATA_SCOPE_CHANGED，资料创建／修订／删除在原事务重验，常规列表拒绝 AllAuthorized，目录映射 as_of，no_scope 表示无有效范围规则。客户域仍直接依赖身份域，须完成 S2-12，不得登记本项整体完成 |
...
| S2-07 | 跨页查询与完整导出的版本一致性、最后一页之后及下载前的撤权重验 | 部分接入：销售 CSV、客户目录 CSV 已携带范围版本；客户完整结果收集后、生成 CSV 前再次查询校验。合同及采购未接入；真实撤权与导出验收未执行，客户接入仍受 S2-12 约束 |
...
| S2-12 | 客户消费方 Port、组合层 adapter、窄组织事实、公共单对象判定及条件等价验证 | 待整改：`erp-customer` 直接依赖 `erp-identity` 并构造 DataScopeService；必须按上位合同第 9.4 节形成 CustomerDataScopePort，移除跨域依赖及身份内部类型，保留客户归属规则；A33、A34、A36 未核销 |
...
| 客户 | `erp-customer/src/service/customer/{access,scope}.rs`；`repository/scope.rs` | CustomerDataScopePort 及生产 adapter 待实施；当前为跨域直连 | S2-12；同事务和时点的窄组织事实；列表／详情／候选／导出／命令共同验证 |
...
| 合同／采购 | v2 对象范围消费未完成；既有归属或责任字段不构成接入 | 相应范围 Port／adapter 未完成登记 | S2-02、S2-04、S2-13；接入前拒绝将初始化目录作为授权生效依据 |
...
1. 先完成 S2-12 的消费方 Port 与 adapter 合同，并核对 S2-13 的接入登记；客户必须形成符合主合同的完整样例，后续领域不得复制当前跨域直连接法。
2. 客户整改必须保留已实现的主责／协作口径、组织条件、版本与 CSV 行为；新接法通过功能和架构检查后，方可核销 S2-01、S2-12 的对应缺口。
...
1. 客户交付参考提交为 `515a4ff1`、`1ba83e8b`；截至 `3988ae56`，代码已包含第 4 章记录的客户功能，跨业务域直接依赖仍存在，S2-12 保持待整改。
