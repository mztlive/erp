...
| S2-08 | 组织页面、影响预览、范围配置页面、URL 与 Query 缓存同步及窄屏交互 | 功能已接入：组织树、成员调岗、管理授权、变更前后影响预览、提交重验 `expected_version`、范围配置按资源/动作、URL 与 Query 缓存同步及 390px 窄屏布局。组织配置权不授予业务执行权，不自动改派任务。浏览器与业务验收未核销，不得登记本项整体完成 |
