...
| 编号 | 必须交付 | 当前状态 |
| --- | --- | --- |
| S2-01 | M01 客户：统一范围、主责及协作、组织筛选、列表与详情、候选及导出 | 列表、详情、全量授权、导出、写命令与归属变更已接入 DataScope v2；主责与协作分别解释；org_unit_ids 按当前主负责人所属组织筛选；跨页与 CSV 携带 scope_version。评审修复：HTTP 输出 DATA_SCOPE_CHANGED；资料创建/修订与删除在原事务内重验；常规列表拒绝 AllAuthorized；目录映射 as_of；no_scope 仅表示无有效范围规则 |
