...
| 编号 | 必须交付 | 当前状态 |
| --- | --- | --- |
| S2-01 | M01 客户：统一范围、主责及协作、组织筛选、列表与详情、候选及导出 | 未接入新解析器 |
| S2-02 | M02 合同：当前客户归属及合法单据参与、组织筛选、附件、候选与导出 | 未接入新解析器 |
| S2-03 | M04 销售：业务组织范围、列表与详情、打印、候选、导出和写命令范围 | 已采集业务组织与快照；范围消费未完成 |
...
| S2-06 | 统一 `scope_summary`、`as_of`、权限、组织及业务归属版本；跨页变化错误 | 未接入四类资源响应 |
| S2-07 | 跨页查询与完整导出的版本一致性、最后一页之后及下载前的撤权重验 | 未接入 |
