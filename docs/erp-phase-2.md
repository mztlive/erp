...
| **商城停单与 ERP 全面服务** | T 起商城停止创建 B2B 销售单，全部 B2B 销售单统一由 ERP 服务（创建、审批、销售变更）。不存在任何存量单迁移动作：无迁移操作、无批次、无审计动作、无幂等键、无续跑、无回退。数据仅通过商城同步进入 ERP |
...
| **消费回流启用日** | P0、P1 同批上线后 ERP 开始实时接收商城关键业务事实的系统级时间点，记为 `T`。以支付成功事实的发生时间划分履约归属：`T` 之前支付成功的订单沿用原人工履约链，历史回填只记账、不触发供应商下单；`T` 及之后支付成功的订单进入 ERP 自动履约链。T 起商城不再开设新的 B2B 销售单，不存在任何迁移动作，数据仅通过商城同步进入 ERP |
...
| --- | --- | --- | --- |
| 卡券销售单 | 商城开单并管理，ERP 定时同步数据 | 卡券与实物服务销售入口分离，ERP 无法承接客户变更 | 商城停止建单，全部 B2B 销售单统一由 ERP 服务；草稿统一作废 |
| 卡券销售单审批 | 商城内部完成生效处理，ERP 不参与 | 商城停止建单后缺少 ERP 侧线上销售单提交、生效和责任衔接 | 销售提交后依次由销售领导和运营审批，运营审批通过后销售单生效并自动同步商城 |
...
| 接口运维 | 已有一期 `outbox_message` 内部可靠事件和投影消费；商城卡券销售单仅由 ERP 单向轮询读取，无对外发送或回调接收能力 | 缺少外部 dispatcher、inbox、业务接口任务、重试和通用差异中心 | 复用 outbox 建设外部投递，新增 inbox、接口任务、错误中心和对账任务 |
...
第二期上线后，全部 B2B 销售单统一由 ERP 服务。T 起商城停止创建 B2B 销售单，商城草稿统一作废，不进入 ERP；不存在存量单迁移动作。T 后不再由商城发起商业变更，正式销售单商业变更一律走 ERP 销售变更单。
...
| 项目 | 切换前（第一期） | 切换后（第二期） |
| --- | --- | --- |
| 建单入口 | 商城开单，ERP 定时同步 | 商城停单，全部销售单由 ERP 建单 |
...
- 切换完成后停止第一期五分钟轮询并封存为只读历史，启用 P0、P1 联合发布的完整链路，把开放时点记为 `T`，再开放 ERP 卡券销售单新建和商城员工业务；
...
供应商订单状态由三条正交轨道组成，互不折算。履约轨表达供应商履约进度；取消轨和退款轨分别表达取消与退款进度，不得折入履约轨。

**履约轨 `fulfillment_status`：**
...
三轨互不折算：取消、退款结果不得写入 `fulfillment_status`；履约完成与否不影响取消、退款轨取值。同一条订单可同时处于「履约已完成、部分退款」或「履约中、取消处理中」等组合，页面可把三轨映射为综合展示状态。

同一轨内状态只允许向前迁移。供应商重复或乱序回调不得使已完成、已取消或已退款订单回到处理中：履约轨已完成不得回退；取消轨已取消或取消失败不得回到取消处理中；退款轨全部退款不得回到退款中；部分退款不得把已完成履约改回处理中。
...
- T 后商城不再开单，切换范围内不再产生新的关键业务事实；`T` 附近实时与回填的重叠只可能来自同一既有事实的重复投递，两者按第 13.1 节的业务事实键去重，同一支付、取消、退款、订单完成或卡券余额恢复事实只形成一份正式记录；
...
| 切换前的卡券销售单同步 | 来源商城 + 来源销售单号 + 来源更新时间 + 内容指纹 |
| 卡实例基线同步 | 商城 + 卡实例引用 |
...
- 第二期复用第一期的 `outbox_message`，正式业务事务与外部待发送消息同事务提交；外部 dispatcher 才在本期启用；
- `inbox_message`、`integration_attempt`、`integration_error_task` 以及通用对账在本期新增；一期的内部 outbox 消费、商城主动轮询和专用销售快照核对不替代这些能力；
...
T 起商城停止开设新的销售单，全部 B2B 销售单统一由 ERP 服务（创建、审批、销售变更），数据仅通过商城同步进入 ERP；不存在存量单迁移动作。销售单只使用 `sales_order.origin_system` 字段表达最初创建入口，创建后永久不变。
...
| 实体 | 作用 | 关键关系 |
| --- | --- | --- |
| `outbox_message` | ERP 待发送消息；一期仅由内部投影消费，二期复用后由外部 dispatcher 投递 | 聚合类型/身份/修订、事件类型、幂等键、目标、状态 |
...
6. T 后商城不再开单，不再产生本次切换范围内的销售单新建或商业变更；
...
| P2-D37 | 二期上线切换 | P0、P1 同批上线；T 后商城不再开单，停止一期轮询并封存为只读历史，记录 `T`；不重开商城建单入口 | 8.5.3、8.5.5、18 章 |
...
1. T 起商城停止创建 B2B 销售单，全部 B2B 销售单统一由 ERP 服务，见第 8.5 节；
...
| 依赖 | 第一期出处 | 第二期用途 |
| --- | --- | --- |
| 卡券销售单与实物与服务销售单共用同一实体，创建入口是字段而非单据类型 | 一期 4.6.1 | T 起商城停单、ERP 统一服务，不迁数据、不换单号，见 8.5 |
