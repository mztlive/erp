...
只选择已有公司 SKU；没有对应公司 SKU 时可前往 W14 使用普通新建页独立创建，当前不读取
供应商来源参数进行预填，也不自动返回或建立映射。目录底价与可供状态不进入公司主档。详见
`docs/ui-workspaces/w21-supplier-catalog.md`。

### 7.3 商品映射与审核

- 供应商目录 SKU 只有在采购逐项选择入池时才关联已有公司 SKU。W14 普通新建页保存公司
  商品/SKU 并取得稳定 `sku_id` 后，采购再回 W21 建立
  `supplier_catalog_sku_id → sku_id`；W21“加入公司商品池”本身不创建公司 SKU；
