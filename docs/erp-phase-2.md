...
形成来源修订（`skus[]`）。中心页不展示来源版本差异、供给版本时间线与发布影响。采购逐项
选择供应商 SKU 时，有同款公司 SKU 的正向分支只建立映射与供给；没有同款时，“加入公司
商品池”是反向创建公司商品/SKU 的业务名称，对话框使用来源同名字段自动预填公司商品/SKU
草稿，采购二次修改并确认独立必填 `product_kind`、销售可见价、市场价后原子创建公司商品/SKU
及其修订、映射和供给。
`product_kind` 决定商品业务作用，不能由分类推导；分类只能校验兼容性。目录底价与可供状态不得写入
公司主档；它们只可作为正式供给确认的建议值。详见
`docs/ui-workspaces/w21-supplier-catalog.md`。

### 7.3 商品映射与审核

- 供应商目录 SKU 只有在采购逐项确认时才关联公司 SKU：已有稳定 `sku_id` 的正向分支只建立
  精确 `supplier_catalog_sku_id → sku_id` 映射与该供应商供给；没有同款时，“新建公司商品/SKU
  并加入公司商品池”的反向原子命令创建目标 SKU 后立即建立同一精确映射。两种分支都不得以
  supplier product/SPU 代替；
- 一个公司 SKU 可以关联多个供应商 SKU；
- 每条供应商供给关系在同一修订中同时保存一件代发供给价、集采供给价、集采起订量、库存、区域、能力和有效期，不设置供给方式字段；
- 反向创建命令同时确认该供应商的两项供给价与集采起订量，并把销售可见价与市场价写入
  新公司 `sku_revision`；已有 SKU 的正向分支只写映射与供给，不得修改公司 SKU 或其价格。
