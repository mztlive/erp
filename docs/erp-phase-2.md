...
4. 必须建设 API 供应商商品、订单、履约和售后的统一接入；
...
| **供应商供给关系** | 一条“公司 SKU ↔ 供应商 SKU”的供货绑定，独立保存一件代发供给价、集采供给价、集采起订量、库存、区域、能力和有效期；不设置供给方式字段，默认两种采购方式均可用；同一公司 SKU 可有多家供应商 |
...
| API 供应商商品 | 第一期已有 Excel/手工供应商商品库与供给 | 缺少 API 自动同步和变化处理 | API 作为新来源写入同一供应商 SPU/SKU、映射和供给模型 |
...
- API 供应商连接和能力登记；
- 供应商原始商品同步和版本留痕；
- 供应商 SKU 与公司 SKU 的逐 SKU 映射；供应商 SPU 只提供页面与批量选择上下文；
- 商品分类、定价、固定供应关系和上架审核；
- ERP 商品向商城发布、更新和停止销售；
- 全部 B2B 销售单创建，含原线上业务；
...
| P2-C02 | API 供应商目录自动协同 | API 商品同步、暂存、变化审核，以及对一期既有映射和固定供给的追加处理 | 采购、运营 |
...
flowchart TD
  ...
  Manual[一期手工录入]
  Catalog[一期统一供应商商品库]
  Review{采购逐 SKU 关联或反向建公司 SKU}
  Existing[关联已有公司 SKU]
  New[补齐资料并创建公司商品/SKU\n确认销售可见价和市场价]
  Supply[确认逐供应商代发价、集采价与集采起订量]
...
API 同步数据与 Excel、手工录入使用同一供应商商品库，不得另建 API 专属商品实体。
供应商商品入库是正式、可审计的采购目录事实，但不得直接写入公司 SKU、公司 SKU 的
销售价格/销售资格或商城商品。

暂存数据至少包括：

- 供应商、来源类型，以及 API 来源才有的连接；
- 供应商 SPU 编码和供应商 SKU 编码；
- 商品名称、可选来源商品类型、规格、分类、图片和说明；
- 一件代发底价（含税运）、集采底价（含税）、集采起订量；未确认前不等于采购成本；
- 库存或可供状态；
- 供应商商品版本、更新时间和原始数据摘要。

同一次同步必须区分新增、变化、无变化、停止供应和异常数据。API 连接被停用不得
删除已经入库的供应商商品或供给历史。

**工作面约定（W21）**：供应商商品中心与公司商品（W14）采用同构字段分区，且分类/
品牌/单位/规格使用同一套字典与规格维度编辑。多规格组合生成**可编辑 SKU 表**；每行
维护供应商 SKU 编码、1:1 主图与价格字段（一件代发底价含税运、集采底价含税、集采起订量）
及可供数量/状态，对应 `supplier_catalog_sku_revision`。目录不含统一含税报价、进项税率、
运费/其他费用/可供区域/预计发货/售后说明/商品能力。手工录入使用全页详情即编辑，保存
形成来源修订（`skus[]`）。中心页不展示来源版本差异、供给版本时间线与发布影响。
采购以供应商 SPU 为上下文打开 **「入池」**：系统先返回各供应商 SKU 的池内状态
（`MAPPED` / `HAS_CANDIDATES` / `UNMATCHED`）与匹配证据（条码、名称/规格、单位等，不自动合并）；
有同款走 **关联入池**（只写映射 + 双价供给）；无同款走 **反向入池**（同构新建公司商品/SKU 并原子映射与供给）。
第二家供应商同款必须关联，禁止再次反向建档。入池确认即生效，不向用户采集供给生效日；
进项税率 UI 为整数百分比、提交换算小数。详见 `docs/ui-workspaces/w21-supplier-catalog.md`。
`product_kind` 决定商品业务作用，不能由分类推导；分类只能校验兼容性。目录底价与可供状态不得写入
公司主档；它们只可作为正式供给确认的建议值。

### 7.3 商品映射与审核

- 供应商目录 SKU 只有在采购确认入池时才关联公司 SKU：关联分支建立精确
  `supplier_catalog_sku_id → sku_id` 映射与该供应商供给；反向分支原子创建公司 SKU 后立即建立
  同一精确映射。正式粒度不得以 supplier product/SPU 代替映射键；
- 一个公司 SKU 可以关联多个供应商 SKU；
- 每条供应商供给关系在同一修订中同时保存一件代发供给价、集采供给价、集采起订量、库存、区域等，不设置供给方式字段；
  入池路径不要求用户填写供给生效日（确认即生效）；
- 反向入池同时确认该供应商的两项供给价（起订量取自目录）并把销售可见价与市场价写入
  新公司 `sku_revision`；关联入池只写映射与供给，不得修改公司 SKU 或其价格。
  来源报价、正式供给价、销售可见价和市场价不得互相覆盖；
- 公司商品池不是独立表或修订，而是销售查询视图：启用 SKU 已维护销售可见价且存在至少一条
  业务时点有效的 `supplier_offering_revision` 时，才具备销售资格；
- 同一个商城商品发布版本只能绑定一条供应商供给关系；
- 商品名称、规格、供货价和可供范围变化进入待处理队列；
- 新商品及关键字段变化确认前不得自动发布商城；
- 供应商商品停止供应后，相关发布版本立即变为不可下单并生成待办。
...
- 审核通过后形成新的供应关系版本；只有公司 SKU 内容/价格或发布内容变化时才分别形成相应新版本；
...
- 一条商城订单明细不得拆量给多个供应商；
- 供应商子订单保存商城订单、商品发布版本、供应商商品、供应商和成本快照；
- 后续修改供应关系不影响已经支付的订单。
...
卡券消费能算出成本，依赖两件事同时成立：**消费事实回流**，以及 **ERP 侧存在该商品的成本**。供应商商品库、映射和供给版本已经在第一期建成；第二期 `P2-C02` 只通过 API 自动同步在既有模型上追加来源和供给变化，`P2-C03` 再把选定供给绑定到商品发布版本。供应商供给版本携带供货价和有效期，是消费成本的取值来源。
...
供应商商品库、供应商 SPU/SKU、来源修订、供应商 SKU 映射、供应商供给及
MANUAL/EXCEL 摄取批次已经属于第一期正式模型。第二期不新增这些实体，只允许 API 连接和
自动同步在同一稳定身份上追加 API 来源修订、映射候选与供给变化。下表只列第二期新增实体：
...
多家供应商的同款供应商 SKU 各自保留身份和来源版本，可通过多对一映射指向同一公司 SKU；
同一供应商 SPU 下的不同 SKU 也可分别指向不同公司 SKU，任何映射都不得隐式作用于兄弟 SKU。
公司 SKU 没有独立商品池条目；“公司商品池”只是其销售查询视图。新增第二供应商默认只追加
该供应商 SKU 的映射和供给，不修改公司商品/SKU 修订、销售可见价或市场价。
...
| `supplier_fulfillment_item` | 供应商订单明细 | 商城明细、供应商商品、数量、成本快照 |
...
| P2-P02 | API 供应商商品自动同步与变化处理 | API 同步批次、变化/异常队列、对一期映射与供给的追加处理 | W21 队列 + 详情 |
...
| 角色或部门 | 第二期职责 |
| --- | --- |
| 采购 | 供应商连接业务资料、供应商商品映射、供给关系和成本变更确认 |
