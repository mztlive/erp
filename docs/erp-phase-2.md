...
除第 21 章明确列出的衔接修订外，第二期不重新设计第一期的合同、销售、采购、卡券、库存和结算单据；第二期在其上补齐 ERP、福利商城及 API 供应商之间的自动协同，切换后全部 B2B 销售单统一由 ERP 服务，并使商城消费、实际成本和经营结果能够完整追溯。
...
> **切换后全部 B2B 销售单统一由 ERP 服务，建设 ERP、福利商城和 API 供应商之间的标准化业务协同，并形成卡券业务从企业销售、员工消费、供应商履约到实际成本和经营结果的闭环。**
...
第二期不再使用“所有数据只能有一个全局事实源”的绝对表述，而是按业务对象明确管理与服务系统。
...
销售单的"业务来源"和"服务系统"是两个独立维度，定义见第一期第 4.1 节。**卡券销售单**指企业客户的线上福利业务，这一分类终身不变；切换改变的只是服务系统，不改变它仍然是卡券销售单。因此第二期文档中不再出现"商城渠道销售单""ERP 原生销售单"等按创建系统命名的旧称谓。
...
已生效及之后状态、且未作废的正式存量卡券销售单继续由 ERP 服务。不按"已关闭"筛选：线上卡券销售单的履约期限一般为两年，履约完成的实际条件只有履约期限到期，因此绝大多数存量单在 T 时仍处于履约中，由 ERP 承接后续商业变更。商城现存草稿统一作废，不在 ERP 补建。
...
因此本章不是一个可以独立交付的模块。第一期商品基础资料和供应商供给关系未落实时，第二期消费数据同步过来只有金额没有成本；这属于一期前置能力缺失，不表示供给关系应延后到第二期建设。第 18.1 节的实施顺序必须保留该依赖。
...
- 第 5 项与第 6 项都取不到时才是 `NONE`。由于第 5 项不依赖商品映射，`NONE` 的实际占比预期远低于只靠 ERP 商品基础资料的情形。
...
卡券的成本可信度不是整张卡的属性，而是**每一笔消费**的属性：同一张卡的两笔消费，可能一笔买到有 ERP 商品基础资料的商品、一笔买到早已下架且从未映射的商品。因此成本口径按消费记录逐笔标记：
...
第二期物理表字段和索引统一承载在 `erp-data-model.md`。接口 OpenAPI、消息 Schema 和状态枚举可以在本文件确认后继续拆分，但不得改变本文件已经明确的系统服务边界、固定供应关系、组合支付分摊、追加式纠错和三期边界。
