...
> 注：商城（mall）域已移除，`mall_*` 测试、种子与脚本引用已清理；卡券销售单（Voucher）流程保留。本文 mall 章节仅存档，以代码为准。

本文规定第二期必须补齐的功能、系统边界、核心流程、逻辑数据扩展、接口契约、页面范围和验收标准。

> 冻结说明（2026-09-04）：下文第 1 章“必须补齐”均为未与商城沟通的预测性需求，现冻结存档。
> 商城自动发布、订单回流、组合支付等内容以未来重做文档为准，禁止按本文继续实现。

除第 21 章明确列出的衔接修订外，第二期禁止重新设计第一期的合同、销售、采购、卡券、库存和结算单据。第二期必须在其上补齐 ERP、福利商城及 API 供应商之间的自动协同；切换后全部 B2B 销售单必须统一由 ERP 服务；商城消费、实际成本和经营结果必须完整可追溯。
...
1. ERP 正式商品必须自动发布到商城，保证商品、价格和状态一致；
2. 切换后全部 B2B 销售单必须统一由 ERP 创建与变更；ERP 必须承接客户变更，商城不得再创建或变更正式销售单；
3. 商城订单、组合支付、卡券消费和退款明细必须自动回流 ERP；
4. 必须建设 API 供应商供给、订单、履约和售后的统一接入；
5. 必须归集卡券消费侧实际采购成本、未消费余额和完整经营结果；
6. 跨系统消息必须具备统一的幂等、重试、差异核对和人工补偿。

第二期必须实现：

> **切换后全部 B2B 销售单统一由 ERP 服务；建设 ERP、福利商城和 API 供应商之间的标准化业务协同；形成卡券业务从企业销售、员工消费、供应商履约到实际成本和经营结果的闭环。**

卡券生产、印刷、绑定和激活不在第二期范围内，继续留在商城闭环，见第 8.4 节。
...
| **Supplier Connector** | ERP 内部的供应商接入适配器，把各供应商协议转换为统一的适配器接口，隔离签名、鉴权、字段映射和供应商专用错误                                                                                                                                             |
| **供应商供给关系**     | 一条“公司 SKU ↔ 供应商”的供货绑定，直接保存供应商订货编码；商业条款修订保存一件代发价、集采价、集采起订量、区域、能力和有效期，实时可供投影保存状态和数量；不设置供给方式字段；同一公司 SKU 可有多家供应商                                             |
...
| **消费回流启用日**     | P0、P1 同批上线后 ERP 开始实时接收商城关键业务事实的系统级时间点，记为 `T`。以支付成功事实的发生时间划分履约归属：`T` 之前支付成功的订单沿用原人工履约链，历史回填只记账、不触发供应商下单；`T` 及之后支付成功的订单进入 ERP 自动履约链                |
...
| ------------------ | ----------------------------------------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| 卡券销售单         | 商城开单并管理，ERP 定时同步数据                                  | 卡券与实物服务销售入口分离，ERP 无法承接客户变更      | 切换后全部 B2B 销售单统一由 ERP 服务；草稿统一作废                                                     |
| 卡券销售单审批     | 商城内部完成生效处理，ERP 不参与                                  | 切换后缺少 ERP 侧线上销售单提交、生效和责任衔接       | 销售提交后按 `VoucherSalesOrder` 已发布审批定义逐节点审批，全部节点通过后销售单生效并自动同步商城      |
...
| 卡券生产与激活     | 商城闭环内完成，ERP 不建单据                                      | 无控制能力缺口                                        | 保持商城执行，不建设生产、绑定和激活控制接口；只同步稳定卡实例与余额事实，见 §8.4                      |
| ERP 商品与商城商品 | 采用人工协同                                                      | 商品、价格、状态可能不一致                            | ERP 发布正式商品版本，商城自动接收                                                                     |
| API 供应商供给     | 第一期已有 Excel/手工供给                                         | 缺少 API 自动同步和变化处理                           | API 作为新来源写入同一供给身份、商业条款修订和可供投影                                                 |
...
| 接口运维           | 一期商城卡券销售单仅由 ERP 单向轮询读取，无对外发送或回调接收能力 | 缺少外部投递任务表、定时重试、错误任务和差异处理      | 外部对接用任务表 + 定时重试 + 人工处理，新增错误任务表和对账                                           |
...
采购询价继续由采购部门在线下完成。ERP 第二期接收采购确定后的供给条件和接口数据，不把线下沟通过程改造成 ERP 询价单。
...
第二期按业务对象确定管理与服务系统，不以“全部数据仅有一个全局事实源”为约束。
...
| 供应商原始商品目录                         | API 供应商 | ERP 仅把与已建公司 SKU 对应的数据归一为供给写入；原始报文保留为集成证据，不形成商品主档                |
...
同一事实可以在多个系统中保存，但正式变更只能由服务系统发起。其他系统保存的是缓存、同步数据、快照或派生结果。

第二期上线后，全部 B2B 销售单统一由 ERP 服务，商城草稿统一作废，不进入 ERP；不存在存量单迁移动作。T 后不再由商城发起商业变更，正式销售单商业变更一律走 ERP 销售变更单。

销售单的"业务来源"和"服务系统"是两个独立维度，定义见第一期第 4.1 节。**卡券销售单**指企业客户的线上福利业务，这一分类终身不变；切换改变的只是服务系统，不改变它仍然是卡券销售单。第二期文档禁止使用"商城渠道销售单""ERP 原生销售单"等按创建系统命名的旧称谓。
...
卡券类目、面额、数量、金额和履约期限属于 ERP 销售单商业事实，变更走 ERP 销售变更单；玩法规则由商城自管。完整职责划分和不建设权益方案对象见第 8.1 节，变更流程见第 8.3 节。
...
- API 供应商连接和能力登记；
- 把供应商 API 的价格、订货编码和可供事实归一到既有公司 SKU 的供给；
- 商品分类、定价、固定供应关系和上架审核；
- ERP 商品向商城发布、更新和停止销售；
- 全部 B2B 销售单创建，含原线上业务；
- 卡券销售单的销售提交、按已发布审批定义逐节点审批和生效；
...
第二期上线后，商城不得自行创建 B2B 销售单或与 ERP 无关的正式在售商品，不得直接修改 ERP 管理的销售商业事实、卡券类目、面额、数量、履约期限、商品身份、销售价格、供应关系和上下架状态。
...
| P2-C02 | API 供应商供给自动协同     | API 供给条款和可供事实同步、变化审核                                             | 采购、运营                   |
| P2-C03 | 商品发布协同               | ERP 商品版本发布、商城接收确认、暂停和重新发布                                   | 采购、运营                   |
| P2-C04 | 卡券销售审批与商城执行同步 | 销售提交、按已发布审批定义逐节点审批、执行投影下发、销售变更单下发和接收确认     | 销售、各审批节点审批人、运营 |
...
不建设统一能力代码清单。每个供应商连接通过**适配器接口**和**连接配置**声明其实际支持的操作，按需启用：

| 配置项     | 说明                                                                                                                 |
| ---------- | -------------------------------------------------------------------------------------------------------------------- |
| 适配器接口 | 按供应商协议实现的接入接口，提供商品同步、价格和库存同步、下单、查询、取消、退款、物流查询、状态回调和结算同步等操作 |
| 连接配置   | 地址、鉴权凭据、字段映射、超时和重试参数，以及能力声明（该连接支持哪些操作）                                         |

能力声明只表示供应商接口具备该操作，不表示每个商品都必然可用。商品发布版本还需保存商品级能力标识。

### 6.2 Supplier Connector

`Supplier Connector` 是 ERP 内部的供应商接入适配器，负责：

- 按各供应商协议实现适配器接口，隔离签名、鉴权、字段映射和供应商专用错误；
- 把调用结果归一为统一错误分类（可重试/不可重试/需人工）；
- 处理请求幂等、超时、重试和回调验签；
...
| 错误类型 | 处理规则                                                                                     |
| -------- | -------------------------------------------------------------------------------------------- |
| 可重试   | 网络超时、临时不可用和限流按退避策略自动重试，重试前先按幂等键查询原请求结果；超过上限转人工 |
| 不可重试 | 鉴权或签名失败、参数或映射错误、能力不支持：不自动重试，生成错误任务并告警                   |
| 结果未知 | 不直接再次创建订单，先查询原请求结果，无法确认时转人工                                       |
| 需人工   | 供应商业务拒绝、回调重复或乱序：生成人工任务并通知业务人员，不允许状态倒退                   |

---

## 7. API 接入供应商供给与 ERP 商品发布

### 7.1 供给同步流程

```mermaid
flowchart TD
  Product[W14 创建公司商品与 SKU]
  Excel[供应商 Excel]
  Api[供应商 API]
  Manual[手工录入]
  Supply[为公司 SKU 添加供给]
  Terms[形成商业条款修订]
  Availability[形成实时可供投影]
  Eligible[派生公司 SKU 销售资格]
  Publish[形成商品发布版本]
  Sales[销售选品/导出/下单]
  Mall[商城接收并确认]
...
公司商品及 SKU 必须先在 W14 创建。W21、Excel 和 API 只允许为已有公司 `sku_id`
添加供给，不得隐式建品，不得创建供应商商品/SPU/SKU 主档或映射。

供给身份至少包括供应商、供应商订货 SKU 编码、可选供应商商品编码、来源类型和可选 API
连接。新增供给必须同一事务创建首版商业条款与初始可供投影。

### 7.3 供给条款与可供投影

- 一个公司 SKU 可以关联多个供应商供给；
- 每条供给的商业条款修订同时保存一件代发供给价、集采供给价、集采起订量、税率、费用、区域、能力和有效期；
- 可供状态、数量、来源更新时间独立保存在 `supplier_offering_availability`，不得进入商业条款修订；
- 供给价、销售可见价和市场价不得互相覆盖；
- 公司商品池不是独立表或修订，而是销售查询视图：启用 SKU 已维护销售可见价且存在至少一条
  业务时点有效的供给修订且当前可供时，才具备销售资格；
- 同一个商城商品发布版本只能绑定一条供应商供给关系；
- 供货价变化形成新商业条款修订；库存或可供变化只更新可供投影；
- 供给停止后，相关发布版本立即变为不可下单并生成待办。

### 7.4 价格和库存变更

- 可供状态与数量允许按供应商接口自动同步；来源时间倒退必须拒绝；
- 供应商库存为零、不可供或数据过期时，ERP通知商城暂停下单；
- 供应商供货价变化不得直接修改公司 `sku_revision` 的销售可见价/市场价或商城销售价；
- 审核通过后形成新的供给商业条款修订；只有公司 SKU 内容/价格或发布内容变化时才分别形成相应新版本；
...
第二期不建设"卡券权益方案"这一独立对象，也不建设商城权益配置选项目录、ERP 权益方案版本发布和全卡一致的原子版本切换。客户确认的权益内容在 ERP 侧就是销售项本身，必须由卡券销售单的唯一卡券明细完整表达；玩法规则不属于 ERP 管理范围，ERP 不得建模、版本化和校验，也不得要求商城回报。

ERP 不实现商城购买权限判断、支付校验、卡券余额扣减和余额计算。

#### 8.1.1 卡券销售单审批与生效

第二期由 ERP 承接卡券销售单的创建和审批。**阶段**顺序固定，审批内部的节点数量、顺序和审批人不固定：

```text
销售提交
→ 按 VoucherSalesOrder 已发布审批定义逐节点审批（节点由流程管理员配置）
→ 全部节点通过后销售单生效
→ ERP 自动向商城发送执行投影
→ 商城确认接收
→ 运营在商城配置玩法、制卡、绑定和激活
...
- 销售依据有效客户、合同和结算约定创建卡券销售单并提交；创建时冻结绑定的审批定义版本，提交时冻结 `subject_version`；
- 每个审批节点的审批人只审批本节点职责范围内的事实（例如客户商务条件与合同一致性、卡券类目与商城执行条件），任何节点的审批人都不得在审批时修改销售商业字段；
- 最后一个节点通过后销售单生效，ERP 形成首个不可变销售单版本并派生应收；
- 任一节点驳回不改变销售单内容和 `subject_version`，审批实例轮次加一并回到第一节点重新审批。销售不需要、也不得为了重启审批而修改单据内容再重新提交（合同 §4.4.2、§11）；
- 销售单生效前不形成应收，不向商城发送执行投影；
- 销售单生效后商业字段锁定，后续调整按第 8.3 节通过销售变更单处理，不直接修改原单。

销售单生效与商城执行是两个连续但独立的阶段：商城接收失败不回退已经生效的销售事实和应收，但会阻断受影响的商城执行。

该流程不得由代码注册固定步骤。卡券销售单对应固定 `DocumentType::VoucherSalesOrder`，审批政策为
`PROCESS_REQUIRED`（`approval-workflow-contract.md` §4.3）。审批节点数量、顺序和每个节点的具体审批人
由流程管理员在审批流程配置工作面创建草稿并发布，单据创建时冻结绑定的定义版本。

固定约束：

- 每个审批节点只指定一个具体用户。全系统只有「指定到人」一种责任模型，`AssignmentMode` 与 `POOL`、角色池、候选人集合和处理人解析器均已删除；
- 审批任务直接进入该用户“待我处理”，不存在“开始处理”“退回团队”和责任池认领动作；
- 任何时刻实例只允许一个活动节点执行和一个开放审批任务；
- 当前节点指定审批人失效时，实例和当前执行进入 `BLOCKED`，不得猜测管理员、回退责任池或跳过该节点。
  恢复方式只有三种，见合同 §12.2—§12.5；
- 任一节点驳回时，销售单和 `subject_version` 保持不变，审批实例轮次加一并回到第一节点重新审批。
  不得把实例置为终态，也不得要求销售修改内容后重新提交以创建新实例；
- 节点通过、当前任务完成和下一节点执行创建必须在同一事务；最终通过与销售版本、应收、执行投影修订、
  当前任务完成及审批实例终结必须在同一事务。

运营环节已于 2026-08-17 签署为**普通审批节点**（合同 §4.4.3）：它是审批链中的一个单人节点，与其他节点没有差别，
不再是责任池。现有 `POOL` 语义、`card_sales_operations_pool` 解析器、“开始处理 / 退回团队”动作和
`WorkItemType::CardSalesOperationApproval`、`CardSalesManagerApproval` 必须按合同 §13.3 一并删除。

合同 §4.4.3、§13.2 废止「团队业务任务」：实物及服务销售单的采购二次确认必须实现为带固定
`node_purpose=SALES_ORDER_PROCUREMENT_CONFIRMATION` 的普通审批节点，低毛利上级确认整体删除，`AssignmentMode`、`claim`、`start_processing`、`release_to_team`
和「团队待处理」队列视图从全系统删除。因此两类销售单的审批责任模型完全一致，卡券销售单不存在特例。
完整规则以 `approval-workflow-contract.md` 为准。
...
sequenceDiagram
  participant 销售
  participant 审批人 as 各审批节点审批人
  participant ERP
  participant 运营
  participant 商城 as 福利商城

  销售->>ERP: 创建并提交卡券销售单
  ERP->>审批人: 按已发布定义逐节点创建待我审批
  审批人-->>ERP: 逐节点通过（任一节点驳回则轮次加一回到第一节点）
  ERP->>ERP: 末节点通过 销售单生效 形成首个 ERP 版本并派生应收
...
更换卡券类目等于更换销售的标的，属于销售单的商业变更，必须遵守：
...
卡券销售变更单对应 `DocumentType::SalesChangeOrder`。节点数量、顺序和审批人只来自创建时冻结的已发布定义；合同不固定“运营→财务”两级。业务需要商城执行判断或金额影响复核时，由流程管理员把具备相应职责的具体用户配置为普通审批节点，不得为其建立专用状态机、任务类型或运行分支。
...
sequenceDiagram
  ...
  ERP->>商城: 查询存量单第一份执行投影版本或最后已接收执行投影
  商城-->>ERP: 返回当前执行投影版本标识和投影字段
  Note over ERP: 执行投影范围不一致时阻断提交并生成差异任务，见 8.5.4
  销售->>ERP: 提交并启动冻结定义版本的审批实例
  loop 按定义逐节点
    ERP->>审批人: 创建指定到人的审批任务
    审批人->>商城: 需要执行判断时人工核对
    审批人-->>ERP: 通过或驳回并填写意见
  ...
  alt 任一节点驳回
    ERP-->>销售: 保持原提交，实例下一轮回到入口节点
  else 末节点通过
    ERP->>ERP: 生效变更单并形成新的销售单版本
    ERP->>商城: 按新版本下发执行投影
    商城-->>ERP: 返回接收结果
  end
  Note over 销售,审批人: 已登记的回款和发票通过第一期纠错单据处理
```

任一节点驳回后，变更单内容和 `subject_version` 不变，审批实例轮次加一并回到入口节点。销售要修改变更内容时必须先撤回审批；撤回后修改并再次提交才形成新的不可变提交和新 `subject_version`。最终生效时必须重新校验基准销售版本仍为当前版本，且末节点决定属于当前实例、当前执行和冻结提交。
...
- 承担商城执行可行性判断的审批节点审批人自行进入商城核对当前执行情况，判断商城能否按变更后的内容继续执行；该节点在 `SalesChangeOrder` 已发布审批定义中的位置和审批人由流程管理员配置；
- ERP 只保存各节点的审批结论、审批意见、审批人和审批时间；卡实例基线仅用于追溯和余额核对，不作为销售变更的执行数量依据；
- 确认无法执行时在该节点驳回，审批实例轮次加一回到第一节点，由销售与客户重新协商后再推进；变更单内容和 `subject_version` 不因驳回而改变；
- 该节点通过表示已确认商城可以承接新版本；变更单在已发布定义的末节点通过后才生效；
...
第二期只改变**开单权和非敏感事实回流**：销售单必须由 ERP 开出并下发执行投影；商城继续执行卡券，同时必须回流卡实例基线、余额快照和关键业务事实。卡券的生产与激活方式本身不变。

第二期不建设卡券生产与激活控制，仅同步卡实例与余额事实（见第 2 节）。卡券生产与激活收归 ERP 不在第二期范围内。

### 8.5 切换时点 T 与上线 checklist

第二期上线时，商城停止创建新的 B2B 销售单，T 起全部线上和线下 B2B 销售单统一由 ERP 服务（创建、审批、销售变更）。不存在存量单迁移动作：无迁移操作、无批次、无审计动作、无幂等键、无续跑、无回退。数据仅通过商城同步进入 ERP。

已生效及之后状态、且未作废的正式存量卡券销售单继续由 ERP 服务。不得按"已关闭"筛选：线上卡券销售单履约完成的唯一条件是履约期限到期；T 时仍处于履约中的存量单必须由 ERP 承接后续商业变更。商城现存草稿统一作废，不在 ERP 补建。

#### 8.5.1 切换前后对比

| 项目               | 切换前（第一期）                | 切换后（第二期）                         |
| ------------------ | ------------------------------- | ---------------------------------------- |
| 建单入口           | 商城开单，ERP 定时同步          | 全部销售单由 ERP 建单                    |
| 商业变更来源       | 商城变更后由 ERP 同步形成新版本 | ERP 销售变更单形成 ERP 版本              |
| ERP 侧可否发起变更 | 不可                            | 可，通过销售变更单                       |
| 是否可直接编辑单据 | 不可                            | 不可                                     |
| 历史版本           | 第一期轮询实际观察并形成的版本  | 完整保留已观察版本，作为切换前的来源版本 |
| ERP 轮询同步       | 持续同步商业字段                | 停止同步商业字段                         |
| 应收、回款、发票   | ERP 派生和登记                  | 不变                                     |

T 前由第一期轮询实际观察并形成的版本完整保留，与 T 后产生的 ERP 版本在同一条版本序列上按时间顺序展示，并标注版本来源是商城同步还是 ERP 变更单；不把轮询间隔内未被观察到的中间状态表述为已保存版本。
...
商城不得自行修改 T 前商城开单卡券销售单的商业字段。需要修改时由销售在 ERP 发起销售变更单，ERP 下发后商城执行。
...
4. 商城侧已完成商业字段只读化改造，B2B 销售单创建入口可在切换时点关闭并在切换后保持永久关闭；入口关闭后可统一作废现存草稿；
5. 商城已具备稳定卡实例引用、原销售单身份、初始余额和周期余额快照同步能力；
6. 存量单清单已由销售和财务确认；
7. 切换时点后商城不再开单，本次切换范围内不再产生新的销售单新建或商业变更；
8. 切换时点前完成最后一次第一期同步，以每张存量单当时的 ERP 当前销售单版本作为其第一份执行投影版本（见第 8.5.4 节）；所有会被实时或历史关键事实引用的卡实例均已完成稳定引用、原销售单身份和初始余额基线同步。

以上条件以**上线 checklist 文档**形式维护，由上线负责人人工逐项确认；不建设自动化切换校验表（cutover_check）、证据链或切换验收门槛。确认通过后把切换时点记为 `T` 并写入系统的 cutover 时间戳字段。

#### 8.5.4 存量单的第一份执行投影版本

存量单切换前完成最后一次第一期同步，全部存量单的商城最终商业事实已经形成 ERP 当前版本，且客户、合同、卡券类目映射和唯一明细约束没有未处理差异。

每张存量单以切换时点的 ERP 当前销售单版本作为第一份执行投影版本，用于切换后的商城执行一致性校验；该动作不产生新的销售单版本，不单独登记基线。切换不核对历史发卡、激活、消费和玩法执行过程；卡实例引用、原销售单身份和初始余额按第 8.4 节形成事实基线。

T 后对存量单发起第一次销售变更时，ERP 以该第一份执行投影版本校验商城执行关系；后续变更以商城最后确认接收的执行投影版本校验。校验只覆盖第 8.2 节执行投影字段，不再通过第一期单据查询接口比较完整商业快照；成交金额、配赠、税率、开票要求和应收等 ERP 内部字段只以 ERP 当前版本为变更基线。执行投影范围不一致时阻断该次变更并生成差异任务，不直接覆盖任一侧正式事实。

#### 8.5.5 切换执行

- 切换时点前第一期五分钟轮询保持运行，只用于形成存量单的 ERP 当前版本；
- 切换时点先关闭商城 B2B 销售单创建入口并启用商业字段只读，之后商城不再开单，也不再产生本次切换范围内的新业务事实；
- 确认建单入口已经关闭且不会产生新草稿后，把商城现存 B2B 销售单草稿全部作废，不在 ERP 补建；
- 存量单的任何商业变更都只能通过 ERP 销售变更单形成新版本，见第 8.3 节；
- 切换完成后停止第一期五分钟轮询并封存为只读历史，启用 P0、P1 联合发布的完整链路，把开放时点记为 `T` 并写入系统的 cutover 时间戳字段，再开放 ERP 卡券销售单新建和商城员工业务；
- 商城 B2B 销售单创建入口在切换后保持永久关闭，不设计应急重开路径。
...
- 待基础资料或成本条件补齐后重新归集，仍引用原业务事实键，不生成第二份消费事实；
...
- 同一商城订单包含多家供应商供给时，ERP 按供应商拆成多张供应商子订单；
- 一条商城订单明细不得拆量给多个供应商；
- 供应商子订单保存商城订单、商品发布版本、供应商订货编码、供应商和成本快照；
- 后续修改供应关系不影响已经支付的订单。
...
供应商订单使用**单轨状态机**，取消和退款不另设独立状态轨道：

| 状态             | 含义                                         |
| ---------------- | -------------------------------------------- |
| `PENDING_SUBMIT` | 待提交：已接收商城支付事件，尚未向供应商提交 |
| `ACCEPTED`       | 已接单：供应商已接单                         |
| `FULFILLING`     | 履约中：供应商履约中（含已发货）             |
| `COMPLETED`      | 已完成                                       |
| `CANCELED`       | 已取消                                       |
| `REFUNDED`       | 已退款：退款全部完成                         |
| `EXCEPTION`      | 需人工处理（含供应商明确拒绝、结果未知）     |

状态只允许向前迁移。供应商重复或乱序回调不得使已完成、已取消或已退款订单回到处理中。

另设两个标志位，只作业务标记，不参与状态迁移：

| 标志位               | 含义                                       |
| -------------------- | ------------------------------------------ |
| `settled`（已结算）  | 订单已进入结算账单并形成应付               |
| `refunded`（已退款） | 存在部分退款事实，订单可保持履约中或已完成 |
...
- ERP 以商城退款事实和供应商退款事实分别记账。供应商退款只在已经形成需退回的供应商
  ...
  正式事实判定为不适用。所有适用的商城退款、卡券余额恢复和供应商退款未全部完成
...
卡券消费能算出成本，依赖两件事同时成立：**消费事实回流**，以及 **ERP 侧存在该商品的成本**。公司商品和供给版本已经在第一期建成；第二期 `P2-C02` 通过 API 自动同步供给条款和可供事实，`P2-C03` 再把选定供给绑定到商品发布版本。供应商供给版本携带供货价和有效期，是消费成本的取值来源。

本章能力的交付必须以第一期商品基础资料和供应商供给关系已落实为前提。该前置未落实时，消费事实仅有金额而无成本；不得据此将供给关系延后到第二期建设。第 18.1 节的实施顺序必须保留该依赖。
...
- **第 5 项优先于第 6 项。** 历史消费必须优先取第 5 项（商城订单快照中记录的商品成本）；仅当第 5 项取不到时，才允许取第 6 项。第 5 项不依赖 ERP 商品映射是否覆盖该历史商品；
- **第 6 项必须按消费发生时间匹配供给版本的有效期**，禁止取当前最新版本。消费时点无有效供给版本时必须落到 `NONE`，禁止向前或向后就近取值；
- 第 5 项与第 6 项都取不到时才是 `NONE`。

每笔成本保存**取值来源、成本口径、取值时间和关联单据**（供应商订单号或商城订单号）。成本口径的三级定义见第 12.3.1 节。成本发生变化时保存原值、新值和变化来源，不覆盖历史成本。

第 5 项的成本字段口径必须是**供应商供货成本**，禁止使用商城内部结算价、平台分成价或含服务费的转移价。该字段在商城全部历史区间内语义一致，禁止按区间降级。
...
- 第二期起，卡券实际消费成本必须进入卡券经营核算；第一期"卡券成本费用不进入实际经营盈亏"的规则在卡券消费成本上不再适用；
- 修订只影响经营分析口径，不得改变第一期任何已发生的销售、履约、票款和费用事实；
- 第一期的印刷费、仓储费和配送费继续按第一期规则记录，作为卡券直接履约费用并入卡券经营结果，口径见第 12.5 节。

#### 12.3.1 按消费记录标记成本口径，不按卡券售出时间分组

成本口径必须按消费记录逐笔标记，不得按整张卡统一标记：
...
禁止按"成本完整卡券 / 成本不完整卡券"对卡分组后整体排除指标。

#### 12.3.2 历史消费数据回填

第二期必须建设商城历史消费数据回填。回填必须形成"订单 → 商品 → 支付来源 → 销售单 → 卡券"的完整追溯链。
...
- 切换范围内不再产生新的关键业务事实；`T` 附近实时与回填的重叠只可能来自同一既有事实的重复投递，两者按第 13.1 节的业务事实键去重，同一支付、取消、退款、订单完成或卡券余额恢复事实只形成一份正式记录；
...
历史事实入库不以商品基础资料和供应商供给关系全部建成为前置条件；完成成本归集仍依赖第 12.0 节所述商品基础资料和供应商供给关系。
...
**任何利润类指标都必须与成本覆盖率同屏展示。** 成本覆盖率不足 100% 时，消费毛差、当前经营贡献和最终经营盈亏不得作为成本完整结论使用：`NONE` 口径的消费只贡献金额不贡献成本。必须计算利润类指标并标明可信范围；禁止因成本不完整而整体拒绝计算。
...
卡券到期未消费金额不退款、不延期、不转入其他卡券，也不形成其他余额，作为公司留存。卡券销售收入已在企业客户销售单中完整记录，到期留存禁止再追加收入；最终经营盈亏必须按完整卡券销售收入减全部实际消费成本和直接履约费用计算，未消费金额通过未发生对应消费成本自然体现，禁止重复计入收入。
...
| 口径       | 指标                                                                                           | 规则依据                                                                                         |
| ---------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| **含税**   | 卡券销售金额、卡券面值、配赠金额、累计卡券消费金额、未消费余额、未履约余额、消费率、成本覆盖率 | 对应卡券面值和卡券实际支付，本身为含税金额；消费率和成本覆盖率是同口径比值，含税与不含税结果一致 |
...
业务事实落库以**业务事实唯一键**去重：每类业务事实在数据库层建立唯一约束（DB unique），重复投递时按唯一键命中既有记录，不重复执行任何业务动作。
...
| 供应商状态回调             | 连接（connection_id）+ 外部事件 ID                                                             |
| 供应商退款结果             | 连接（connection_id）+ 外部退款单号（external_refund_no）+ 退款版本（external_refund_version） |
| 供应商供给同步             | 供应商 + 供应商订货 SKU 编码 + 外部版本                                                        |
| ERP 商品发布               | ERP 商品 + 发布版本 + 目标商城                                                                 |
| ERP 销售单执行投影下发     | ERP 销售单号 + ERP 销售单版本 + 目标商城                                                       |
| 商城执行投影接收确认       | 商城 + ERP 销售单号 + ERP 销售单版本                                                           |
| 切换前的卡券销售单同步     | 来源商城 + 来源销售单号 + 来源更新时间                                                         |
| 卡实例基线同步             | 商城 + 卡实例引用                                                                              |
| 卡券余额快照同步           | 商城 + 卡实例引用 + 快照时间                                                                   |
| 结算账单同步               | 供应商 + 账单号 + 账单版本                                                                     |

同一业务事实无论来自实时事件还是历史回填，都使用相同的业务事实键，只形成一份正式记录。商城订单号不能单独作为业务事实键；同一订单的支付、取消、完成、多次部分退款和多次卡券余额恢复必须分别保存。商城不发送 `PAYMENT_PENDING`、`REFUND_PENDING` 或其他处理中间态，因此这些中间态不定义业务事实键。

### 13.2 投递可靠性

- 外部投递和回调接收使用普通任务表记录，状态包括待发送、已发送、重试和人工处理；
- 重复投递按业务事实唯一键命中既有记录，不重复执行任何业务动作；
- 失败任务记录重试次数、下次重试时间和最终结果；
- 重试采用有限次数和退避策略；
- 结果未知的下单、取消和退款不得盲目重复调用；
- 超过自动处理上限后进入人工错误任务表（错误中心）；
- 人工重放仍使用原业务事实唯一键。

### 13.3 周期对账

周期对账只保留两项涉钱核对，按结算周期执行：

1. 供应商完成订单与结算账单；
2. 供应商退款与 ERP 成本、应付冲减。

差异只生成差异记录和处理任务，不由对账任务直接修改正式业务事实。

其余项目不做周期全量对账，改为按差异线索抽检或在具体业务环节核验：商品发布版本与商城生效版本、ERP 销售单版本与商城已接收的执行投影版本、商城关键事实与 ERP 对应事实、支付来源与分摊合计、供应商订单状态等。商城卡券余额快照仅作为参考信息保存，不要求 ERP 按公式推导余额与其核对。

### 13.4 监控指标

监控不建设多维度指标面板，规则简化为**失败即告警**：接口调用失败、投递重试耗尽、定时任务失败、人工错误任务新增或超期未处理，立即告警通知责任人处理。
...
公司商品是唯一商品主数据。供应商供给、商业条款修订、可供投影和 MANUAL/EXCEL 写入
已经属于第一期正式模型。第二期不新增供应商商品或映射实体，只允许 API 连接和自动同步
在同一供给身份上追加商业条款修订或更新可供投影。下表只列第二期新增实体：
...
同一公司 SKU 可以拥有多家供应商供给，每条供给直接保存该供应商的订货 SKU 编码和来源。
公司 SKU 没有独立商品池条目；“公司商品池”只是其销售查询视图。新增第二供应商只追加
供给，不修改公司商品/SKU 修订、销售可见价或市场价。
...
| `sales_order_projection_revision` | 不可变执行投影版本                 | 来源（存量单切换时点版本或 ERP 版本）、ERP 销售单版本、销售单级卡券类目与履约期限、唯一明细的面额、数量和卡形态 |
| `sales_order_projection_delivery` | 执行投影发送与商城接收结果         | 投影版本、发送时间、商城接收结果                                                                                |

切换后全部 B2B 销售单统一由 ERP 服务（创建、审批、销售变更），数据仅通过商城同步进入 ERP；不存在存量单迁移动作。销售单只使用 `sales_order.origin_system` 字段表达最初创建入口，创建后永久不变。

执行投影版本与 ERP 销售单版本一一对应。销售单版本由销售单生效和后续销售变更单生效产生，不单独建模。

每张正式存量单以切换时点的 ERP 当前销售单版本作为第一份执行投影版本；该动作不产生新的销售单版本。之后每个 ERP 销售单版本仍只对应一份新的执行投影版本。

第一期通过轮询实际观察并形成的版本历史完整保留，与切换后的 ERP 销售单版本在同一条版本序列上按时间顺序展示，并标注版本来源是商城同步还是 ERP 变更单。
...
| `mall_item_funding_allocation`     | 支付来源到商品的分摊                | 商品明细、支付来源、实际支付金额；成本分摊与尾差归属见 `mall_consumption_cost_assessment`                               |
| `mall_consumption_entry`           | 支付成功形成的有效消费事实          | 业务事实键、商品明细、支付来源、客户、原销售单、唯一卡券明细、归集状态、数据来源（实时或回填）                          |
...
| `supplier_fulfillment_item`      | 供应商订单明细             | 商城明细、供应商订货编码快照、数量、成本快照                |
...
| 实体                                   | 作用                                           | 关键关系                                                                                                            |
| -------------------------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `inbox_message`                        | 已接收商城关键事实或供应商回调的普通入站记录表 | 来源系统、来源事件 ID、业务事实键、签名、载荷摘要、状态                                                             |
...
| `reconciliation_difference_resolution` | 差异解决动作                                   | 对账差异、递增处理序号、动作类型（查询、重放、补证、补偿、确认或关闭）、处理结果；待办责任变更只记 `work_item` 审计 |

`integration_message` 仅是外部对接消息的概念总称，不创建同义物理表，也不替代各自的唯一键、签名和状态约束。
...
卡券销售单创建、详情和版本历史复用第一期统一销售单页面，并在第二期嵌入通用审批区：冻结路线、当前轮次、当前节点、当前审批人、决定入口、驳回原因和轮次历史，一律取审批实例投影，不做前端推断。新增的跨系统页面如下：

下表只保留第二期增量**能力与工作面**索引（交互落点见 `erp-ui-design.md` 工作面 W20–W30 与 `erp-ui-flows.md`）。列表/详情不再强制拆成互不关联的独立菜单页，默认挂在统一对象中心、连续处理队列或治理工作面内。

| 编号   | 能力 / 工作面                    | 主要功能                                   | UI 落点                     |
| ------ | -------------------------------- | ------------------------------------------ | --------------------------- |
| P2-P01 | API 供应商连接                   | 连接状态、能力、同步与异常                 | W20 列表 + 对象中心         |
| P2-P02 | API 供应商供给自动同步与变化处理 | API 商业条款修订、可供投影更新和异常处理   | W21 供给列表                |
| P2-P03 | 商品发布                         | 发布版本、固定供应关系、商城确认           | W22                         |
| P2-P04 | 销售单执行投影                   | 下发内容、接收状态、重试与历史             | 销售单中心「协同」+ W23     |
| P2-P05 | 商城消费订单                     | 关键事实、组合支付分摊、原销售单           | W25                         |
| P2-P06 | 供应商订单                       | 接单履约取消退款与异常补偿                 | W26；可从 W25 钻取          |
| P2-P07 | API 供应商结算                   | 成本汇总、差异、应付形成                   | W27 → 复用供应商往来核销    |
| P2-P08 | 卡券消费与经营                   | 消费台账、成本口径、经营分析               | W28                         |
| P2-P09 | 接口错误与对账                   | 重试、转人工、差异核对                     | W29 详情内闭环              |
| P2-P10 | 历史消费回填                     | 回填批次、去重与未归集清单                 | W30                         |
| P2-P11 | 卡券销售审批                     | 通用审批区：逐节点决定、驳回原因与轮次历史 | W01 统一工作台 + 销售单中心 |
...
| 角色或部门             | 第二期职责                                                                                                                                                         |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 采购                   | 供应商连接业务资料、供给关系和成本变更确认                                                                                                                         |
| 运营                   | 作为已发布审批定义中指定审批人参与卡券销售单和销售变更单的执行可行性审批、ERP 商品发布、执行投影接收协同、商城玩法规则配置、卡号生产与激活执行、消费异常和订单协同 |
| 客服                   | 员工取消、退款、售后和异常沟通                                                                                                                                     |
| 财务                   | 供应商结算、差异确认、应付形成和卡券经营核算                                                                                                                       |
| 销售                   | 创建并提交销售单和销售变更单，处理审批驳回，查看商城接收结果、卡券消费及经营结果                                                                                   |
| 销售领导               | 被已发布定义指定为审批人时，审批卡券销售单或销售变更单的客户商务条件、销售内容及合同一致性                                                                         |
| 系统管理员             | 接口权限、能力配置、错误中心和运行监控                                                                                                                             |
| 上线负责人（项目指定） | 组织切换协同，不替代销售、财务和系统管理员的业务职责                                                                                                               |
...
- 销售只查看其负责客户的汇总和授权明细；
- 卡券销售单必须按 `VoucherSalesOrder` 已发布审批定义逐节点通过，末节点通过后才生效；
- 各节点审批人只能通过或驳回，不得在审批时直接修改销售单内容；
- 任一节点驳回不改变销售单内容，审批实例轮次加一并回到第一节点；
...
1. P0、P1 使用同一生产发布批次上线；切换后，商城无法创建 B2B 销售单，全部新单在 ERP 创建；
2. 销售提交卡券销售单后按创建时冻结的 `VoucherSalesOrder` 定义版本逐节点审批；切换前必须已发布该类型定义，否则单据创建失败关闭并返回 `APPROVAL_PROCESS_NOT_CONFIGURED`；
3. 末节点通过后销售单生效，ERP 形成首个正式版本、派生应收并自动发送执行投影；
4. 任一节点驳回不改变销售单和 `subject_version`，审批实例轮次加一并回到第一节点，不需要销售改单重提；
5. 销售单生效前不形成应收，也不向商城发送执行投影；
6. 切换后不再产生本次切换范围内的销售单新建或商业变更；
7. 全部已生效及之后状态、且未作废的正式存量卡券销售单继续由 ERP 服务；商城草稿已在建单入口关闭后统一作废，不在 ERP 补建；
8. 切换前完成最后一次第一期同步；全部存量单均有 ERP 当前版本、完整映射和恰好一条卡券明细，以切换时点的当前版本作为第一份执行投影版本；
9. 切换不产生新单号，不改变已登记的应收、回款和发票；
10. 切换后第一期轮询实际观察并形成的版本历史完整保留，并标注版本来源；
11. 切换完成后停止一期五分钟轮询并封存为只读历史，启用 P0、P1 完整链路并记录 `T`，再开放 ERP 卡券销售新建和商城员工业务；商城 B2B 建单入口永久关闭；
12. 恰好包含一条卡券明细的销售单生效后向商城下发执行投影，商城返回接收结果；零条或多条卡券明细不能生效；
13. 商城接收失败时 ERP 销售单商业事实和应收保持生效，执行进度显示接收失败并进入错误中心；
14. 商城确认接收前，新销售单不得开始玩法配置、制卡、绑定和激活；变更版本只阻断其影响的新增或变更部分，已经完成的存量执行事实不回退；
15. 每个销售单版本必须恰好只有一条卡券明细；销售变更可以在新版本中整体替换该明细，只有原条件和新条件需要在同一生效结果中同时存在时才拆单；
16. 更换卡券类目等商业事实必须走 ERP 销售变更单，变更生效后按新版本下发执行投影；
17. 商城运营无法在商城修改 T 前商城开单卡券销售单的卡券类目、面额、数量、配赠条件、卡形态和履约期限；
18. 商城运营可以自行调整玩法规则，ERP 不接收、不校验、不告警；
19. 重复下发同一 ERP 销售单版本不产生重复调整；
20. 对存量单发起第一次销售变更时，以切换时点的 ERP 当前销售单版本（第一份执行投影版本）校验，后续校验商城最后已接收执行投影；只比较执行投影字段，不用商城旧快照校验 ERP 内部金额、税率、开票和应收字段；
21. 卡券销售变更单走运营确认商城执行影响的时序，不经过实物及服务销售单的采购确认节点；
22. 运营自行进入商城核对变更可执行性并审批通过或驳回；ERP 只保存审批结论和意见，卡实例基线不作为自动变更判断依据，也不按发卡数或激活数自动阻断变更；
23. ERP 从商城关键事实归集的消费金额进入消费台账和经营分析，不作为销售变更自动判断条件。
...
P0、P1 表示研发和联调的先后优先级，不表示可以分成两个生产上线批次。**P0 与 P1 必须同批上线；P1 所列补偿、结算、对账和异常能力未就绪前，不得开放 `T` 后自动下单链路。**
...
2. ERP 卡券销售单创建、按已发布审批定义逐节点审批、生效和销售变更单能力；
3. ERP 销售单生效后自动发送执行投影、商城接收确认及未确认时的运营执行门禁；
4. 商城侧销售单商业字段只读化改造和 B2B 建单入口永久关闭能力；
5. API 供应商连接、商品同步暂存和商品映射；
6. ERP 商品发布版本和商城接收确认；
7. 业务事实唯一键去重、错误任务表、重试和失败告警。

**P0-B 切换与消费回流**

8. 正式存量卡券销售单映射差异处理、唯一明细校验、卡实例引用及初始余额基线准备；商城草稿统一作废；
9. 完成最后一次第一期同步；关闭商城 B2B 建单入口并作废现存草稿，停止第一期轮询并封存为只读历史；
10. 以时间点 `T` 开启支付、取消、退款、订单完成和余额恢复等不可变关键事实回流；
11. 固定供应关系下的供应商订单创建和状态同步。

P0-B 第 9 项必须在 P0 第 2 至第 4 项、第 8 项以及 P1 全部能力完成后执行，操作条件见第 8.5.3 节。切换后不重开商城建单入口。
...
第 6 项的回填能力与 P0 同批上线，实际回填任务在实时回流启用后执行，范围截止 `T`。实时与回填若重复投递同一既有事实，按业务事实键去重。历史消费必须优先按第 12.1 节第 5 项取商城订单成本快照；仅当第 5 项取不到时，才允许按第 12.1 节第 6 项取消费时点的供给版本价。商品映射范围必须把商城历史在售商品一并纳入，禁止仅覆盖当前在售商品。
...
P0、P1 和 P2 均属于第二期范围。P0、P1 允许按优先级先后研发，但必须同批生产上线；P2 允许在 P0、P1 同批上线之后按运营分析需要分批交付，不得取消。
...
以下事项不进入 ERP 第二期与三期建设，继续在线下或外部系统完成：
...
| P2-D26 | 历史消费回填       | **必须建设**商城历史消费回填                                                                                                                                                                                                                                                            | 12.3.2                                                        |
...
| P2-D29 | 存量单执行投影基准 | 存量单以切换时点的 ERP 当前销售单版本作为第一份执行投影版本；不单独登记基线，不建设自动化阶段退出或切换验收门槛                                                                                                                                                                         | 8.5.3、8.5.4                                                  |
...
| P2-D07 | API 供应商         | 第二期建设标准 API 供应商供给、订单、履约和售后接入                                                                                                                                                                                                                                     | 6、7、10、11 章                                               |
| P2-D08 | H5 供应商          | 第二期不建设，不在范围内                                                                                                                                                                                                                                                                | 2.1、19.2                                                     |
...
| P2-D16 | 卡券销售单开单入口 | 第二期上线后全部 B2B 销售单由 ERP 创建，商城建单入口永久关闭，不设计应急重开                                                                                                                                                                                                            | 8.5、8.5.5                                                    |
| P2-D17 | 存量卡券销售单     | 已生效及之后状态、且未作废的正式存量单继续由 ERP 服务；商城草稿统一作废、不补建；切换不产生新单号                                                                                                                                                                                       | 8.5、8.5.2                                                    |
| P2-D18 | 销售单可编辑性     | ERP 正式销售单不允许直接编辑，商业变更一律走销售变更单并形成 ERP 版本                                                                                                                                                                                                                   | 8.5.1、8.3                                                    |
...
| P2-D24 | 切换数据准备       | 正式存量销售单具备 ERP 当前版本、完整映射和唯一明细；商城草稿统一作废；所有会被实时或历史关键事实引用的卡实例具备稳定引用、原销售单身份和初始余额基线                                                                                                                                   | 8.4、8.5.3                                                    |
| P2-D35 | 卡券销售单审批     | 固定 `DocumentType::VoucherSalesOrder`，政策 `PROCESS_REQUIRED`；节点、顺序和审批人由管理员配置并发布，创建时冻结绑定版本；每节点指定单个具体用户，全系统无责任池、无开始处理、无退回团队；最终通过使销售单生效；任一节点驳回不改变单据和 `subject_version`，实例轮次加一并回到第一节点 | 8.1.1、16 章、`approval-workflow-contract.md` §4.3、§4.4、§11 |
| P2-D36 | 商城执行门禁       | 销售事实和应收不因投影失败而回退，但商城确认接收前不得执行受该版本影响的玩法配置、制卡、绑定和激活                                                                                                                                                                                      | 8.2、8.4                                                      |
| P2-D37 | 二期上线切换       | P0、P1 同批上线；切换后商城建单入口永久关闭，停止一期轮询并封存为只读历史，记录 `T`；不重开商城建单入口                                                                                                                                                                                 | 8.5.3、8.5.5、18 章                                           |
| P2-D38 | 实时回流与历史回填 | 以时间点 `T` 开启关键事实回流并后台回填 `T` 以前的历史事实；`T` 前支付只补台账、不触发供应商下单，重复投递按业务事实键去重                                                                                                                                                              | 9.2、12.3.2、18.1、18.2                                       |
| P2-D39 | 关键事实幂等       | 支付、取消、退款、订单完成和余额恢复以业务事实唯一键去重（DB unique），重复投递只命中既有记录；订单号不得单独作为幂等键                                                                                                                                                                 | 13.1                                                          |
...
| **本文**                        | 第二期缺口、销售单切换、商城消费、供应商协同、数据扩展和验收基线                                      |
| `approval-workflow-contract.md` | 审批定义、运行实例、节点分派、待办责任（全系统只有指定到人一种）、多级推进、统一工作台和 BPM 接入边界 |
| `erp-data-model.md`             | 覆盖两期的规范实体、字段、关系、版本、约束和索引                                                      |
| `erp-mall-data-mapping.md`      | 商城旧表、历史数据和接口到规范模型的兼容与迁移规则                                                    |
| `erp-ui-design.md`              | 应用壳、工作面、页面模式、导航与组件约定（UI 事实源）                                                 |
| `erp-ui-flows.md`               | 主业务路径如何在工作面内连续完成、避免跨页割裂                                                        |

两期文档在以下五处形成显式衔接：

1. 切换后全部 B2B 销售单统一由 ERP 服务，见第 8.5 节；
...
| 依赖                                                                                     | 第一期出处        | 第二期用途                                         |
| ---------------------------------------------------------------------------------------- | ----------------- | -------------------------------------------------- |
| 卡券销售单与实物与服务销售单共用同一实体，创建入口是字段而非单据类型                     | 一期 4.6.1        | 切换后 ERP 统一服务，不迁数据、不换单号，见 8.5    |
| ERP 销售单稳定身份统一，且一张卡券销售单恰好只有一条明细；一期来源同步键只用于商城来源单 | 一期 4.3.2、8.2.1 | 卡实例按原销售单身份归集到唯一明细，见 9.2、12.3.2 |

第二期物理表字段和索引统一承载在 `erp-data-model.md`。接口 OpenAPI、消息 Schema 和状态枚举可另行拆分，但不得改变本文件已经明确的系统服务边界、固定供应关系、组合支付分摊、追加式纠错和三期边界。
