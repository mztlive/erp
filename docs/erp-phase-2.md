...
| 卡券销售单审批 | 商城内部完成生效处理，ERP 不参与 | 切换后缺少 ERP 侧线上销售单提交、生效和责任衔接 | 销售提交后依次由销售领导和运营审批，运营审批通过后销售单生效并自动同步商城 |
...
| P2-C04 | 卡券销售审批与商城执行同步 | 销售提交、销售领导审批、运营审批、执行投影下发、销售变更单下发和接收确认 | 销售、销售领导、运营 |
...
该流程必须注册为版本化审批定义 `CARD_SALES_APPROVAL`，步骤固定如下：

| 顺序 | `step_key` | 分派模式 | 处理人解析 | 通过结果 |
| --- | --- | --- | --- | --- |
| 1 | `SALES_MANAGER_APPROVAL` | `DIRECT` | 解析该销售单负责销售的唯一有效销售领导，并重验数据范围和岗位分离 | 完成当前待办并激活运营步骤 |
| 2 | `OPERATIONS_APPROVAL` | `POOL` | 解析具备卡券运营审批权限及该对象数据范围的运营责任池 | 完成当前待办、结束审批并使销售单生效 |

销售领导任务创建后直接进入该用户“我的待办”，不需要开始处理。运营步骤只有在领导步骤通过后才激活并
创建一条“团队待处理”任务，合格运营人员必须先执行“开始处理”。任何时刻只允许活动步骤存在一个开放待办。

销售领导无法解析为唯一有效用户时，审批实例和步骤进入 `BLOCKED`，不得猜测管理员、回退运营池或跳过该步骤。
步骤通过、当前待办完成和下一步骤激活必须在同一事务；运营最终通过与销售版本、应收、执行投影修订、
当前待办完成及审批实例结束必须在同一事务。完整规则以 `approval-workflow-contract.md` 为准。
...
| P2-P11 | 卡券销售审批 | 领导/运营审批、驳回原因与记录 | 待办队列 M3 + 销售单中心 |
...
21. 卡券销售变更单走运营确认商城执行影响的时序，不进入采购二次确认；
...
| P2-D35 | 卡券销售单审批 | 使用版本化 `CARD_SALES_APPROVAL` 串行定义；领导 `DIRECT`、运营 `POOL`；运营通过后销售单生效，修改驳回内容并重提时创建新实例 | 8.1.1、16 章、`approval-workflow-contract.md` |
...
| `approval-workflow-contract.md` | 审批定义、运行实例、步骤分派、待办责任、多级推进和 BPM 接入边界 |
