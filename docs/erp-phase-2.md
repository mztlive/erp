...
供应商商品入库是正式、可审计的采购目录事实，但不得直接写入公司 SKU、公司商品池
或商城商品。
...
形成来源修订（`skus[]`）。中心页不展示来源版本差异、供给版本时间线与发布影响。入池
可以选择已有公司 SKU；没有同款时在对话框中使用来源同名字段自动预填公司商品/SKU 草稿，
采购二次修改并确认独立必填 `product_kind`、销售可见价、市场价后原子创建和入池。
...
- 供应商目录 SKU 只有在采购逐项选择入池时才关联公司 SKU；目标可为已有稳定 `sku_id`，
  也可由“新建公司商品/SKU并入池”原子命令创建后立即取得。两种分支都必须建立精确
  `supplier_catalog_sku_id → sku_id`，不得以 supplier product/SPU 代替；
- 一个公司 SKU 可以关联多个供应商 SKU；
- 每条供应商供给关系在同一修订中同时保存一件代发供给价、集采供给价、集采起订量、库存、区域、能力和有效期，不设置供给方式字段；
- 入池命令同时确认该供应商的两项供给价与集采起订量，并设置公司商品池销售可见价；来源报价、正式供给价和销售可见价不得互相覆盖；
...
- 供应商供货价变化不得直接修改公司商品池销售可见价或商城销售价；
- 供货价变化进入采购或运营审核队列；
- 在成本变化未确认前，相关商品默认暂停新订单，避免按失效成本继续销售；
- 审核通过后形成新的供应关系版本；只有商品池或发布内容变化时才分别形成新版本；
...
公司 SKU 最多一个商品池条目。新增第二供应商默认只追加该供应商 SKU 的映射和供给，不修改
公司商品内容或商品池修订。
