...
| **主责迁移** | 第二期上线时把已生效及之后状态、且未作废的正式存量卡券销售单的主责系统标记从福利商城改为 ERP 的一次性运营行为。商城草稿不迁移。数据上只改 `sales_order.owner_system` 字段并记录通用审计，不建设专用迁移表，不产生新单号 |
...
| **消费回流启用日** | P0、P1 同批上线并完成主责迁移后，ERP 开始实时接收商城关键业务事实的系统级时间点，记为 `T`。以支付成功事实的发生时间划分履约归属：`T` 之前支付成功的订单沿用原人工履约链，历史回填只记账、不触发供应商下单；`T` 及之后支付成功的订单进入 ERP 自动履约链。迁移时点后商城不再开设新的 B2B 销售单 |
...
| P2-C04 | 卡券销售审批、主责迁移与商城执行同步 | 销售提交、销售领导审批、运营审批、正式存量单一次性主责迁移、执行投影下发、销售变更单下发和接收确认 | 销售、销售领导、运营、系统管理员 |
...
商城草稿不进入迁移范围，不在 ERP 生成销售单；第二期上线时关闭商城 B2B 建单入口后，现存草稿统一作废，不在 ERP 补建。
...
4. 商城侧已完成商业字段只读化改造，B2B 销售单创建入口可在迁移时点关闭并在迁移后保持永久关闭；入口关闭后可统一作废现存草稿；
5. 商城已具备稳定卡实例引用、原销售单身份、初始余额和周期余额快照同步能力；
6. 迁移清单已由销售和财务确认；
7. 迁移时点后商城不再开单，本次切换范围内不再产生新的销售单新建或商业变更；
8. 迁移时点前完成最后一次第一期同步，以每张存量单当时的 ERP 当前销售单版本作为其第一份执行投影版本（见第 8.5.5 节）；所有会被实时或历史关键事实引用的卡实例均已完成稳定引用、原销售单身份和初始余额基线同步。

以上是本次人工切换的操作条件，不另建设自动化阶段退出指标或切换验收门槛。

#### 8.5.5 存量单的第一份执行投影版本

存量单迁移前完成最后一次第一期同步，全部迁移单的商城最终商业事实已经形成 ERP 当前版本，且客户、合同、卡券类目映射和唯一明细约束没有未处理差异。

主责迁移本身只把 `owner_system` 从 `MALL` 改为 `ERP`，不单独登记迁移基线、不产生新的销售单版本；每张存量单以迁移时点的 ERP 当前销售单版本作为第一份执行投影版本，用于迁移后的商城执行一致性校验。迁移不核对历史发卡、激活、消费和玩法执行过程；卡实例引用、原销售单身份和初始余额按第 8.4 节形成事实基线。

迁移后对存量单发起第一次销售变更时，ERP 以该第一份执行投影版本校验商城执行关系；后续变更以商城最后确认接收的执行投影版本校验。校验只覆盖第 8.2 节执行投影字段，不再通过第一期单据查询接口比较完整商业快照；成交金额、配赠、税率、开票要求和应收等 ERP 内部字段只以 ERP 当前版本为变更基线。执行投影范围不一致时阻断该次变更并生成差异任务，不直接覆盖任一侧正式事实。

#### 8.5.6 迁移执行与失败处理

- 第二期上线时执行一次性主责迁移；第一期五分钟轮询在迁移完成前保持运行，只用于形成存量单的 ERP 当前版本；
- 迁移时点先关闭商城 B2B 销售单创建入口并启用商业字段只读，之后商城不再开单，也不再产生本次切换范围内的新业务事实；
- 确认建单入口已经关闭且不会产生新草稿后，把商城现存 B2B 销售单草稿全部作废，不迁移、不在 ERP 补建；
- 存量单的任何商业变更都只能通过 ERP 销售变更单形成新版本，见第 8.3 节；
- 完成最后一次第一期同步后，一次性把全部存量单的主责系统标记从福利商城改为 ERP；迁移只改 `owner_system` 字段并记录通用审计（销售单、迁移时间、执行人和迁移前主责系统），以来源商城加来源销售单号为幂等依据；
- 任一张存量单迁移失败时，修复后以原幂等依据续跑该单，不把已经迁移的销售单回退为商城主责，也不恢复第一期轮询；
- 迁移完成后停止第一期五分钟轮询并封存为只读历史，启用 P0、P1 联合发布的完整链路，把开放时点记为 `T`，再开放 ERP 卡券销售单新建和商城员工业务；
- 商城 B2B 销售单创建入口在迁移后保持永久关闭，不设计应急重开或主责回退路径。
...
- 迁移时点后商城不再开单，切换范围内不再产生新的关键业务事实；`T` 附近实时与回填的重叠只可能来自同一既有事实的重复投递，两者按第 13.1 节的业务事实键去重，同一支付、取消、退款、订单完成或卡券余额恢复事实只形成一份正式记录；
...
| `sales_order_projection_revision` | 不可变执行投影版本 | 来源（存量单迁移时点版本或 ERP 版本）、ERP 销售单版本、销售单级卡券类目与履约期限、唯一明细的面额、数量和卡形态 |
| `sales_order_projection_delivery` | 执行投影发送与商城接收结果 | 投影版本、发送时间、商城接收结果 |

主责标记只在第二期上线迁移时变化一次，迁移后生命周期内不再切换。

主责迁移是第二期上线时的一次性运营行为：商城停止开设新的销售单，存量商城主责卡券销售单的 `owner_system` 由 `MALL` 改为 `ERP`。数据上只使用 `sales_order.origin_system`（最初创建入口）和 `sales_order.owner_system`（当前主责）两个字段，不建设专用迁移表；迁移只改 `owner_system` 字段并记录通用审计，不产生新的销售单版本。

执行投影版本与 ERP 销售单版本一一对应。销售单版本由销售单生效和后续销售变更单生效产生，不单独建模。

每张正式存量单以迁移时点的 ERP 当前销售单版本作为第一份执行投影版本；该动作不产生新的销售单版本。之后每个 ERP 销售单版本仍只对应一份新的执行投影版本。
...
| P2-P05 | 主责迁移 | 一次性运营行为：存量单 `owner_system` MALL→ERP + 迁移审计；不建页面（W24 已取消） | W05 来源/主责展示 |
| P2-P07 | 商城消费订单 | 关键事实、组合支付分摊、原销售单 | W25 |
| P2-P08 | 供应商订单 | 接单履约取消退款与异常补偿 | W26；可从 W25 钻取 |
| P2-P09 | API 供应商结算 | 成本汇总、差异、应付形成 | W27 → 复用供应商往来核销 |
| P2-P10 | 卡券消费与经营 | 消费台账、成本口径、经营分析 | W28 |
| P2-P11 | 接口错误与对账 | 重试、转人工、差异核对 | W29 详情内闭环 |
| P2-P12 | 历史消费回填 | 回填批次、去重与未归集清单 | W30 |
| P2-P13 | 卡券销售审批 | 领导/运营审批、驳回原因与记录 | 待办队列 M3 + 销售单中心 |
...
| 系统管理员 | 接口权限、能力配置、一次性主责迁移执行、错误中心和运行监控 |
| 上线负责人（项目指定） | 组织一次性主责迁移和切换协同，不替代销售、财务和系统管理员的业务职责 |
...
- 一次性主责迁移由系统管理员执行并留审计；销售和财务在迁移前确认存量单清单；
- 财务可以查看成本、结算和经营结果；
- 连接密钥不通过普通业务页面展示；
...
6. 迁移时点后商城不再开单，不再产生本次切换范围内的销售单新建或商业变更；
7. 全部已生效及之后状态、且未作废的正式存量卡券销售单的主责标记迁为 ERP；商城草稿已在建单入口关闭后、主责迁移前统一作废，不迁移、不在 ERP 补建；
8. 迁移前完成最后一次第一期同步；全部存量单均有 ERP 当前版本、完整映射和恰好一条卡券明细，以迁移时点的当前版本作为第一份执行投影版本；
9. 迁移只改变主责标记，不产生新单号，不改变已登记的应收、回款和发票；
10. 迁移后第一期轮询实际观察并形成的版本历史完整保留，并标注版本来源；
11. 任一张存量单迁移失败时，修复后以来源商城加来源销售单号幂等续跑；不回退已迁移单主责，不恢复第一期轮询；
12. 迁移完成后停止一期五分钟轮询并封存为只读历史，启用 P0、P1 完整链路并记录 `T`，再开放 ERP 卡券销售新建和商城员工业务；商城 B2B 建单入口永久关闭；
...
21. 对已迁移存量单发起第一次销售变更时，以迁移时点的 ERP 当前销售单版本（第一份执行投影版本）校验，后续校验商城最后已接收执行投影；只比较执行投影字段，不用商城旧快照校验 ERP 内部金额、税率、开票和应收字段；
...
9. 完成最后一次第一期同步，一次性执行主责迁移：把全部存量单的 `owner_system` 从 `MALL` 改为 `ERP`，只改字段并记录通用审计；
10. 全部迁移完成后停止第一期轮询，以时间点 `T` 开启支付、取消、退款、订单完成和余额恢复等不可变关键事实回流；
11. 固定供应关系下的供应商订单创建和状态同步。

P0-B 第 9 项必须在 P0 第 2 至第 4 项、第 8 项以及 P1 全部能力完成后执行，操作条件见第 8.5.4 节。迁移失败时修复后以原幂等依据续跑，不重开商城建单入口。
...
| P2-D29 | 存量单执行投影基准 | 存量单以迁移时点的 ERP 当前销售单版本作为第一份执行投影版本；不单独登记迁移基线，不建设自动化阶段退出或切换验收门槛 | 8.5.4、8.5.5 |
...
| P2-D37 | 二期上线切换 | P0、P1 同批上线；上线时执行一次性主责迁移，迁移时点后商城不再开单，迁移完成后停止一期轮询并封存为只读历史；失败修复后续跑，不重开商城或回退主责 | 8.5.4、8.5.6、18 章 |
