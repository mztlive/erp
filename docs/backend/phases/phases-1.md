...
- 幂等操作结果、请求追踪和一期统一 `outbox_message` 的内部可靠事件契约。
...
- 文件扫描通过且属于允许保留类型后才能关联正式对象；成功白名单资产、失败诊断、
  临时导出不得复用同一资产身份。
- 定义统一 `outbox_message` 的原子写入、唯一 `event_id` / `idempotency_key`、聚合修订追溯和投递状态语义；领域正式事实与内部可靠事件必须同事务提交。
- 一期 outbox 只授权工作台、预警和经营分析等 ERP 内部可重建投影消费；内部消费者按原 `event_id` 幂等推进水位，不能反向修改正式事实。
- 一期不注册外部目标、HTTP endpoint、webhook、商城或供应商 API 事件，不创建 `inbox_message`、`integration_attempt`、通用接口错误中心或通用接口对账；外部 dispatcher 和双向消息治理属于第二期。
...
8. outbox 写入失败使业务事务整体失败；内部投影重复消费不重复领域动作且按原水位恢复。
