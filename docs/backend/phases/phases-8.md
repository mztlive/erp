...
明确排除第二期的切换（T 时点）、ERP → 商城执行投影、卡实例、卡号/卡密、绑定/激活、
...
- 快照白名单只包含商业事实：状态、客户、合同、结算主体、卡券类目、
  履约期限、唯一卡券明细、金额、税率和开票要求。项目名称和业务备注随快照同步但
  不进入指纹。
...
- 更早快照直接丢弃且不回退当前版本；同一来源时间不同指纹必须标记来源冲突，
  不按到达顺序选胜者。
...
- W17 文档含第二期冻结/封存内容；本 phase 仅实现第一期运行态，第二期切换（T 时点）
  不属于本 phase。
