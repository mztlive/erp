...
- 来源报价、采购确认成本、销售可见价和未来商城发布价是四类不同价格事实。
...
6. 供给必须明确 `supply_mode`；现有最小入池表单和命令不足以唯一生成时，返回
   `SUPPLY_MODE_REQUIRED`，不能猜测默认供货方式。
...
7. SPU 多 SKU 提升入池、缺 `supply_mode`、错误 offering revision 全部 fail-closed。
