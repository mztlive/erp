...
1. 口径胶囊是筛选器：点击写入 URL 查询参数并筛选左列，不打开第二个页面、不跳路由；禁止独立 MetricStrip 统计卡；
2. 左列是唯一待办列表，跨领域混排，按任务类型页签收敛，承接原 W02 的全部查询能力；全页只渲染一份；
...
7. 移动端与窄屏下详情降级为全屏抽屉，返回后保留列表筛选与滚动位置；待办列表仍只渲染一份。
8. 无任务或筛选为空时主面板不拆左右栏，只保留一个空态。
...
- 工作台无团队分区：口径胶囊筛选不跳页，审批决定可在详情内连续提交；无独立统计卡，待办列表只渲染一份；
