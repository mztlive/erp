...
创建草稿对话框必须要求管理员显式选择“空白流程”或“复制当前已发布版本”，并分别提交 `draft_source=EMPTY|CURRENT_PUBLISHED`。当前无已发布版本时必须禁用复制选项；前端不得提交源定义 ID、任意历史版本或隐式默认值。`SalesOrder + EMPTY` 打开编辑器时必须渲染一个未保存的采购确认必填槽位；首次保存把它作为顺序第一节点提交，`node_purpose` 仍不得进入请求。
...
`SalesOrder` 草稿必须渲染服务端返回的唯一「采购确认」用途节点：允许修改名称、顺序和审批人，不允许删除、复制或改用途。其他类型不得显示该用途。`node_purpose` 只读展示且不得进入写请求；用途完整性以服务端发布校验为准。
...
14. `SalesOrder + EMPTY` 首次编辑显示采购确认必填槽位；首次保存后该用途节点不可删除、复制或改用途，其他类型不显示该用途，写请求不含 `node_purpose`。
...
- [ ] `SalesOrder` 必备用途节点受控显示，客户端不能提交或改写用途。
