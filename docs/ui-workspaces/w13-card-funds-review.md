...
| 复核通过 | 决策区 | 领取有效；证据完整；当前指纹、账户版本、票款事实版本与任务一致；登记事实已确认入账 | `FormalActionConfirmDialog` | 用共享完成信封追加新复核链尾、`workflow_action`、更新可重建缓存并完成任务，固定展示复核号后可自动下一项 | 失败保留当前项；结果未知查询原幂等操作，不跳下一项 |
