...
4. 发布新版本并持续看到商城确认结果；
5. 在供应停止、零库存、不可供、数据过期或成本/关键供给变化未确认时，直接看见系统已触发的安全暂停、商城投递以及已注册后续任务或强类型 follow-up blocker/证据；人工不承担首次暂停，且禁止发起从安全暂停到 `ON_SALE` 的恢复发布；
6. 从发布失败直接进入接口错误处理，不需要在多个技术菜单中寻找消息。

### 1.2 业务目标

- 以服务端已分配的 `product_publication.publicationId` 表达现有发布对象的稳定身份。禁止前端以 `SKU + 商城` 推导唯一键或创建新对象；禁止新建发布对象，列表必须固定返回 `PUBLICATION_IDENTITY_POLICY_UNCONFIRMED`。
...
- 供货价变化不得自动修改商城销售价；销售价、最小购买量和主动业务暂停必须由服务端权限/政策判定。销售价或销项税率变化且服务端复核政策未配置时必须 fail-closed，固定返回 `REVIEW_POLICY_UNCONFIGURED`；无价格/税率变化也不得由前端跳过服务端政策。任何从安全暂停转为 `ON_SALE` 的提交必须固定返回 `RECOVERY_RESPONSIBILITY_UNCONFIRMED`。
- `STOPPED`、零库存、不可供、可供数据过期，以及成本或其它关键供给变化未确认时，系统对受影响在售发布实行 fail-closed：立即幂等形成不可变暂停修订/明确暂停动作和投递记录；人工任务不阻塞首次暂停。

### 1.3 不在本工作面完成

- 不同步供应商供给、不修改供给身份；进入 W21。
...
| 角色 | 默认入口 | 可见范围 | 主要动作 |
| --- | --- | --- | --- |
| 运营 | 发布列表 | 被授权商城、类目和业务组织内的现有发布对象 | 按发布编号查看/维护现有发布、准备普通新版本、按政策提交发布、暂停下单、查看投递；不得创建新发布对象，不得恢复安全暂停对象 |
| 采购 | W21，可进入发布详情 | 其负责供应商或供给关系关联的发布 | 查看固定供给、处理供给阻塞；不改销售价与销售文案 |
| 财务 | 按需从商品或经营页面进入 | 被授权组织的价格、税率与发布历史 | 只读核对价格及税口径；不得代替服务端 `publishGate` 判定复核，不得在本工作面执行发布确认 |
...
- 安全暂停由供给领域事件触发，不是运营页面按钮，也不等待运营、采购或管理员先人工介入。触发范围固定为：供给关系 `STOPPED`、可供数量为零、明确不可供、可供数据超过服务端新鲜度阈值，以及供货价、进项税率、费用、MOQ、区域或商品能力变化尚未确认。
...
对象中心 TaskTabs 身份为 `publication:{publicationId}`，标题为 `发布 · {SKU编码}`。同一稳定发布对象重复打开只聚焦原页签；版本切换不产生新任务页签。会话内编辑存在时页签显示脏标记，关闭或刷新前必须明确提示“输入将丢失”。发布工作副本仅允许 TaskTab 会话内编辑；禁止服务端草稿保存 mutation、自动保存、`localStorage` / `IndexedDB` 持久化及 TanStack Query 持久缓存。
...
| 安全暂停 | `safetyPause` | 安全暂停 | 暂停操作、后续任务或强类型 follow-up blocker 查询投影 | 原因、提交时间、来源对象（业务名，由调用方解析）；`SUPPLIER_STOPPED` 显示唯一后续任务，其它已落库原因显示与 cause 匹配的唯一 blocker；不展示内部操作号/来源 ID/信封号 | 业务摘要按对象权，技术错误脱敏 |
...
| 来源深链 | 无 | `skuId` / `supplierOfferingRevisionId` | 从供应商供给等进入时静默过滤；页内以共享 `FilterChip` 展示业务名（SKU 编号 / 供应商名）并可单独移除，「清除筛选」一并清除 |

- 指标点击使用按钮语义、选中态和筛选摘要；切换指标会清除发送/发布状态维度避免空结果陷阱；浏览器后退恢复上一筛选。
- 分页 `page` 写回 URL，筛选变更自动回第一页。
- 「清除筛选」只清筛选参数（含来源锁定 `skuId`/`supplierOfferingRevisionId`）并回第 1 页，不清导航上下文；预览 Sheet 为本地状态，不随清除筛选变化。
...
| --- | --- | --- | --- | --- | --- |
| 新建发布对象 | 列表页头 | 入口必须强制禁用；列表响应必须含 `creationBlocker` | 不打开确认框 | 无创建命令；固定显示 `PUBLICATION_IDENTITY_POLICY_UNCONFIRMED` | 保持阻断；禁止用 `SKU + 商城` 在前端去重或创建；仅当服务端不再返回该 blocker 且提供稳定创建命令时可启用 |
...
| 提交发布 | 编辑区主动作 | `PUBLISH`；必填字段、主图、唯一供给、类目和有效期校验通过；媒体安全检查状态只作治理记录；服务端返回 `publishGate.kind=READY` | `FormalActionConfirmDialog` 展示商城、价格、状态、供给、生效时间和复核政策结论 | 形成不可变发布修订；固定展示版本号与“等待商城确认” | 销售价/销项税率变化且政策未配置时返回 `REVIEW_POLICY_UNCONFIGURED`；安全暂停恢复返回 `RECOVERY_RESPONSIBILITY_UNCONFIRMED`；结果未知时查询原请求，不创建新版本重试 |
...
- 安全原因仍有效时，服务端必须阻断任何 `ON_SALE` 提交；来源重新可用只移除恢复 blocker 的一部分，不自动创建上架版本。即使供给已有效，任何从安全暂停恢复 `ON_SALE` 的提交仍必须固定返回 `RECOVERY_RESPONSIBILITY_UNCONFIRMED`；只允许安全暂停或人工暂停继续执行。
- 结果未知时先查询原请求或商城确认，不以再次点击“发布”解决。
- 商城业务拒绝、鉴权失败和字段映射错误直接转人工；临时网络或限流才按策略自动重试。商城确认 SLA 与自动重试上限必须按错误类别由服务端配置，页面必须展示服务端返回的下次动作时间，禁止前端自定 SLA 或重试上限。
...
列表响应的 `creationBlocker` 必须固定为 `PUBLICATION_IDENTITY_POLICY_UNCONFIRMED`，创建入口必须禁用；已有行仅按服务端 `publicationId` 查看和维护，禁止前端假定 `SKU + 商城` 唯一键。Query Key 必须包含当前用户、角色、数据范围版本及全部筛选。
...
- 发布工作副本仅保留 TaskTab 会话内输入；禁止服务端草稿保存 mutation、自动保存和本地持久存储；刷新/关闭前必须明确提示输入将丢失。
...
- 可在表单内提示必填和图片数量，但服务端必须再次校验唯一固定供给、类目映射、有效期和权限；安全检查状态不得参与发布门禁。
...
| 空数据 | “尚无商品发布”，同时展示 `PUBLICATION_IDENTITY_POLICY_UNCONFIRMED` | 无可用新建动作 | 保持阻断；仅当服务端不再返回该 blocker 且提供创建命令后重查 |
| 筛选无结果 | 展示筛选摘要 | 清除筛选 | 恢复默认视图 |
| 无数据范围 | 不展示 0 指标和对象内容 | 查看当前角色/申请范围 | 权限更新后重查 |
| 查询失败 | 有缓存则保留并标记失败；无缓存显示 `BusinessFailureState` | 重试 | 查询成功 |
| 数据陈旧 | 显示查询更新时间；投递状态超过服务端返回的 SLA/下次动作时间时提示 | 刷新、查看 W29 | 服务端状态追平；SLA 与重试上限仅以服务端配置为准 |
...
| 新建已阻断 | 新建入口禁用并显示 `PUBLICATION_IDENTITY_POLICY_UNCONFIRMED`；已有对象可按 `publicationId` 维护 | 查看/维护已有对象；禁止新建 | 保持阻断；仅当服务端不再返回该 blocker 且提供创建命令后重查 |
| 复核政策未配置 | 销售价/销项税率变化时显示 `REVIEW_POLICY_UNCONFIGURED`，提交禁用 | 查看 diff 和责任提示；禁止发布 | 仅当服务端政策已配置且 `publishGate` 重算为可发布后重查 |
| 恢复已阻断 | 安全暂停对象的 `ON_SALE` 编辑/提交显示 `RECOVERY_RESPONSIBILITY_UNCONFIRMED` | 只允许查看、查询投递和安全/人工暂停；禁止恢复 | 保持阻断；仅当服务端 `publishGate` 不再返回该 blocker 时可恢复 |
| 发布提交中 | 防重复提交，显示请求编号 | 查询请求状态 | 明确成功/失败后结束 |
| 发布成功、待商城确认 | 固定结果区显示发布版本和投递编号 | 查看投递、继续其它工作 | 商城确认后刷新状态轨 |
| 安全暂停已落库 | `COMMITTED` / `ALREADY_SAFE` 高风险 Alert 展示固定原因、来源对象（业务名）与提交时间（不展示内部 ID）；`SUPPLIER_STOPPED` 强制显示唯一 `followUpWorkItem`，其它原因强制显示唯一 `followUpBlocker`；本地已不可下单 | 查询投递；有已注册任务时前往供应商供给核对，其它原因只查看 blocker/说明；禁止直接恢复 | 商城确认暂停后更新投递轨；本地必须始终保持暂停，直至服务端允许恢复 |
...
| 供应商供给 | W21 | `supplierOfferingRevisionId`、`skuId`、阻塞原因 | 处理后回 W22 重新校验，不自动提交 |
...
- [x] 类目、销售说明、最小购买量和至少一张主图由 mock API 按服务端契约校验；安全检查状态只展示，不参与校验。
- [x] 最小购买量不会从供应商最小订购量自动复制。
- [x] 供货价变化不会自动修改商城销售价。
- [x] 图片、固定供给、价格或销售状态变化均形成新修订，不覆盖历史。
- [ ] 已支付订单仍可钻取其下单时发布修订，后续修订不改变历史快照。
- [x] `STOPPED`、零库存、不可供、数据过期或成本/关键供给变化未确认时，mock 领域事件处理器不等待人工即执行安全暂停；来源恢复也不会自动上架。
- [x] 发布工作副本仅允许 TaskTab 会话内编辑；禁止草稿保存 mutation、自动保存与本地持久化；刷新或关闭前必须明确提示输入将丢失。
- [x] 列表固定返回 `PUBLICATION_IDENTITY_POLICY_UNCONFIRMED` 并禁止新建；已有对象仅按服务端 `publicationId` 查看/维护；禁止前端假定 `SKU + 商城` 唯一键。
...
- [x] 商城成功确认前不显示为“商城已生效”。
- [x] 商城确认 SLA 与自动重试上限按错误类别由服务端配置，页面展示服务端返回的下次动作时间，前端不得自定。
...
- `erp-phase-2.md` §7：供应商供给同步、条款/可供变化和 ERP 向商城发布。
