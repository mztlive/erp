...
- 角色池任务是否计入“待我处理”是待确认事项 Q1；确认前服务端须分别返回个人任务数与角色池任务数。
...
| 指标 | `assignedActiveCount` | 待我处理 | 服务端聚合 | 当前权限快照内的有效任务数 | 不用加载列表求和 |
| 指标 | `dueTodayCount` | 今日到期 | 服务端聚合 | 当前工作时区今日结束前到期且未完成 | 与 Q2 的时区决策一致 |
...
| 内容 | 数据来源 | 展示规则 | 操作 |
| --- | --- | --- | --- |
| 预警 | 服务端预警统计 | 最多展示 5 条；按严重度和发生时间排序 | 打开对应 Wxx，不在卡片内直接修复 |
| 数据陈旧提示 | 统计数据更新时间 | 超过 1 分钟显示 warning；刷新失败显示 failed | 刷新或进入后台任务中心 |
| 最近打开 | 当前用户 TaskTabs 最近记录 | 最多 6 项；仅保存稳定对象身份、标题和最近时间 | 聚焦已有页签或重开对象页签 |

“最近打开”不是审计记录，不影响业务事实，也不得包含敏感字段快照。存储位置见待确认事项 Q3。
...
| 最近打开 | 非正式 UI 偏好 | 只保存稳定身份与展示标题，不缓存敏感业务内容 |
...
| ID | 问题 | 影响 | 建议决策人 | 当前建议 |
| --- | --- | --- | --- | --- |
| Q1 | 未分配到个人、仅分配给角色池的任务是否计入“待我处理”？ | 指标口径、默认列表和领取方式 | 业务负责人 + 权限负责人 | 分开展示个人任务与角色池任务；默认待我处理只含个人任务 |
...
| Q5 | 每个任务族的 `pagePreviewLimit` 应如何按实际峰值配置？ | 首屏密度和 W02 导流 | 各业务部门 | 由服务端配置每组预览上限；未确认前仅以 5 条作安全的临时展示参数，精确条数不进入验收硬约束，再用真实任务数量校准 |
