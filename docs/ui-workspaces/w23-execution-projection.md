...
- 不领取、移动确认或完成 `work_item`；W01/W02 的正式错误待办统一进入 W29，W23 只执行投递对象级查询、重试和升级动作。
- 不处理商品发布；进入 W22。
...
| 处理投递对象 | W23 失败指标、W05 协同摘要或 W29 证据链接 | 打开 W23 投影对象并定位 `deliveryId`；不接收任务租约参数 | 返回原筛选、W05 或 W29 上下文 |
| 处理正式错误待办 | W01 / W02 | 统一打开 W29 并携带 `workItemId`；不直接路由 W23 | 由 W29 完成或转交后回原任务队列 |
...
| `workItemId` / `errorTaskId` | 正式待办 / 错误对象 | W29 关联 | 仅显示稳定引用、责任方、状态和 W29 入口；W23 不领取或完成任务 |
...
- Q3 未确认时不提供“当前筛选全部”；批量操作只接收用户逐项显式勾选且仍在当前授权结果中的稳定 ID，并受服务端保守数量上限约束。服务端策略确认后，才可增加冻结全筛选目标集合的 `bulk_selection_snapshot`。
...
| 批量查询结果 | 选择栏 | `BULK_QUERY`；仅逐项显式选择且服务端数量上限内 | `BatchImpactPreview` | 后台任务逐项查询并汇总 | 显示成功/仍未知/跳过/失败；全筛选选择在 Q3 未确认时拒绝 |
| 批量重试 | 选择栏 | `BULK_RETRY`；仅逐项显式选择、明确可重试且服务端数量上限内 | 预览状态、商城和数量 | 后台任务沿各项原幂等键执行 | 逐项保留原因，不改变销售事实；全筛选选择在 Q3 未确认时拒绝 |
| 升级到 W29 | 对象中心 | `ESCALATE`；已超自动上限或明确人工错误 | 说明责任队列与影响 | 幂等创建或关联 W29 错误对象及正式 `work_item`，只返回入口 | 重复请求返回既有对象与任务；后续领取、处理和完成均在 W29 |
...
- `ESCALATE` 只登记升级证据，并由服务端按固定类型幂等创建 `work_item`；W23 不提供领取、续租、转交、关闭或完成任务动作。
...
- `QUERY_RESULT`、`RETRY`、`ESCALATE` 都是投递对象级命令，不携带 `claimToken`、`leaseVersion` 或任务完成决定。
- `ESCALATE` 使用稳定 `idempotencyKey` 幂等创建或复用 W29 错误对象，并由服务端从已注册类型中选择 `INTEGRATION_RESULT_UNKNOWN` 或 `BUSINESS_EXCEPTION` 创建正式 `work_item`；错误对象、升级证据与任务引用同一事务落地。W23 只读取返回的 `workItemId` / `errorTaskId` 并打开 W29。
- Q3 未确认时，批量动作只冻结本次显式选择的稳定 ID、对象版本与授权水位，再以唯一 `requestId` 注册 `background_job`；请求不得表达“当前筛选全部”。服务端确认全筛选治理规则后，才可增加对应选择快照分支。逐项结果可为成功、仍未知、跳过或失败。
...
- 页面同时显示销售来源事实、投影生成和投递状态三个水位；任一水位陈旧时只标记对应分区，不用新投递状态覆盖旧投影版本。
...
| 已升级人工 | 显示只读 `workItemId`、W29 错误对象和责任队列 | 打开 W29；W23 不领取或完成 | W29 返回后刷新投递明确结果 |
...
W23 与 W29 的任务边界固定：W23 只执行 `QUERY_RESULT`、`RETRY`、`ESCALATE` 三类投递对象动作；`ESCALATE` 可以创建正式待办，但 W01/W02 的领取、移动确认和完成统一由 W29 的任务处理器承接。
...
- [ ] W01/W02 正式错误待办统一打开 W29；W23 不接收任务租约，也不领取、移动确认或完成任务。
- [ ] `ESCALATE` 幂等返回既有或新建的 `workItemId` / `errorTaskId`，后续处理只在 W29 发生。
...
| Q2 | 运营与系统管理员分别可对哪些投递错误执行对象级 `QUERY_RESULT`、`RETRY` 或 `ESCALATE`？ | W23 `allowedActions`、升级责任队列和审计；不决定 W29 任务领取/完成权限 | 运营负责人 + 系统负责人 | 业务拒绝由运营查询或升级；鉴权/连接由运维查询或升级；映射差异直接升级到 W29 对应责任队列 |
| Q3 | W23 是否允许“当前筛选全部”批量重试，还是只允许逐页选择？ | 高风险批量影响与后台任务规模 | 运维负责人 | 未确认前仅允许逐项显式选择并受服务端保守上限约束；若未来开放全筛选，必须由服务端冻结目标快照，结果未知项仍只查询、不直接重试 |
| Q4 | 商城执行基线稳定引用对销售和运营展示到什么粒度？ | 字段权限和排障能力 | 商城负责人 + 安全负责人 | 展示策略未配置时只返回不可复制完整值的短引用；配置后也仅按服务端字段策略向明确授权的排障角色开放完整值 |
