...
| 等待时长 | 全部 | `latency` | 正常、接近超时、已超时；阈值来自服务端配置 |
...
| 批量重试 | 选择栏 | `BULK_RETRY`；仅逐项显式选择、明确可重试且服务端数量上限内 | 预览状态、商城和数量 | 后台任务沿各项原投递身份执行 | 逐项保留原因，不改变销售事实；全筛选选择在 Q3 未确认时拒绝 |
| 升级到 W29 | 对象中心 | `ESCALATE`；已超自动上限或明确人工错误 | 说明责任队列与影响 | 按稳定身份创建或复用 W29 错误对象及正式 `work_item`，只返回入口 | 重复请求返回既有对象与任务；后续领取、处理和完成均在 W29 |
...
- `ESCALATE` 只登记升级证据，并由服务端按固定类型唯一创建 `work_item`；W23 不提供领取、转交、关闭或完成任务动作。
...
- Q3 未确认时，批量动作只冻结本次显式选择的稳定 ID、对象版本与授权状态，再以唯一 `requestId` 注册 `background_job`；请求不得表达“当前筛选全部”。服务端确认全筛选治理规则后，才可增加对应选择快照分支。逐项结果可为成功、仍未知、跳过或失败。
...
- [x] W01/W02 正式错误待办统一打开 W29；W23 不参与任务领取与完成。
