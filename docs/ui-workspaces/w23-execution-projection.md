...
- 不开始处理、移动确认或完成 `work_item`；W01/W02 的错误待办统一进入 W29，W23 只执行投递对象级查询、重试和升级动作。
- 不处理商品发布；进入 W22。
...
| `workItemId` / `errorTaskId` | 待办 / 错误对象 | W29 关联 | 仅显示稳定引用、责任方、状态和 W29 入口；W23 不开始处理或完成任务 |
...
| 升级到 W29 | 对象中心 | `ESCALATE`；已超自动上限或明确人工错误 | 说明责任队列与影响 | 按稳定身份创建或复用 W29 错误对象及 `work_item`，只返回入口 | 重复请求返回既有对象与任务；后续开始处理、领域动作和完成均在 W29 |
...
- `ESCALATE` 只登记升级证据，并由服务端按固定类型唯一创建 `work_item`；W23 不提供开始处理、转交、关闭或完成任务动作。
...
| 已升级人工 | 显示只读 `workItemId`、W29 错误对象和责任队列 | 打开 W29；W23 不开始处理或完成 | W29 返回后刷新投递明确结果 |
...
W23 与 W29 的任务边界固定：W23 只执行 `QUERY_RESULT`、`RETRY`、`ESCALATE` 三类投递对象动作；`ESCALATE` 可以创建待办，但 W01/W02 的开始处理、移动确认和强类型完成统一由 W29 的任务处理器承接。
...
- [x] W01/W02 错误待办统一打开 W29；W23 不参与开始处理与完成。
