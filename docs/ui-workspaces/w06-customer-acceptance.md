...
- 不提供独立顶层验收列表作为唯一入口。跨单查找必须使用 W05 销售单筛选。普通客户验收必须固定注册一种 `work_item_type` 并按销售单聚合任务；进入后按销售明细与履约事实分配。仅当该类型与粒度已写入 `erp-data-model.md` 注册表且 API 契约同步后，才允许从 W01/W02 汇聚到本工作面；注册前禁止启用 W01/W02 验收入口。

## 2. 用户、权限与数据范围

| 角色 | 默认入口 | 可见范围 | 主要动作 |
| --- | --- | --- | --- |
| 负责销售 / 协作销售 | 必须从 W05 履约子区进入；仅当正式验收任务已注册后可从 W01/W02 进入 | 自己负责或协作客户的销售单及授权履约事实 | 新建验收草稿、保存、过账、发起异常处理 |
...
| 待办处理（注册前禁用） | W01/W02 的已注册客户验收任务 | 仅当注册表与 API 生效后，同一路由追加 `workItemId`、可选 `salesLineId`；注册前直接访问必须显示配置 blocker，禁止降级成直接过账 | 完成后回来源队列或定位下一待办 |
...
| 销售明细 | 全部 | `salesLineId` | 从 W09 进入时定位明细；仅当任务已注册后可从 W02 定位；无权 ID 不得泄露存在性 |
...
| 验收任务类型/粒度未登记 | W01/W02 不展示验收入口；直接携带 `workItemId` 时显示配置 blocker，不返回任务或敏感对象 | 去 W05 按销售单直接登记 | 仅当固定类型与粒度写入 `erp-data-model.md` 注册表且 API 契约同步并上线后，才启用 W01/W02 与 `WORK_ITEM` 契约 |
...
| --- | --- | --- | --- |
| 今日工作台 / 统一待办（注册前禁用） | W01 / W02 | 仅当固定类型与粒度已注册后传 `workItemId`、销售单 ID、可选销售明细 ID；注册前禁止生成或展示普通验收任务 | 事实与任务同事务完成后刷新来源队列；注册前必须从 W05 直接进入 |
...
- [x] 当前用户从 W05 进入无需再次搜索销售单；仅当正式类型与粒度写入 `erp-data-model.md` 注册表且 API 同步后才开放 W01/W02 待办入口。
...
- [x] 固定类型与粒度未写入注册表且 API 未同步前，W01/W02 验收入口与 `WORK_ITEM` 提交 fail-closed；不得用 `BUSINESS_EXCEPTION` 或页面私有码代替。
