...
- 不提供独立顶层验收列表作为唯一入口。跨单查找必须使用 W05 销售单筛选。普通客户验收必须固定注册一种 `work_item_type` 并按销售单聚合任务；进入后按销售明细与履约事实分配。仅当该类型与粒度已写入 `erp-data-model.md` 注册表且 API 契约同步后，才允许从 W01 汇聚到本工作面；注册前禁止启用 W01 验收入口。

## 2. 用户、权限与数据范围

| 角色 | 默认入口 | 可见范围 | 主要动作 |
| --- | --- | --- | --- |
| 负责销售 / 协作销售 | 必须从 W05 履约子区进入；仅当正式验收任务已注册后可从 W01 进入 | 自己负责或协作客户的销售单及授权履约事实 | 新建验收草稿、保存、过账、发起异常处理 |
...
- 普通客户验收必须固定注册一种 `work_item_type`，任务按销售单聚合；进入后按销售明细与履约事实分配。在该类型与粒度写入 `erp-data-model.md` 注册表且 API 契约同步前，W01 验收入口、`workItemId` 路由和 `WORK_ITEM` 正式提交必须 fail-closed，只允许从 W05 直接对象过账。
- 不得用 `BUSINESS_EXCEPTION` 或页面私有码伪装普通验收任务。仅当固定类型与粒度写入 `erp-data-model.md` 注册表且 API 契约同步后，才可启用 W01。
- 启用后，页面必须携带 `workItemId`；任务创建时必须指定到具体用户，当前用户必须是 `owner_user_id`，否则 fail-closed。正式提交必须使用验收强类型命令，不得在验收过账后再单独调用“完成待办”。
...
| 待办处理（注册前禁用） | W01 的已注册客户验收任务 | 仅当注册表与 API 生效后，同一路由追加 `workItemId`、可选 `salesLineId`；注册前直接访问必须显示配置 blocker，禁止降级成直接过账 | 完成后回来源队列或定位下一待办 |
...
```text
┌ （挂 W05）DocumentHeader compact：客户 · 销售单号 · 主状态 · 三轨进度 ───┐
├ 概览 | 明细 | 履约与验收（选中）| 票款 | 变更与异常 | 审计 ─────────────┤
│ 数据更新时间 · 待验收 3 批 / 128 件                    [退出登记] [保存草稿] │
├────────────────────────────────────────┬───────────────────────────────┤
...
| 区域 | 目的 | 主组件 | 是否固定 |
| --- | --- | --- | --- |
| 销售单头 | 保留客户承诺、主状态和多轨进度上下文 | W05 `PageHeader object-chrome` + `DocumentHeader density="compact"` `StatusTrackSummary` | 顶部 sticky |
| 子区导航 | 在同一对象中心切换履约、票款和异常 | W05 锚点导航 | 顶部 sticky |
| 待验收摘要 | 展示服务端汇总与数据更新时间 | `MetricStrip` `DataFreshness` | 否 |
...
| 单据头 | `customerName` | 客户 | 销售版本客户快照 | 不追随基础资料静默变化 | 按字段权限掩码 |
...
| 摘要 | `factsUpdatedAt` | 数据更新时间 | 正式履约/验收查询更新时间 | 绝对时间 | 全部对象查看者 |
...
| `evidenceAttachmentId` | 客户签收或验收凭证 | `customer_acceptance_line.evidence_attachment_id` | 类型、大小与安全状态由附件服务校验；过账前必须按业务类型执行证据规则；拒收与服务不通过默认必须上传客户签收单、邮件或服务确认附件，否则禁止过账 |
...
| 销售明细 | 全部 | `salesLineId` | 从 W09 进入时定位明细；仅当任务已注册后可从 W01 定位；无权 ID 不得泄露存在性 |
...
| 保存草稿 | 动作栏 / 自动保存 | 草稿编辑权有效，`draftVersion` 匹配 | 无 | 返回新版本与保存时间 | 保留输入；冲突时不覆盖服务端草稿 |
| 确认并过账验收 | 底部主动作 | `POST_ACCEPTANCE` 可用；守恒、附件证据规则和版本校验通过 | `FormalActionConfirmDialog` 展示单据、各结果数量及异常影响 | 形成正式验收、分配和工作流审计；固定结果显示验收单号与剩余量；短少/拒收/服务不通过时必须生成强提醒待办，禁止在过账事务内自动创建异常处理草稿 | 失败保留草稿；结果不确定停留并查询最终结果 |
| 创建后续异常处理 | 短少/拒收/服务不通过结果区 | 服务端返回对应 `allowedAction`；用户已确认影响范围 | 展示原验收与影响范围，用户确认后再建单 | 进入 W05“变更与异常”预填原销售单、验收单与影响行；不得由验收过账自动创建处理单 | 创建失败不回退验收事实，可从结果区重试 |
| 冲正误录验收 | 历史详情 | `REVERSE_ACCEPTANCE` 可用；原验收已过账且未被完整冲正；高影响冲正必须已进入复核队列且复核通过 | 强确认原事实、影响分配与理由；高影响冲正禁止普通经办单人完成 | 新增反向验收及 `REVERSE` 分配；原验收保留 | 不确定时查询反向事实；不得本地标记已冲正 |
...
短少、拒收和服务不通过的过账只记录客户验收结论。界面不得把它表述成“已退货”“已退款”“已扣库存”或“已减少应收”。过账成功后必须先生成强提醒待办；仅当用户确认影响范围后，才允许创建异常处理单，禁止验收过账事务内自动创建处理草稿。
...
Query Key 至少包含用户、当前角色、权限版本、数据范围版本、销售单稳定 ID、当前版本、子视图和筛选。固定 `work_item_type` 与粒度未写入 `erp-data-model.md` 注册表且 API 未同步前，查询不得接受 `workItemId`，不得返回 `workItem` 处理信息。正式履约与验收事实事务内同步可见；不得用一分钟级分析投影驱动过账。
...
当前只启用从 W05 直接登记的 `DIRECT_OBJECT` 分支。仅当固定 `work_item_type` 与按销售单聚合粒度已写入 `erp-data-model.md` 注册表且 API 契约同步后，才允许启用 `WorkItemPostCustomerAcceptanceCommand`。注册前服务端必须拒绝 `WORK_ITEM` 分支，前端不得显示 W01 验收入口，也不得把失败请求降级成直接过账。启用后，从 W01 正式待办进入只允许 `WORK_ITEM` 分支，任务必须已指定给具体用户且当前用户必须是 `owner_user_id`；服务端必须校验 `workItemId`、查询返回的 `expectedTaskVersion`、任务针对的 `expectedSubjectVersion` 以及验收自身版本，不得降级成仅传任务 ID 的命令。
...
网络超时后先查询原结果；重复提交由服务端唯一约束返回同一结果；未确认前不关闭草稿、不推进本地进度、不自动进入下一任务。
...
| 验收任务类型/粒度未登记 | W01 不展示验收入口；直接携带 `workItemId` 时显示配置 blocker，不返回任务或敏感对象 | 去 W05 按销售单直接登记 | 仅当固定类型与粒度写入 `erp-data-model.md` 注册表且 API 契约同步并上线后，才启用 W01 与 `WORK_ITEM` 契约 |
| 查询失败且无缓存 | `BusinessFailureState` | 重试、返回 W05 | 重试成功 |
| 查询失败但有缓存 | 保留旧内容并标“刷新失败” | 查看；正式过账前必须重验 | 重试成功 |
| 数据陈旧 | 展示事实更新时间；禁用正式过账 | 刷新 | 获取当前版本后恢复 |
...
| 375×812 | 仅当单一来源、无差异（无短少/拒收/服务不通过）且证据规则已满足时，允许简单“整批通过”；否则只读查看并提示转桌面 | 任务身份、来源、数量、结果查看 | 多来源分配、短少/拒收复杂登记、冲正禁止在手机完成 |
...
| --- | --- | --- | --- |
| 今日工作台 / 统一待办（注册前禁用） | W01 | 仅当固定类型与粒度已注册后传 `workItemId`、销售单 ID、可选销售明细 ID；注册前禁止生成或展示普通验收任务 | 事实与任务同事务完成后刷新来源队列；注册前必须从 W05 直接进入 |
| 销售单对象中心 | W05 | 同一销售单页签、`section=acceptance` | 退出编辑回 W05 履约阅读态 |
| 履约处理 | W09 | 履约事实类型/ID、销售单 ID、来源任务 | W09 页签保留；验收后其事实不被修改 |
...
- [x] 当前用户从 W05 进入无需再次搜索销售单；仅当正式类型与粒度写入 `erp-data-model.md` 注册表且 API 同步后才开放 W01 待办入口。
- [x] W06 没有平行顶层路由和脱离销售单上下文的唯一入口。
- [x] 一次验收可分配多个履约批次，同一履约批次可被多次验收。
- [x] 1440×900 下销售单身份、至少两条履约来源、本次验收摘要和主动作同屏可见。
- [x] 短少、拒收和服务不通过结果明确说明“仅记录验收事实”，不暗示库存/票款已处理；过账后仅生成强提醒待办，禁止在过账事务内自动创建异常处理草稿。

### 12.2 数据、权限与正式动作

- [x] 页面所有字段能追溯到销售版本、履约事实、验收事实、正式投影或权限结果。
- [x] 可验收量和履约完成采用服务端净事实，前端不按表头状态推断。
- [x] 固定类型与粒度未写入注册表且 API 未同步前，W01 验收入口与 `WORK_ITEM` 提交 fail-closed；不得用 `BUSINESS_EXCEPTION` 或页面私有码代替。
...
- [ ] 注册后，从 W01 进入的验收过账使用强类型命令，逐行校验同销售明细、净数量上限、任务对象版本、权限和当前责任人。
...
- [x] 正式成功固定展示验收单号；超时不得误报成功或重复过账。
- [ ] 刷新、后退和从 W09 返回均恢复销售单、子区与草稿；启用任务入口后还必须恢复来源任务。
