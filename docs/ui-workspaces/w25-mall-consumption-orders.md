...
7. 每笔消费当前成本口径是实际、标准还是无成本（枚举原值不直接上屏），而不把缺失成本显示为零；
8. 遇到下单失败或结果未知时，从当前订单直接进入 W26/W29 处理，同时保留“商城支付已发生”的事实。

### 1.2 业务目标

- 本页是商城消费记录的只读快照，不是商城可变订单的实时副本，也不是第二个商城订单写入口（用户提示不出现 W25 编号与“追溯视图”等实现词）。
...
- 第一份有效 `PAYMENT_SUCCEEDED` 必须无条件在一个领域事务内保存不可变支付事实、唯一 `mall_order` 及可保存的订单/商品/支付来源/分摊来源快照；客户、卡实例、商品映射或供给暂缺只让归集待处理并产生差异，绝不能造成“有支付事实、无商城订单”。归集条件齐全时形成消费归属；`T` 后固定供给完整时再以同一正式处理事务形成确定性供应商子订单、首个 `PLACE` 动作。消息投递、重试和分析投影才异步执行。
- 卡实例基线缺失或来源暂未归属属于待归集差异；已存在卡实例的原销售单或初始余额与新来源冲突时，必须保留原基线并进入 `FINANCE_CORRECTION_REVIEW`，经复核只追加纠错，不能由运营直接覆盖。

### 1.3 不在本工作面完成

- 不创建、编辑、取消或删除商城员工订单；商城继续负责可变订单与员工端进度。
...
| 收货人、手机号、地址 | 默认掩码 | 仅当前责任供应商子订单履约所需角色可在 W25 短时揭示并记录审计；其余角色禁止在 W25 揭示，必须从 W26 受控查看；日志和导出不得含未授权完整值 |
...
| --- | --- | --- | --- | --- | --- |
| 身份 | `mallOrderId` / `externalOrderNo` | 商城订单 | `mall_order` | ERP 订单编号 + 商城单号 | 有订单范围可见 |
...
| `ORDER_CANCELED` | 订单已取消 | 售后请求 ID、取消版本、范围、数量/金额、原因 | 商城取消结果事实 | 与支付事实分轨，显示接收进度 | 客服/运营按订单范围；原因字段分权 | 只记录取消结果，不冲减消费或资金 |
...
- 同一稳定卡实例已经存在基线，但新来源声称的原销售单或初始余额不同，页面显示“基线冲突，禁止覆盖”；领域服务保留原基线，创建 `FINANCE_CORRECTION_REVIEW`，财务按对象版本和证据复核后追加 `mall_card_instance_correction`。

### 5.5 供应商履约协同

| 情形 | W25 表现 | 数据来源 / 新鲜度 | 权限规则 | 下一步 |
| --- | --- | --- | --- | --- |
| `paidAt < T` | “原人工履约链”；明确历史回填只记账 | 支付事实、唯一 `T` 与回填来源；显示各自更新时间 | 对象可见即显示链路类型 | 不创建 W26 子订单；售后仍按结果事实回流 |
| `paidAt >= T` 且发布/供给完整 | 展示按供应商确定性拆分的一个或多个子订单 | 支付时固定的发布/供给快照与 W26 当前摘要 | 供应商成本按字段权 | 钻取 W26 查看接单、履约、取消、退款 |
| `paidAt >= T` 但商品、发布或供给缺失 | 支付事实保留，显示“自动履约条件不足” | 原支付事实与 W21/W29 blocker 状态 | 技术摘要脱敏 | 进入 W29 / W21 修复，使用原事实归集 |
...
| 能力 | 默认值 | URL 状态 | 行为 |
| --- | --- | --- | --- |
| 搜索 | 空 | `q` | 商城订单号精确优先；客户、ERP 销售单、供应商子订单按权限搜索；防抖 300ms 即时生效 + Enter 兜底 + `/` 聚焦，**不设**独立「搜索」按钮 |
| 来源商城 | 全部有权商城 | `mall` | 服务端过滤 |
| 事实发生期间 | 服务端角色期间策略；未配置时不预填 | `occurredFrom` / `occurredTo` | 必须使用事实发生时间，禁止用 ERP 接收时间；默认期间由服务端按角色策略配置，禁止前端固定 30 天/90 天或私自恢复“上次选择”；策略未配置时必须由用户显式选择完整 `occurredFrom/occurredTo` 后才查询，禁止静默预填 |
...
| 成本口径 | 全部 | `costBasis` 可多选 | ACTUAL、STANDARD、NONE；同单多口径必须按分项摘要与金额占比展示。成本口径策略未配置时，服务端禁止返回归一主标签，列表不得自造“混合”标签；策略已配置时，服务端必须返回 `MIXED` 或选定可信口径作为归一主标签，前端禁止自行挑选 |
| 数据来源 | 全部 | `dataSource` | 实时 / 历史回填 |
| 排序 | 事实发生时间降序 | `sort=occurredAt.desc` | 服务端排序；相同时间使用稳定事实 ID |

- 指标点击改变列表筛选并写 URL，必须有选中态和筛选摘要。
- 工具栏常驻「清除筛选」，清全部筛选参数并回第 1 页；空态筛选无结果时同样提供。
...
| --- | --- | --- | --- | --- | --- |
| 刷新 | 页头 | 有 W25 权限 | 无 | 重新获取订单、事实、归集和履约进度 | 保留旧数据并显示重试 |
...
| 揭示履约地址 | 地址区 | `REVEAL_ADDRESS`；必须为当前责任供应商子订单履约所需角色，且职责确有履约需要 | 明确用途与短时期限 | 短时展示并写访问审计 | 超时必须自动重新掩码；结果未知禁止展示完整值；非当前责任子订单角色禁止在 W25 揭示，仅可从 W26 受控查看 |
...
指标、总数与列表使用同一权限快照和事实更新时间。Query Key 至少包含用户、角色、权限/数据范围版本、筛选和时区。

列表行的 `costBasisBreakdown` 必须始终按口径分项返回行数与金额。`costBasisPolicyState` 为 `UNCONFIGURED` 时，禁止出现 `normalizedCostBasis`，前端必须只展示分项摘要与金额占比，禁止归一为单一主标签；为 `CONFIGURED` 时，服务端必须返回 `normalizedCostBasis`（`ACTUAL` / `STANDARD` / `NONE` / `MIXED` 之一），前端必须原样展示，禁止按最低可信口径或其它规则自行改写。
...
- 地址揭示返回短时凭据，离开页面、权限变化或到期后必须立即清除。仅当前责任供应商子订单履约所需角色可在 W25 发起揭示；其余角色禁止在 W25 揭示，必须从 W26 受控查看。
- 导出通过 `background_job` 执行；发起和下载时分别鉴权，结果文件按统一规则到期。
- 请求结果未知时使用 `requestId` 查询既有审计/任务，禁止重复揭示或创建第二个导出。
- 消费事实接收不因用户刷新触发。第一份有效支付通过验签、金额与分摊守恒校验后，按 `(mallId, externalOrderNo)` 串行处理：
  ...
  - 客户、卡实例和商品归集条件完整时，锁定上述原订单与事实形成消费归属；`paidAt >= T` 且发布、固定供给和能力完整时，同一正式处理事务创建确定性供应商子订单及明细、首个 `PLACE` 动作。若首次接收时条件已齐，这些写入属于首次事实/订单事务；若条件后补齐，则属于锁定原事实和订单的续归集事务。两种情况下“子订单 + 动作”都必须原子，不能留下“有子订单无动作”。
  - 前置条件缺失时保留已经形成的不可变事实和唯一商城订单，并同事务登记待归集状态、差异/正式任务；不能因归集失败删掉或推迟创建订单。修复后锁定原事实与订单，继续使用同一 `businessFactKey` 和稳定唯一键只补齐缺失归属或履约对象；唯一冲突加载既有对象，不生成第二份订单、消费或动作。
- 投递、结果查询与重试，以及成本/经营分析投影刷新可以异步；它们不得反向改变上述正式事实或唯一履约链。
...
- 异步消费/成本分析投影必须显示独立来源消息消费位置、`projectionUpdatedAt` 和 `lagSeconds`；正常消费消息后 60 秒内可见，超过 60 秒必须标记陈旧并告警。每日全量重建/对账校验投影，重建失败只影响查询新鲜度，禁止修改正式事实；商城余额快照使用独立 `balanceSnapshotAt`，不得用分析更新时间冒充余额时点。
- 商城关键事实从发生到 ERP 可见的正常延迟、告警与 SLA 状态必须由服务端按事实类型返回；前端必须原样展示，禁止按本地时钟或固定阈值推断延迟/告警级别。投影 `lagSeconds` 仅用于分析投影新鲜度，不得冒充关键事实 SLA。
...
| 刷新 | 保留事实、矩阵和履约摘要，显示各自更新时间 | 允许阅读；敏感揭示不跨刷新保留 | 成功更新时间，失败保留旧数据 |
...
| 数据陈旧 | 分别标记事实、归集、供应商与成本更新时间 | 刷新、进入 W29 | 数据追平 |
...
| T 切换 | 消费回流启用时点（T 起商城停单） | `cutoverId`、`enabledAt` | 只读追溯，W25 不改履约链 |
...
- [x] 本页明确是消费记录的只读快照，不显示商城处理中间态，也没有修改商城订单入口（用户提示不出现 W25 编号）。
...
- [ ] 第一份有效支付无论归集条件是否完整，都同事务形成不可变事实、唯一商城订单及可保存来源快照；条件齐全后按同一事实键补齐消费归属，`T` 后供给完整时原子形成确定性供应商子订单、首个 `PLACE` 动作。
