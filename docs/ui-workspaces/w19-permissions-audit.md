...
- 权限双人复核任务不在 W19 建设范围内：不接收 `workItemId`，不提供领取、移动确认或完成任务入口；仅当正式任务模型已注册固定 `work_item_type` 时由 W02 统一动作命令处理。
...
- 扩大敏感字段访问、全公司数据范围、权限管理能力等服务端标记为需复核的高风险变更，仅当正式任务模型已注册双人复核 `work_item_type`、岗位分离与唯一完成动作时，才可经 W02 统一动作命令生效；否则必须以 `REVIEW_POLICY_UNCONFIGURED` fail-closed 阻塞。普通缩权仅当服务端明确允许对象级直接提交时才可在 W19 提交，否则同样阻塞。
...
W19 路由仅支持权限对象级配置、解释与审计查询。禁止接收 `workItemId`，禁止提供领取、移动确认或完成任务入口。W01/W02 不得将权限复核任务路由到 W19。仅当正式任务模型已注册固定 `work_item_type`、岗位分离与唯一完成动作时，复核由统一任务注册表与 W02 统一动作命令处理，不得以 W19 路由或页面内动作模拟任务完成。
...
| 停用角色 | 角色行更多 | `DISABLE_ROLE`；服务端无不可停用 blocker，且当前安全策略允许对象级直接提交 | 强确认影响用户、任务责任池和替代角色 | 角色停用结果常驻；不删除历史角色身份 | 服务端要求复核但复核策略未注册时保持启用并以 `REVIEW_POLICY_UNCONFIGURED` 阻塞；其它 blocker 必须列出解决入口 |
...
- `AccessChangeCommand` 是权限对象命令，禁止携带 `workItemId` 或页面自定义完成动作。用户角色时间策略与字段粒度策略字段只用于服务端并发校验，前端禁止补默认值。仅当正式模型已注册固定复核任务时，任务决定必须由已注册处理器包入 W02 统一动作命令；禁止向本命令追加私有任务字段。
...
W01/W02 不得直接打开 W19 执行权限复核或移动确认。权限复核不在 W19 建设范围内：仅当正式任务模型已注册固定 `work_item_type` 时，由统一任务注册表声明目标处理器；处理器必须遵守 W02 统一动作命令，禁止以 W19 路由参数或 `AccessChangeCommand` 模拟任务完成。
...
- [x] 服务端要求复核但复核策略未注册时，命中动作 fail-closed 阻塞；W19 禁止创建、领取、移动确认或完成 `work_item`。
- [ ] `AccessChangeCommand` 仅是对象级命令；仅当正式模型已注册固定复核任务时，必须复用 W02 统一动作命令，禁止向本命令追加私有任务字段。
