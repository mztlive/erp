...
- 权限双人复核任务不在 W19 建设范围内：不接收 `workItemId`，不提供开始处理、移动确认或完成任务入口；仅当任务模型已注册固定 `work_item_type` 和强类型命令时由 W02 路由到相应处理器。
...
W19 路由仅支持权限对象级配置、解释与审计查询。禁止接收 `workItemId`，禁止提供开始处理、移动确认或完成任务入口。W01/W02 不得将权限复核任务路由到 W19。仅当任务模型已注册固定 `work_item_type`、岗位分离与唯一强类型领域命令时，复核才由统一任务注册表路由到受控处理器，不得以 W19 路由或页面内动作模拟任务完成。
...
W01/W02 不得直接打开 W19 执行权限复核或移动确认。权限复核不在 W19 建设范围内：仅当正式任务模型已注册固定 `work_item_type` 时，由统一任务注册表声明目标处理器；处理器必须使用该任务唯一的强类型复核命令，禁止以 W19 路由参数或 `AccessChangeCommand` 模拟任务完成。
...
- [x] 服务端要求复核但复核策略未注册时，命中动作 fail-closed 阻塞；W19 禁止创建、开始处理、移动确认或完成 `work_item`。
