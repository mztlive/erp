...
TaskTabs 身份为 `purchase-order:{purchaseOrderId}`；新草稿在服务端取得稳定采购单 ID 后使用相同身份。草稿阶段标题必须采用 `PO · 采购草稿 + 短 ID`（不得预分配正式采购单号）；首次提交财务审核成功时服务端形成不可复用正式 `purchaseNo`，页签仅更新标题文案，不得改变页签身份。
...
├ MetricStrip：全部 | 草稿 | 待财务审核 | 待履约 | 先款门禁阻塞（真筛选）│
├ BusinessTableFrame ───────────────────────────────────────────────────┤
...
| --- | --- | --- | --- | --- | --- |
| 身份 | `purchaseNo` | 采购单号 | `purchase_order.purchase_no` | 稳定业务号；草稿阶段禁止预分配正式号，展示为「采购草稿 + 短 ID」；仅当首次提交财务审核成功时分配不可复用正式编号 | 对象可见者 |
...
- 财务驳回必须填写结构化 `reasonCode`；代码集合至少区分：成本/税率、费用、付款条件、供应商资料、分配错误、其它。禁止仅自由文本而无代码；采购修改提示与审核统计必须按 `reasonCode` 归类。
...
- 变更是否触发采购负责人或更高层额外审批，仅由服务端规则判定并返回额外任务与 `allowedActions`/`actionBlockers`；W08 禁止硬编码影响阈值、审批层级或额外状态机。
...
type PurchaseOrderReviewDecision = PurchaseOrderReviewDecisionBase &
  (
    ...
    | {
        ...
        /** 必须；至少：成本/税率、费用、付款条件、供应商资料、分配错误、其它 */
        reasonCode: string
      }
  )
