...
「添加供应商并登记成本」是**最小对话框**（区别于手工录入全页表单）：固定当前 `sku_id`，名称/规格/分类/品牌/单位/条码/图文反向复用为公司商品资料，形成供应商商品来源快照；要求填写供应商、供应商 SKU 编码、一件代发供给价（含税运）、集采供给价（含税）和集采起订量，不收集供给方式与生效日期（提交默认当日生效）。两项供给价分别写入同一供给修订，不再自动择一生成单一采购确认成本。同一业务事务内创建供应商商品、SKU 映射、供给修订，并在目标 SKU 尚无商品池条目时设置首次销售可见价形成商品池条目；已有商品池则默认 `KEEP_EXISTING`。W21 携带 `skuContext` 进入列表页时，页头「添加供应商并登记成本」按钮使用同一对话框。
