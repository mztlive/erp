...
> 最后更新：2026-08-04（确认 W21 一期启用、双价格、SKU 级映射及当前前端 SKU 创建边界）
...
SPU/SKU、供应商 SKU 到公司 SKU 的映射、多供应商供给修订和加入公司商品池；第一期不
对接 API 供应商。第二期只增加 W20 API 连接、自动同步及 API 来源变化/异常处理，继续复用
第一期的供应商商品、映射、供给和商品池实体，不建立第二套目录或供给模型。前端可以完整
...
- 采购先拥有完整的供应商商品库，再从供应商 SPU 页面逐个选择供应商 SKU，**分别关联已有公司 SKU**（公司 SKU 在 W14 创建），并加入公司商品池；SPU 只作为页面容器和批量选择范围。
- 一个公司 SKU 可以关联多个供应商 SKU；每个关联分别维护一件代发供给价、集采供给价、集采起订量、税率、费用、区域和有效期。系统不设置供给方式字段，每条供给默认同时支持一件代发与集采。
- 第二家供应商的同款供应商 SKU 继续保留独立身份和来源版本，但可映射到同一公司 SKU；不得重复创建公司商品或商品池条目。
...
- 公司商品/SKU 正向创建：W14 `/master-data/products/new` 使用独立空白表单创建公司 SPU
  及一个或多个公司 SKU；当前不读取供应商来源进行预填。
- 供应商商品/SKU 独立创建：W21 `/procurement/supplier-catalog/new` 使用完整手工表单创建
  供应商 SPU 及一个或多个供应商 SKU，保存后仍处于待映射状态。
- 反向创建：W14 固定已有公司 SKU 打开“添加供应商并登记成本”，使用公司 SKU 快照预填
  供应商商品/SKU，并同时建立精确映射、供给与必要的商品池修订。
- W21“加入公司商品池”不创建公司 SKU，只能从已有且启用的公司 SKU 中选择目标。W21
  「前往商品资料」当前仅进入 W14 列表；URL 中的 `supplierProductId/returnTo/queueContextId`
  尚未被 W14 消费，不能视为已实现来源预填、自动返回或自动映射。
...
6. 采购从供应商 SPU 页面选择一个或多个供应商 SKU，逐项**映射到已有公司 SKU**（公司商品须先在 W14 维护），再确认供给和商品池价格。未选择的兄弟 SKU 不得被隐式映射；不提供「从来源新建公司商品」。
...
- W14 商品 SKU 行「添加供应商并登记成本」：固定当前公司 SKU，使用**最小对话框**一次性填写该供应商 SKU、两项供给价和供给条件（仍写 W21 实体）。名称、规格、分类、品牌、单位、条码与图文从公司 SKU **反向复用**为供应商商品来源快照，对话框只补录供应商侧独有差异：供应商、供应商 SKU 编码、**一件代发供给价（含税运）/ 集采供给价（含税）**、集采起订量；仅当公司 SKU 尚无商品池条目时追加首次销售可见价。两项供给价分别写入供给修订，不再折叠成单一采购确认成本；**不收集生效日期**，提交默认当日生效；进项税率与供给区域使用默认值（0.13 / 全国）。系统不设置供给方式字段，默认两种方式都可用。不在该入口展示供应商商品媒体上传、来源描述、规格属性等字段——需维护时进入供应商商品中心全页编辑。
...
「添加供应商并登记成本」是**最小对话框**（区别于手工录入全页表单）：固定当前 `sku_id`，名称/规格/分类/品牌/单位/条码/图文反向复用为公司商品资料，形成供应商商品来源快照；要求填写供应商、供应商 SKU 编码、一件代发供给价（含税运）、集采供给价（含税）和集采起订量，不收集供给方式与生效日期（提交默认当日生效）。两项供给价分别写入同一供给修订，不再自动择一生成单一采购确认成本。同一业务事务内创建供应商商品及供应商目录 SKU、精确的 `supplier_catalog_sku_id → sku_id` 映射和供给修订，并在目标 SKU 尚无商品池条目时设置首次销售可见价形成商品池条目；已有商品池则默认 `KEEP_EXISTING`。W21 携带 `skuContext` 进入列表页时，页头「添加供应商并登记成本」按钮使用同一对话框。
...
PromoteSupplierProductToPool {
  ...
  bulk_minimum_order_quantity
  input_tax_rate / fees
  supply_region / valid_from / valid_to?
...
单项映射与入池的正式粒度始终是 `supplier_catalog_sku_id`。供应商 SPU 页面可以多选 SKU，
但批量提交必须拆成 SKU 级 `items[]`；每项都显式携带来源 SKU 与目标公司 SKU，独立执行
原子校验和幂等控制，不得生成一条 SPU 级映射或隐式处理未选择的兄弟 SKU。

```text
PromoteSupplierSkusToPoolBatch {
  items[]: PromoteSupplierProductToPool
}

PromoteSupplierSkusToPoolBatchResult {
  items[] {
    supplier_catalog_sku_id
    idempotency_key
...
成功结果至少返回 `supplier_product_id`、`supplier_catalog_sku_id`、`supplier_product_mapping_id`、
`supplier_offering_revision_id`、复用或新形成的 `product_pool_entry_revision_id`、
`pool_entry_change`、当前有效供应商数量、审计引用和记录时间。单项执行中出现并发冲突、
来源修订已变化或价格无效时该项整体失败，不允许只完成映射、供给或商品池中的一部分。
...
2. 一期 Excel/手工入库、二期 API 入库后的商品都默认只在供应商商品库，未选择入池前销售不可见。
...
16. 系统不存在供给方式字段；每条正式供给默认同时支持一件代发与集采，并分别保存两项供给价。供应商商品目录**不包含**：统一含税报价、进项税率、运费、其他费用、可供区域、预计发货、售后说明、商品能力（入池供给确认另有字段，不进目录主档）。
...
19. 同一供应商 SPU 下的多个供应商 SKU 可以分别映射不同公司 SKU；只选择其中部分 SKU
    入池时，未选择的兄弟 SKU 保持未映射，且任何一项都不能覆盖另一项的映射。
