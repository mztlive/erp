...
- W14 商品 SKU 行「添加供应商并登记成本」：固定当前公司 SKU，使用**最小对话框**一次性填写该供应商 SKU、双底价和供给条件（仍写 W21 实体）。名称、规格、分类、品牌、单位、条码与图文从公司 SKU **反向复用**为供应商商品来源快照，对话框只补录供应商侧独有差异：供应商、供应商 SKU 编码、**供给方式（多选）**、**一件代发底价（含税运）/ 集采底价（含税）双底价**、供给起订量；仅当公司 SKU 尚无商品池条目时追加首次销售可见价。采购确认成本不再单独填写，自动取所选方式的底价（集采优先）；**不收集生效日期**，提交默认当日生效；进项税率与供给区域使用默认值（0.13 / 全国）。不在该入口展示供应商商品媒体上传、来源描述、规格属性、集采起订量等字段——需维护时进入供应商商品中心全页编辑。W21 手工录入/Excel 批次对话框同样使用供给方式多选与双底价条件必填，且不收集生效日期。
...
「添加供应商并登记成本」是**最小对话框**（区别于手工录入全页表单）：固定当前 `sku_id`，名称/规格/分类/品牌/单位/条码/图文反向复用为公司商品资料，形成供应商商品来源快照；只要求填写供应商、供应商 SKU 编码、供给方式（多选）、双底价（一件代发含税运 / 集采含税，按勾选方式必填）和供给起订量，不收集生效日期（提交默认当日生效）。采购确认成本自动取所选方式底价（集采优先）写入供给修订。同一业务事务内创建供应商商品、SKU 映射、供给修订，并在目标 SKU 尚无商品池条目时设置首次销售可见价形成商品池条目；已有商品池则默认 `KEEP_EXISTING`。W21 携带 `skuContext` 进入列表页时，页头「添加供应商并登记成本」按钮使用同一对话框。
