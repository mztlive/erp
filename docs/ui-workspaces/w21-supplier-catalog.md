...
- 采购先拥有完整的供应商商品库，再从供应商 SPU 页面逐个选择供应商 SKU：有同款时关联
  已有公司 SKU；没有同款时在入池 Dialog 中使用来源同字段预填并创建公司商品/SKU。SPU
  只作为页面容器和批量选择范围。
...
- 反向创建：先有供应商 SKU；W21“加入公司商品池”在没有同款公司 SKU 时显示“新建公司
  商品/SKU”分支。名称、描述、来源商品类型、分类、品牌、单位、规格、条码、已归档主图/图文等语义相同
  字段自动预填，采购可二次修改；独立 `product_kind`、销售可见价与市场价均必填。提交原子创建公司商品/SKU、
  精确映射、双价供给修订和首个商品池条目。
...
6. 采购从供应商 SPU 页面选择一个或多个供应商 SKU，逐项选择“关联已有公司 SKU”或
   “新建公司商品/SKU并入池”。新建分支自动预填同字段、允许采购修改，并要求独立
...
- W14 商品 SKU 行「添加供应商并登记成本」：固定当前公司 SKU，使用**最小对话框**一次性填写该供应商 SKU、两项供给价和供给条件（仍写 W21 实体）。名称、商品类型、规格、分类、品牌、单位、条码与图文从公司 SKU **正向复用**为供应商商品来源快照；对话框补录供应商、供应商 SKU 编码、**一件代发供给价（含税运）/ 集采供给价（含税）**、集采起订量、进项税率、供给区域和生效日期，仅当公司 SKU 尚无商品池条目时追加首次销售可见价。两项供给价分别写入供给修订，不折叠成单一确认成本。税率、区域、生效日期有可靠版本化来源时自动预填并显示来源，采购可修改；无可靠来源时空白必填。禁止静默使用 `0.13`、“全国”或浏览器当天。系统不设置供给方式字段，默认两种方式都可用。不在该入口展示供应商商品媒体上传、来源描述、规格属性等字段——需维护时进入供应商商品中心全页编辑。
...
1. 选择目标分支：**关联已有公司 SKU**，或在无同款时**新建公司商品/SKU并入池**。新建
...
4. 已有分支原子写入供应商 SKU 映射、供给修订和必要的商品池修订；新建分支还须在同一
   数据库事务创建公司商品/SKU及其修订。任一步失败全部回滚，返回各稳定 ID/修订 ID。
...
- 入池 Dialog 提供“关联已有公司 SKU / 新建公司商品并入池”两个分支；已有分支展示匹配
  证据、当前有效供应商数量、商品池状态与销售可见价，已有商品池默认沿用现有价格。
...
| 映射与入池 | 公司商品/SKU 新建草稿、销售可见价、市场价 | 映射状态、映射历史、商品池价摘要 | 主动作：**加入公司商品池**；可关联已有公司 SKU，或自动预填并新建公司商品/SKU；不展示来源版本差异、供给版本时间线、发布影响 |
...
「添加供应商并登记成本」是**最小对话框**（区别于手工录入全页表单）：固定当前 `sku_id`，名称/商品类型/规格/分类/品牌/单位/条码/图文从公司商品资料正向复用，形成供应商商品来源快照；要求填写供应商、供应商 SKU 编码、一件代发供给价（含税运）、集采供给价（含税）、集采起订量、进项税率、供给区域和生效日期，不收集供给方式。税率、区域和生效日期只有可靠版本化来源时自动预填并显示来源，采购可修改；无可靠来源时空白必填，禁止静默使用 `0.13`、“全国”或浏览器当天。两项供给价分别写入同一供给修订，不再自动择一生成单一采购确认成本。同一业务事务内创建供应商商品及供应商目录 SKU、精确的 `supplier_catalog_sku_id → sku_id` 映射和供给修订，并在目标 SKU 尚无商品池条目时设置首次销售可见价形成商品池条目；已有商品池则默认 `KEEP_EXISTING`。W21 携带 `skuContext` 进入列表页时，页头「添加供应商并登记成本」按钮使用同一对话框。

公司 SKU 可由 W14 独立创建，也可由 W21 反向入池复合命令创建；两种路径都必须由服务端
分配稳定 `sku_id`。正向添加供应商与反向入池最终都只能引用该稳定 ID，不得由前端临时生成。
...
```text
PromoteSupplierCatalogSkuToPool {
  ...
  sales_visible_price_gross? // SET_PRICE 或新建分支必填
...
媒体未归档或启用 SKU 缺主图时必须补齐；不得伪造字典 ID 或长期媒体地址。销售可见价和
市场价均必填，且不得由来源底价、正式供给价或彼此自动推导。

`CREATE_COMPANY_PRODUCT_AND_SKU` 是单一数据库事务：创建公司 product/SKU 及修订、精确
SKU 映射、双价 offering 修订、首个 product pool 修订、审计、幂等结果与 outbox 必须一起
...
PromoteSupplierSkusToPoolBatch {
  items[]: PromoteSupplierCatalogSkuToPool
}
...
`company_sku_id`、`supplier_product_mapping_id`、`supplier_offering_revision_id`、复用或新形成的
`product_pool_entry_revision_id`、
...
3. 采购可把供应商 SKU 关联已有公司 SKU，也可在无同款时选择“新建公司商品/SKU并入池”；
...
6. 加入公司商品池的已有分支同时形成映射、供给修订和必要的商品池修订；新建分支还原子
   形成公司商品/SKU及修订。第二供应商选择沿用时返回原商品池修订；失败时没有半完成状态。
