...
| 角色 | 默认入口 | 可见范围 | 主要动作 |
| --- | --- | --- | --- |
| 系统管理员 | 我的接口任务 / 结果未知 | 授权系统、商城、连接和环境 | 开始处理、查询原结果、按规则重放、转交、关联终态证据 |
...
- `POOL` 任务“开始处理”为 W02 条件更新原子完成；只有当前责任人可执行处理，查看权限不等于处理权限。
...
| 从待办进入 | W01 / W02 中 `destinationWorkspaceId=W29` 的 `INTEGRATION_RESULT_UNKNOWN` / `BUSINESS_EXCEPTION` | 仅当注册 handler 的业务对象为 `integration_error_task` / `reconciliation_difference` 时，先打开 `/governance/integration-errors?resolveWorkItemId=:workItemId&queueContextId=...`；服务端解析后以 replace 导向领域详情，URL 不携带处理令牌 | 完成/退回团队后回原队列 |
...
- 所有开始处理、查询、重放、转交、补偿、解决、退回团队或关闭动作的追加式记录。
...
| 责任人 | 我 / 团队待处理 | `owner=` | 区分本人负责、尚无个人责任人和他人负责 |
...
| --- | --- | --- | --- | --- | --- |
| 开始处理 | 当前项 / 队列 | 责任角色匹配、开放 `POOL` 任务尚无个人责任人 | 无 | W02 条件更新原子写入当前用户 | 已由他人处理时转只读并显示当前处理人 |
...
| 转人工 / 转交 | 能力不足或责任错误 | 有转交权限、目标责任明确 | 原因与业务影响必填 | 用 W02 `REASSIGN` 更新原开放任务责任人与转交审计，不创建同义后继任务 | 失败保留原任务状态和输入 |
...
| 退回团队 / 跳过本轮 | 连续处理条 | 当前责任人为本人，`RELEASE_TO_TEAM` 可用 | 原因按规则 | 用 W02 `RELEASE_TO_TEAM` 清空原任务个人责任并保持 `OPEN`，焦点可进入下一项 | 明确播报当前项仍未完成及恢复方法 |
| 下一项 | 连续处理条 | 当前快照内有下一有效项 | 无 | 只切队列焦点，不写任务状态 | 目标失效时定位下一有效项 |
| 保存当前进度 | 连续处理条 | 当前责任人为本人 | 无 | 保存草稿与处理进度，不改变任务状态 | 保存失败保留输入；需要释放责任时使用“退回团队” |
...
- 未取得可验证终态前禁止 `RESOLVE`；仅允许 `ADD_EVIDENCE`、`RELEASE_TO_TEAM` 或 `REASSIGN`。
...
凡当前项关联 `work_item`，查询、重放、重新归集、补偿关联和补证使用 `IntegrationTaskActionCommand`；`RESOLVE` 使用 `IntegrationTaskCompletionCommand`；开始处理、退回团队、关闭和转交分别使用 W02 `START_PROCESSING`、`RELEASE_TO_TEAM`、`CLOSE` 和 `REASSIGN`。只有未创建 `work_item` 的直接对账差异处理才使用差异对象命令；该命令不得顺带完成、转交或关闭任何任务。
...
- 非终结与完成命令，以及 W02 的关闭、退回团队、转交责任命令，都校验当前责任人或管理权限、`expectedTaskVersion` 与业务对象版本；各类语义不得混用。
...
- 重复或误派只使用 W02 `CLOSE`；结构化原因必填，重复项还必须引用替代任务。服务端确认任务类型允许关闭后，关闭记录和任务 `CLOSED` 在同一事务提交，且不写业务解决结论。
- 转交只使用 W02 `REASSIGN`；原开放任务责任人与转交审计在同一事务更新，转交不写解决结论、不创建同义后继任务。
- `DirectReconciliationCommand` 只允许操作无 `work_item` 的 `reconciliation_difference`，以差异版本做并发保护。终结分支只能使用“确认无误”或“确认有效差异”，且 `reasonCode` 必须取自固定原因枚举（至少含 `SOURCE_CORRECTED_AND_REATTRIBUTED`、`BUSINESS_CONFIRMED_NO_ERROR`、`COMPENSATION_CLOSED`），禁止自由字符串。该命令不接受 `CLOSE_DUPLICATE`、不返回任务 `CLOSED`；若存在 `work_item`，重复/误派关闭必须改用 W02 `CLOSE` 与关闭原因枚举。
...
- `RESOLVE` 通过 `IntegrationTaskCompletionCommand` 完成任务并返回 `COMPLETED`；`CLOSE_DUPLICATE/CLOSE_MISROUTED` 使用 W02 `CLOSE`，要求结构化关闭原因和替代任务（如适用）并返回 `CLOSED`。
...
| 开始处理成功 | 显示当前处理人 | 允许对应处理 | 完成或退回团队 |
| 处理权变化 | 输入保留，正式动作禁用 | 刷新任务、复制非敏感说明 | 重新取得责任并重验版本后处理 |
| 对象版本冲突 | 展示任务针对版本与当前事实的结构化差异，禁用旧决定 | 刷新证据、进入替代任务 | 基于当前版本按分派模式处理 |
| 自动重试中 | 显示服务端下发的不可编辑退避时间与最近尝试 | 查看、必要时转人工；禁止改退避 | 自动终态或超限转人工 |
| 结果未知 | 固定警示；主动作只为“查询原结果” | 查询、补证、退回团队、转人工；未取得可验证终态前禁止 `RESOLVE` | 得到终态/明确无结果后再允许验证后解决 |
...
| 非终结动作成功 | 固定展示查询、重放、归集、关联或补证结果；任务明确仍为 `OPEN` | 继续查询、验证后解决、退回团队或转交 | 不自动下一项 |
...
| 375×812 | 单列只读任务卡；允许开始处理、查询结果、退回团队 | 任务身份、影响、结果未知提示和查询结果 | 不提供重放、补偿、差异修复、原文查看；提示桌面处理 |
...
- [ ] 开始处理、转交、退回团队和连续处理位置均可恢复并有审计。
- [ ] `QUERY/REPLAY/REATTRIBUTE/LINK/ADD_EVIDENCE` 使用 `IntegrationTaskActionCommand`，成功或结果未知时任务均保持 `OPEN`，不会自动下一项。
- [ ] 只有 `RESOLVE` 使用 `IntegrationTaskCompletionCommand` 并返回 `COMPLETED`；重复/误派关闭使用 W02 `CLOSE` 并强制结构化原因、替代任务（如适用）后返回 `CLOSED`；转交使用 W02 `REASSIGN` 更新原开放任务责任并记录审计。
- [ ] 所有 task-bound 写动作均携带查询返回的 `expectedTaskVersion`；完成、关闭或转交只按各自明确定义改变任务状态。
