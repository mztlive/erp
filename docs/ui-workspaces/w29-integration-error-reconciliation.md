...
- 所有人工查询、重放、补偿、确认以及重复/误派关闭动作追加审计，不覆盖原消息、尝试、差异或正式事实。
...
| 角色 | 默认入口 | 可见范围 | 主要动作 |
| --- | --- | --- | --- |
| 系统管理员 | 我的接口任务 / 结果未知 | 授权系统、商城、连接和环境 | 查询原结果、按规则重放、管理员转交非审批任务、关联终态证据 |
...
- 所有开放任务在创建时指定唯一具体责任人；只有当前责任人可执行处理，管理员 `REASSIGN`（转交）仅用于非审批任务，查看权限不等于处理权限。
...
| 从工作台进入 | W01 非审批任务摘要的「打开单据」中 `destinationWorkspaceId=W29` 的 `INTEGRATION_RESULT_UNKNOWN` / `BUSINESS_EXCEPTION` | 仅当注册 handler 的业务对象为 `integration_error_task` / `reconciliation_difference` 时，先打开 `/governance/integration-errors?resolveWorkItemId=:workItemId&queueContextId=...`；服务端解析后以 replace 导向领域详情，URL 不携带处理令牌 | 完成后返回 W01；W29 详情保留原筛选和位置 |
| 从业务对象进入 | W17/W20–W27 | 带 `objectType/objectId`及稳定错误任务或对账差异 ID；目标页按对象类型选择明确详情路由 | 返回原对象异常子区 |
| 打开错误任务详情 | 错误列表、全局搜索或 W01 解析结果 | `/governance/integration-errors/errors/:taskId`；`:taskId` 固定为 `integration_error_task` 稳定 ID，相同错误任务聚焦 `integration-error:{taskId}` | 返回原队列位置 |
...
连续队列 TaskTab 身份为 `integration-resolution-queue:{queueContextId}`；单个差异对象页签身份为 `integration-error:{taskId}` 或 `reconciliation-difference:{differenceId}`。`:taskId` 与 `workItemId` 是不同身份：前者定位 `integration_error_task`，后者只定位开放任务；解析入口必须由服务端同时校验 handler 的 `destinationWorkspaceId=W29`、业务对象类型和二者关系，再 replace 到领域详情，前端不得把两个 ID 互换。其它 `BUSINESS_EXCEPTION`（例如 W21 的 `STOPPED`）严格按注册 handler 进入自己的工作面，W29 不按任务类型抢路由。刷新恢复队列筛选、当前项和自动下一项偏好；责任状态从服务端重取。重复打开同一任务聚焦已有页签。未提交处理说明为脏状态；离开页签按全局契约确认。
...
| 状态 | `status` | 待处理 / 自动重试中 / 待人工 / 已解决 / 已关闭（仅重复/误派） | 错误任务或处理链 | 与原消息状态分开 | 按角色 |
| 滞留 | `createdAt/dueAt` | 发生 / 已滞留 / SLA | 任务时间 | 服务端计算风险级别 | 全员有权任务可见 |
| 责任 | `ownerRole/user` | 责任角色 / 当前处理人 | `work_item` / 错误任务 | 当前责任人可处理 |
...
- 所有查询、重放、转交、补偿、解决以及重复/误派关闭动作的追加式记录。
...
| 模式 | 错误 + 差异统一列表 | `mode=all/errors/reconciliation` | 身份和操作仍按各自类型表达 |
...
| 责任人 | 我 / 他人负责 | `owner=` | 只区分本人负责和他人负责；开放任务不存在空责任人 |
...
队列处理时筛选摘要、当前项 ID、自动下一项偏好和位置写入 URL/可恢复状态。指标点击采用按钮语义并与筛选同步。刷新保留旧队列，不能因查询失败把本人负责项从屏幕清空。
...
| --- | --- | --- | --- | --- | --- |
| 查询原结果 | 结果未知主动作 | 有查询权限/能力、原动作可定位 | 无破坏性确认 | 用 `IntegrationTaskActionCommand` 保存查询证据；无论得到终态、明确无结果或仍未知，任务仍为 `OPEN` | 网络失败保留当前项和原状态，可重试 |
| 重放原动作 | 查询结果后的次动作 | **仅明确无结果且服务端判定安全**；当前责任人为本人 | 展示对象、原动作、影响和服务端锁定的原键摘要 | 用 `IntegrationTaskActionCommand` 发起；服务端自行取原记录的 `originalActionIdempotencyKey` 重放并保存结果，任务仍为 `OPEN` | 再次未知时保持非终结状态并回到查询原结果；客户端不能传入或替换原键 |
| 修复后重新归集 | 映射/归集错误 | 目标 Wxx 已形成正式修复，当前事实仍待归集 | 展示原事实和修复证据 | 用 `IntegrationTaskActionCommand` 和原业务事实键重新归集；保存证据但任务仍为 `OPEN` | 失败保留原事实和最新错误 |
| 转人工 / 转交 | 能力不足或责任错误 | 管理员非审批任务转交权限、目标责任明确 | 原因与业务影响必填 | 用管理员 `REASSIGN`（转交）更新原开放任务责任人与转交审计，不创建同义后继任务 | 失败保留原任务状态和输入 |
| 关联正式补偿 | 处理区 | 对应业务允许且补偿对象已创建或本动作可幂等创建 | `FormalActionConfirmDialog` 展示影响 | 用 `IntegrationTaskActionCommand` 创建/关联纠错、退款、冲正或补偿对象证据；当前任务仍为 `OPEN` | 结果未知查询本次任务动作，不重复创建 |
| 补充终态证据 | 处理区 | 有证据追加权 | 无终结确认 | 用 `IntegrationTaskActionCommand` 追加终态证据，任务保持 `OPEN` | 追加失败时保留当前任务并说明缺失项 |
| 标记已解决 | 处理区 | 已取得可验证终态或已完成补偿并对账通过；未取得前禁止 `RESOLVE` | 展示原因枚举、备注与终态 | 用 `IntegrationTaskCompletionCommand` 追加“已解决”记录并完成任务 | 保存失败保留当前任务和输入，可重试 |
| 关闭重复/误派 | 更多 | 仅重复或误派；管理员非审批任务关闭权限；重复项必须有替代任务或误派证据 | 结构化原因必填；重复项必须给出替代任务 | 仅用管理员 `CLOSE` 动作追加关闭记录并返回 `CLOSED`，不影响正式事实 | 原因或替代证据缺失不允许关闭；失败保持原任务有效 |
...
| 保存当前进度 | 连续处理条 | 当前责任人为本人 | 无 | 保存草稿与处理进度，不改变任务状态 | 保存失败保留输入；需要变更责任时由管理员使用“转交” |
...
- 未取得可验证终态前禁止 `RESOLVE`；可继续追加 `ADD_EVIDENCE`，责任变更仅允许管理员使用 `REASSIGN`（转交）。
...
任务重复/误派关闭必须单独使用管理员 `CLOSE` 动作与关闭原因枚举，不得复用上述对账结论原因。

凡当前项关联 `work_item`，查询、重放、重新归集、补偿关联和补证使用 `IntegrationTaskActionCommand`；`RESOLVE` 使用 `IntegrationTaskCompletionCommand`；重复/误派关闭仅使用管理员 `CLOSE`，责任变更仅使用管理员 `REASSIGN`（转交）。只有未创建 `work_item` 的直接对账差异处理才使用差异对象命令；该命令不得顺带完成、转交或关闭任何任务。

本工作面禁止提供责任池、团队待处理、领取、开始处理、退回团队、`scope=team`、`assignment_mode=POOL`、`START_PROCESSING` 或 `RELEASE_TO_TEAM`；任何 `OPEN` `work_item` 必须已指定唯一 `owner_user_id`。
...
type IntegrationTaskCompletionCommand = {
  ...
  expectedTaskVersion: string
  expectedSubjectVersion: string
  decision: IntegrationTaskCompletionDecision
  idempotencyKey: string
}
...
- 非终结与完成命令，以及管理员的重复/误派关闭、转交责任命令，都校验当前责任人或管理权限、`expectedTaskVersion` 与业务对象版本；各类语义不得混用。
- `IntegrationTaskActionCommand` 只追加查询、重放、重新归集、补偿关联或补证记录。成功必须返回 `workItemStatus: "OPEN"`；即使查询或重放取得可验证终态，也必须再以当前版本执行 `RESOLVE` 才能完成任务。仍为 `RESULT_UNKNOWN` 时保留当前任务、输入和查询入口，不得自动下一项。
- `IntegrationTaskCompletionCommand` 只接受 `RESOLVE`，携带结构化原因枚举和备注；服务端在可验证终态或补偿闭环前提下，把处理记录与任务 `COMPLETED` 在同一事务提交。
- 重复或误派只使用管理员 `CLOSE`；结构化原因必填，重复项还必须引用替代任务。服务端确认任务类型允许关闭后，关闭记录和任务 `CLOSED` 在同一事务提交，且不写业务解决结论。
- 非审批任务转交只使用管理员 `REASSIGN`（转交）；原开放任务责任人与转交审计在同一事务更新，转交不写解决结论、不创建同义后继任务。
- `DirectReconciliationCommand` 只允许操作无 `work_item` 的 `reconciliation_difference`，以差异版本做并发保护。终结分支只能使用“确认无误”或“确认有效差异”，且 `reasonCode` 必须取自固定原因枚举（至少含 `SOURCE_CORRECTED_AND_REATTRIBUTED`、`BUSINESS_CONFIRMED_NO_ERROR`、`COMPENSATION_CLOSED`），禁止自由字符串。该命令不接受 `CLOSE_DUPLICATE`、不返回任务 `CLOSED`；若存在 `work_item`，重复/误派关闭必须改用管理员 `CLOSE` 与关闭原因枚举。
...
- `RESOLVE` 通过 `IntegrationTaskCompletionCommand` 完成任务并返回 `COMPLETED`；`CLOSE_DUPLICATE/CLOSE_MISROUTED` 仅使用管理员 `CLOSE`，要求结构化关闭原因和替代任务（如适用）并返回 `CLOSED`。
- mutation 返回 `operationId`、处理记录、业务终态或关闭证据引用，以及适用时的任务完成、重复/误派关闭或转交结果。非终结动作只更新当前项；完成、重复/误派关闭或转交得到确定结果后才可推进队列。
...
| 处理权变化 | 输入保留，正式动作禁用 | 刷新任务、复制非敏感说明 | 重新取得责任并重验版本后处理 |
| 对象版本冲突 | 展示任务针对版本与当前事实的结构化差异，禁用旧决定 | 刷新证据、进入替代任务 | 基于当前版本由当前责任人处理 |
| 自动重试中 | 显示服务端下发的不可编辑退避时间与最近尝试 | 查看、必要时转人工；禁止改退避 | 自动终态或超限转人工 |
| 结果未知 | 固定警示；主动作只为“查询原结果” | 查询、补证、管理员转交；未取得可验证终态前禁止 `RESOLVE` | 得到终态/明确无结果后再允许验证后解决 |
| 安全故障置顶 | 生产鉴权/签名失败固定告警标识并队列置顶 | 停止自动重试、授权运维排查 | 连接修复后由服务端解除置顶 |
| 明确无结果 | 展示查询证据与安全判断 | 允许时由服务端沿锁定的原幂等键重放 | 固定展示重放结果，任务仍为 `OPEN` |
| 保存/提交失败 | 保留原因、证据和说明 | 重试同一操作 | 成功后写处理记录 |
| 非终结动作成功 | 固定展示查询、重放、归集、关联或补证结果；任务明确仍为 `OPEN` | 继续查询、验证后解决或管理员转交 | 不自动下一项 |
| 非终结动作仍结果未知 | 固定警示和本次动作证据；任务保持 `OPEN` | 查询本次动作最终结果、沿同一任务动作幂等键重试 | 得到确定动作结果后仍需显式解决 |
...
| 转交成功 | 固定展示原任务、转交记录和新责任人 | 打开原任务、下一项 | 原开放任务由新责任人继续，不把转交显示为解决或完成 |
...
| 375×812 | 单列只读任务卡；允许查询原结果和查看当前责任 | 任务身份、影响、结果未知提示和查询结果 | 不提供重放、补偿、差异修复、原文查看；提示桌面处理 |
...
| --- | --- | --- | --- |
| 今日工作台 | W01 | 非审批任务摘要、任务 ID、`workItemId`、队列上下文与「打开单据」入口 | 从 W01 打开 W29；正式动作在 W29 完成后刷新原任务 |
...
- [x] 标记已解决使用原因枚举和备注，并通过 `IntegrationTaskCompletionCommand` 完成任务；未取得可验证终态或补偿未闭环时不能解决。
- [x] 无任务直接对账的“确认无误/有效差异”必须选择固定原因枚举（至少含来源已更正并重新归集、业务确认无误、已补偿闭环），不接受自由字符串原因。
- [ ] 关闭重复/误派必须引用替代任务或误派证据，并始终使用管理员 `CLOSE` 动作与关闭原因枚举；直接对账命令不伪造 `CLOSED`、不复用对账结论原因。
- [x] 对账只生成差异，修改业务必须进入正式变更、纠错、重新归集或重放入口。
- [ ] 管理员转交和连续处理位置均可恢复并有审计。
- [ ] `QUERY/REPLAY/REATTRIBUTE/LINK/ADD_EVIDENCE` 使用 `IntegrationTaskActionCommand`，成功或结果未知时任务均保持 `OPEN`，不会自动下一项。
- [ ] 只有 `RESOLVE` 使用 `IntegrationTaskCompletionCommand` 并返回 `COMPLETED`；重复/误派关闭使用管理员 `CLOSE` 并强制结构化原因、替代任务（如适用）后返回 `CLOSED`；非审批任务转交使用管理员 `REASSIGN` 更新原开放任务责任并记录审计。
- [ ] 所有 task-bound 写动作均携带查询返回的 `expectedTaskVersion`；完成、重复/误派关闭或转交只按各自明确定义改变任务状态。
...
- `erp-data-model.md` §6.1 `work_item`：分派、转交、关闭限制和处理证据。
