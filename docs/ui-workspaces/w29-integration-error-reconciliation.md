...
| 采购 | 供应商与商品差异 | 负责供应商、供应商商品和订单 | 修复业务映射、确认供应商业务拒绝、提供人工证据 |
...
| 标记已解决 | 处理区 | 已取得可验证终态或已完成补偿并对账通过 | 展示原因枚举、备注与终态 | 标记已解决=选原因枚举 + 备注，用 W02 统一动作命令追加“已解决”记录并完成任务 | 保存失败保留当前任务和输入，可重试 |
| 关闭重复/误派 | 更多 | 仅重复或误派，任务类型允许关闭，且有替代任务/终态证据 | 结构化原因必填；重复项必须给出替代任务 | 用 W02 `CLOSE` 动作追加关闭记录并返回 `CLOSED`，不影响正式事实 | 原因缺失不允许关闭；失败保持原任务有效 |
| 确认直接对账结论 | 无 `work_item` 的差异详情 | 原因=枚举下拉框 | 展示“确认无误”或“确认有效差异”、原因枚举和全部证据 | 只追加 `reconciliation_difference_resolution` 并进入对应差异终态；不完成或关闭任务 | 保存失败保留差异和输入，可重试 |
...
- `DirectReconciliationCommand` 只允许操作无 `work_item` 的 `reconciliation_difference`，以差异版本做并发保护。终结分支只能使用原因枚举下拉框的“确认无误”或“确认有效差异”。该命令不接受 `CLOSE_DUPLICATE`、不返回任务 `CLOSED`；若存在 `work_item`，重复/误派关闭必须改用 `IntegrationTaskCloseCommand`。
...
| 供应商商品映射 | W21 | 供应商商品、SKU、映射差异 | 映射生效后回当前项 |
...
- [x] 无任务直接对账的“确认无误/有效差异”必须选择原因枚举，不接受自由字符串原因。
- [ ] 关闭重复/误派必须引用替代任务或终态证据，并始终使用 W02 `CLOSE` 动作；直接对账命令不伪造 `CLOSED`。
...
| Q3 | 结果未知且供应商无查询能力时，哪些证据足以形成可验证终态？ | 任务能否解决 | 业务部门 + 运维 + 财务 | 由各责任角色按错误类型与资金影响约定可验证终态；未取得前禁止 `RESOLVE`，只允许补证、暂挂或转交 |
| Q4 | 安全故障是否需要独立强制值班升级时限？ | 排序、通知和告警 | 安全负责人 + 运维负责人 | 生产鉴权/签名失败立即告警并置顶，不等待普通 SLA |
| Q5 | 对账差异“确认无误”与“确认有效差异”的原因枚举有哪些？ | 防止把真实差异以自由文案终结 | 各业务负责人 | 采用固定原因枚举下拉框（如来源已更正并重新归集、业务确认无误、已补偿闭环等），不接受自由字符串；任务重复关闭仍单独使用 W02 `CLOSE` 动作 |
...
对应 `docs/ux-review/governance-integration-errors.md`、`governance-integration-error-task-detail.md`、`governance-integration-difference-detail.md` 的修复：

- 终态动作（转交/关闭重复/标记已解决/确认无误/确认有效差异）统一加 `FormalActionConfirmDialog` 二次确认；「标记已解决」在确认框中明示将自动登记的处理凭证清单。
- 「关闭重复」的替代任务由手输内部 ID 改为从队列选择（任务号 · 业务单），默认不再预填 `wi_*`。
- 详情路由（focusMode）取数解耦：`detailItemQuery` 失败/为空有「未找到该任务」错误态 + 重试 + 返回队列；加载骨架单栏；「刷新当前任务」同时 refetch 详情。
- 未领取时「下一项」只读翻页解锁（`processNextDisabled=false`）；详情模式隐藏「下一项」；「处理当前」改为滚动到动作区并聚焦首个可用按钮。
- 筛选摘要走中文映射（视图/模式/环境/类别不再英文原值）；详情路由不渲染筛选摘要。
- 错误码徽章显示中文分类名（`classification.label`）；证据策略 Alert 去掉 policyId 与 `DISTINCT_REVIEWER` 原值（岗位分离中文映射）；已关联证据不再展示 recordId。
- 「已解决」「自动重试」两个恒空视图从切换栏移除；「对账差异」不再与视图重复出现在模式下拉（`mode` 仅 all/errors）。
- 差异对比面板改为「对照项/左证据/右证据/差异说明」列文案，删除「边界」「更新时间」伪差异行，计数只计真实差异；`differenceType` 走中文映射。
- 「处理当前」之外的演示开关「模拟仍未知」收敛为「演示控制」行内标注；删除死代码 `sessionAutoNext`；「自动下一项」默认关闭。
- 队列行按钮嵌套 Link 拆解为可聚焦 div + 行内「详情」链接；状态徽章 tone 改按状态语义；`dueAt` 传截止时间而非滞留时长；原因位展示中文分类名。
- 实现词清理：集成更新时间→数据更新时间、载荷摘要→消息摘要、追加式→历史保留、强类型证据→核验记录、等待退避→请稍后重试、限流→调用次数受限、终结→处理完成、非终结→暂不完成对账、重复请求标识→按原任务号重发。
- 搜索草稿随 URL 前进/后退回写；处理说明与替代任务选择随任务切换重置；自动领取失败不再静默。
- 修复跳转按钮去掉 W 编号（供应商订单、商城同步映射、API 供应商连接、API 结算等）；尝试历史技术原值（MAPPING_REQUIRED 等）中文化。
