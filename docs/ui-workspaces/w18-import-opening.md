...
- 不用“重新上传一份 Excel”覆盖已经形成的正式事实；冲突进入映射、主数据修订、库存调整或财务纠错流程。
...
| 采购确认人 | 待办进入指定批次 | 供应商、能力、资质、可销售项目和商品 SKU | 确认或退回责任范围结果 |
...
| 可销售项目、商品 SKU、卡券类目 | 全部启用对象；停用对象按批准清单导入 |
...
| 主数据 | W14 | 主数据稳定 ID、冲突类型、来源身份 | 修订或映射完成后回 W18 重新校验 |
...
- [ ] 任一批次都能明确看到环境、基准日、对象集、规则版本和六段阶段。
- [ ] 上传成功后界面仍明确写“尚未形成正式数据”。
- [ ] 1440×900 下阶段、影响摘要、首批问题和当前主动作同屏可见。
- [ ] 问题表能按固定错误码、对象、行列和处理状态筛选，不混入成功长表。
- [ ] 刷新或重新打开页签能恢复批次、阶段、问题筛选和后台进度。

### 12.2 业务与数据

- [ ] 验证环境校验与业务确认是生产应用前置条件。
- [ ] 期初库存只接受统一基准日实盘数量；不导入历史流水。
- [ ] 卡券草稿不迁移，期初已收和已开票为 0，后续进入 W13。
- [ ] 已成功对象不会因取消、失败重跑或新文件被直接覆盖、删除或回滚。
- [ ] 来源身份、规则版本、manifest、成功结果和映射谱系可追溯。
- [ ] 部分成功正确区分成功、跳过和失败，并支持幂等修复批次。

### 12.3 安全与权限

- [ ] 原始 SQL、数据库连接头和禁止字段不会进入长期 `file_asset` 或普通页面。
- [ ] 成功资产与失败诊断资产分离，分别执行长期/30 天保留规则；导出 7 天到期。
- [ ] 下载时重验当前对象、数据范围和字段权限，并记录下载审计。
- [ ] 业务确认人只能确认本人责任范围，系统管理员不能代替业务确认。
...
- [ ] 业务确认入口上线前，导入确认的固定 `work_item_type` 已写入权威数据模型并与 W01/W02 展示映射一致；未登记时必须保持实施 blocker。
