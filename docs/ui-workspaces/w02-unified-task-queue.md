...
| 业务主管 | 团队视图 | 团队及授权组织范围内任务 | 查看责任分布；必须可直接转交并记录原因 |
...
- 转交直接更新责任人并记录审计；不需要创建后继任务或转交链。业务主管在团队视图中，对授权范围内任务必须可直接执行转交，且必须记录原因；禁止将「仅请求原处理人暂挂」作为主管唯一管理路径。
...
筛选归属遵循 `docs/ui-filter-design.md` §2.3（M3：一张 sticky 处理面，禁止工具条裸飘画布）。

```text
┌ PageHeader：统一待办   我的待办 18 · 待领取 7 · 超期 2   [刷新] ─────┐
┌ sticky 处理面（surface 单卡）─────────────────────────────────────────┐
│ 第0层：[我的待办] [待领取] [团队] [已跳过]  （scope 分段，可带计数）   │
│ 第1层：ListToolbar  搜索 | 类型 | 时限 | …（主筛≤3） | 共 N · 清除筛选 │
│ 第2层：FilterChip（深链 workItem / 对象锁定，有则显示）               │
└──────────────────────────────────────────────────────────────────────┘
├────────────────────────────────┬────────────────────────────────────────────┤
│ 任务队列 34%                 │ 当前任务处理区 66%                         │
│ 筛选摘要 · 共28项             │ SequentialProcessBar 第 3/28 · 自动下一项  │
│                                │ 对象身份、原因、影响、截止、附件           │
│ [当前条目]                    │                                            │
...
| sticky 处理面 | 范围、搜索、主筛、清除与深链 chip | surface 卡 + `ListToolbar` + `FilterChip` | 第 0/1/2 层同卡；主筛 ≤3；不裸飘画布 |
| 左侧队列 | 识别上下项、超期和领取状态 | `WorkTaskItem` 紧凑变体 | 独立滚动，当前项始终有文字选中态 |
| 处理导航 | 告知位置和下一项偏好 | `SequentialProcessBar` | 右区 sticky；可与处理面同 sticky 上下文 |
...
| 能力 | 默认值 | URL 状态 | 行为 |
| --- | --- | --- | --- |
| 责任范围 | `mine` | `scope=mine|role_pool|team` | 控件文案统一「我的待办 / 团队待认领」；无对应权限不展示切换项 |
...
主工具条常驻筛选项 ≤3；其余（优先级多选、状态历史等）进筛选面板/高级筛选。交互与清除语义同 `docs/ui-filter-design.md`。
...
| 转交 | 更多动作；业务主管团队视图 | `TRANSFER` 可用，目标用户/角色有资格；主管对授权范围内任务必须可直接转交 | 目标人选择器 + 确认弹窗，明确「任务将转交 ×××」；必须填写转交原因 | 直接更新责任人并记录审计，任务状态不变 | 失败时任务保持服务端当前状态，刷新后可重试 |
...
- `TRANSFER` 直接更新责任人并写审计记录；必须携带原因；不完成任务、不写正式业务结论。业务主管在团队视图中对授权范围内任务必须可直接执行 `TRANSFER`；禁止仅允许请求原处理人暂挂。
...
- [ ] 转交直接更新责任人并写审计，不直接覆盖责任人权限之外的任务；业务主管团队视图必须可直接转交并必须记录原因。
