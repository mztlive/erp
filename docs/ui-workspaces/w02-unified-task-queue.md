...
2. 读懂任务对象、事项简报、原因、影响和下一步，以及时限、责任角色和当前责任人；
3. 不进入处理页也能说出这是谁的单、卖了什么、大概多大；正式结论仍在对应页面提交；
4. 对 `POOL` 任务执行原子“开始处理”，对 `DIRECT` 任务直接进入处理；
5. 在同类任务中连续处理，并在责任变化、版本冲突或结果未知时安全恢复。
...
├──────────────────────────────┬───────────────────────────────────────┤
│ 任务队列 34%                 │ 当前任务处理区 66%                    │
│ 对象 · 一行事项摘要 · 责任   │ SequentialProcessBar 第 3/28         │
│ [当前条目]                   │ 事项简报、业务影响、去确认采购计划     │
│ [普通条目]                   │                                       │
...
| 任务摘要 | 展示对象、原因、影响、事项简报和责任 | 只读；简报字段由任务类型白名单提供，不在本页提交业务结论 |
...
| `summarySections` | 事项简报键值 | 只读白名单：客户、含税金额、提交来源、提交人；空值不上屏 |
| `briefLines` / `briefMoreCount` | 事项明细 | 采购二次确认展示前 3 行品名、数量、期望交期；其余写「还有 N 行」 |
| `listSummary` | 列表一行摘要 | 例如「办公椅 ×20 · 8/20 交 · 另 2 行 · ¥12,800」 |
