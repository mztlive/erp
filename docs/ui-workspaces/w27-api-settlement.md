...
| 任务处理权 / 提交版本冲突 | 显示当前复核任务处理人或提交快照变化，保留阅读位置 | 刷新任务；不能改走普通对象确认 | 重新取得责任且提交版本和岗位分离满足后再操作 |
...
- [ ] `REJECT` / `CONFIRM` 只接受 `SettlementReviewCommand`，校验 `work_item` 的当前责任人、对象版本和注册任务类型，并与业务状态变化同事务；对象级命令不含可选任务字段。
