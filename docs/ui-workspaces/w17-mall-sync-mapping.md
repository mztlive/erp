...
对应 `docs/ux-review/governance-mall-sync.md` 的修复：

- 确认依据对照面板去掉 `external_identity_map` 表名（改为「确认后建立身份对应关系，不改动来源单」）。
- 跳转按钮去掉 W 编号（执行信息、接口错误与对账中心，走 `workspaceLabel`）。
- 映射表「待办」列与详情卡不再展示 `wi_*` workItemId（改为任务状态/责任）；确认成功结果不再把 `eim_*` 作为结果编号（展示「已确认目标」业务值）。
- `wm_*` 水位 token 全部替换为业务时间（最终同步点/当前同步进度/游标前后/历史卡片/来源不可用提示均格式化日期）。
- 错误码/枚举原值清零：`MANUAL_GOVERNANCE_POLICY_MISSING`、`OWNER_ROUTING_MISSING`、blockers 的 `action · code` 前缀、`FIRST_PHASE_MALL_OWNED` 均改为中文业务文案；错误分类（SOURCE_PAGE_TIMEOUT 等）走中文映射。
- 指标「失败任务」「待重新归集」不再写入页面不消费的 `status`/`mappingStatus` URL 参数。
- 搜索框按视图提示生效范围（来源数据/同步任务/映射任务），`q` 同时过滤任务号。
- 封存态闭环：`confirmMapping` / `reapplyMallSnapshot` 增加阶段校验，封存后返回业务阻断文案。
- 运行中任务 4 秒轮询刷新进度；重试失败任务补确认对话框并在成功后 refetch；确认映射成功后自动定位下一项（与「先跳过」一致）。
- 领取按钮文案由共享组件按未领取/失效区分（领取任务 / 重新领取）；admin 不再渲染一排禁用的进程条按钮（`showProcess=false`）。
- 演示控制卡折叠为一行入口；「页/数据/错」列头改为「页 / 条 / 错」；「已安全推进」改为「已推进」；内容哈希列改为业务「数据版本」值。
- 术语清理：队列上下文→当前处理位置、单人理由/双人授权→单人执行/双人复核、安全同步点→当前同步进度、来源安全时间→来源数据更新时间、T 前/T 后→第一阶段/已封存；封存 banner 标题去重。
- DataFreshness 不再显示「刚刚」；面包屑「治理」指向治理首页；最近同步任务空态补齐。
