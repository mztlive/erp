...
- 二期供应商供给进入 W21，销售执行投影进入 W23，T 切换后商城停单、ERP 全面服务，通用错误和对账进入 W29；W17 不复制这些工作面。
...
| 财务 | 我的映射任务 / 核对摘要 | 明确分派且授权的税率、开票、金额格式，以及 `ownerRole=FINANCE` 时的结算归属与结果摘要 | 确认财务语义或追加责任依据；不可修正来源金额 |
...
| 人工同步动作 | “立即增量”和“按单号补拉”必须对有权限管理员直接可执行并追加审计，禁止额外服务端二次确认门禁，禁止手工编辑水位；服务端按阶段校验并拒绝封存后的人工动作。系统定时增量仍按调度契约运行 |
| T 后（封存） | W17 转为只读历史态：不再从商城拉取新商业事实，同步、核对及执行类写动作全部禁用；同步任务、快照谱系、映射与核对证据必须长期可查，原始受控报文禁止普通导出；当前动作引导至 W23/W29 |
...
| 业务人员从工作台进入 | W01 非审批任务摘要的「打开单据」 | `view=mapping&workItemId={id}&mappingTaskId={id}&queueContextId={id}`；以 `workItemId` 校验当前责任并在 W17 执行强类型完成，以 `mappingTaskId` 查询领域证据 | 返回 W01 时保留原位置 |
...
| 历史 | 已封存轮询、最终水位、历史版本和处理证据 | 授权只读角色；同步任务、快照谱系、映射与核对证据必须长期可查；原始受控报文按独立安全策略，禁止普通导出 |
...
| `mappingTaskStatus` | 待处理/已解决/无法处理/已关闭 | `master_mapping_task` | 领域关闭必须有替代任务或证据；不复用来源快照或重新归集状态 |
| `ownerRoutingState/workItemStatus/owner` | 待责任配置，或由你处理/由他人处理/已完成/已关闭（仅重复/误派） · 责任角色/人员 | 责任路由配置 + `work_item` | 责任未配置时不创建可执行确认待办；配置后只创建一个由具体用户负责的开放任务。管理员可转交但不能替代业务确认 |
...
映射任务的处理区左右对照：左侧来源白名单事实，右侧 ERP 候选/当前对象，中部明确“确认的是身份关系，不是修改来源销售单”。每个已完成责任路由、需要人工处理的有效 `master_mapping_task` 必须关联一个有效 `work_item`；开放任务必须指定唯一具体用户，转交直接更新原任务责任并记录审计，两者仍关联同一映射任务，页面不维护第二套责任字段。结算主体责任未配置时，领域差异可保存但不进入业务确认队列，待配置生效后原子创建唯一且指定到人的开放任务。

责任路由固定遵守唯一责任人语义：客户映射进入销售、合同映射进入销售、卡券类目/唯一明细进入运营、税率和金额口径进入财务；结算主体由服务端 `ownerRole` 配置在销售与财务中二选一。结算主体未配置唯一 `ownerRole` 时，领域差异显示“待责任配置”且不得创建 `work_item`，`CONFIRM_TARGET` 必须禁用；不得同时向销售和财务生成两个可完成待办，也不得由前端按当前登录人选择责任。税率与金额口径必须由财务确认；技术解析失败必须由管理员排障，不得由业务映射确认动作代为修复。

映射任务默认 SLA 必须按类型执行：客户/合同类 1 个工作日；卡券类目/唯一明细类 4 小时。超期必须进入工作台预警、列表排序与管理指标；页面不得省略超期标识。
...
| 搜索 | 空 | `q` | 商城销售单号、ERP 销售单号、任务号；来源单号按协议精确语义；使用 `InputGroup` 控件，防抖 300ms 即时生效 + Enter 兜底 + `/` 聚焦，变更以 `replace` 写 URL |
...
「清除筛选」范围必须补全各视图的对象定位参数（`jobId`/`snapshotId`/`mappingTaskId` 等），不只清 `q`；切换子视图时清理其它视图的跨视图残留参数（保留当前视图归属的对象参数）。筛选/搜索变更一律 `replace`。
...
“立即增量”禁止让用户输入或移动高水位；服务端按当前水位、重叠窗口、`safeNow` 和稳定分页形成范围。有权限管理员必须可直接执行“立即增量”和“按单号补拉”，每次人工动作必须追加审计，禁止额外服务端二次确认门禁。来源不可用时商城继续运行，ERP 水位保持不变。按单补拉、普通失败重试和普通每日核对均是 `FIRST_PHASE_MALL_OWNED` 专属动作：迁移时点后普通增量与核对停止，即使用户保留旧页面或重放旧请求，服务端也必须拒绝；W17 转只读历史态，仅保留历史查询与证据追溯。
...
| 转交映射责任 | 任务更多菜单 | 当前责任路由已配置但实际责任人需变更；管理员非审批任务转交权限 | 展示目标责任和原因 | 使用管理员 `REASSIGN`（转交）更新原开放任务责任并记录审计；`mappingTaskStatus` 保持待处理，不创建同义后继任务 | 失败保留原任务和输入；不得用来源修复动作顺带改责任人 |
...
“确认映射”固定使用 `ConfirmMappingCommand`：它携带 `mappingTaskId`、`workItemId`、`expectedTaskVersion`、`expectedSubjectVersion`、映射任务版本、来源快照 ID 和当前映射谱系版本，领域映射变化与当前待办完成在同一事务提交。“请求来源修复”使用强类型非终结命令，同样校验任务版本，只追加当前映射任务内部证据/说明，不创建外部协同对象、外部责任人或外部状态，也不引入 `WAITING_SOURCE_FIX`。改变责任人只允许管理员使用 `REASSIGN`（转交），不得创建同义后继任务。重新归集使用独立 operation ID 和状态。映射提交结果不确定时不得在前端标记已解决；重新归集结果不确定时不回滚已经服务端确认的映射结论，且不得形成前端应收或自动跳下一项。
...
- 无通用“关闭”绕过结果未知、映射未完成或未形成终态证据；任务 `CLOSE` 仅可用于重复或误派。
- 禁止提供责任池、领取、开始处理、退回团队、`scope=team`、`assignment_mode=POOL`、`START_PROCESSING` 或 `RELEASE_TO_TEAM`；任何 `OPEN` `work_item` 必须已指定唯一 `owner_user_id`。
...
- 运行中后台任务必须按服务端返回间隔轮询或订阅；离开页面不得取消已提交任务。
- 来源快照 detail 仅返回当前角色允许的白名单字段；禁止字段不得先返回再在组件隐藏。
- 管理员的人工立即增量与按单补拉必须直接可用并在 `allowedActions` 返回，每次操作必须追加审计；禁止额外服务端二次确认门禁；系统定时增量不依赖任何人工配置。
- `MappingTaskView` 以 `ownerRoutingState` 强判别：`MISSING` 分支禁止 `ownerRole` 和 `workItem`，`CONFIGURED` 分支强制返回唯一 `ownerRole`、`ownerUserId` 与正式 `workItem`；所有 `OPEN` `work_item` 必须返回非空 `ownerUserId`，前端不得把未配置字段补成可执行责任路由。
- 封存（`ARCHIVED`）后 `allowedActions` 不再返回任何同步、核对或映射写动作；同步任务、快照谱系、映射与核对证据必须长期可查，历史查询与证据追溯仍可用；原始受控报文按独立安全策略管理，禁止普通页面导出。T 切换由服务端运营行为完成，不属于 W17 可执行工作面，契约不提供任何切换执行上下文。
...
W17 的非审批任务只允许管理员使用 `REASSIGN`（转交）；映射确认、请求来源修复和重新归集分别使用已注册的强类型命令。
...
- 若需改变非审批任务责任人，只允许管理员使用 `REASSIGN`（转交）；原开放任务责任人与转交审计原子更新，不写映射结论、不改变 `mappingTaskStatus`、不创建同义后继任务。
...
| 映射/应用失败 | 来源快照与已推进捕获水位保持不变，独立显示 `mappingTaskStatus`、责任待办、重新归集状态和业务影响 | 由当前责任人处理映射待办、修复映射、按原快照重新归集 | 映射与重新归集分别取得终态 |
...
| --- | --- | --- | --- |
| 今日工作台 | W01 | 非审批任务摘要、`workItemId`、`mappingTaskId`、`queueContextId` 与「打开单据」上下文 | 打开 W17 完成正式动作后返回 W01，或在 W17 内继续下一项 |
...
- [ ] 每个已完成责任路由、需要人工处理的映射差异同时具有 `master_mapping_task` 与 `work_item` 双身份；路由缺失时只保留领域差异且不可执行。配置后按唯一责任人规则建立责任，开放任务必须有 `owner_user_id`，映射结论与待办完成在同一事务，转交可追溯。
- [ ] 确认映射与请求来源修复分别使用强类型命令；来源修复只追加当前映射任务内部说明/证据并保持任务 `OPEN`，不创建外部协同对象、外部责任或 `WAITING_SOURCE_FIX`；非审批任务改变责任人只使用管理员 `REASSIGN`（转交）。
- [ ] 结算主体只有一个服务端配置的 `ownerRole`；未配置时确认动作阻断，不会向销售和财务同时生成可完成待办。
- [x] `MappingTaskView` 按 `ownerRoutingState` 强判别：`MISSING` 不含 `ownerRole/workItem`，`CONFIGURED` 必含唯一 `ownerRole`、`ownerUserId` 与 `workItem`；开放 `work_item` 必须有 `owner_user_id`。
