...
| 搜索 | 空 | `q=` | 精确匹配任务号、商城、报告号；防抖 300ms 即时生效 + Enter 兜底 + `/` 聚焦 |
...
列表与明细工具栏常驻「清除筛选」，清全部筛选参数并回第 1 页；搜索位于 `ListToolbar` 的 search 槽（防抖 + Enter + `/`）。
...
| --- | --- | --- | --- |
| 供应商供给 | W21 | 历史商品、消费时点、供给缺口 | 修复后回 W30 重新归集 |
