...
- 不修改消费回流启用时间 `T`，不在此重新执行主责迁移；切换与基线进入 W24。
...
| 从主责迁移查看准备 | W24 切换记录 | 携带 `cutoverId`，只读显示 `T` 和能力证据 | 返回 W24 切换子区 |
...
| 前置未满足 | 展示缺失的 `T`、能力检查、来源契约或权限 | 去 W24/W29 处理 | 前置满足后重验 |
...
| --- | --- | --- | --- |
| 主责迁移批次 | W24 | `cutoverId`、T、能力检查证据 | 返回保持 W24 切换上下文 |
| 外部商品映射 | W21 | 历史商品、消费时点、映射缺口 | 修复后回 W30 重新归集 |
...
- [ ] `rangeStart` 固定等于目标范围最早业务事实或已登记历史边界 `requiredHistoryStart`，正式任务范围严格为 `[requiredHistoryStart,T)`。
- [ ] 来源覆盖起点晚于 `requiredHistoryStart`、存在任一区间缺口或边界无法证明时阻断正式执行；不能缩晚起点后标记“全历史完成”。
- [ ] `occurredAt = T` 不进入历史回填，按实时/补投契约处理。
- [ ] `T` 前支付只补台账，全部标记 `LEGACY_MANUAL`，不创建供应商订单。
- [ ] 五类关键事实完整回填，同一订单下支付、取消、完成、多次退款和多次余额恢复不会被合并。
- [ ] 实时与回填按同一业务事实键去重，只形成一份正式事实。
- [ ] 失败或中断只续跑原任务、原范围和原幂等键，不新建重叠正式批次。

### 12.2 追加式事实与成本

- [ ] 回填不覆盖现有实时事实、消费、退款、余额恢复、成本或成本评估。
- [ ] 商城订单成本有完整税口径时标 ACTUAL；否则按消费时点供给版本标 STANDARD；仍无来源标 NONE。
- [ ] 不使用当前供给价、不猜测税率、不用销项税率替代进项税率。
- [ ] NONE 成本为空而不是 0，只进入消费金额和覆盖率分母。
- [ ] 映射修复后重新归集引用原事实，成本改善通过追加评估和差额表达。

### 12.3 报告、权限与体验

- [ ] 技术报告及其确认后版本均包含范围、T、总笔数/金额、去重数、ACTUAL/STANDARD/NONE、覆盖率、未归集和失败清单。
- [ ] 报告、列表和明细统计使用同一任务快照并可追溯规则/Schema 版本。
- [ ] `processingStatus` 与 `reportReviewStatus` 分开验收；技术 `COMPLETED` 不等于报告已确认或全业务完成。
- [ ] 报告复核策略未配置或报告未确认时，下载文件固定标“未确认”，确认动作和正式下游门禁 fail-closed；不会仅因技术完成解锁。
- [ ] 普通页面、日志和导出不泄露卡号、卡密、绑定手机号、完整地址或原始敏感报文。
- [ ] §9 状态和 §10 五档视口全部验收。
- [ ] 后台任务不伪装同步完成；进度滞留、部分完成和失败均有明确恢复路径。
