...
| Q2 | 历史来源无法提供成本含税标识/进项税率的预期比例是多少？ | NONE/ STANDARD 水位和项目准备 | 商城负责人 + 财务 | 切换前试算并设监控目标，但不通过前端猜测补齐 |
| Q3 | 执行中是否需要“停止尚未开始项目”的取消能力？ | 操作契约和后台任务状态 | 系统管理员 + 技术负责人 | 默认不提供；紧急停止由运维控制，已提交事实永不回滚 |
...
对应 `docs/ux-review/governance-history-backfill.md`、`governance-history-backfill-job.md` 的修复：

- 列表页补手动「刷新」按钮；详情错误态补「重试」。
- 详情明细接入分页（`page` 入 URL，pageSize 20），删除「当前页明细 N 条」与「可能因分页存在差异」的免责文案；切换任务时重置 result/factType/costBasis/q 过滤，杜绝隐形过滤跨任务继承。
- 详情 `ItemFilters` 补「搜索」输入框（含清除按钮），移除「URL：result / factType / costBasis」参数名说明。
- 结果横幅不再展示 operationId / idempotencyKey 原文；「结果未知」下一步文案改为「按原任务号查询处理结果」。
- 去重证明列只展示命中来源与业务摘要，删除 formalFactId / originalMessageId 原文；「原任务标识」不再展示幂等命名空间值。
- 阶段指示器传回填专属六步标签（范围确认/来源校验/记录入库/归集评估/报告/完成）。
- 进度卡「失败」恢复为纯 failedCount（与描述行口径一致）；「最近心跳」改为「最近更新于」；版本徽章由 `lv-*` 改为「版本 N」。
- 校验来源被覆盖阻断时不再推进任务状态/锁版本；START/RESUME 演示会模拟进度推进，避免运行中任务长期停在 0%。
- 创建面板：删除「回填起点（固定）」重复行；非管理员禁用时透出原因；BLOCKED 结果横幅在面板内展示。
- 错误码/阻断码不再前缀上屏（ROLE_DENIED、failure.errorCode 等走中文映射或仅展示消息）。
- 「下游功能」改为「后续流程：已可用 / 保持关闭」；「下载报告」改为「下载报告（示例）」并统一两处提示。
- 未归集跳转去掉 `queueContextId=queue:W29:...` 与 `wi_*` 参数；未归集/失败项提供逐项「重新归集」确认入口。
- 成本口径统一走 `COST_BASIS_LABEL` 中文映射（实际成本/时点标准成本/未覆盖），指标与列文案同步；「来来源更新时间」错字修正。
- 明细表轮询刷新条收敛（`showRefreshingBanner=false`）；报告确认（CONFIRM_REPORT）补齐确认对话框入口。
- mock 明细摘要由 `pay·…` 等英文前缀改为中文（支付/取消/退款/完成/恢复）。
