...
- 驳回完成后清楚告知销售只能改品/改价后重提、照原条件申请低毛利上级确认或不做作废；采购能从后续新任务识别其新提交、新指纹及来源链。

### 1.2 业务目标

- 把采购二次确认作为销售单生效前的正式行为闸门，而不是业务单据或普通详情页按钮。
- 通过时在同一事务校验完整确认覆盖并使实物与服务销售单生效；驳回时把销售单退回销售处理。
- 以不可变 `sales_order_submission` 和 `subject_hash` 作为确认对象，禁止对仍可编辑的销售草稿确认。
- 以 `work_item` 租约保障一个任务同一时间只有一个有效处理人，并支持高峰期连续处理。
...
| 页面期间权限收回 | 立即停止续租、清除成本与联系人敏感缓存，切换无权限态 |
...
1. 打开指定任务时，服务端以条件更新原子领取并返回 W02 统一租约中的一次性 `claimToken`；客户端只保存令牌原值于当前会话内存。
2. `claimToken` 不得写 URL、日志、本地持久化或埋点；服务端仅保存不可逆摘要。
3. 处理期间按服务端返回的续租窗口续期；后台页签不得无限续租占用任务。
4. 通过、驳回、保存和暂挂均校验 `claimed_by`、`claimToken`、`leaseVersion`、对象版本和内容指纹；W07 不定义第二套任务领取令牌。
5. 租约丢失时保留本地输入供查看/复制，但所有写动作禁用；重新领取后必须重取当前确认数据再处理冲突。
6. “暂挂”不是采购确认的新业务状态：它通过 W02 非终结任务动作让任务保持 `PENDING/IN_PROGRESS`；服务端可释放当前租约，再把队列指针移到下一项，但不得完成任务。
...
| 从销售单查看进度 | W05 下一步/审计 | 只读定位指定任务或确认结果，不抢占他人租约 | 返回 W05 原子区 |
| 深挖销售单 | 当前项“打开销售单” | 新开/聚焦 `sales-order:{salesOrderId}` 页签，W07 保留 | 关闭/返回后恢复当前项 |
| 刷新浏览器 | 任意队列状态 | URL 恢复筛选、排序、`currentWorkItemId`、`queueContextId` 和显式 `autoNext` 临时值；未配置 `preferenceScope` 时不从本地或服务端恢复持久偏好；重新申请租约 | W07 |
...
临时确认层、驳回 Dialog、`claimToken` 和未保存敏感值不进入 URL。刷新后若任务已由他人领取或已完成，页面展示确定结果并定位到队列中下一条有效任务。
...
```text
┌ PageHeader：采购二次确认 · 待我处理 28              数据水位 09:36 ───┐
├ SequentialProcessBar：第 3/28 · 仅我的 · 自动下一项 开 [上一项][下一项]│
│ 当前任务：已领取 · 剩余 08:42            [暂挂] [打开销售单中心]      │
├─────────────────────────────────────────────┬──────────────────────────┤
...
| 区域 | 目的 | 主组件 | 是否固定 |
| --- | --- | --- | --- |
| 页头 | 展示队列名称、有效任务数和数据水位 | `PageHeader` `DataFreshness` | 顶部 |
| 连续处理条 | 展示位置、筛选、租约和上下项 | `SequentialProcessBar` | 顶部 sticky |
...
- 明确显示提交序号、提交时间、提交人和 `subjectHash` 的短摘要，避免采购误以为正在确认销售当前草稿。
...
- 每条分行完整展示供应商、确认数量、最新含税成本、进项税率、预计交期、履约方式和能力版本。
- 页面实时显示“已确认数量 / 承诺数量”，但最终覆盖校验由服务端完成。
- 供应商选择器只返回当前业务日期有效、能力匹配、资质有效且在用户数据范围内的供应商。
- 参考供应商/参考成本只作为参考快照，不自动成为正式确认结果。
...
- 改品/改价重提形成的新任务必须指向递增提交号和新 `subjectHash`；采购重新从零确认供应商、成本、数量、交期与履约方式。
- 低毛利上级通过后形成的新任务也必须指向新的不可变提交/新指纹，并展示上级确认的受控证据引用；该证据只说明公司愿意承担低毛利，不能让 W07 自动通过或沿用旧确认分行。
...
| 租约 | `claimedBy` / `leaseExpiresAt` | 处理人 / 剩余占用时间 | `work_item` 租约 | 不展示令牌 | 当前查看者 |
...
| `subjectHashSummary` | 内容指纹摘要 | `sales_order_submission.subject_hash` | 用于并发说明，不暴露完整内部值 |
...
| `referenceSupplier` / `referenceCost` | 参考供应商 / 参考成本 | 可销售项目提交快照 | 只读，不是正式结果 |
| `supplierId` | 确认供应商 | `procurement_confirmation_line.supplier_id` | 需能力/资质有效，可多供应商拆分 |
...
| 供应商 | 全部 | `supplierId` | 仅对已有暂存/参考供应商的任务筛选，不泄露无权供应商 |
...
| --- | --- | --- | --- | --- | --- |
| 领取任务 | 打开当前项 / 角色池“领取” | `CLAIM` 可用；任务无有效他人租约 | 无 | mutation 返回 W02 `WorkItemLease`；`claimToken` 仅存会话内存 | 被他人领取则转只读并可去下一项 |
| 续租 | 系统提示 / 自动续租 | 当前领取人、页签前台、租约仍有效 | 无 | mutation 返回新的 `WorkItemLease`，含仅存会话内存的 `claimToken`、过期时间和 `leaseVersion` | 失败立即停止写入并提示重新领取 |
| 保存确认数据 | 自动保存 / `⌘S` | 有效租约，确认仍待处理，`editVersion` 匹配 | 无 | 返回新编辑版本与校验摘要 | 输入保留；版本冲突显示差异 |
| 新增供应商分行 | 明细行“拆分供应商” | 供应商能力/资质有效且明细可拆 | 无 | 在当前确认编辑数据新增分行 | 校验失败保留原分行 |
| 暂挂 | 连续处理条 / 底栏 | `DEFER` 可用；当前任务仍待处理且租约有效 | 有未保存输入时确认保存或放弃 | 使用 `WorkItemActionEnvelope` 返回 `PENDING/IN_PROGRESS`；可释放租约但不完成任务，固定结果后打开下一项 | 失败停留当前项；不得假装已暂挂或已完成 |
| 驳回 | 底栏 | `REJECT` 可用；有效租约、提交/指纹未变；结构化原因和说明完整 | 确认销售单将退回销售处理，并展示销售后续三条固定出路 | 原子写驳回确认、工作流审计并完成当前任务；不创建后继任务；固定结果提供 W05 驳回处理入口后可继续下一项 | 失败保留输入；结果不确定停留并查询；不得复用当前任务重提 |
| 确认通过 | 底栏主动作 | `APPROVE` 可用；全部外采明细覆盖、能力资质有效、版本/指纹/租约有效 | 展示供应商拆分、成本、交期、履约方式及“销售单将生效” | 原子形成确认记录、销售正式版本/应收和采购创建依据并完成当前任务；固定结果后仅在当前 `autoNext` 生效时自动下一项，否则停留结果页 | 失败不本地生效；不确定时同幂等键查最终结果 |
| 打开销售单中心 | 页头/当前项 | 有 W05 对象权限 | 无 | 新开或聚焦 W05，W07 租约按后台策略处理 | 返回时重验租约与提交版本 |
...
- 驳回事务不创建任何后继 `work_item`。旧 `PROCUREMENT_CONFIRMATION` 任务保持 `COMPLETED`，其提交、指纹、确认分行、结论和审计永久留痕且不得复用。
- “改品或改价”出路：销售与客户重新确认后，由 W05 冻结递增 `submission_no` 的新提交和新 `subjectHash`，原子创建新的 `PROCUREMENT_CONFIRMATION`。
- “照原条件承接”出路：W05 先冻结新提交/新指纹、创建已注册 `LOW_MARGIN_MANAGER_CONFIRMATION`；只有上级通过事务才能完成该任务并创建新的 `PROCUREMENT_CONFIRMATION`，且采购仍须重新确认。
- “不做”出路：由 W05 原子作废生效前销售单，不创建任务。
- 无论改品/改价还是低毛利上级通过，新 W07 任务都只能关联新 `submissionId` / 新 `subjectHash`。不得改绑旧任务、复制旧确认分行、把上级同意当采购通过或绕过采购生效闸门。
...
查询响应不得包含 `claimToken`。它只由 W02 `ClaimWorkItemCommand` 或续租 mutation 的 `WorkItemLease` 返回，客户端仅保存在当前会话内存；列表刷新、当前项查询和跨工作面导航均不能再次携带或恢复它。
...
type SaveProcurementConfirmationCommand =
  WorkItemActionEnvelope<SaveProcurementConfirmationAction>
...
type DeferProcurementConfirmationCommand =
  WorkItemActionEnvelope<DeferProcurementConfirmationAction>
...
保存的是待处理确认的工作数据，不形成业务单据、不使销售生效、不完成或转交待办。`SaveProcurementConfirmationCommand` 直接复用 W02 `WorkItemActionEnvelope`，共享信封统一提供 `workItemId`、`claimToken`、`leaseVersion`、`expectedSubjectVersion`、`expectedSubjectHash`、本次保存动作专用的 `idempotencyKey` 和 `action`；W07 不复制这些任务字段，也不另建私有租约协议。`action.expectedEditVersion` 保护确认工作数据版本。成功结果的任务状态仍为 `PENDING` 或 `IN_PROGRESS`，并返回新 `editVersion`、规范化数值和完整校验摘要。

`DeferProcurementConfirmationCommand` 使用独立幂等键追加暂挂动作证据，并携带当前 `queueContextId`；返回 `DeferProcurementConfirmationResult`。成功结果的 `workItemStatus` 只能是 `PENDING` 或 `IN_PROGRESS`；服务端可使原 `claimToken` 失效并返回不含 token 的租约释放证据，也可保留原会话租约并返回 `WorkItemLeaseState`，但绝不签发/回显 `claimToken`、返回完成结果或改变采购确认正式结论。前端只在结果的 `queueContextId` 与当前上下文一致时，才把 `currentWorkItemId` 切到服务端返回的 `nextWorkItemId`。
...
type CompleteProcurementConfirmationCommand =
  CompleteWorkItemEnvelope<ProcurementConfirmationDecision>
...
type CompleteProcurementConfirmationResult =
  CompleteWorkItemResult<ProcurementConfirmationBusinessResult>
```

`CompleteProcurementConfirmationCommand` 直接复用 W02 `CompleteWorkItemEnvelope`，不得在 W07 重定义同名或等价私有信封。请求完整携带 `workItemId`、`claimToken`、`leaseVersion`、`expectedSubjectVersion`、`expectedSubjectHash`、`idempotencyKey` 和 `decision`；其中 `expectedSubjectVersion` 对应不可变销售提交版本，`expectedConfirmationEditVersion` 保护采购确认工作数据版本。
...
新 W07 任务的 `resubmissionContext` 由服务端从提交谱系生成。`CHANGED_TERMS_AFTER_REJECTION` 必须关联改品/改价后的新提交；`LOW_MARGIN_MANAGER_APPROVED` 必须同时关联已完成上级确认的受控证据。两种来源都使用本任务的新 `submissionId` / `subjectHash`，并在领取和决定时重新校验；旧驳回任务、旧指纹或上级任务租约均不能作为本任务的完成依据。

重复提交同一幂等键返回同一正式结果。超时或断网后用幂等键查询：

- 已成功：展示同一销售单版本和处理结果，再按原队列快照寻找下一项；
- 明确失败：保留当前输入并允许同幂等键重试；
- 仍未知：停在当前项，继续受控查询或转人工支持，不在本工作面临时创建新的任务类型，也不自动下一项。

服务端必须对同一不可变提交的有效 `PROCUREMENT_CONFIRMATION` 做唯一约束。重复查询或重放旧驳回的幂等结果只能返回同一 `REJECTED` 事实和三条销售出路，不能顺带创建后继任务；只有 W05 的改品/改价重提事务或 `LOW_MARGIN_MANAGER_CONFIRMATION` 上级通过事务，才能针对各自新提交原子创建一个新采购确认任务。

### 8.4 前端边界

- 前端只做输入格式、覆盖提示和服务端结果展示；供应商有效性、资质、数量守恒、正式成本/毛利、销售生效均由服务端决定。
- 前端不得把参考成本复制为最新成本后静默提交；自动带入值必须标明来源并要求用户确认。
...
| 刷新 | 保留当前数据，显示水位；不重置表单 | 查看；正式动作等待重验 | 成功更新未编辑字段 |
...
| 他人有效租约 | 当前项只读，显示处理人和到期时间 | 去下一项、打开销售单 | 租约释放或任务完成后重查 |
| 租约即将到期 | 连续条 warning 倒计时 | 续租、保存、暂挂 | 续租成功 |
| 租约丢失 | 本地输入只读保留，正式动作禁用 | 重新领取、复制输入 | 重取数据并处理冲突 |
| 正式动作进行中 | 锁定当前项与动作，禁止重复点击 | 无其它正式动作 | 返回确定结果 |
| 正式动作成功 | 固定结果展示销售单状态/版本、确认结果和采购创建依据 | 自动/手动下一项、打开销售单/W08 | 用户明确继续 |
| 驳回正式成功 | 固定结果展示旧提交/指纹、`REJECTED`、已完成旧任务和销售三条出路；不显示后继任务 | 打开 W05 驳回处理、继续队列 | 销售选择合法出路后，可能以新任务再次进入队列 |
| 改品/改价后新任务 | 来源 Banner 展示新提交/新指纹及上一驳回引用；不加载旧确认分行 | 领取、重新填写并独立通过/驳回 | 按新任务正式事务处理 |
| 低毛利上级通过后新任务 | 来源 Banner 展示新提交/新指纹和上级确认受控证据，明确“仍待采购确认” | 领取、重新填写并独立通过/驳回 | 上级证据不预选供应商、不自动通过 |
| 旧驳回任务被再次打开 | 只读固定结果，写动作和租约入口隐藏 | 打开 W05、返回队列 | 不恢复、转交或改绑旧任务 |
| 正式结果不确定 | 不显示成功，不跳下一项 | 查询最终结果、同幂等键重试 | 确定成功或失败 |
| 字段级隐藏 | 只读跨角色视图掩码；采购处理角色缺关键字段权限时不可处理 | 查看 blocker | 权限修正后重查 |
| 权限收回 | 停止续租，清除敏感缓存，显示无权限 | 返回有权模块 | 权限恢复后重查 |
...
| 1024×768 | 单列：销售摘要 → 明细 → 决策摘要；底栏固定 | 当前项、租约、阻塞、通过/驳回/暂挂 | 右栏改可展开摘要 |
...
- 租约告警、校验、成本变化和状态均使用文字，不仅依赖颜色；触控目标至少 44×44px。

## 11. 与其他工作面的关系

| 来源 / 去向 | Wxx | 携带上下文 | 返回规则 |
| --- | --- | --- | --- |
| 今日工作台 | W01 | `currentWorkItemId=workItemId`、`queueContextId` | 完成后 W01 正式待办立即刷新，指标投影按水位刷新 |
...
- [ ] 采购默认着陆后无需返回列表，可连续处理通过、驳回和暂挂。
- [ ] 二次确认页面和文案始终表达为“行为/任务”，不生成确认单号或纸质单据。
- [ ] 1440×900 下队列位置、不可变销售提交、至少两条确认分行、覆盖摘要和主动作同屏可见。
- [ ] 多供应商拆分能逐明细说明确认数量，不能用总量掩盖单行缺口。
- [ ] 打开 W05 深挖后返回仍恢复队列位置、筛选、当前项和显式 URL / 当前会话的自动下一项临时值；`preferenceScope` 未配置时不产生本地或服务端持久偏好。

### 12.2 数据、权限与正式动作

- [ ] 全部确认引用具体 `submissionId` 和 `subjectHash`，不读取可变销售草稿。
- [ ] 供应商、能力、资质、数量覆盖、成本、交期与履约方式均由服务端提交时重验。
- [ ] 通过事务原子完成确认、销售生效、正式版本/应收、当前任务完成和采购创建依据；未注册“采购建单任务”不成为销售通过 blocker。
- [ ] 驳回形成本次采购确认的正式 `REJECTED` 结论并完成当前任务，不创建采购单、变更单或任何后继任务；固定结果完整展示销售三条出路。
- [ ] 改品/改价重提只以新 `submissionId`、递增提交号、新 `subjectHash` 和新 `PROCUREMENT_CONFIRMATION` 返回 W07；旧确认分行与旧任务不复用。
- [ ] 照原条件承接先进入已注册 `LOW_MARGIN_MANAGER_CONFIRMATION`；上级通过前 W07 不产生新任务，通过后仍创建全新采购确认且不能自动通过。
- [ ] 不做路径在 W05 作废销售单且不创建任务；W07 只读保留旧驳回结果和审计链。
- [ ] 查询 View 不返回 `claimToken`；仅领取/续租 mutation 返回令牌，且只存在会话内存。
- [ ] 领取与续租维持 W02 的统一 `claimToken` / `leaseVersion`；保存和暂挂使用 `WorkItemActionEnvelope`，暂挂只返回 `PENDING/IN_PROGRESS`；正式决策使用 `CompleteWorkItemEnvelope`。
- [ ] 重复点击和超时重试不重复推进销售状态或生成多个采购创建依据。
- [ ] 驳回结果重放不创建后继任务；新提交与固定任务类型存在服务端唯一约束，低毛利上级任务和采购任务的幂等身份彼此独立。

### 12.3 状态与无障碍

- [ ] §9 全部状态完成组件测试或浏览器验证。
- [ ] 1440、1280、1024、768、375 五档视口符合 §10.1。
- [ ] 正式动作结果不确定时停留当前项，不自动下一项。
- [ ] 键盘可完成分行编辑、保存、驳回/通过和连续切换。
- [ ] 读屏可识别队列位置、租约变化、行覆盖、错误与固定结果。

## 13. 待确认事项

| ID | 问题 | 影响 | 建议决策人 | 当前建议 |
| --- | --- | --- | --- | --- |
| Q1 | 暂挂后是立即释放租约，还是在短时间内保留给当前采购？ | 队列公平性、并发与恢复 | 采购负责人 + 产品 | 立即释放并保持待处理，避免后台页签长期占用 |
...
| Q3 | 最新成本相对参考成本偏离到何种程度时，必须要求采购填写说明并形成“成本上涨”驳回提示？ | W07 校验摘要、采购说明和驳回引导 | 销售/采购负责人 + 财务 | 阈值由服务端业务规则配置，W07 不硬编码百分比；达到阈值只要求说明/提示驳回，不得自动创建 `LOW_MARGIN_MANAGER_CONFIRMATION`。只有销售在 W05 明确选择“照原条件承接”时才创建该任务 |
...
- `erp-data-model.md` §6.1 `work_item`：固定任务类型、租约、转交、完成与关闭约束。
