...
| `warnings` | 可确认但需注意 | 服务端校验 | 警告不等同于阻塞；相对上一采购确认成本的偏离阈值由服务端业务规则配置，W07 禁止硬编码百分比；服务端判定需说明时采购必须填写说明 |
| `rejectReasonCode` | 驳回原因 | `procurement_confirmation.reject_reason_code` | 无法履约、成本上涨、交期不满足、资质失效等固定码；`COST_INCREASE` 仅按服务端规则触发，销售端只接收风险结论，不得接收原始成本 |
| `comment` | 补充说明 | `procurement_confirmation.comment` | 驳回必填业务说明；服务端要求成本偏离说明时必须填写；不记录技术堆栈 |
...
当前权威 `work_item_type` 注册表没有“采购建单任务”。本期不建设该类任务。W07 通过不得创建或等待该类任务，也不得把其缺失作为阻断销售通过的 blocker；W08 只从 W07 固定结果或 W05 销售上下文读取采购创建依据并显式建草稿。仅当权威注册表已增加固定类型并补齐处理器契约后，才允许任务化。
...
- “照原条件承接”出路：仅当销售在 W05 明确选择该出路时，才创建已注册 `LOW_MARGIN_MANAGER_CONFIRMATION`；W05 先冻结新提交。W07 不得因成本偏离或毛利提示自行创建低毛利任务。只有上级通过事务才能完成该任务并创建新的 `PROCUREMENT_CONFIRMATION`，且采购仍须重新确认。
...
`preferenceScope` 缺失时：切换“自动下一项”必须只更新显式 URL / 当前会话状态，禁止写 `localStorage`、IndexedDB 或服务端用户偏好。仅当服务端明确返回 `DEVICE` 时允许写设备本地偏好，返回 `USER` 时允许通过用户偏好接口跨设备保存；无论哪种范围，显式 URL 仍只作当前会话覆盖。
...
- [x] 打开 W05 深挖后返回仍恢复队列位置、筛选、当前项和显式 URL / 当前会话的自动下一项临时值；`preferenceScope` 缺失时禁止产生本地或服务端持久偏好。
