...
│  ├ 单据头：合同来源 + 合同/客户结算            │  合同 / 客户 / 结算     │
│  │   · 已有合同：Combobox + 只读摘要条         │  业务性质 / 明细行数   │
│  │   · 上传 PDF：上传 + 客户/结算 + 编号/有效期 │  含税预估 / 下一步     │
│  │   · 商业约定六格：性质/负责人/场景/付款/期限/税率 │                 │
...
- 已有合同：`ContractCombobox`；客户 / 结算主体 / 合同精确版本由合同快照**只读摘要条**带出（不渲染 disabled 输入框）。
- 同步上传合同：客户 `CustomerCombobox`，结算主体 `SettlementPartyCombobox`；合同编号/签订日/有效期**直接展示**在表单中（不使用「合同档案信息」折叠条）。
