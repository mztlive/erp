...
> 最后更新：2026-08-03（列表：行点纸质预览、无侧栏 Sheet；合同列下载原始 PDF；去掉创建来源/主责列）
...
├ ListToolbar：搜索 | 业务性质 | 主责筛选 | 主状态 | 进度 | 高级筛      ┤
├ BusinessTableFrame ─────────────────────────────────────────────────┤
...
| ~~创建来源~~ / ~~当前主责~~ | **列表不展示**；主责仍可在筛选与对象中心表达 |
...
| 区域 | 目的 | 主组件 | 关键规则 |
| --- | --- | --- | --- |
| 列表 | 高频查单与分流 | `BusinessTableFrame` `DataTable layout="flush"` | 身份列与「查看详情」固定；**不**列创建来源/当前主责 |
...
| `originSystem` | 创建来源 | `sales_order.origin_system` | ERP / 商城 | **列表默认不展示**；对象中心/协同可展示；筛选不以此为主 |
| `ownerSystem` | 当前主责 | 主责登记 | 商城 / ERP，与创建来源分列语义 | **列表默认不展示**；工具栏可筛；对象中心必须可见 |
...
| 主责 | 创建系统、当前主责、唯一切换时间 | W05 `owner_system` / `origin_system` 事实 | 二期仅允许商城→ERP 一次性迁移，不提供回退动作 |
...
筛选、排序、游标和对象子区可恢复；表单输入不进入 URL。W05 **不**用 `preview=` 保留半屏 Sheet 状态（列表预览为瞬时浮层）。指标与列表使用同一查询水位和权限版本。导出只覆盖当前筛选、当前字段权限和服务端快照，大范围导出使用后台任务。
...
| 执行投影 | W23 | 销售版本、投影修订 ID | 返回协同区，不传主责结论快照 |
...
- [x] 创建来源与当前主责在**数据模型与对象中心**分列；任一时点只有一个写入主责。列表**默认不展示**这两列，避免与业务性质/进度抢视线。
