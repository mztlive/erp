...
| 销售单中心 | W05 | `salesOrderId`、基线版本 | 只修复主数据/版本问题；返回后重新预检 |
...
- [ ] 迁移范围只包含已生效及之后、未作废的正式存量卡券销售单。
- [ ] 商城草稿明确显示为不迁移、不补建，不会进入批次项目或统计。
- [ ] 每个批次只含一个客户；同批所有项目属于该客户。
- [ ] 销售、财务和最终基线三类确认由各自责任角色独立完成，管理员不能代签。
- [ ] 销售与财务确认均通过 W02 完整任务信封提交；确认事实、`workflow_action` 和对应任务完成在同一事务落地。
- [ ] `scopeHash` 或相应分面变化会使旧确认失效，并保留审计。

### 12.2 冻结、基线与执行

- [ ] 冻结期间所有受影响工作面显示不可忽略的维护 Banner。
- [ ] 最终基线确认只在冻结、最后同步和全量核对后可用。
- [ ] 基线登记不生成新销售版本；迁移执行基线明确标注为第一份投影修订。
- [ ] 迁移成功只改变主责标记，不换单号、不复制销售单、不改变应收、回款和发票。
- [ ] 批次任一项失败时全批未提交，界面不出现部分成功语义。
- [ ] 失败保持冻结并使用原批次续跑；其它完成批次不回退。

### 12.3 唯一 T 与不可回退

- [ ] 全部目标客户批次完成、一期轮询封存和固定检查链尾全部通过前，无法登记 `T`。
- [ ] 旧、失败、过期或已被替代的检查证据不能被当成当前通过。
- [ ] `T` 以商城为粒度原子登记，结果未知时先查询，不创建第二个切换。
- [ ] `T` 一经登记不可修改或删除。
- [ ] 页面不存在恢复商城主责、重开商城 B2B 建单或恢复一期轮询的动作。
- [ ] `T` 前支付只回填台账，`T` 及以后支付才进入自动供应商履约；界面不按接收时间判断。

### 12.4 状态、权限与响应式

- [ ] 无模块权限、无客户范围、无批次、筛选无结果、确认失效和字段掩码可区分。
- [ ] 执行进度不会被表达为正式项目成功数。
