...
| 场景 | 入口 | URL / 页签行为 | 返回位置 |
| --- | --- | --- | --- |
| 角色默认着陆 | 仓储/采购登录或侧栏 | `/fulfillment?scope=mine`，可见类型由角色收敛（§2.1），不需要额外的类型偏好 | 不适用 |
...
```text
/fulfillment?demoRole=warehouse&scope=mine&type=receipt&currentWorkItemId=wi_123&queueContextId=queue:W09:warehouse:mine&autoNext=1
```
...
> 线框已按实现更新（2026-08-03）。指标格与类型分段**只渲染当前角色可见的类型**，不是固定五格。

```text
┌ PageHeader：履约作业                              数据更新时间 09:36 ┐
├ MetricStrip：待入库 3 | 待仓发 3      ← 仅当前角色可见类型            │
├ 类型：[全部] [入库] [仓发]            ← 同上；越权类型不渲染          │
├ ListToolbar：搜索 | 仅我的/全组 | 仓库 | 到期 | 货款 | 身份 | 自动下一项│
├────────────── 待办 32% ───────────────┬──────── 当前任务 68% ─────────┤
│ 第 3 条，共 6 条 · 你正在处理这一条   │ 入库 · XS20260327018          │
│ [已超期] 入库 · XS… · CG…             │ 先款条件（PrepaymentGate）    │
│          待处理 100 个                │                               │
│ [待处理] 仓发 · XS…                   │ 当前类型表单                  │
│          待处理 80 套 · 另 1 行明细   │                               │
...
W09 的口语化措辞**不能**通过修改共享组件默认文案实现 —— 那会波及 W08/W10/W12。
