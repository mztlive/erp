...
- 敏感收货信息单次揭示有效时长必须为 5 分钟；离开对象页、关闭对象中心或权限变化时必须立即清除已揭示值，禁止跨页或跨会话保留。
- 转人工任务的默认责任边界必须按异常类型分配：供应商接单 / 履约异常归采购；员工售后沟通归客服；系统 / 接口异常归系统管理员。不得把默认责任混挂到未对应角色。
...
| 从异常任务处理 | W01 / W29 | 打开订单中心并携带 `workItemId`、`queueContextId` | 返回原队列上下文 |
| 从结算明细钻取 | W27 | 打开订单中心并聚焦“成本与结算” | 返回 W27 原结算明细 |
| 单击列表行 | W26 列表 | 打开 `detail` 半屏预览，URL 写入 `preview={id}` | 关闭后焦点回原行 |
| 打开详情 | detail 底栏「查看详情」或行操作「详情」 | 以稳定订单 ID 聚焦或新建任务页签 | 返回列表仍保留预览来源 |
...
├ MetricStrip：待提交 | 结果未知 | 履约异常 | 售后待处理 | 全部订单
├ ListToolbar：视图 | 搜索 | 供应商 | 三轨状态 | 支付时间 | 高级筛选
├ BusinessTableFrame
│ 订单号 | 商城单号 | 供应商 | 履约状态 | 取消 | 退款 | 外部单号 | 成本（含税） | 商品数 | 更新时间 | 操作
└ 分页
                                      ┌ detail 半屏预览 ───────────┐
                                      │ 来源、明细、三轨进度       │
                                      │ 最近动作、异常原因、下一步 │
...
| 成本 | `cumulativeCostGross` | 成本（含税） | 下单成本快照 | 无字段权限时以 `•••` 打码 | 采购/财务/管理员可见 |
| 商品数 | `items.length` | 商品数 | 明细行数 | 数字 | 全员有权对象可见 |
| 时间 | `lastBusinessAt` | 更新时间 | 状态历史 / 正式动作 | 发生时间 + 数据更新时间 | 全员有权对象可见 |

### 5.2 对象中心

| 子区 | 必须展示 | 事实来源 | 特别规则 |
| --- | --- | --- | --- |
| 概览 | 商城订单、支付凭证号、履约链归属、供应商、连接环境、供给/发布数据版本 | `mall_order`、`supplier_fulfillment_order` | 明确提示“商城支付已发生，当前处理供应商履约”；不展示内部支付事实键、乐观锁版本或连接内部 ID |
| 商品明细 | 商品快照、数量、供应商订货编码快照、发布/供给数据版本、下单成本快照、合计数量与合计成本行 | `supplier_fulfillment_item` | 一条商城明细只属于一个供应商子订单，不显示可改供应商；版本代号（SV-/PV-）不原样上屏 |
...
收货地址只在“履约与物流”按需揭示，单次有效时长必须为 5 分钟；离开对象页、权限变化或超时后必须立即清除。禁止出现在列表、导出默认列、URL、分析埋点或浏览器持久缓存中。

## 6. 搜索、筛选、排序与默认视图

| 能力 | 默认值 | URL 状态 | 行为 |
| --- | --- | --- | --- |
| Saved View | `actionable` | `view=actionable` | 默认仅显示结果未知、履约/取消/退款异常与售后待处理；长时间无状态变化但未超 SLA 的订单禁止计入异常指标，也不得进入 `actionable` 默认可操作集合，仅可在“全部”视图检索；可切“全部”“最近完成” |
| 搜索 | 空 | `q=` | 精确优先匹配 ERP 订单号、商城订单号、供应商外部单号 |
| 供应商 | 全部有权供应商 | `supplierId=` | 服务端按稳定 ID 过滤 |
| 履约状态 | 可操作集合 | `fulfillmentStatus=` | 支持多选，不与取消/退款状态混成一个枚举 |
| 取消 / 退款 | 全部 | `cancelStatus=` / `refundStatus=` | 两个独立筛选器，URL 支持多值（逗号分隔） |
| 售后待处理快捷筛选 | 关闭 | `aftersalePending=1` | 与指标口径一致：取消/退款异常态任一命中即计入；与指标数字完全对应 |
...
| 排序 | 优先级降序、异常发生时间升序 | `sort=` / `dir=` | 支持订单号/商城单号/外部单号/更新时间四列排序；排序变更重置页码 |
| 分页 | 50 条 | `page=` / `pageSize=` | 每页条数选择器与 URL 联动；浏览器后退可逐步撤销筛选 |

列表首屏在 1440×900 下至少显示 6 条有效数据行。刷新保留旧行；筛选/分页/搜索变更统一使用 `router.replace`，不膨胀历史；视图选择控件位于工具条内，使用 `ToggleGroup`；「清除筛选」只清筛选键、不重置视图，空态筛选无结果时同样提供。导出使用服务端选择快照，执行时重验权限和字段遮罩；导出结果不展示内部权限版本号与任务号，文件名使用日期。
...
| 管理员改派 | 管理操作 | 当前用户具备任务管理权限；目标责任人明确且仍具备处理资格 | 确认目标责任人、原因与影响 | 使用 `REASSIGN` 更新原开放任务责任人与审计，任务保持 `OPEN`；不创建后继任务或改变订单事实 | 失败时停留当前任务，刷新任务版本后重试 |
...
任何动作都不得把 `RESULT_UNKNOWN` 直接改成成功。明确业务拒绝不自动重试；供应商无查询能力时必须进入 W29 人工异常，且仅当外部工单或书面回复、核对时间、经办人、外部单号四项证据齐备时，才允许人工确认可验证结果。订单或售后已产生正式结果时，重复操作返回原结果而不是再次推进状态。查询 / 重放成功只表示证据和处理记录已追加，不表示任务完成；只有强类型终态确认才能完成任务。任务稍后继续时只保留当前已分配责任并维持 `OPEN`；不改变责任、不移动队列游标，也不自动进入下一任务。管理员改派只更新原开放任务责任；因业务版本变化确需替代任务时，必须关闭旧任务并由领域命令创建新的 `OPEN` 任务。
...
- W26 任务入口只允许本节注册的查询、重放和终态确认强类型命令；任务必须存在 `owner_user_id` 且已指定给具体用户，当前责任人与对象版本校验失败时不得执行。管理员改派另用 `REASSIGN`，仅更新原开放任务责任。
...
- 查询 / 重放成功或稍后继续时，原任务必须保持 `OPEN` 且责任人不变；前端不得移动队列游标、释放责任或创建后继任务。
- `SupplierOrderAfterSalesCommand` 是取消 / 退款领域直接动作：它只校验售后请求、订单版本和服务端固定幂等键，追加供应商动作及结果，不读取或改变 `work_item`。任务处理器不得把这类对象命令包装成“提交即完成”。
- 只有已经取得可验证终态时，任务处理器才可用 `SupplierOrderTaskCompletionCommand`；服务端必须重新校验 `verifiedSupplierActionResultId`、订单版本、当前责任人、主体版本和注册任务类型，并在同一事务固定业务结果与任务 `COMPLETED` 结果。证据仍未知时拒绝完成并保持原任务为 `OPEN`；重复/误派等关闭场景只能另用服务端授权的 `CLOSE` 动作，不得复用本完成命令。
- 管理员改派只使用 `REASSIGN`。服务端在同一事务更新原开放任务责任人与改派审计；不得创建同义后继任务或伪造业务终态。
...
| 正式动作成功 | `FormalActionResult` 固定展示供应商订单、履约/取消/退款轨结果、时间和下一步 | 返回 W25、进入 W27、继续查看订单 | 用户明确关闭结果 |
| 任务内查询 / 重放成功 | 固定展示本次证据、动作记录号与“任务仍待处理”；即使取得终态也不自动完成 | 继续查询、确认可验证终态；稍后继续时保留当前责任 | 同一任务保持 `OPEN` 且责任人不变；刷新任务版本 |
| 结果未知 | 固定警示，不改变本地订单状态 | 查询原结果；不能直接再次下单 | 得到可验证终态或进入 W29 人工异常 |
...
| 供应商供给 | W21 / W22 | SKU、供应商订货编码快照、供给/发布版本 | 返回保持订单页签 |
...
- [ ] 查询 / 重放后或稍后继续时，原任务仍为 `OPEN` 且保持当前责任人；不自动下一项，不创建后继任务。
- [ ] 取消 / 退款领域命令不读写任务；只有可验证终态能通过 `SupplierOrderTaskCompletionCommand` 完成任务；管理员改派仅更新原开放任务责任，不创建同义后继任务。
- [ ] 管理员改派仅可由任务管理权限调用 `REASSIGN`，目标责任人明确且原任务保持 `OPEN`。
- [ ] 供应商无查询能力时，人工确认必须齐备外部工单或书面回复、核对时间、经办人、外部单号；缺一禁止改写终态。
- [ ] `actionable` 默认可操作视图与异常指标均不包含长时间无状态变化但未超 SLA 的订单；此类订单仅可在“全部”视图检索。

### 12.2 数据、权限与安全

- [x] 下单时发布版本、固定供给、商品和成本快照不受后续基础资料变化影响。
- [ ] 地址、手机号、成本和技术摘要按字段权限独立控制；权限收回后无缓存泄漏。
- [ ] 敏感收货信息单次揭示有效时长为 5 分钟；离开对象页、超时或权限变化后立即清除，无跨页/跨会话残留。
...
- [x] 页面文案使用业务语言，不出现表名、HTTP 状态、堆栈或组件名称。
