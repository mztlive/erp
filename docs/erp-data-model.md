...
- 商品规格编辑命令必须在一个事务内把提交前后的规范化签名集合分类为“保留、新增、移除”：
  仅签名未变的行保留原 `sku_id` 并形成所需修订；新增签名必须分配新 `sku_id`；移除签名的
  旧 SKU 保留全部历史并转为停用，不得物理删除；
- 请求把既有 `sku_id` 绑定到不同签名、用相同 `sku_no` 恢复另一个签名的身份，或让一个
  既有/历史业务编码重绑到不同 SKU 时必须整体拒绝；`sku_no` 全局唯一约束不因停用而释放；
- 上述保留、新增和停用使用商品与受影响 SKU 的期望修订进行并发校验；任一冲突或校验失败
  整体回滚，不得留下部分规格组合；
- SKU 名称、规格、类型和状态搜索索引；
...
- 规格集合变化不能改写既有 `sku_revision_attribute_value`：签名不变时追加同一 SKU 的修订，
  签名变化时创建新 SKU，并在同一商品编辑事务中停用已移除签名对应的旧 SKU；
