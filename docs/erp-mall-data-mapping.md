...
4. 一次性迁移动作把存量单 `sales_order.owner_system` 从 `MALL` 改为 `ERP`，记录通用
   变更审计；迁移时点的当前 ERP 销售单版本作为第一份执行投影修订，不生成销售版本；
5. 迁移不换单号、不复制销售单、不改变应收、回款和发票；
6. 迁移完成后停止一期轮询，记录唯一 `mall_consumption_cutover` 时间点 `T`，
...
- 主责迁移是一次性迁移动作，把存量单 `sales_order.owner_system` 从 `MALL` 改为 `ERP`
  并记录通用变更审计；同一销售单不重复迁移，无批次、冻结或基线确认流程。
