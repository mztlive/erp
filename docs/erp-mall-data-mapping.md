...
| `sellable_item` / `sellable_item_revision` | 采购维护的可销售项目、参考供给和有效期 |
| `supplier_external_product` / `supplier_external_product_revision` | 二期 API 供应商外部商品稳定身份及原始目录版本 |
| `supplier_product_mapping` | 外部商品到 ERP SKU 的确认映射 |
| `supplier_offering` / `supplier_offering_revision` | 二期供应商供给稳定身份，以及供货价、区域、可供状态和有效期版本 |
...
| 规范对象 | 主要职责 |
| --- | --- |
| `sales_order` | ERP 稳定身份、销售单号、业务性质、业务来源和当前主责系统 |
...
| `sales_order_owner_migration_batch` / `sales_order_owner_migration_item` | 第二期主责迁移批次、基线和逐单审计 |

第二期主责迁移只改变同一张销售单的主责标记，不新建销售单、不换单号、不替换稳定身份。
...
客户编码、供应商编码、税号、条码和供应商外部商品编号都可能为空或重复，不作为未经核验的来源身份键。
...
5. 比当前已应用来源时间更早的迟到快照保留为“迟到丢弃”，不回退当前销售版本；
...
配置。当前两期均不进入 ERP 商业模型。若未来合同明确把签收升级为客户履约要求，
应新增正式履约字段和变更规则，不能把旧代码直接塞入备注。
...
`low_balance_warning`、`balance_available_warning`、`days_available_warning`、`credit_status` 使用“0 开启、1 关闭”的反向语义。即使需要迁移预警配置，也必须转换为正向命名的 `enabled`，不得按数据库布尔值直接复制。
...
只有经过采购确认的有效能力和必要资质才能用于新可销售项目、供给关系和采购单。
...
| `category` | 商品性质候选 | `PHYGOODS`、`VIRGOODS` 等代码须确认；不得与分类树混用，并补齐线下服务等 ERP 类型 |
...
| 正式启用 | 商品性质未知；没有可解释的 SKU；SKU 缺基础单位或规范规格冲突 | 可继续保留来源身份和历史追溯 |
...
| `supplier_sku_id`、`supplier_type` | 供应商外部 SKU 候选 | 必须结合补充的供应商连接和外部商品身份，确认后建立 `supplier_product_mapping` / `supplier_offering` |
...
五张旧表不能完成主责迁移。迁移还必须取得正式卡券销售单清单、稳定卡实例引用、
原销售单身份、初始余额及商城写入口状态。迁移固定执行：

1. P0 与 P1 能力同一生产发布批次就绪，销售和财务分别确认按客户拆分的迁移清单；
2. 进入维护窗口，商城永久关闭 B2B 建单入口并启用商业字段只读，ERP 和商城冻结
   本次范围内的新建、变更、制卡、卡实例、余额、支付、取消、退款、完成和余额恢复；
3. ERP 完成最后一次一期增量同步及全量指纹核对，记录最后水位；
4. 上线负责人确认最终权威基线、卡实例与初始余额基线，并为每张销售单登记第一份
   `sales_order_projection_revision` 迁移执行基线；
5. 系统管理员按客户创建并执行 `sales_order_owner_migration_batch`；一个批次内任何
   对象失败都不提交该批主责变化，保持冻结并使用原批次幂等续跑；
6. 成功批次只把原 `sales_order.owner_system` 从 `MALL` 改为 `ERP`，写迁移项和审计，
   不换单号、不复制销售单、不改变应收、回款和发票；
7. 全部批次完成后停止一期轮询，记录唯一 `mall_consumption_cutover` 时间点 `T`，
   再开放 ERP 卡券建单和商城员工业务；不设计商城建单重开或主责回退。
...
| 数据集 | 必需内容 | 规范目标 |
| --- | --- | --- |
| 外部商品身份与版本 | 供应商、连接、外部商品/SKU ID、供应商版本或摘要、名称、规格、分类、图片、说明、供货价、进项税率、费用、区域、可供状态、预计发货和商品能力 | `supplier_external_product`、`supplier_external_product_revision`；批次结果另记 `catalog_sync_job`、`catalog_sync_item` |
| ERP 商品映射 | 外部商品、ERP SKU、确认人、依据和有效状态 | `supplier_product_mapping` |
| 固定供给版本 | ERP SKU、供应商、外部商品、含税/不含税供货价、进项税率、运费、服务费、区域、可供状态、商品能力、最低订购量和有效期 | `supplier_offering`、`supplier_offering_revision` |
...
| SPU | 全部被历史订单引用的对象均有可追溯身份；新启用商品另需明确商品性质、分类处理结果和至少一个可用 SKU；商城发布信息与 ERP 核心信息已拆分 |
...
1. 冻结字段字典、状态字典、单位规则和安全白名单；
2. 在验证环境导入来源快照并完成自动校验；
3. 销售确认客户和卡券销售单映射；
4. 采购确认供应商、能力、资质、商品和供应关系；
5. 运营确认卡券类目、卡形态和商城发布关系；
6. 财务确认税率、卡券期初票款处理和银行信息，并确认旧供应商累计余额不生成 ERP 期初事实；
7. 仓储按统一基准日实盘确认 SKU 期初库存；
8. 输出成功、拒绝、差异和未覆盖清单；
9. 业务负责人签字确认后执行生产导入；
10. 成功形成正式数据的白名单导入包、manifest、规则版本、成功结果和映射审计长期
    保留；失败合规包及行列明细保留 30 天，失败批次元数据、汇总计数、脱敏错误码和
    审计长期保留。原始 SQL 只在隔离临时区使用，完成安全清洗和可选 keyed HMAC
    审计关联后按批准期限销毁。

一旦导入结果已经形成正式销售单版本、应收、付款、库存或其他下游事实，不通过覆盖或删除回滚，必须使用业务变更或纠正事实处理。
...
- 商品名称、规格、分类、图片引用、说明、供应商外部商品 ID、价格和可供信息可以进入商品暂存。
...
| 供应商连接、渠道和外部商品身份 | 解释 `supplier_type`、`third_channel_id`、`supplier_spu_id`、`supplier_sku_id` |
