...
| 编号 | 范围 | 必须完成 | 当前状态 |
| --- | --- | --- | --- |
| S3-01 | S2 必需依赖、M01／M02／M04／M05 | 统一范围消费者、业务组织筛选、详情／附件／打印、写命令和关联任务检查 | M04 列表、详情、候选、跨页、完整导出及主单命令已接入；客户功能接入与 S2-12 架构整改分别登记；合同／采购消费、关联入口、变更单及任务依赖未核销 |
...
8. 增量接口合同：[销售及成本范围读取接口合同](organization-data-scope-s3-openapi.yaml)。销售组织筛选、关联客户与合同范围、变更单与任务命令、附件及独立打印消费者仍未核销，不得据此登记 M04 全组完成。
