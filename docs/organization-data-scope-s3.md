...
| 编号 | 范围 | 必须完成 | 当前状态 |
| --- | --- | --- | --- |
| S3-01 | S2 必需依赖、M01／M02／M04／M05 | 统一范围消费者、业务组织筛选、详情／附件／打印、写命令和关联任务检查 | M04 列表、详情、候选、跨页、完整导出及主单命令已接入；关联客户／合同、变更单、任务及其他 S2 依赖未核销 |
...
6. 销售主单的历史参与仅用于 `list/detail`，不得用于任何写动作。销售建单所选客户仍使用既有客户归属入口，客户及合同 v2 接入前不得将建单依赖登记为完整；变更单、任务执行和责任交接须独立核销。
...
4. S3-02、S3-03、S3-04、S3-05、S3-07 及 S3-01 中未核销部分仍须实施；本批不得登记 S3 完成。
