...
7. 第 2、5 章已实施能力不自动视为符合主合同 v1.2 新增架构验收；按第 6 章复核。客户的架构整改沿用 S2-12，工作台／工作流仍为待接入项；不得将两者混记为已完成或同类实施错误。
...
| 编号 | 范围 | 必须完成 | 当前状态 |
| --- | --- | --- | --- |
| S3-01 | S2 必需依赖、M01／M02／M04／M05 | 统一范围消费者、业务组织筛选、详情／附件／打印、写命令和关联任务检查 | M04 列表、详情、候选、跨页、完整导出、主单命令、组织筛选、关联客户／合同、变更单及附件／独立打印消费者已接入；客户功能接入与 S2-12 架构整改分别登记；任务依赖未核销 |
| S3-02 | M03 | 选品册显式销售责任与组织、方案责任关联、列表／汇总／详情／候选／导出和命令范围 | 未实施 |
| S3-03 | M07／M08／M09 | 回款、付款、票据及正反分配；整单／余额授权依据；部分金额、完整凭证限制、经办人和处理人条件 | 未实施 |
| S3-04 | M06／M12 | 来源销售与采购、处理人查询、管理范围、受阻任务展示、独立改派候选和原有原子级联 | 未实施；现有工作台／工作流为尚未迁移的旧读取器，后续接入执行第 6 章，不登记为本批已改造但错误 |
| S3-05 | M10 | 当前负责客户经营情况与历史订单贡献分离；列表、汇总、下钻与导出 | 未实施 |
| S3-06 | M11 剩余项 | 独立销售／成本详情权限、跨功能客户端范围缓存联动、代表性索引检查及真实权限验收 | 读取及缓存代码已接入；索引计划与真实权限验收未执行 |
| S3-07 | 销售责任交接与验收联动 | 显式交接命令、目标资格、开放验收任务原子处理、新任务来源切换、审计幂等；审批与历史贡献保持 | 未实施 |
| S3-08 | 既有及新增消费者架构接入 | 公共规则复用、业务条件等价、纯判定归属、Port／adapter、事务与资源准入登记；A33—A36 | 待复核及整改；销售／成本既有调用公共入口不等于全部架构要求通过，按第 6 章核销 |
...
本表以 `3988ae56` 为静态登记基线；完整资源动作登记沿用主合同第 9.5 节与 [S2 接入登记](organization-data-scope-s2.md)。实现入口只说明当前代码位置，不构成架构或业务验收通过。
...
| 销售数据库条件与创建判定 | `erp-sales/src/repository/sales_order/scope.rs` | 数据库条件编译可保留在 Repository；其中 allows_creation 等纯授权判定必须按主合同第 9.3 节归属公共规则并经适配复用，不得与查询另设算法；条件等价性待验证 |
...
| 客户／合同／采购依赖 | 客户功能已接入但存在 S2-12；合同／采购范围消费未完成 | 客户整改以 S2 登记为准；后续业务 Service 通过本域 Port，生产 adapter 在 erp-processes；关联读取与命令各自核销，不以独立接口接入替代 |
| 选品册、票款、履约、工作台、客户经营质量等剩余链路 | 第 3 章对应项未实施 | 先登记事实来源、资源动作及必需维度，再接入公共解析；工作流 Port 必须取得已解析事实，禁止继续传原始范围供业务域解释；当前责任与历史归属各守其口径 |
...
| 验收项 | S3 必须提供的证据 | 当前登记 |
| --- | --- | --- |
| A33 | 本域 Port、组合层 adapter、实际公共入口及依赖图；违规依赖拒绝夹具与调用链检查结果 | 未核销；客户缺口见 S2-12，其余按资源补证 |
| A34 | 公共单对象范围判定与数据库条件编译对相同动作、对象事实、业务边界及筛选的集合等价证据，覆盖多角色、空范围、上限和历史参与 | 未执行新增等价验收；纯创建判定归属待整改 |
| A35 | 未接入资源配置／初始化拒绝、未装配 Port 失败关闭及旧消费者不解释 v2 的验证 | 未核销；准入基础缺口见 S2-13 |
| A36 | 列表、详情、候选、汇总、导出和命令复用对象映射，各自动作、事务、版本及撤权重验的覆盖记录 | 部分功能已有历史记录；新增架构与完整入口复核未执行 |
