...
1. 不可见详情返回 404 时必须同时撤下旧详情缓存，不得保留先前成功响应。`features/data-scope/cache.ts` 必须统一处理账号权限、组织版本、同查询业务范围版本变化和服务器撤权错误；撤下旧数据、取消未完成请求，防止旧响应重新写回；已观察到较新权限或组织版本后，拒绝较旧版本响应。单个查询的 `scope_version` 或 `DATA_SCOPE_CHANGED` 只使同类目录／业务查询失效，不得因此清理其他功能缓存；确认账号、全局 `policy_version`、`organization_version` 变化、会话失效或 401 时，才跨功能清除私有缓存。适用边界见[列表筛选候选独立查询执行合同](query-selector-decoupling-contract.md) §11.8。
