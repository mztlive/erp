...
1. 系统支持的正式单据由强类型 `DocumentType` 枚举确定，固定为第 4.3 节矩阵列出的 21 个值。
...
下表是全部 21 个固定 `DocumentType` 的唯一确定审批政策。实施人员不得增删行、不得留空、不得在编码阶段重新选择。
...
| `SalesInvoiceRequest` | 销项开票申请 | `PROCESS_REQUIRED` | `sales_invoice_request` |
...
2. 当前工作项责任人提交付款时，Service 必须在一个事务内校验任务身份、任务版本、应付对象、供应商、收款账户版本和核销分配，随后登记并过账付款、更新应付余额、同步任务进度并写审计；合并付款时还必须校验全部已勾选附加任务的身份、版本和当前责任人，且核销分录只能属于这些任务绑定的应付；
2a. 一张供应商付款单允许核销同一供应商、同一当前收款账户下多条开放付款执行任务对应的应付分录；分配合计必须等于本次打款金额，且每条已勾选任务都必须有正数分配；
...
销项发票 `Invoice` 维持 `NO_APPROVAL`；开票授权由独立的 `SalesInvoiceRequest` 审批。提交、额度占用、任务执行与切换操作按 [销项开票申请执行合同](sales-invoice-request-contract.md) 执行。
...
| `SalesInvoiceRequest` | `DRAFT`（创建与提交在同一事务） | `DRAFT` | `IN_APPROVAL` | `APPROVED` | `approval_subject_version` |
...
5. 12 个类型的撤回和受阻取消一律回到该类型的 `DRAFT` / `NOT_SUBMITTED`，审批驳回又不改变业务状态，因此审批导致的业务 `REJECTED` 已无可达路径，必须删除：资金类 5 个 `PROCESS_REQUIRED` 类型的 `REJECTED`、`ReviewStatus::REJECTED`、`SalesChangeOrderStatus::Rejected`、`PurchaseChangeOrderStatus::Rejected`、`StockAdjustmentState::Rejected`，以及采购单上与审批实例事实重复的整个 `PurchaseReviewStatus`（`Pending` / `Approved` / `Rejected`）字段。作废仍由各类型现有 `VOIDED` 表达，审批结果一律取审批实例投影。
...
4. **选源与履约路线后移到供给分配**：现有库存、供应商供给、最新成本、供货数量、采购预计交付日和履约责任均不属于销售单或销售审批，一律在销售单生效后的供给分配环节确认（`erp-phase-1.md` §7.4）。实物先推荐现有库存，剩余缺口再推荐采购；纯库存覆盖不得创建采购单。销售仅保存客户承诺的明细履约期限；审批决定不携带任何选源或履约路线字段，第 14.3 节的决定请求合同保持不变；
...
| `SalesInvoiceRequest` | `InvoiceRequestProcess::submit` | `InvoiceRequestProcess::approve` | `InvoiceRequestProcess::cancel_approval` | 授予额度并生成开票执行任务，不形成发票 |
...
5. 销售单生效并按 `erp-phase-1.md` §7.4 确认供给分配后，采购缺口的实际成本以采购单事实为准，现有库存直配的实际成本以库存成本事实为准；估算值不得继续展示为实际结论。
...
| -------------------------- | ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| 固定类型目录读取           | `approval_process:read`             | 返回第 4.3 节 21 行非敏感目录；不得包含定义节点或审批人详情                                                                |
...
目录必须逐行列出第 4.3 节全部 21 个固定类型，`NO_APPROVAL` 行固定显示「无需审批 / 不适用」且不提供任何写入口。`PROCESS_REQUIRED` 但无已发布定义时必须显示为「配置缺失」阻断状态，不得显示为「无需审批」。
...
9. 每个开放任务必须命中 `work_item_type + business_object_type` 的显式处理器登记。退役任务类型、未登记组合和无法解析的路由上下文必须在服务端失败关闭，前端不得回落到 W01 通用详情或其它业务页面；
10. 非审批任务的生产、处理器、正式命令、完成语义和信息要求必须符合 `workbench-workitem-contract.md`，不得由页面自行补充旁路语义。
...
13. 全部 21 个类型的 P3/P4 批次完成后执行 P0-D，删除旧注册表、bootstrap、角色解析、审批责任池、团队任务、旧销售确认实现和驳回终态路径。
...
- [ ] 第 4.3 节 21 行政策矩阵在代码中由对 `DocumentType` 的穷尽 `match` 实现，新增类型必触发编译失败或完整性测试失败。
