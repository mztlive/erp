...
5. 新对象目录才承诺内容版本：覆盖稳定排序后的 ID、编号、名称、状态及授权版本，且版本绑定该次规范化搜索条件。翻页必须携带相同规范化搜索条件的首页版本；旧版本遇撤权到空范围仍返回冲突。原 `/admin/warehouses` 保留业务参数并新增 scope_version 与范围元信息，第二页起必须传版本；该 scope_version 只承诺授权范围版本，名称、停用、修订和经办人变化不进入该版本，不具备内容哈希。跨页改名、停用或经办人变更须按该旧入口单独验收，不得用 `/admin/warehouse-directory` 的内容版本证明旧接口已有内容快照。
