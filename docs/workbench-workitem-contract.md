...
| `SUPPLIER_PAYMENT_EXECUTION`                         | `payable_account`                                                                                                                                                                                                           | 采购单最终通过并形成开放应付时，由付款责任规则解析具体财务人员                                                                    | 应付核销与付款登记面；允许一键合并同一供应商付款任务 | 付款登记、核销和过账同事务执行；开放应付清零时完成任务，分次付款或未勾选的合并任务保持开放                            |
...
2. 右侧详情必须直接嵌入对应业务作业面；嵌入模式必须锁定当前 `work_item_id`、业务对象、`subject_version`、任务版本和必要路由上下文，不得让用户切换到队列中的其它对象。付款任务允许出纳明确发起「一键合并付款」：弹窗默认勾选同一供应商、同一当前收款账户下的开放付款任务，当前任务不可取消，其它任务可取消；确认后作业面锁定已勾选任务集合，不得借合并切换到其他往来对象或任务类型。
3. 所有写命令必须携带任务身份、期望任务版本、业务对象身份、期望业务版本和幂等键；涉及敏感收款账户、映射候选、外部动作证据或安全暂停证据时还必须携带页面已核对事实的 ID 与版本。合并付款还必须携带已勾选附加任务的身份与期望任务版本。
4. Service 必须在命令事务内重验当前责任人、权限快照、任务开放状态、对象身份、冻结版本、岗位分离和领域前置条件。
5. 成功结果必须由正式领域命令完成、关闭或保持任务开放。前端不得先删除队列行，也不得调用通用 `complete` 补结束状态。合并付款时，一条命令可以完成已结清的多条付款任务，并保持仍有开放余额的任务开放。
...
1. 销售、采购及其已接入关联任务必须独立校验当前订单读取范围。负责人身份、审批指派、任务参与和管理身份均不得替代订单详情授权。
2. 负责人账号、完整执行权限或关联订单读取资格失效时，授权管理视图必须显示 `EXECUTION_BLOCKED` 与 `WORK_ITEM_OWNER_INELIGIBLE`。任务责任、状态和版本不得因读取而改变。
3. 失效任务不得返回执行、审批通过或驳回动作；已授权的非审批改派及关闭动作可以保留。审批任务不得通过该状态获得运行时转交能力；已有审批受阻状态和原因优先保留。
4. 任务列表、统计、详情和改派候选必须复用同一订单来源。采购责任级联中的全部开放履约任务均须在原写事务执行器重验接收人资格，任一失败不得部分转交。
