...
5. **内部词只出现在代码注释里**：租约、投影、幂等键等词禁止进入任何用户可见字符串。
...
| idempotency key / 幂等键 | `features/*/api.ts` | 原任务号 |
| subjectHash / 内容指纹 | `mock/*`、`features/*` | 数据版本 |
| work item / 任务项 | `features/workspace-kit/`、`mock/work-items.ts` | 任务 |
...
| 第 3 轮 | P2 长尾（正式/投影/幂等键/快照/事实） | 全量扫描通过 |
...
```bash
rg -n "专用处理器|租约|令牌|角色池|幂等键|投影|正式结果|正式提交|内容指纹|事实复核|快照" erp-client --glob '*.{tsx,ts}' -l
```

剩余命中必须逐条确认：属于注释 / 字段名 / 错误对象 key 的放行，用户可见字符串全部清零。
...
| G4 | 术语表是否需要落在代码层（`lib/ui-text.ts` 常量） | 防止再次漂移 | — | 建议第 2 轮后引入 |
