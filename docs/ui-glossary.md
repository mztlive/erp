...
> **「基线」保留场景**：主责迁移（基线确认 / 最终基线 / 卡实例/余额基线）、商城同步（期初基线 / 迁移基线）、供应商结算（成本基线）、商品发布（基线修订）为业务领域词，保留。
...
剩余命中必须逐条确认：属于注释 / 字段名 / 错误对象 key / 类型名的放行（`DISABLED_Q1`、`subject_hash` 数据字段、内部枚举值属性等），**用户可见字符串**（JSX 文本、title、description、label、message、header、placeholder、Alert、toast、事实表的 label/value）全部清零。验收注意区分「业务保留词」：核销、对账、差异、试算、冻结、回填、期初、基线（主责迁移/商城同步/成本基线）为业务词，不替换。
