...
> **状态**：基线 v1.5
> **依据**：`erp-client` 全量扫描（2026-08-03）+ W21 供应商供给模型（2026-08-08）+ `approval-workflow-contract.md`（2026-08-17 生效合同）；`erp-ui-design.md` 的语言原则
...
5. **内部词禁止上屏**：work_item、领取/重新领取、团队待处理、开始处理、退回团队、动作命令、审批步骤实例、对象版本、幂等键、水位等实现词**禁止**进入任何用户可见字符串；租约、令牌、信封、指纹等分布式协议概念**禁止**出现在用户界面。
...
| 处理租约已释放 | `workflow.tsx:370` | 处理权已变化，请刷新 |
...
| 领取后取得租约方可正式处理 | `unified-task-queue-page.tsx:1252` | 选中任务后即可提交 |
| 角色池任务待领取 | `unified-task-queue-page.tsx:1250` | 待我处理 |
| 角色池待领取 | `fulfillment-operations/api.ts:123`、`card-funds-review/api.ts:194`、`procurement-confirmation/api.ts:176`、`mock/workspace.ts:592` | 待我处理 |
...
| 映射异常处理 | 打开单据 |
| 回款核对 | 打开单据 |
| 收货与发货 / 交付与代发 / 电子履约 / 实物履约 / 服务履约 | 打开单据 |
| 其他已登记非审批任务 | 打开单据 |
| 任意未登记类型 | 打开单据（兜底） |

审批任务的主按钮**必须**是「通过 / 驳回」。非审批任务的主按钮**必须**是「打开单据」；正式动作在对应业务工作面提交。**禁止**「处理器」「领取」「开始处理」「退回团队」。
...
| 未提交单据升级 | 更新审批流程版本 |
| 原提交人取消运行 | 撤回审批 |
| 运行管理员代替原提交人取消运行 | 应急撤回审批 |

审批运行时不得提供转交、改派、转签或委托文案与入口。

流程配置页面不得显示 `ProcessKind`、instance、execution、binding、CAS、幂等键、内部 ID、`DIRECT`、`POOL` 或责任池。采购确认节点显示为「采购确认」，不得显示旧任务类型名。
...
| claim / 领取 / 重新领取 / 开始处理 / 退回团队 / 团队待处理 | `features/*/api.ts`、任务组件状态 | 待我处理 / 刷新任务 |
...
| role pool / 角色池 | `features/*/api.ts`、`mock/workspace.ts` | （不出现；任务创建即指定到人） |
...
| P1 | 状态条与提示（领取/租约/令牌/角色池/团队待处理） | 界面**禁止**出现「领取」「重新领取」「团队待认领」「团队待处理」「开始处理」「退回团队」「租约」「令牌」「角色池」 |
...
```bash
rg -n "专用处理器|领取|重新领取|团队待认领|团队待处理|开始处理|退回团队|租约|令牌|角色池|幂等|投影|正式结果|正式提交|正式操作|正式待办|正式终态|正式水位|内容指纹|对象指纹|事实复核|快照|work_item|任务信封|fail-closed|subject_hash|水位|对象版本" \
...
| 导出 | 用途 |
| --- | --- |
| `responsibilityText` | 当前责任、待我处理与处理权变化提示 |
...
完整路由显示名仍以 `lib/workspace-registry.ts` 为准；`workspaceLabel` **必须**提供提示语短名（如「我的工作台」而非内部编号）。
...
| 领取 / 处理权 | （删除领取）/ 你正在处理这一条 | 与合同 §13.2、§16.4 一致 |
| 作业队列 / 作业类型 | 待办 / 任务类型 | |
