...
- 退回团队、驳回、处理失败都必须说明当前项是否仍在本队列、下一项是否已打开，以及如何恢复。
...
审批规则 UI 约束（业务已定）：审批人**不能改单**；驳回后销售改内容须创建新审批实例并从领导步骤重新开始。
中心用 `SequentialProcessBar` 展示审批链位置；只有活动步骤创建待办，直接指派任务不显示“开始处理”，
责任池任务使用“团队待处理 / 开始处理”。完整规则以 `approval-workflow-contract.md` 为准。
...
| ⌘↵ | 正式提交（在编辑区且校验通过时） |
| ⌘S | 保存草稿 |
| g 然后 t | 必须提供：连续键 g 再 t 回到今日工作台（vim 风格双键） |
| Esc | 关闭最上层浮层；不关闭任务页签 |
...
| M3 二次确认队列 | `/procurement/confirm` | 连续处理、确认、驳回原因、退回团队、自动下一项与结果反馈 |
