...
| 建单 | 销售 | W05 新建页签；合同来源二选一（已有有效合同 / 同步上传 PDF）与公司商品池选择器 | 草稿保存或提交成功结果面板；失败不留下空合同 |
...
采购单中心与履约作业均使用 `PrepaymentGate`，展示密度不同：

- **W08 采购单中心**：完整 `panel` 卡片，展示门槛、有效已付、差额与结论。
- **W09 履约作业**：`presentation="badge"` 挂在连续处理条状态区；顶栏只显示结果徽章（可收货 / 暂时不能收货 / 无先款要求），悬停展开详情。
