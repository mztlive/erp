...
- 第二期上线时停止从商城开设新销售单，存量商城主责卡券销售单 `owner_system` 由 `MALL` 改为 `ERP`。
- 仅改字段并记录通用变更审计，无批次、冻结或基线确认流程；创建来源由 `sales_order.origin_system` 表达，来源与主责信息在 W05 销售单对象中心展示。
...
| 2026-08-03 | W05 列表改为行点纸质投影（无 Sheet / 无打印钮）；合同列下载原始 PDF；§1.1.1 |
| 2026-08-03 | W05 列表改为行点纸质投影（无 Sheet/无打印钮）；合同列下载原始 PDF；§1.1.1 |
