...
| 销售单审批 | 已配置审批人 | W01 右侧详情；决定只有通过/驳回 | 通过后自动下一项；驳回不改单，下一轮从入口节点重新开始 |
| 看结果 | 销售 | 列表纸质投影或原销售单中心 | 状态=已生效；下一步提示「待采购建单」 |
| 采购单 | 采购 | 从销售单中心「履约/采购」子区「去建单」或 W08；**在此选源**（供应商供给、成本、数量、交期、履约方式） | 提交审核 |
| 审核 | 财务 | W01 待我处理（待办同源） | 通过后应付可见 |
...
- 原「采购二次确认」是 `SalesOrder` 已发布定义中的普通单人审批节点，**必须**在 W01 详情内连续处理。
- 决定请求只允许通过 / 驳回；驳回原因必填。**禁止**在决定里提交供应商、成本、交期或履约方式。
- 成功：通过或驳回后当前任务完成并进入下一项（或本筛选项已处理完）。
- 失败：校验不通过或提交失败时停留当前项，展示错误，不得静默跳过。
- **禁止**再建设 W07 独立队列，也**禁止**领取、开始处理或退回团队。

### 4.2 选源在采购单

选源明细（选择供应商供给、最新成本、供货数量、预计交期、履约方式）**必须**在销售单生效后创建采购单时录入，见 `erp-phase-1.md` §7.4 与 W08。

1. **只读**：已生效销售明细、客户交期、附件。
2. **可写（采购单创建）**：供应商供给、成本、数量、交期、履约方式。
3. **链出**：打开销售单中心（新页签），工作台页签保留。

### 4.3 销售单生效后

- 销售单状态随审批最终通过更新为已生效。
- 下一步提示「待采购建单」；建采购单仅在 W08 / 销售单中心「去建单」路径完成。
- **禁止**提供「通过并创建采购单草稿」一键动作。
...
入口 B：W05 销售单票款 → 登记回款（预选当前应收）
  ...
  同一 Allocation 会话（任务页签标题：核销 · 客户名）
      ↓
  提交审批 → 回款 IN_APPROVAL → 末节点通过后过账并形成核销
      ↓
  FormalActionResult → 停留会话或返回来源页签
```
...
- 审批中的回款不计入已收、开放应收或卡券票款复核；只有末节点通过事务形成正式回款和分配。
...
```
销售提交 → 启动审批实例 → 已发布定义中的有序单人节点 → 最终通过并生效 → 自动下发执行信息
```

- `VoucherSalesOrder` 绑定已发布定义；节点、顺序和审批人由管理员配置，创建时冻结版本。
- 每个节点只指定一个具体用户，任务直接进入该用户 W01「待我处理」。
- 审批人在 W01 详情内只读 + 通过/驳回；销售单中心同步展示运行摘要。
- 驳回不改变单据和 `subject_version`，实例轮次加一并回到入口节点。
- 审批人**无**字段编辑器。
- 节点通过、当前待办完成和下一节点激活必须同事务；最终通过与销售单生效必须同事务。
- 完整运行规则以 `approval-workflow-contract.md` 为准。
