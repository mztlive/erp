...
- 采购二次确认是销售单生效闸门，**必须**在 W07 **M3 队列**中连续处理；默认「完成后自动下一项」。
- **禁止**以「列表点进普通详情 → 再找按钮 → 返回列表」作为主处理路径。
- 成功：通过 / 驳回后当前任务完成并进入下一项（或队列空）；退回团队只清空个人责任并保持任务开放，用户手动进入下一项。
...
3. **决策**：通过 | 驳回（原因必填）| 退回团队。
4. **链出**：打开销售单中心（新页签），队列页签保留。

### 4.3 通过后

- 销售单状态**必须**更新为已生效（同步域）。  
- **必须**生成「待建采购单」任务进入采购员工作台（可按供应商拆分提示）。  
- **禁止**提供「通过并创建采购单草稿」一键动作；建采购单仅在 W08 / 销售单中心「去建单」路径完成。
...
```
销售提交 → 启动审批实例 → 销售领导步骤 → 运营步骤 → 审批通过并生效 → 自动下发投影
```

- 固定注册 `CARD_SALES_APPROVAL` 定义：销售领导步骤为 `DIRECT`，运营步骤为 `POOL`；
  只有当前活动步骤创建一个开放待办，后续步骤等待激活。
- 销售领导直接在“我的待办”处理；运营成员从“团队待处理”执行“开始处理”。
- 审批人中心只读 + `ApprovalDecisionPanel`。
- 驳回原因结构化；销售改单并重新提交后创建新审批实例，进度从领导步骤重新开始。
- 审批人**无**字段编辑器。
- 步骤通过、当前待办完成和下一步骤激活必须同事务；运营最终通过与销售单生效必须同事务。
- 完整运行和 BPM 兼容规则以 `approval-workflow-contract.md` 为准。
