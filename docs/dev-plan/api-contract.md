...
- **产出位置（双落点）**：`backend/apps/web-api/build.rs` 同时写入
  - `fronts/admin/src/constants/permissions.generated.ts`（import `@/types/admin` 的 `PermissionItem`）
...
- **CI 漂移校验**：P3 验收 gate `permissions_no_drift`（`_meta.json`）；
  ...
  （P3-service-api.md §5.1，fronts/admin 侧同法）。P0-6.2 将落地
