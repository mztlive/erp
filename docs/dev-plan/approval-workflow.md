...
| 团队业务任务禁止规则（采购确认为可选普通节点，低毛利环节删除） | 合同 §4.4.3、§5.2 | 已签署 |
...
`P6-PILOT` 之后按**文件所有权**分轨，不再要求跨域串行。同一 `owns`/`ownsWithin` 路径仍须串行；类型专属文件可以并行。精确 `dependsOn` 与 `owns` 以 `_meta.json` 为准。

SCHED-2 起：未开始的退货无审批 P3、履约剩余无审批 P3 只拥有各自类型文件，不再占用 `returns/**` / `fulfillment/**` 整目录，因此可与回款冲正 P3、已解锁的 P4 同波并行。回款冲正与付款冲正仍共享 `adapter.rs` / `start_approval.rs` / `dto.rs` / handler，必须串行。履约 P4 仍共享 `fulfillment-operations/**`，必须串行。
...
| 退货资金 | `returns` 共享文件 | 见上列应收/应付/销售/采购 P4 | CustomerRefund → SupplierRefund → ReceiptReversal → PaymentReversal（共享 adapter/start/dto） |
| 退货无审批 | 类型专属文件 | 销售/采购页 | SalesReturnCase ∥ PurchaseReturnOrder（P3 互不共享文件） |
| 履约 | `fulfillment` 共享文件 / 类型专属文件 | `fulfillment-operations` | PurchaseReceipt → Delivery；其后 ElectronicDelivery ∥ ServiceFulfillment ∥ CustomerAcceptance（P3 类型文件）；P4 仍串行 |
...
| `SalesReturnCase` | `P3-NO-APPROVAL-SALES-RETURN-CASE` | `P4-SALES-RETURN-CASE` | 类型文件 P3，可与冲正并行；P4 接销售页 |
| `PurchaseReturnOrder` | `P3-NO-APPROVAL-PURCHASE-RETURN-ORDER` | `P4-PURCHASE-RETURN-ORDER` | 类型文件 P3，可与冲正并行；P4 接采购页 |
...
| `ElectronicDelivery` | `P3-NO-APPROVAL-ELECTRONIC-DELIVERY` | `P4-ELECTRONIC-DELIVERY` | P3 类型文件，可与其他履约无审批 P3 并行；P4 仍串行 |
| `ServiceFulfillment` | `P3-NO-APPROVAL-SERVICE-FULFILLMENT` | `P4-SERVICE-FULFILLMENT` | P3 类型文件并行；P4 仍串行 |
| `CustomerAcceptance` | `P3-NO-APPROVAL-CUSTOMER-ACCEPTANCE` | `P4-CUSTOMER-ACCEPTANCE` | P3 类型文件并行；P4 履约终点 |
...
| 编号 | 主题 | 分支 | 状态 |
| --- | --- | --- | --- |
| P0-A | `bpm` 地基与依赖边界 | `chore/erp-p0-amend-approval-workflow-foundation` | 已合并 |
...
| SCHED-2 | 收窄未开始 P3 的 `owns` 到类型专属文件，拆开退货无审批与履约剩余 P3 的 `dependsOn` | 计划元数据，无独立代码分支 | 已登记 |
