...
10. 审批与待办必须按 `docs/approval-workflow-contract.md` 和 `api-contract.md` §8 迁移。
    此项允许并要求修改既有导出签名、`queries.ts`、页面动作和类型；不得以本文件第 1 节
    “保持导出签名不变”为由保留旧 `claim / complete / UNCLAIMED / IN_PROGRESS` 契约。
11. W01/W02 以及承接审批、复核、确认任务的业务工作面必须共用服务端责任事实：
    `DIRECT` 直接进入“我的待办”，`POOL` 通过“开始处理”建立个人责任；正式结果只能调用
    已注册的强类型领域接口，页面不得选择下一审批步骤或补发任务完成请求。
...
| 新鲜度 | 分析类页面展示数据更新时间 |
| 任务责任 | `DIRECT` 无“开始处理”；`POOL` 仅团队待处理可开始，同人重试幂等、他人冲突后刷新 |
| 任务版本 | 队列和嵌入任务投影均返回 `taskVersion`；所有 task-bound 写命令原样回传，禁止用 `subjectVersion` 代替 |
| 查询范围 | `managed` 可见主管授权范围内全部开放团队任务；`history` 仅展示只读终态任务 |
| 审批受阻 | 保留的开放待办展示受阻原因且无普通动作；管理员恢复使用独立阻塞视图 |
| 正式处理 | 只调用注册的强类型领域接口；响应同时返回业务结果与任务结果，不补发公共完成请求 |
| 多级审批 | 页面只呈现当前活动步骤；不得预建未来步骤待办或由客户端选择下一步骤 |
| 阻塞恢复 | 管理员只可重试当前步骤；请求不含决定、指定处理人或目标下一步骤 |
...
| 为兼容旧 F7 证据保留领取、租约或通用完成接口 | 打回；历史证据不具有现行契约效力 |
