...
3. 任务责任人固定取销售单显式负责销售 `sales_owner_user_id`（S3-07 独立联动批次起；此前为 `stable.created_by`），责任组织固定取销售单 `settlement_party_id`；新任务形成前必须校验账号可用及 W06 完整执行权限。销售责任交接必须在同一事务原子转交同一销售单下全部开放验收任务，后续新任务取交接后负责销售；开放审批任务、已完成验收与历史经营归属不随交接改写。
4. 客户验收任务必须写入下列稳定事实：
