...
async function main() {
    ...
    for (const it of body.items ?? []) {
        console.log(JSON.stringify({ work_item_id: it.work_item_id, status: it.status, processing_state: it.processing_state, object: it.business_object_id, type: it.work_item_type }))
    }
}
