 ...
 *   W08 采购从已生效销售单核对创建依据、创建采购单并提交(caigou) → 财务审核通过(caiwu) →
...
async function pickFirstComboboxOption(
    page: Page,
    placeholder: string,
): Promise<void> {
...
async function pickDayInCalendar(page: Page, day: string): Promise<void> {
    ...
    await calendar
        .locator(
            `[data-day="${m}/${d}/${y}"],[data-day="${y}/${m}/${d}"],[data-day="${day}"]`,
        )
        .first()
        .click()
 ...
 */
async function approveInWorkspace(
    page: Page,
    guardText?: string,
): Promise<void> {
...
async function ensureWarehouse(request: APIRequestContext): Promise<void> {
    ...
    await api(request, "POST", "/admin/warehouses", {
        token,
        body: {
...
async function openAllocationSession(
    page: Page,
...
test("flow-01 实物销售单仓发完整基准流程", async ({ page, request }) => {
    ...
    await expect(customerDialog.getByText("新建客户")).toBeVisible({
        timeout: 10_000,
    })
    ...
    await pickComboboxOption(
        salesPage,
        salesPage.getByLabel("默认付款条件"),
        "货到 15 天",
    )
    await clickButton(salesPage, "创建客户")
    await salesPage
        .getByRole("link", { name: CUSTOMER_SHORT, exact: true })
        .click()
    ...
    await expect(
        uploadDialog.getByRole("heading", { name: "上传合同 PDF" }),
    ).toBeVisible({
        timeout: 10_000,
    })
    await salesPage
    ...
    await expect(
        uploadDialog.getByRole("combobox", { name: "结算主体" }),
    ).toHaveValue(CUSTOMER_LEGAL, { timeout: 20_000 })
    await expect(
        uploadDialog.getByRole("combobox", { name: "付款条件" }),
    ).toHaveValue("货到 15 天")
    ...
    const contractInput = salesPage
        .getByPlaceholder("搜索合同编号或客户")
        .first()
    ...
    const salesOwnerInput = salesPage.getByLabel("负责销售")
    await expect(salesOwnerInput).toHaveValue(/.+/, {
        timeout: 20_000,
    })
    const salesOwnerName = (await salesOwnerInput.inputValue()).trim()
    // 单据头：福利场景 / 付款条件（合同已带出则跳过）/ 履约期限
    await pickComboboxOption(
        salesPage,
        salesPage.getByLabel("福利场景"),
        "年节礼包",
    )
    ...
    await expect(submitDialog.getByText("提交销售单")).toBeVisible({
        timeout: 10_000,
    })
    await submitDialog.getByRole("button", { name: "确认提交" }).click()
    await salesPage.waitForURL(/\/sales\/orders\/[0-9a-f]{24,32}/, {
        timeout: 30_000,
    })
    const salesOrderId = salesPage
        .url()
        .match(/\/sales\/orders\/([0-9a-f]{24,32})/)![1]
    const salesOrderNo = (await salesPage
        .getByText(/XS\d{8,}\d*/)
        .first()
        .textContent())!.trim()
    ...
    await expect(poPage.getByText("已生效").first()).toBeVisible({
        timeout: 20_000,
    })

    // ── W08 从销售单直达采购建单，核对上下文后创建并提交 ─────────────────────
    await gotoPage(poPage, `/sales/orders/${salesOrderId}?section=fulfillment`)
    const createPurchaseLink = poPage.getByRole("button", {
        name: "去建单",
        exact: true,
    })
    const purchaseLaneBeforeCreate =
        createPurchaseLink.locator("xpath=ancestor::li")
    await expect(purchaseLaneBeforeCreate).toContainText("0 笔", {
        timeout: 20_000,
    })
    ...
    const basisLine = basisDialog
        ...
        .first()
    await expect(basisLine).toBeVisible()
    await expect(
        basisLine.locator('[data-slot="quantity-value"]'),
    ).toContainText("1")
    ...
    const purchaseOrderId = poPage
        .url()
        .match(/\/procurement\/orders\/([0-9a-f]{24,32})/)![1]
    await expect(poPage.getByText("采购草稿").first()).toBeVisible({
        timeout: 20_000,
    })
    await expect(poPage.getByText(salesOrderNo).first()).toBeVisible()
    await expect(poPage.getByText(salesOrderId, { exact: true })).toHaveCount(0)

    // 草稿创建后销售单立即反映真实关联数，动作切换为仅查看；列表按关系键过滤。
    await gotoPage(poPage, `/sales/orders/${salesOrderId}?section=fulfillment`)
    const openPurchaseLink = poPage.getByRole("button", {
        name: "打开采购",
        exact: true,
    })
    const purchaseLaneAfterCreate =
        openPurchaseLink.locator("xpath=ancestor::li")
    await expect(purchaseLaneAfterCreate).toContainText("1 笔", {
        timeout: 20_000,
    })
    await expect(openPurchaseLink).toBeVisible()
    await expect(
        purchaseLaneAfterCreate.getByText("去建单", { exact: true }),
    ).toHaveCount(0)
    await openPurchaseLink.click()
    await expect(poPage).toHaveURL(
        new RegExp(`/procurement/orders\\?.*salesOrderId=${salesOrderId}`),
        { timeout: 20_000 },
    )
    await expectTableRow(poPage, salesOrderNo)

    await gotoPage(poPage, `/procurement/orders/${purchaseOrderId}?mode=edit`)
    // 供应商/成本/交期来自创建依据（只读或已带出），数量/单价/税率已带出；直接提交审批
    await clickButton(poPage, "提交审批")
    const poSubmitDialog = dialogOn(poPage)
    await expect(poSubmitDialog.getByText("提交采购单")).toBeVisible({
        timeout: 10_000,
    })
    ...
    await expect(
        poPage.getByText("审批中", { exact: true }).first(),
    ).toBeVisible({
        timeout: 20_000,
    })
    ...
    await expect(financePage.getByText("已生效").first()).toBeVisible({
        timeout: 20_000,
    })
    ...
    await gotoPage(
        whPage,
        `/fulfillment?lane=warehouse&purchaseOrderId=${purchaseOrderId}&autoNext=0`,
    )
    ...
    await gotoPage(
        whPage,
        `/fulfillment?lane=warehouse&salesOrderId=${salesOrderId}&autoNext=0`,
    )
    ...
    const factCheckbox = salesPage
        .getByRole("checkbox", { name: /仓发/ })
        .first()
    ...
    await expect(salesPage.getByText("客户验收已登记")).toBeVisible({
        timeout: 20_000,
    })
    ...
    await expect(receiptDialog.getByText("提交回款")).toBeVisible({
        timeout: 10_000,
    })
    await receiptDialog.getByRole("button", { name: "确认提交" }).click()
    await expect(financePage.getByText("回款已提交审批")).toBeVisible({
        timeout: 20_000,
    })
    const receiptNo = (await financePage
        .getByText(/已进入审批。单号 \S+/)
        .first()
        .textContent())!.match(/单号 (SK-\d+)/)![1]
    // 销售领导复核
    ...
    await expect(receiptRow.getByText("已过账")).toBeVisible({
        timeout: 20_000,
    })
    ...
    const salesHeader = salesPage.locator('[data-slot="document-header"]')
    await expect(salesHeader.getByText("已关闭", { exact: true })).toBeVisible({
        timeout: 20_000,
    })
    const parallelStatuses = salesHeader.getByRole("list", {
        name: "单据并行状态",
    })
    await expect(
        parallelStatuses.getByText("已结清", { exact: true }),
    ).toBeVisible()
    const amountSummary = salesHeader.getByLabel("销售单金额摘要")
    const receivedAmount = amountSummary
        .locator(":scope > div")
        .filter({ hasText: "已回款" })
    await expect(receivedAmount).toContainText("¥100.00")
    await expect(
        salesHeader.getByRole("button", { name: "去验收" }),
    ).toHaveCount(0)
    await expect(
        salesPage.getByText("可以做客户验收", { exact: true }),
    ).toHaveCount(0)
