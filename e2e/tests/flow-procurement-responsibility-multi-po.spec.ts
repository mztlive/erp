...
async function pickToday(page: Page, label: string): Promise<void> {
    ...
    const dayAttribute = await page.evaluate(() => new Date().toLocaleDateString())
...
async function expectCoverage(
    ...
    await expect
        .poll(
            async () => {
                const detail = await salesOrderDetail(
                    request,
                    token,
                    salesOrderId,
                )
                return {
                    ...
                    total: numericQuantity(
                        detail.purchase_coverage.total_quantity,
                    ),
                    covered: numericQuantity(
                        detail.purchase_coverage.covered_quantity,
                    ),
            ...
            order.lines
                ...
                .reduce(
                    (sum, line) =>
                        sum +
                        numericQuantity(
                            line.allocated_quantity ?? line.quantity ?? "0",
                        ),
                    0,
                ),
        )
...
function visibleBasis(dialog: Locator): Locator {
    return dialog
        .locator(
            '[data-testid^="purchase-basis-"]:not([data-testid^="purchase-basis-line-"])',
        )
        .first()
}

function visibleBasisQuantity(dialog: Locator): Locator {
    return visibleBasis(dialog)
        .locator('[data-testid^="purchase-basis-line-quantity-"]')
        .first()
}
...
test("采购责任在销售最终生效后派发建单任务，并按 4 + 6 建立两张采购草稿", async ({
    ...
    await test.step("销售创建数量 10 的外采销售单并看到只读采购负责人", async () => {
        ...
        await pickComboboxOption(
            page,
            page.getByLabel("福利场景"),
            "年节礼包",
        )
        await pickComboboxOption(
            page,
            page.getByLabel("履约方式"),
            "供应商直发",
        )
        await pickToday(page, "履约期限")
        await pickComboboxOption(
            page,
        ...
        await expect(
            page.getByText("审批中", { exact: true }).first(),
        ).toBeVisible({ timeout: 20_000 })
    })
    ...
    await test.step("采购确认通过后仍停在销售领导审批，且没有采购建单任务", async () => {
        ...
        expect(
            (await listProcurementTasks(
                request,
                procurementToken,
                "mine",
            )).filter((item) => item.business_object_id === salesOrderId),
        ).toHaveLength(0)
        await expect(
            page.locator(
                '[data-testid^="work-item-procurement-order-creation-"]',
            ),
        ).toHaveCount(0)
    })
    ...
    await test.step("第一次建立数量 4 的采购草稿，销售与任务剩余数量变为 6", async () => {
        ...
        expect(bases[0]?.sales_order_no).toBe(salesOrderNo)
        expect(numericQuantity(bases[0]?.lines[0]?.sales_quantity)).toBe(10)
        expect(numericQuantity(bases[0]?.lines[0]?.covered_quantity)).toBe(0)
        expect(numericQuantity(bases[0]?.lines[0]?.remaining_quantity)).toBe(10)

        const quantity = visibleBasisQuantity(dialog)
        ...
        await expect(visibleBasisQuantity(dialog)).toHaveValue("6", {
            timeout: 30_000,
        })
        ...
        const mobileProgress = page.getByTestId(
            "sales-order-procurement-progress",
        )
    ...
    await test.step("第二次建立数量 6 的另一张采购草稿，剩余归零且任务完成", async () => {
        ...
        await expect(progress).toContainText(
            "销售总数量 10 · 已覆盖 10 · 剩余 0",
            { timeout: 20_000 },
        )
        await expect(progress).toContainText("2 笔")
        await expect(
            page.getByTestId("sales-order-continue-purchase"),
        ).toHaveCount(0)
