...
/**
 * 校验仓储账号具备入库与仓发完整执行权限，否则不能绑定仓库经办人。
 */
async function verifyWarehouseEligibility(adminToken, warehouseAccount) {
  const options = await call("GET", "/admin/warehouse-fulfillment-handler-options", {
    token: adminToken,
  });
  const option = (Array.isArray(options) ? options : []).find(
    (row) => row.user_id === warehouseAccount.id,
  );
...
async function listWarehouses(adminToken) {
  const page = await call(
    "GET",
    "/admin/warehouses?page=1&page_size=100&sort_by=warehouse_code&sort_dir=asc",
    {
      token: adminToken,
    },
  );
...
async function main() {
  ...
  console.log("销售账号 id:", salesUserId);
  ...
  customer = {
    ...
    legal_name: detail.legal_name ?? detail.current_revision?.legal_name ?? customer.legal_name,
  };
