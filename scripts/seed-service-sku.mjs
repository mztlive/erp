...
async function main() {
  ...
  if ((existing.items ?? []).length > 0) {
    console.log(
      "已存在 OFFLINE_SERVICE 可售 SKU:",
      existing.items[0].sku_no,
  ...
  // 验证可售
  const verify = await api("/admin/sellable-skus?product_kind=OFFLINE_SERVICE&page=1&page_size=5", {
    token,
  });
  console.log(
    "可售列表:",
    (verify.items ?? [])
      .map((i) => `${i.sku_no} ${i.name} suppliers=${i.supplier_count}`)
      .join(", "),
  );
