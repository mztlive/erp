...
export type UrlStateCodec<TState extends object> = {
  ...
  buildParams(state: TState, options?: Record<string, unknown>): URLSearchParams
}
...
function parseField(
  ...
  switch (spec.type) {
    ...
    case "number": {
      ...
      if (!Number.isFinite(parsed) || parsed < min) return spec.defaultValue
...
function defaultBuildWhen(spec: UrlFieldSpec): (value: unknown) => boolean {
  switch (spec.type) {
    ...
    case "number":
      return (value) => typeof value === "number" && value !== spec.defaultValue
    case "boolean":
...
export function createUrlStateCodec<TState extends object>(
  ...
  const build = (state: TState, options?: Record<string, unknown>): string => {
