...
interface BusinessStatusBadgeProps
  extends Omit<React.ComponentProps<typeof StatusBadge>, "children"> {
...
interface StatusTrackSummaryProps
...
function StatusTrackSummary({
  ...
  const statusContext: BusinessStatusContext =
    variant === "inline" ? "list" : variant === "stacked" ? "preview" : "detail"
  ...
  return (
    <dl
      ...
      className={cn(
        ...
        variant === "table" && "grid gap-3 sm:grid-cols-2 lg:grid-cols-3",
        className
      )}
      {...props}
    >
...
interface BusinessObjectRefProps
...
function withPrecision(
  ...
  const fraction =
    precision === 0 ? "" : incremented.slice(-precision)
  const isZero = integer === "0" && !/[1-9]/.test(fraction)
