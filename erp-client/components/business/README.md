...
| 页面公共区         | `PageScaffold`、`PageHeader`（`page` / `object-chrome`）、`PageActions`、`MetricStrip`、`MetricItem`（`detailMode` / `status` 指标徽章）、`DataFreshness`、`surfacePanelClassName` / `surfaceClassName(flat)` / `surfaceInsetClassName`、`workspaceTaskSurfaceClassName` / `workspaceEmbeddedScaffoldClassName` | `page.tsx`                                                                           | 内容区脚手架与浮起表面；工作台作业面用扁平分区，禁止再套一层卡片；**固定模式见** `docs/erp-ui-design.md` §2.5；一级 `page` 不展示面包屑；M4 用 object-chrome + 面包屑                                                                                                                                                                                                                                                  |
...
| 列表工作面筛选     | `ListWorkspaceFilterBar`、`ListSearchField`、`ListWorkspaceFilterField`、`ListWorkspaceInlineFilter`                                                                                                                                                                                                                                 | `list-workspace-filter-bar.tsx`、`list-search-field.tsx`                             | 查询按钮、常驻常用筛、卡片式更多筛、已生效 chip、待查询提示；列表页筛选区以此为唯一布局/交互口径                                                                                                                                                                                                                                                                                                                       |
...
| 值与状态           | `BusinessStatusBadge`、`StatusTrackSummary`、`BusinessObjectRef`、`MoneyValue`、`QuantityValue`、`RateValue`、`DocumentTotals`、`taxAmountToneClass`                                                                                                                                                                                 | `values.tsx`                                                                         | 多维状态、稳定对象引用、精确十进制展示；`MoneyValue` 的 `taxBasis` 会叠「含税/不含税」Badge——**列头/标签已写明口径时不要传**（如销售建单「含税小计」）；`BusinessStatusBadge` 对 `PENDING` 等枚举原值 label 有兜底中文映射（业务页仍需自己映射领域枚举，兜底只防漏网）；`taxAmountToneClass` 按「含税金额/不含税金额/税额」标签取税额三角数字的语义配色（含税=信息蓝、税额=深橙、不含税=默认色），金额数字展示统一走它 |
| 正式单据/对象详情  | `DocumentHeader`（M4 用 `density="compact"` + `meta` / `titleExtra` / `summary`）、`DocumentSummary`、`DocumentSection`、`RevisionTimeline`、`RelatedDocumentList`、`ResponsibilityPanel`                                                                                                                                            | `document.tsx`                                                                       | 销售、采购、客户、票款、发票、结算等详情与版本追溯；唯一身份头                                                                                                                                                                                                                                                                                                                                                         |
...
2. **M4 对象中心**使用：
    ...
    - `DocumentHeader density="compact"`：唯一身份头（名称 / 单号 / 版本 / 状态 / 主动作）；
      ...
      性质等身份标签放 `titleExtra`；金额/KPI 摘要放 `summary`（身份带下方通栏），
      不要再把金额摘要塞进 `children`，也不要宽屏并排以免大块留白；
    - 可选 `MetricStrip density="compact"` + `MetricItem`：业务风险明细用 `detailMode="inline"`，
      口径旁白用 `tooltip` 或 `none`；
      统计摘要统一使用无边框、无阴影的紧凑样式；标签、数值、说明分行展示。
      统计项只展示数据，不绑定点击、选中态或跳转；筛选放入页签或筛选控件。
...
3. 正式详情页的版本、关联单据和并行责任分别使用 `DocumentSummary`、`RelatedDocumentList`、
   `ResponsibilityPanel` 等专用组件，不合并成单一“状态”。关联采购单等需要履约/付款并行进度时，
   给 `RelatedDocumentList` 传 `tracks`，不要把进度塞进 `status.label` 长文案。
...
文案本身的用词口径见 `erp-client/AGENTS.md` §5 与 `erp-client/lib/ui-text.ts`。
