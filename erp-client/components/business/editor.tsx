...
interface EditableLineItemTableProps<TItem>
              ...
              items.map((item, rowIndex) => {
                ...
                return (
                  <React.Fragment key={rowId}>
                    <TableRow aria-invalid={rowErrors.length > 0 || undefined}>
                      {columns.map((column) => {
                        const content =
                          mode === "edit" && column.renderEditor
                            ? column.renderEditor(context)
                            : column.renderValue(context)
                          ...
                          <div className="flex justify-end gap-1">
                            ...
                            {onRemoveItem ? (
                              <GuardedBusinessAction
                                ...
                                onClick={() =>
                                  onRemoveItem(item, rowId, rowIndex)
                                }
                              >
          ...
          <DescriptionList columns="two" aria-label="审批摘要">
            {summaryItems.map((item) => (
              <DescriptionItem key={item.id}>
                ...
                <DescriptionDetails className={cn(item.numeric && "num")}>
                  {item.value}
                </DescriptionDetails>
              </DescriptionItem>
            ))}
        ...
        <CardFooter className="justify-end gap-2 border-t border-border">
          ...
          >
            {pendingDecision === "reject" ? (
              ...
              <CircleXIcon data-icon="inline-start" aria-hidden="true" />
            )}
            {rejectLabel}
          </GuardedBusinessAction>
          <GuardedBusinessAction
...
interface AllocationWorkspaceProps<TAllocation>
  extends Omit<React.ComponentPropsWithoutRef<"section">, "children"> {
  readonly title?: string
  readonly description?: React.ReactNode
