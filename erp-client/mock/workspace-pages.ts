...
export const MASTER_DATA_FIXTURES: Record<MasterDataResource, MasterDataFixture> = {
  ...
  categories: {
    searchPlaceholder: "分类代码、名称",
    metrics: [
      { key: "enabled", label: "当前启用", value: 6, detail: "可被 SKU 选用" },
      { key: "disabled", label: "当前停用", value: 0, detail: "历史保留" },
      { key: "expiring", label: "有效期将尽", value: 0, detail: "30 日内" },
      { key: "pending", label: "待生效修订", value: 0, detail: "计划生效" },
    ],
    columns: [
      { key: "code", header: "分类编号" },
      { key: "name", header: "分类名称" },
      { key: "catCode", header: "分类代码" },
      { key: "kind", header: "适用类型" },
      { key: "version", header: "当前版本" },
      { key: "status", header: "启停状态", status: true },
    ],
    rows: listRows([
  ...
  brands: {
    searchPlaceholder: "品牌代码、名称",
    metrics: [
      { key: "enabled", label: "当前启用", value: 5, detail: "可被 SKU 选用" },
      { key: "disabled", label: "当前停用", value: 0, detail: "历史保留" },
      { key: "expiring", label: "有效期将尽", value: 0, detail: "30 日内" },
      { key: "pending", label: "待生效修订", value: 0, detail: "计划生效" },
    ],
    columns: [
      { key: "code", header: "品牌编号" },
      { key: "name", header: "品牌名称" },
      { key: "brandCode", header: "品牌代码" },
      { key: "version", header: "当前版本" },
      { key: "status", header: "启停状态", status: true },
    ],
    rows: listRows([
