...
const STATUS_UI: Record<
  string,
  { code: WorkItemStatusCode; label: string; tone: QueueWorkItemView["status"]["tone"] }
> = {
...
export function mapWorkItemDto(dto: WorkItemDto): QueueWorkItemView {
  ...
  return {
    ...
    status: {
      label: dueMeta.overdue && statusUi.code !== "COMPLETED" && statusUi.code !== "CLOSED"
        ? `${statusUi.label} · 超期`
        : statusUi.label,
...
export function computeQueueCounts(items: readonly QueueWorkItemView[]): {
  ...
  const overdue = items.filter(
    (i) =>
      i.status.tone === "destructive" || i.dueAt.includes("超期")
  ).length
...
export async function fetchUnifiedTaskQueue(
  ...
  if (filters.scope === "hold" || filters.scope === "team") {
    ...
    const empty: UnifiedTaskQueueView = {
      ...
      filterSummary: buildFilterSummary(filters, 0, FAMILY_LABELS[filters.family!] ?? "全部类型"),
      total: 0,
  ...
  if (filters.due === "overdue") {
    ...
    items = items.filter((i) => i.dueAt.includes("今天") || i.dueAt.includes("今日"))
  }
