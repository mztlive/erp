...
function toListItem(
        ...
        enabledCaps.length === 0
            ...
            : enabledCaps
                  .map(
                      (c) =>
                          CAPABILITY_LABEL[
                              mapCapabilityCode(c.capability_code)
                          ],
                  )
                  .join("、")
    ...
    return {
        ...
        supplier: {
            id: conn.supplier_id,
            name: supplierName ?? conn.supplier_id,
        },
        environment: env,
...
export async function fetchConnectionList(
    ...
    await Promise.all(
        pageResult.items.map(async (c) => {
            if (!supplierNames.has(c.supplier_id)) {
                supplierNames.set(
                    c.supplier_id,
                    await resolveSupplierName(c.supplier_id),
                )
            }
        }),
    )
    ...
    const metrics = {
        ...
        pendingConfig: items.filter((i) => i.status === "PENDING_CONFIG")
            .length,
...
export async function updateCapabilities(input: {
    ...
    for (const ch of input.changes) {
        if (
            !existing.some(
                (e) => mapCapabilityCode(e.capability_code) === ch.code,
            )
        ) {
            existing.push({
