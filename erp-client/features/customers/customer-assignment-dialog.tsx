...
function mutationMessage(error: unknown): string {
  ...
  const response = apiError.responseData as { errorMessage?: string } | undefined
...
export function CustomerAssignmentDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{ending ? "结束协作归属" : "调整客户归属"}</DialogTitle>
          <DialogDescription>
            {ending
        ...
        <form className="space-y-4" onSubmit={submit}>
          {ending ? (
            <div className="rounded-lg bg-muted px-3 py-2 text-sm">
              {target.userName} · 协作销售 · {target.effectiveFrom} 起
            </div>
          ) : (
            <>
              <div className="space-y-2">
                <Label htmlFor="customer-assignment-role">责任角色</Label>
                <NativeSelect
                  id="customer-assignment-role"
                  className="w-full"
                  value={role}
                  onChange={(event) =>
                    setRole(event.target.value as "OWNER" | "COLLABORATOR")
                  }
                >
                  <NativeSelectOption value="OWNER">负责销售</NativeSelectOption>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="customer-assignment-from">生效日期</Label>
                  <Input
          ...
          {ending ? (
            <div className="space-y-2">
              <Label htmlFor="customer-assignment-end-date">结束日期</Label>
              <Input
          ...
          <div className="space-y-2">
            <Label htmlFor="customer-assignment-reason">调整原因</Label>
            <Textarea
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={mutation.isPending}>
              {mutation.isPending ? "提交中…" : ending ? "确认结束" : "确认调整"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
