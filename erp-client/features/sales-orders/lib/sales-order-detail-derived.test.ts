...
describe('deriveSalesOrderDetailState', () => {
    ...
    it('shows the approval panel only inside the approval section', () => {
        ...
        expect(
            deriveSalesOrderDetailState(order, baseInput).showApproval,
        ).toBe(false)
        expect(
    ...
    it('requires mode=edit plus resubmit permission for rejected orders', () => {
        ...
        expect(
            deriveSalesOrderDetailState(order, baseInput).showEditor,
        ).toBe(false)
    ...
    it('derives rejection capabilities and the focus task', () => {
        ...
        expect(state.focusTask?.id).toBe('procurement-rejection')
        expect(state.focusTask?.tone).toBe('warning')
        expect(state.focusTask?.description).toBe('无法履约：无法交付')
        expect(state.hasPrimaryTaskAction).toBe(true)
        expect(state.bannerJump).toBe(false)
    })
