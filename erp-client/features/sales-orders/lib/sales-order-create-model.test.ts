...
describe("hasMeaningfulLines", () => {
    ...
    it("detects filled content", () => {
        expect(hasMeaningfulLines([emptyLine({ name: "货物" })])).toBe(true)
        expect(hasMeaningfulLines([emptyLine({ sku: "sku-1" })])).toBe(true)
        expect(hasMeaningfulLines([emptyLine({ quantity: "2" })])).toBe(true)
        expect(
            hasMeaningfulLines([emptyLine({ unitPriceGross: "9.90" })]),
        ).toBe(true)
        expect(hasMeaningfulLines([emptyLine({ faceValue: "100" })])).toBe(true)
        expect(hasMeaningfulLines([emptyLine({ dueDate: "2026-09-01" })])).toBe(
            true,
        )
    })
