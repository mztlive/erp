...
const executionProjectionKeys = {
  ...
  collaboration: (salesOrderId: string) =>
    [...executionProjectionKeys.all, "collaboration", salesOrderId] as const,
...
export function useExecutionProjectionDetailQuery(
  ...
  return useQuery({
    queryKey: executionProjectionKeys.detail(projectionId ?? "", revisionId),
    queryFn: () =>
      fetchExecutionProjectionDetail({
...
export function useBulkProjectionCommandMutation() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (input: BulkCommandInput) => submitBulkProjectionCommand(input),
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: executionProjectionKeys.all,
      })
