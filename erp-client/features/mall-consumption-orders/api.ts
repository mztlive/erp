...
function mapFulfillmentChain(raw: string): FulfillmentChain {
  if (raw === "ERP_AUTOMATED" || raw === "erp_automated") return "ERP_AUTOMATED"
  return "LEGACY_MANUAL"
}

function mapDataSource(raw: string): DataSource {
  if (raw === "history_backfill" || raw === "BACKFILL" || raw === "HISTORY_BACKFILL")
    return "BACKFILL"
...
function mapFactType(raw: string): FactType {
  ...
  if (u === "ORDER_CANCELED" || u === "ORDER_CANCELLED") return "ORDER_CANCELED"
...
function mapCostAssessment(
  ...
  return {
    ...
    taxInclusion:
      a.tax_inclusion == null ? undefined : a.tax_inclusion ? "含税" : "不含税",
...
function mapDetail(d: BackendDetail): MallConsumptionOrderView {
  ...
  return {
    ...
    conservation: {
      itemRowResults: (d.conservation?.item_row_results ?? []).map((r) => ({
        mallOrderItemId: r.id,
        expected: r.expected,
        actual: r.actual,
        valid: r.valid,
      })),
      sourceColumnResults: (d.conservation?.source_column_results ?? []).map(
        (r) => ({
    ...
    consumptionEntries: (d.consumption_entries ?? []).map((ce) => ({
      ...
      currentCostAssessment: mapCostAssessment(ce.current_cost_assessment),
    })),
    supplierOrders: (d.supplier_orders ?? []).map((so) => ({
      ...
      itemIds: so.item_ids ?? [],
      fulfillmentStatus: so.fulfillment_status as MallConsumptionOrderView["supplierOrders"][number]["fulfillmentStatus"],
      cancelStatus: "NONE",
      refundStatus: "NONE",
    })),
    address: {
...
export async function fetchConsumptionOrderList(
  ...
  return {
    ...
    metrics: emptyMetrics(),
    malls: Array.from(mallsMap.entries()).map(([id, name]) => ({ id, name })),
