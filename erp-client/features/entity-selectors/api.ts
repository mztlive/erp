...
export type CustomerSearch = EntitySearch &
  Readonly<{ scope: "mine" | "collaborating" | "assigned" | "all_authorized" }>
...
async function partyItem(row: PartyDto): Promise<SettlementPartyComboboxItem> {
  ...
  try {
    ...
    const revision =
      revisions.items.find((item) => item.id === row.current_revision_id) ??
      revisions.items[0]
...
export async function fetchPartyOption(
  ...
  try {
    return partyItem(
      await apiGet<PartyDto>(`/admin/parties/${encodeURIComponent(partyId)}`),
    )
  } catch {
    return null
  }
...
async function warehouseItem(row: WarehouseDto): Promise<WarehouseComboboxItem> {
  ...
  try {
    ...
    warehouseName = revisions.items[0]?.warehouse_name?.trim() || row.warehouse_code
  } catch {
    // 仓库代码是稳定且可展示的回退值。
  }
  const enabled = activeStatus(row.status)
  ...
  return page.items
    .filter(
      (row) =>
        ...
        row.product_kind.toUpperCase() !== input.excludeProductKind.toUpperCase(),
    )
    .map(productItem)
}
...
async function contractItem(row: ContractDto): Promise<ContractComboboxItem> {
  ...
  try {
    ...
    revision =
      detail.revisions.find((item) => item.id === detail.current_revision_id) ??
      detail.revisions[0]
  } catch {
