...
export function useSupplierSelectorQuery(input: EntitySearch, selectedId?: string) {
  const list = useQuery({
...
export function useCustomerSelectorQuery(input: CustomerSearch, selectedId?: string) {
  const list = useQuery({
...
export function usePartySelectorQuery(input: EntitySearch, selectedId?: string) {
  const list = useQuery({
...
export function useWarehouseSelectorQuery(input: EntitySearch, selectedId?: string) {
  const list = useQuery({
...
export function useContractSelectorQuery(input: ContractSearch, selectedId?: string) {
  const list = useQuery({
...
export function useSalesOrderSelectorQuery(input: EntitySearch, selectedId?: string) {
  const list = useQuery({
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return source.map((item) => ({
        ...
        baseUnit: "张",
        description:
          item.keyFacts.find((fact) => fact.label === "说明")?.value ??
          item.keyFacts.find((fact) => fact.label === "商品类型")?.value,
      }))
    },
    staleTime: STALE_TIME,
  })
