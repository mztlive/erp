...
import type { ReceiptReversalRow } from "@/features/customer-receivables/types"

/** 回款冲正单作为合同 DocumentType 的固定值。 */
export const RECEIPT_REVERSAL_DOCUMENT_TYPE = "ReceiptReversal" as const
 ...
 * @param status 服务端状态码。
 */
export const receiptReversalStatusLabel = (status?: string): string => {
    switch (status) {
        case "DRAFT":
        case "draft":
        case "REJECTED":
...
/**
 * 回款冲正状态对应的列表色调。未知码按进行中处理，不上屏枚举。
 *
 * @param status 服务端状态码。
 */
export const receiptReversalStatusTone = (
    status?: string,
): ReceiptReversalRow["statusTone"] => {
    switch (receiptReversalStatusLabel(status)) {
        case "已过账":
            return "success"
        case "审批中":
            return "warning"
...
/**
 * 按单据生命周期选择回款冲正审批区相位。提交确认由调用方显式传入。
 *
 ...
 * @param status 单据状态码。
 */
export const receiptReversalApprovalPhase = (
    approval?: DocumentApprovalView,
    status?: string,
): Exclude<ReceiptReversalApprovalPhase, "confirm"> =>
    approval?.instance || !isUnsubmittedReceiptReversalStatus(status)
        ? "runtime"
        : "draft"

/**
 * 合并回款冲正单与当前任务的服务端动作白名单。只做并集过滤，不补默认动作。
 *
 * @param documentActions 单据 `allowed_actions`。
 * @param workItemActions 当前任务 `allowed_actions`。
 */
export const mergeReceiptReversalAllowedActions = (
...
/**
 * 把回款冲正详情上的只读审批结构转成通用审批区投影。
 *
 * 缺省返回 undefined，禁止前端补默认审批人或节点。
 *
 * @param dto 详情内嵌的审批 DTO。
 */
export const mapReceiptReversalApproval = (
 ...
 */
export const isReceiptReversalWorkItem = (workItem?: {
    businessObjectType?: string
}): boolean =>
    workItem?.businessObjectType === RECEIPT_REVERSAL_DOCUMENT_TYPE ||
    workItem?.businessObjectType === RECEIPT_REVERSAL_OBJECT_TYPE

/**
 * 构造回款冲正提交审批请求。只允许版本与幂等键。
 *
 * @param input 提交所需字段。
 */
export const buildReceiptReversalSubmitRequest = (input: {
...
/** 回款冲正提交意图对应的幂等槽。 */
export type ReceiptReversalIdempotencySlot = Readonly<{
    key: string
    fingerprint: string
    reversalId?: string
}>
 ...
 * @param reason 冲正原因。
 */
export const receiptReversalIntentFingerprint = (
    scopeId: string,
    reason: string,
 ...
 * @param reason 冲正原因。
 */
export const slotForReceiptReversalIntent = (
    current: ReceiptReversalIdempotencySlot | null,
    scopeId: string,
    reason: string,
): ReceiptReversalIdempotencySlot => {
    const fingerprint = receiptReversalIntentFingerprint(scopeId, reason)
    if (current && current.fingerprint === fingerprint) return current
    return {
        key: `w11-rr-${scopeId}-${crypto.randomUUID()}`,
        fingerprint,
    }
}
