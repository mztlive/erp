...
import { ReceiptReversalApprovalArea } from "./receipt-reversal-approval-area"
...
import { mergeReceiptReversalAllowedActions } from "@/features/customer-receivables/lib/receipt-reversal-approval"
...
vi.mock("@/features/approval-workflow/queries", async () => {
    ...
    return {
        ...
        useRecoveryOptionsQuery: () => ({
            data: { instanceId: "inst-rr-1", actions: recoveryState.actions },
        }),
...
describe("ReceiptReversalApprovalArea", () => {
    it("shows the bound route after create and does not offer a work item", () => {
        ...
        expect(screen.getByText("回款冲正审批 v2")).toBeTruthy()
    ...
    it("keeps rejected history across rounds and increments the displayed round", () => {
        ...
        expect(screen.getByText("冲正金额与原回款不一致")).toBeTruthy()
    })
...
describe("receipt reversal decision whitelist and idempotency", () => {
    it("only emits the contract decision fields", () => {
        const request = buildDecisionRequest({
            workItemId: "wi-rr-1",
            decision: "APPROVE",
            expectedTaskVersion: "2",
            idempotencyKey: "k-dec-rr",
        })
    ...
    it("keeps the same key for the same intent and rotates after a change", () => {
        ...
        const changed = slotForIntent(
            ...
            "decision",
            "wi-rr-1",
            decisionIntentFingerprint("REJECT", "冲正金额与原回款不一致"),
        )
