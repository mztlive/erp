...
import { CustomerReceiptApprovalArea } from "./customer-receipt-approval-area"
...
import { mergeCustomerReceiptAllowedActions } from "@/features/customer-receivables/lib/customer-receipt-approval"
...
vi.mock("@/features/approval-workflow/queries", async () => {
    ...
    return {
        ...
        useRecoveryOptionsQuery: () => ({
            data: { instanceId: "inst-cr-1", actions: recoveryState.actions },
        }),
...
describe("CustomerReceiptApprovalArea", () => {
    it("shows the bound route after create and does not offer a work item", () => {
        ...
        expect(screen.getByText("客户回款审批 v2")).toBeTruthy()
        expect(screen.getByText("回款复核")).toBeTruthy()
    ...
    it("prints the submit confirmation route and fixed reject explanation", () => {
        wrapper(
            <CustomerReceiptApprovalArea phase="confirm" approval={binding} />,
        )
    ...
    it("keeps rejected history across rounds and increments the displayed round", () => {
        ...
        expect(screen.getAllByText("回款复核 · 已驳回")).toHaveLength(1)
        expect(screen.getAllByText("回款复核 · 办理中")).toHaveLength(1)
        expect(screen.getByText("银行流水不一致")).toBeTruthy()
    })
...
describe("customer receipt decision whitelist and idempotency", () => {
    it("only emits the contract decision fields", () => {
        const request = buildDecisionRequest({
            workItemId: "wi-cr-1",
            decision: "APPROVE",
            expectedTaskVersion: "2",
            idempotencyKey: "k-dec-cr",
        })
    ...
    it("keeps the same key for the same intent and rotates after a change", () => {
        ...
        const changed = slotForIntent(
            ...
            "decision",
            "wi-cr-1",
            decisionIntentFingerprint("REJECT", "银行流水不一致"),
        )
