...
function cellContext<TData>(row: TData): CellContext<TData, unknown> {
    return { row: { original: row } } as CellContext<TData, unknown>
}

function renderCell<TData>(column: ColumnDef<TData, unknown>, row: TData) {
...
describe("useMallSyncColumns", () => {
    ...
    it("opens the reconciliation view for the clicked difference row", () => {
        const { patchUrl, result } = renderColumns()
        const column = result.current.diffColumns.find((c) => c.id === "order")
