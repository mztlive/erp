...
describe("usePostFulfillmentMutation", () => {
    ...
    it("does not invalidate approval cache after a delivery post", async () => {
        mockedPost.mockResolvedValue({
            status: "succeeded",
            outcome: {
                kind: "POSTED",
        ...
        await act(async () => {
            await result.current.mutateAsync({
                ...
                draft: {
                    ...
                    carrier: "顺丰",
                    trackingNo: "SF-1",
                    shippedAt: "2026-08-14T09:00:00.000Z",
                    lines: [],
                },
            })
        })
        ...
        expect(invalidate).not.toHaveBeenCalledWith({
            queryKey: ["approval", "document", "Delivery", "dl-1"],
        })
        expect(invalidate).not.toHaveBeenCalledWith({
            queryKey: ["approval"],
        })
    ...
    it("does not invalidate approval cache after a service fulfillment post", async () => {
        mockedPost.mockResolvedValue({
            status: "succeeded",
            outcome: {
                kind: "POSTED",
        ...
        expect(invalidate).not.toHaveBeenCalledWith({
            queryKey: ["approval", "document", "ServiceFulfillment", "sf-1"],
        })
        expect(invalidate).not.toHaveBeenCalledWith({
            queryKey: ["approval"],
        })
