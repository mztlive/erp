...
function emptySourceLine(
  ...
  return {
    ...
    remainingQuantity: partial.remainingQuantity ?? partial.orderedQuantity ?? "",
...
function baseTask(
  partial: Omit<FulfillmentTask, "gate" | "allowedActions" | "actionBlockers" | "impact" | "summary" | "statusTone" | "statusLabel" | "held" | "priority" | "dueAt" | "dueLabel" | "overdue" | "responsibleLabel" | "sourceVersion" | "subjectHash" | "editVersion" | "source" | "lines" | "draft"> &
  ...
  return {
    ...
    allowedActions: partial.allowedActions ?? ["POST", "SAVE", "DEFER", "CLAIM"],
...
function deliveryToTask(d: BackendDelivery): FulfillmentTask {
  const op: FulfillmentOperationType =
    d.delivery_type === "SUPPLIER_DIRECT" ? "SUPPLIER_DIRECT" : "WAREHOUSE_SHIP"
  ...
  return baseTask({
    ...
    lines: [
      emptySourceLine({
        ...
        purchaseLineSalesAllocationId: e.purchase_line_sales_allocation_id,
    ...
    draft: {
      type: "ELECTRONIC",
      occurredAt: secsToIso(e.occurred_at).slice(0, 16) || nowIso().slice(0, 16),
      recipientMasked: "",
  ...
  return baseTask({
    ...
    lines: [
      emptySourceLine({
        ...
        purchaseLineSalesAllocationId: s.purchase_line_sales_allocation_id,
    ...
    draft: {
      type: "SERVICE",
      startedAt: secsToIso(s.occurred_at).slice(0, 16) || nowIso().slice(0, 16),
      endedAt: secsToIso(s.occurred_at).slice(0, 16) || nowIso().slice(0, 16),
      serviceLocation: "",
...
async function hydrateTaskDetail(task: FulfillmentTask): Promise<FulfillmentTask> {
  try {
    ...
    if (
      ...
      const lines = detail.lines.map((l) =>
        emptySourceLine({
          ...
          reservedQuantity: l.stock_reservation_id ? l.quantity : undefined,
...
export async function fetchFulfillmentQueue(
  ...
  if (requestedOutOfRole.length > 0) {
    return {
      ...
      context: {
        queueContextId: filters.queueContextId ?? `queue:W09:${filters.scope}`,
        position: 0,
        total: 0,
  ...
  if (want.has("RECEIPT")) {
    loaders.push(
      apiGet<BackendPage<BackendPurchaseReceipt>>("/admin/purchase-receipts", {
        page: 1,
        page_size: 100,
        status: "DRAFT",
  ...
  try {
    const wh = await apiGet<BackendPage<BackendWarehouse>>("/admin/warehouses", {
      page: 1,
      page_size: 100,
    })
...
function formalFromDelivery(
  ...
  return {
    ...
    occurredAt:
      (isWh ? draft.shippedAt : draft.shippedAt) || nowIso(),
...
export async function postFulfillmentOperation(input: {
  ...
  try {
    if (draft.type === "RECEIPT") {
      ...
      if (receipt.warehouse_id !== draft.warehouseId && draft.warehouseId) {
    ...
    if (draft.type === "WAREHOUSE_SHIP" || draft.type === "SUPPLIER_DIRECT") {
      ...
      if (!delivery) {
        ...
        const created = await apiPost<BackendDelivery>("/admin/deliveries", {
          delivery_no: genDocNo(draft.type === "WAREHOUSE_SHIP" ? "FH" : "DF"),
          delivery_type:
            draft.type === "WAREHOUSE_SHIP"
              ? "WAREHOUSE_SHIP"
              : "SUPPLIER_DIRECT",
          sales_order_id:
            draft.lines[0]?.salesOrderLineId /* wrong — need SO id */ ||
            input.workItemId,
          purchase_order_id: undefined,
          warehouse_id:
            draft.type === "WAREHOUSE_SHIP" ? draft.warehouseId : undefined,
          ...
          lines:
            draft.type === "WAREHOUSE_SHIP"
              ? draft.lines.map((l) => ({
                  ...
                  quantity: l.quantity,
                  stock_reservation_id: l.stockReservationId,
    ...
    if (draft.type === "ELECTRONIC") {
      ...
      const created = await apiPost<BackendElectronicDelivery>(
        "/admin/electronic-deliveries",
        {
          ...
          purchase_line_sales_allocation_id: line.purchaseLineSalesAllocationId,
      ...
      return {
        ...
        outcome: {
          ...
          factId: confirmed.id,
          factNo: confirmed.fulfillment_no,
          formalStatus: confirmed.result === "FAILED" ? "FAILED" : "CONFIRMED",
          occurredAt: secsToIso(confirmed.occurred_at) || nowIso(),
          operationType: "ELECTRONIC",
          inventoryDelta: [],
          reservationDelta: [],
  ...
  const probes: Array<() => Promise<FormalActionResponse | null>> = [
        ...
        if (d.receipt.status === "POSTED") {
          return {
            ...
            outcome: {
              ...
              formalStatus: "POSTED",
              occurredAt: secsToIso(d.receipt.posted_at) || nowIso(),
              operationType: "RECEIPT",
              inventoryDelta: [],
              reservationDelta: [],
              remainingByLine: [],
    ...
    async () => {
      try {
        ...
        if (d.delivery.status === "SHIPPED" || d.delivery.status === "SIGNED") {
          return {
            status: "succeeded",
            outcome: {
              kind: "POSTED",
              ...
              occurredAt: secsToIso(d.delivery.shipped_at) || nowIso(),
              operationType:
