...
import { SupplierPaymentApprovalArea } from "./supplier-payment-approval-area"
...
import { mergeSupplierPaymentAllowedActions } from "@/features/supplier-payables/lib/supplier-payment-approval"
...
vi.mock("@/features/approval-workflow/queries", async () => {
    ...
    return {
        ...
        useRecoveryOptionsQuery: () => ({
            data: { instanceId: "inst-sp-1", actions: recoveryState.actions },
        }),
...
describe("SupplierPaymentApprovalArea", () => {
    it("shows the bound route after create and does not offer a work item", () => {
        ...
        expect(screen.getByText("供应商付款审批 v2")).toBeTruthy()
        expect(screen.getByText("付款复核")).toBeTruthy()
    ...
    it("keeps rejected history across rounds and increments the displayed round", () => {
        ...
        expect(screen.getAllByText("付款复核 · 已驳回")).toHaveLength(1)
        expect(screen.getAllByText("付款复核 · 办理中")).toHaveLength(1)
...
describe("supplier payment decision whitelist and idempotency", () => {
    it("only emits the contract decision fields", () => {
        const request = buildDecisionRequest({
            workItemId: "wi-sp-1",
            decision: "APPROVE",
            expectedTaskVersion: "2",
            idempotencyKey: "k-dec-sp",
        })
