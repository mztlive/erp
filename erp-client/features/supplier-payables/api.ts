...
function mapBackendSourceType(t?: string): string | undefined {
  ...
  if (t === "PURCHASE_ORDER" || t === "purchase_order") return "purchase_order"
...
function projectInvoice(inv: BackendInvoice): PurchaseInvoiceRow {
  ...
  return {
    ...
    allocations: (inv.allocations ?? []).map((a) => ({
      allocationId: a.id,
      action: a.allocation_action === "reverse" ? "REVERSE" : "APPLY",
      payableAccountId: a.payable_account_id ?? a.receivable_account_id ?? "",
      sourceType: "PURCHASE_ORDER" as const,
...
export async function fetchSupplierAccounts(
  ...
  const invoices = (invPage.items ?? [])
    .filter((i) => i.invoice_direction === "purchase" || !i.invoice_direction)
    .map(projectInvoice)
  ...
  else if (query.view === "payment") total = paymentPage.total ?? payments.length
  ...
  return {
    ...
    canExport: true,
    emptyReason:
      total === 0 ? (hasFilters ? "FILTER_NO_RESULT" : "NO_DATA") : undefined,
    // 应付优先级策略后端缺口
    payablePriorityPolicy: {
      state: "MISSING",
      mixedAutoAllocationAllowed: false,
      blockerMessage: "应付优先级策略接口尚未提供，请显式逐项选择分配目标。",
    },
...
export async function fetchPayableDetail(
  ...
  try {
    ...
    try {
      const allocPage = await apiGet<Page<BackendPurchaseInvoiceAllocation>>(
      ...
      invoiceAllocs = (allocPage.items ?? []).map((row) => ({
        allocationId: row.id,
        action: row.allocation_action === "reverse" ? "REVERSE" : "APPLY",
...
export async function fetchAllocationSession(input: {
  ...
  const draftSessionId =
    input.draftSessionId ?? `alloc_sup_${++sessionSeq}`
      ...
      const posted = await apiPost<BackendSupplierPayment>(
        `/admin/supplier-payments/${encodeURIComponent(paymentId)}/post`,
        {
          allocations: targets.map((t) => ({
            payable_entry_id: t.payableEntryId ?? t.payableAccountId,
