...
function setupRouter() {
    ...
    mockedRouter.mockReturnValue(router as unknown as ReturnType<typeof useRouter>)
    return router
}
...
describe('useCardFundsReviewUrlState', () => {
    ...
    it('ignores unknown param values by falling back to defaults', () => {
        mockedSearchParams.mockReturnValue(
            new URLSearchParams('scope=bogus&type=bogus&status=bogus&due=bogus') as unknown as ReadonlyURLSearchParams,
        )
        const { result } = renderHook(() => useCardFundsReviewUrlState())
        expect(result.current.scope).toBe('mine')
        expect(result.current.type).toBe('all')
    ...
    it('replaceUrl sets, deletes and preserves queueContextId', () => {
        mockedSearchParams.mockReturnValue(
            new URLSearchParams('scope=team&queueContextId=qc_1') as unknown as ReadonlyURLSearchParams,
        )
        ...
        expect(router.replace).toHaveBeenCalledWith(
            '/finance/card-funds-review?scope=team&queueContextId=qc_1&type=opening&currentWorkItemId=wi_2',
            { scroll: false },
        )
    })

    it('replaceUrl adds the default queueContextId when absent', () => {
        mockedSearchParams.mockReturnValue(new URLSearchParams('scope=team') as unknown as ReadonlyURLSearchParams)
        mockedPathname.mockReturnValue('/finance/card-funds-review')
        const router = setupRouter()
        const { result } = renderHook(() => useCardFundsReviewUrlState())
        ...
        expect(router.replace).toHaveBeenCalledWith(
            '/finance/card-funds-review?scope=team&due=today&queueContextId=queue%3Acard-funds-review%3Ateam',
            { scroll: false },
        )
    })
    ...
    it('replaceUrl restores the current queueContextId when removed', () => {
        mockedSearchParams.mockReturnValue(
            new URLSearchParams('queueContextId=qc_1') as unknown as ReadonlyURLSearchParams,
        )
        const router = setupRouter()
        const { result } = renderHook(() => useCardFundsReviewUrlState())
    ...
    it('does not write the q param when it already matches', async () => {
        mockedSearchParams.mockReturnValue(new URLSearchParams('q=SO-1') as unknown as ReadonlyURLSearchParams)
        const router = setupRouter()
...
describe('useCardFundsReviewDefaultUrlSync', () => {
    ...
    it('does not write a currentWorkItemId for an empty queue', async () => {
        mockedSearchParams.mockReturnValue(
            new URLSearchParams('scope=mine&queueContextId=qc_1') as unknown as ReadonlyURLSearchParams,
        )
        const router = setupRouter()
        renderHook(() =>
            useCardFundsReviewDefaultUrlSync({
