...
import type { FormalActionResponse, RegisterFundsResult } from '@/features/card-funds-review/types'
...
const mockedResponsibilityHook = vi.mocked(useWorkItemResponsibilityMutation)

function makeResponsibilityMock() {
    return {
        mutateAsync: vi
            .fn()
            .mockResolvedValue({ id: 'wi_1', status: 'OPEN', task_version: 'tv_2' }),
        isPending: false,
    }
}
    ...
    const rendered = renderHookWithProviders(
        () =>
            useCardFundsReviewWorkflow({
                ...
                replaceUrl,
                setSearchInput,
                queueQuery: { refetch } as unknown as Parameters<
                    typeof useCardFundsReviewWorkflow
                >[0]['queueQuery'],
            }),
        { queryClient: client },
    )
    return { ...rendered, replaceUrl, setSearchInput, refetch, task }
}

beforeEach(() => {
    vi.clearAllMocks()
    mockedResponsibilityHook.mockReturnValue(
        makeResponsibilityMock() as unknown as ReturnType<
...
describe('useCardFundsReviewWorkflow · derived state', () => {
    it('derives responsibilityStatus pool_available for startable tasks', () => {
        const { result } = renderWorkflow({
            task: makeTask({
                workItem: { allowedActions: ['START_PROCESSING'] },
            }),
        })
    ...
    it('computes allocation totals from the draft lines', () => {
        ...
        act(() => {
            ...
            result.current.setAllocLines([
                { lineId: 'a1', targetAccountId: 'acct_1', targetLabel: 'SO', amount: '10.00' },
                { lineId: 'a2', targetAccountId: 'acct_1', targetLabel: 'SO', amount: 'abc' },
                { lineId: 'a3', targetAccountId: 'acct_1', targetLabel: 'SO', amount: '5.50' },
            ])
            result.current.setReceiptForm((f) => ({ ...f, grossAmount: '20.00' }))
        })
...
describe('useCardFundsReviewWorkflow · responsibility actions', () => {
    it('releases to the team and records the blocked result', async () => {
        const responsibilityMock = makeResponsibilityMock()
        mockedResponsibilityHook.mockReturnValue(
            responsibilityMock as unknown as ReturnType<
                typeof useWorkItemResponsibilityMutation
            >,
        )
        const { result } = renderWorkflow()
        ...
        expect(responsibilityMock.mutateAsync).toHaveBeenCalledWith(
        ...
        expect(result.current.confirmMode).toBeNull()
        expect(result.current.lastResult?.status).toBe('blocked')
        expect(result.current.lastResult?.title).toBe('当前项已退回团队')
        expect(result.current.lastResult?.outcome?.kind).toBe('RELEASED_TO_TEAM')
    })

    it('uses the draft comment as the release reason when provided', async () => {
        const responsibilityMock = makeResponsibilityMock()
        mockedResponsibilityHook.mockReturnValue(
            responsibilityMock as unknown as ReturnType<
                typeof useWorkItemResponsibilityMutation
        ...
        const { result } = renderWorkflow()
        act(() => {
            result.current.setComment('  证据不足退回  ')
        })
        await act(async () => {
            await result.current.handleReleaseToTeam()
        })
        expect(responsibilityMock.mutateAsync).toHaveBeenCalledWith(
            expect.objectContaining({ reason: '证据不足退回' }),
        )
    })

    it('startProcessing resets scope and refetches the queue', async () => {
        const responsibilityMock = makeResponsibilityMock()
        mockedResponsibilityHook.mockReturnValue(
            responsibilityMock as unknown as ReturnType<
                typeof useWorkItemResponsibilityMutation
        ...
        const { result, replaceUrl, refetch } = renderWorkflow()

        await act(async () => {
            await result.current.startProcessing()
        })
        expect(responsibilityMock.mutateAsync).toHaveBeenCalledWith(
            expect.objectContaining({ kind: 'START_PROCESSING' }),
        )
        expect(replaceUrl).toHaveBeenCalledWith({
            scope: null,
            queueContextId: null,
    ...
    it('surfaces errors from failed responsibility actions', async () => {
        const responsibilityMock = makeResponsibilityMock()
        responsibilityMock.mutateAsync.mockRejectedValue(
            new Error('处理权已变化'),
        )
        mockedResponsibilityHook.mockReturnValue(
            responsibilityMock as unknown as ReturnType<
        ...
        expect(result.current.actionError).toBe('处理权已变化')
    })
})
...
describe('useCardFundsReviewWorkflow · registration', () => {
    ...
    it('generates a receipt number when the draft is blank', async () => {
        ...
        const { result } = renderWorkflow()
        act(() => {
            result.current.setReceiptForm((f) => ({ ...f, grossAmount: '99.00' }))
        })
        await act(async () => {
            await result.current.submitReceipt()
        })
