...
export function SellableItemPreviewPanel({
  row,
}: {
  ...
  return (
    <div className="space-y-5 text-sm">
      ...
      <section className="space-y-3 rounded-xl border bg-card p-4">
        ...
        <div className="flex flex-wrap gap-2">
          {item.supplyRegions.length > 0 ? (
            ...
            <span className="text-muted-foreground">未标注区域</span>
          )}
        </div>
        <div className="flex items-center gap-2 border-t pt-3 text-xs text-muted-foreground">
          <UsersIcon className="size-3.5" aria-hidden />
          <span>
            当前由 <strong className="num text-foreground">{item.supplierCount}</strong>{" "}
            家有效供应商支持供货
          </span>
        </div>
      </section>
        ...
        <dl className="grid grid-cols-2 gap-2">
          {[
            ...
            <div key={label} className="min-w-0 rounded-lg bg-muted/50 p-3">
              ...
              <dd className="num mt-1 truncate font-medium" title={value}>
                {value}
              </dd>
            </div>
          ))}
      ...
      <section className="rounded-xl border bg-muted/30 p-4">
        <h3 className="text-xs font-medium text-muted-foreground">当前可售期间</h3>
        <div className="num mt-1 font-medium">
          {formatEffectiveRange(row.effectiveFrom, row.effectiveTo)}
        </div>
        <p className="mt-2 text-xs leading-5 text-muted-foreground">
          销售资格核对时点：{item.eligibilityAsOf}。价格、商品状态或供给变化后，资格将重新计算。
        </p>
      </section>
    </div>
  )
