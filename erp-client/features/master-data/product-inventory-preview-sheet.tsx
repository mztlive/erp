...
export function ProductInventoryPreviewSheet({
  ...
  React.useEffect(() => {
    if (!open) return
    const initial = skus.find((sku) => sku.skuId === initialSkuId) ?? skus[0]
  ...
  const content = (() => {
    ...
    if (inventoryQuery.isPending) {
      return (
        <div className="space-y-3 p-5" aria-busy="true" aria-label="正在加载商品库存">
        ...
        <div className="space-y-4 p-4 md:p-5">
            ...
            <Table data-density="compact">
              ...
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">账面现存</TableHead>
                  <TableHead className="text-right">有效预占</TableHead>
                ...
                {data.balances.map((balance) => (
                  <TableRow key={balance.balanceId}>
                    ...
                    <TableCell className="text-right">
                      {quantity(balance.onHandQuantity, balance.baseUnit)}
                    </TableCell>
                    <TableCell className="text-right">
                      {quantity(balance.reservedQuantity, balance.baseUnit)}
                    </TableCell>
                    <TableCell className="text-right font-medium text-primary">
                      {quantity(balance.availableQuantity, balance.baseUnit)}
                    </TableCell>
                    <TableCell>
                      <BusinessStatusBadge
                        context="list"
                    ...
                    <TableCell>
                      <div>{balance.lastMovementTypeLabel}</div>
                      <div className="num text-xs text-muted-foreground">
                        {formatDateTime(
          ...
          {data.total > data.balances.length ? (
            <p className="text-xs text-muted-foreground">
              当前仅展示前 {MAX_PREVIEW_ROWS} 个仓库组合；全部结果请在完整库存台账查看。
            </p>
          ) : null}
        </div>
      </ScrollArea>
    ...
    <QuickPreviewSheet
      ...
      summary={
        <div className="space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            库存按已保存的 SKU 身份查询；当前尚未保存的名称、编号或价格修改不会改变台账范围。
          </p>
        </div>
      }
      footer={
