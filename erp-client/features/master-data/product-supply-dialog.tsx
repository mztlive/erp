  ...
  return (
    <Dialog open={product != null} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-5xl">
        <DialogHeader>
          <DialogTitle>{product ? `${product.name} · 供给` : "商品供给"}</DialogTitle>
          <DialogDescription>
            {product
        ...
        {skuError ? (
          ...
          <div className="rounded-lg border bg-muted/30 px-4 py-8 text-center">
            <p className="text-sm font-medium">该商品没有启用中的 SKU</p>
          ...
          <div className="space-y-4">
            ...
            {skus.map((sku) => {
              const skuOfferings = currentOfferingsBySku.get(sku.skuId) ?? []
              return (
                <section key={sku.skuId} className="overflow-hidden rounded-lg border">
                  <div className="flex flex-wrap items-center justify-between gap-3 border-b bg-muted/30 px-4 py-3">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="font-medium">{sku.skuNo}</span>
                        <Badge variant="secondary">{sku.specification}</Badge>
                        {!offeringLoading && !offeringError ? (
                          <Badge variant={skuOfferings.length > 0 ? "success" : "outline"}>
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        {sku.skuName} · 销售价 {money(sku.salesVisiblePriceGross)}
                      </p>
                    </div>
                    <Button
                      type="button"
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead>一件代发价</TableHead>
                          <TableHead>集采价</TableHead>
                          <TableHead>当前可供</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {skuOfferings.map((offering) => (
                          <TableRow key={offering.id}>
                            <TableCell>
                              ...
                              {offering.supplier_no && offering.supplier_name ? (
                                <div className="mt-1 text-xs text-muted-foreground">
                                  {offering.supplier_no}
                                </div>
                              ) : null}
                            </TableCell>
                            <TableCell>{offering.supplier_sku_code}</TableCell>
                            <TableCell>
                              {money(offering.dropship_supply_price_gross)}
                            </TableCell>
                            <TableCell>
                              {money(offering.bulk_supply_price_gross)}
                            </TableCell>
                            <TableCell>
                              <Badge
                                variant={
                              ...
                              <div className="mt-1 text-xs text-muted-foreground">
                                数量 {offering.available_quantity ?? "未提供"}
                              </div>
                            </TableCell>
                          </TableRow>
                        ))}
