    ...
    <PageScaffold density="compact">
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            {returnTo ? (
              <Button type="button" variant="outline" render={<Link href={returnTo} />}>
                返回商品
              </Button>
            ) : null}
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="rounded-lg border bg-card px-4 py-3">
          ...
          <div className="mt-1 text-xl font-semibold">{query.data?.total ?? 0}</div>
        </div>
        <div className="rounded-lg border bg-card px-4 py-3">
          <div className="text-xs text-muted-foreground">本页启用关系</div>
          <div className="mt-1 text-xl font-semibold">{activeCount}</div>
        </div>
        <div className="rounded-lg border bg-card px-4 py-3">
          <div className="text-xs text-muted-foreground">本页当前可供</div>
          <div className="mt-1 text-xl font-semibold">{availableCount}</div>
        </div>
      </div>
            ...
            filters={
              <div className="flex flex-wrap items-center gap-1">
                ...
                <div className="flex items-center gap-1">
                  {([undefined, "ACTIVE", "PAUSED", "STOPPED"] as const).map(
                    (value) => (
                      <Button
                        key={value ?? "all"}
                        type="button"
                      ...
                      >
                        {value ? OFFERING_STATUS_LABELS[value] : "全部"}
                      </Button>
                    )
                  )}
                </div>
                  ...
                  <TableRow key={item.id}>
                    <TableCell>
                      <div className="font-medium">
                        {item.sku_name ?? item.product_no ?? "公司商品"}
                      </div>
                    ...
                    <TableCell>
                      <div className="font-medium">
                        {item.supplier_name ?? item.supplier_no ?? item.supplier_id}
                      </div>
                      <div className="mt-1 text-xs text-muted-foreground">
                        {item.supplier_sku_code} · {SOURCE_TYPE_LABELS[item.source_type]}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div>代发 {money(item.dropship_supply_price_gross)}</div>
                      <div className="mt-1 text-xs text-muted-foreground">
                    ...
                    <TableCell>
                      ...
                      >
                        {item.availability_status
                          ? AVAILABILITY_STATUS_LABELS[item.availability_status]
                          : "未更新"}
                      </Badge>
                      <div className="mt-1 text-xs text-muted-foreground">
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-1">
                        <Button
                          ...
                          size="sm"
                          variant="ghost"
                          onClick={() => setAvailabilityOffering(item)}
                        >
                          更新可供
                        </Button>
                        <Button
                          ...
                          size="sm"
                          variant="outline"
                          onClick={() => setReviseOffering(item)}
                        >
                          修订条款
                        </Button>
                      </div>
        ...
        fixedSku={
          skuId
            ? {
                ...
                specification: items[0]?.specification ?? "默认规格",
                baseUnit: "",
              }
            : undefined
        }
