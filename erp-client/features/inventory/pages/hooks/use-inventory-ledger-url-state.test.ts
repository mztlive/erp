...
describe("useInventoryLedgerUrlState", () => {
    it("applies defaults when no params are present", () => {
        ...
        expect(result.current.workItemIdParam).toBeUndefined()
        ...
        expect(result.current.sortValue).toBe("warehouseCode:asc,skuCode:asc")
    ...
    it("parses every param from the URL", () => {
        ...
        expect(result.current.sortValue).toBe("occurredAt:asc,movementId:asc")
    ...
    it("reads the current work item from workspace deep links", () => {
        mockedSearchParams.mockReturnValue(
            params("currentWorkItemId=wi-1&from=workspace"),
        )
        const { result } = renderHook(() => useInventoryLedgerUrlState())
        expect(result.current.workItemIdParam).toBe("wi-1")
    })
    ...
    it("clamps pageSize: invalid, zero and negative become 20; oversized becomes 100", () => {
        for (const [raw, expected] of [
            ...
            mockedSearchParams.mockReturnValue(params(`pageSize=${raw}`))
    ...
    it("decodes a valid cursor for the active view and ignores mismatches or garbage", () => {
        ...
        mockedSearchParams.mockReturnValue(
            params("view=balance&cursor=w10%3Abalance%3A9007199254740993"),
        )
    ...
    it("derives the default sort per view", () => {
        mockedSearchParams.mockReturnValue(params("view=adjustment"))
        const { result } = renderHook(() => useInventoryLedgerUrlState())
    ...
    it("patchUrl removes null values, keeps the view param and clears the cursor", () => {
        mockedSearchParams.mockReturnValue(
            params("view=balance&cursor=w10%3Abalance%3A40&warehouseId=w1&q=x"),
        )
        ...
        mockedSearchParams.mockReturnValue(params("view=balance&q=SKU-2"))
        rerender()
        act(() => {
    ...
    it("hasActiveFilters covers filters and deep links per active view", () => {
        ...
        const allAvailability = renderHook(() => useInventoryLedgerUrlState())
