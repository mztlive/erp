...
function makeListView(
    overrides: Partial<InventoryListView> = {},
): InventoryListView {
    return {
        view: "balance",
        metrics: {
...
function makeDraftView(
    overrides: Partial<AdjustmentDraftView> = {},
): AdjustmentDraftView {
    return {
...
describe("useInventoryListQuery", () => {
    it("fetches the list with the given query under a stable key", async () => {
        ...
        await waitFor(() => expect(result.current.data).toEqual(makeListView()))
        ...
        expect(
            client
                ...
                .getAll()
                .map((q) => q.queryKey),
        ).toEqual([["inventory", "list", baseQuery]])
    })
