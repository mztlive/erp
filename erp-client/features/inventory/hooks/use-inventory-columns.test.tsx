...
describe('useInventoryColumns', () => {
    ...
    it('passes reservation status label and tone to the badge', () => {
        ...
        const node = callCell(
            ...
            'status',
            makeReservationRow({ statusLabel: '部分消耗', statusTone: 'warning' }),
    ...
    it('renders operator roles with dash fallbacks in the people cell', () => {
        ...
        expect(container.textContent).toContain('仓储复核 —')
        expect(container.textContent).toContain('财务确认 —')
    })
})
