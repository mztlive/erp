use database::{
    AccessControlExt, Executor, NoTransaction, ReceivableExt, SalesOrderExt, SalesReviewExt, Transactional,
    WorkItemExt,
};
...
use entities::sales_review::{
    BusinessType, SalesChangeReview, SalesChangeReviewData, SalesChangeReviewStage,
};
...
impl SalesReviewService {
    ...
    pub async fn confirm_impact(
        ...
        req.validate()?;
        let action = "sales_change_order.impact_confirmed";
        let fingerprint = change_review_fingerprint(id, &req);
        let audit_id = change_review_audit_id(actor.id(), action, &req.work_item_id, &req.idempotency_key);
        if let Some(view) = self
            .replay_change_review_command(&audit_id, &fingerprint, id)
            .await?
        {
            return Ok(view);
        }
        ...
        let work_item = WorkItem::new(
            ...
            WorkItemData {
                ...
                subject_version: submission_id.to_string(),
        ...
        let mut completed = self
            .load_change_review_work_item(&review, &submission_id, &req, actor)
            .await?;
        completed.record_activity(actor.id(), now)?;
        completed.complete_by_domain_command(actor.id(), now)?;
        let audit = actor.clone().resource_log_with_id(
            audit_id.clone(),
            action,
            "sales_change_order",
            id.to_string(),
            Some(change_review_receipt_message(&fingerprint)),
        )?;
        ...
        let transaction_result = client
            .with_transaction(move |session| {
                Box::pin(async move {
                    ensure_change_reviewer_eligible(
                        &db,
                        &completed,
                        &submission,
                        &review_for_tx.base.id,
                        &actor_id,
                        session,
                    )
                    .await?;
    ...
    pub async fn confirm_finance(
        ...
        req.validate()?;
        let action = "sales_change_order.effective";
        let fingerprint = change_review_fingerprint(id, &req);
        let audit_id = change_review_audit_id(actor.id(), action, &req.work_item_id, &req.idempotency_key);
        if let Some(view) = self
            .replay_change_review_command(&audit_id, &fingerprint, id)
        ...
        let delta_work_item_id = WorkItemId::new(next_id());
        let mut completed = self
            .load_change_review_work_item(&review, &submission_id, &req, actor)
            .await?;
        completed.record_activity(actor.id(), now)?;
            ...
            .with_transaction(move |session| {
                Box::pin(async move {
                    ...
                    db.sales_order()
                        ...
                        .await?;
                    if let Some((mut account, entry, is_existing_account)) = delta_for_tx {
                        if is_existing_account {
                            let active = db
                                .work_items()
                                .list_active_by_object("receivable_account", &account.base.id, session)
                                .await?;
                            let existing_delta_task = active
                                .iter()
                                .find(|item| item.work_item_type == WorkItemType::CardFundsDeltaReview);
                            if let Some(item) = existing_delta_task {
                                if item.business_object_type != "receivable_account"
                                    || item.business_object_id != account.base.id
                                    || item.subject_version != revision.revision.base.id
                                    || item.approval_step_instance_id.is_some()
                                {
                                    return Err(Error::ConflictError(
                                        "已有卡券票款差额任务与当前销售版本不一致".to_string(),
                                    ));
                                }
                            }
                            account.update(
                                ReceivableAccountUpdate {
                                    review_status: (submission.business_type == BusinessType::Voucher)
                                        .then_some(AccountReviewStatus::SyncDeltaPending),
                                    reviewed_by: (submission.business_type == BusinessType::Voucher)
                                        .then_some(String::new()),
                                    reviewed_at: None,
                                    review_evidence_reference: (submission.business_type
                                        == BusinessType::Voucher)
                                        .then_some(String::new()),
                                    gross_total: Some(revision.revision.gross_amount),
                                    invoiceable_total: Some(revision.revision.gross_amount),
                                },
                                &actor_id,
                            )?;
                            db.receivable_entries().create(&entry, session).await?;
                            db.receivable_accounts().update(&mut account, session).await?;
                            if submission.business_type == BusinessType::Voucher
                                && existing_delta_task.is_none()
                            {
                                let delta_task = WorkItem::new_at(
                                    delta_work_item_id,
                                    WorkItemData {
                                        ...
                                        impact_summary: Some(
                                            "销售版本差额已追加，需要重新核对卡券回款与开票事实".to_string(),
                                        ),
                                    },
                                    now,
                                )?;
                                db.work_items().create(&delta_task, session).await?;
                            }
                        } else {
                            db.receivable()
    ...
    async fn decide_change_review(
        ...
        req.validate()?;
        let action = "sales_change_order.review_rejected";
        let fingerprint = change_review_fingerprint(id, &req);
        let audit_id = change_review_audit_id(actor.id(), action, &req.work_item_id, &req.idempotency_key);
        if let Some(view) = self
            .replay_change_review_command(&audit_id, &fingerprint, id)
