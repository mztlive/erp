//! 销售变更撤回：调用统一 `prepare_cancel`，再执行业务 `cancel_action`。
...
pub fn build_sales_change_cancel_input(
...
pub(super) struct SalesChangeCancelPersistInput {
...
pub(super) async fn persist_sales_change_cancel(
    db: &Database,
    input: SalesChangeCancelPersistInput,
) -> Result<()> {
    let SalesChangeCancelPersistInput {
        mut change_order,
        prepared,
        open_tasks,
        actor_id,
