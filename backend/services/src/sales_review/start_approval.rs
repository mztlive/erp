//! 销售变更提交启动：加载定义图、构造 `prepare_start` 输入并持久化运行事实。
...
pub(super) async fn load_bound_definition_graph(
    ...
    let graph = db
        ...
        .await?
        .ok_or_else(|| Error::ConflictError("销售变更单绑定的审批定义不存在".to_string()))?;
...
pub(super) async fn replay_sales_change_start_with_executor(
    ...
    let instance = db
        ...
        .await?
        .ok_or_else(|| Error::ConflictError("销售变更启动收据引用的审批实例不存在".to_string()))?;
...
///
/// # 用途
/// 收拢 `build_sales_change_start_input` 的定义图、绑定与提交人参数。
///
/// # 参数
/// 无。
///
...
/// # 关键业务约束
/// 审批人取自已发布节点，不接受客户端选择。
pub(super) struct SalesChangeStartInput<'a> {
    /// 绑定定义图。
...
pub(super) fn build_sales_change_start_input(
    input: SalesChangeStartInput<'_>,
) -> Result<StartExecutionInput> {
    let SalesChangeStartInput {
        graph,
        binding,
        subject,
        subject_version,
    ...
    Ok(StartExecutionInput {
        ...
        process_kind: process_kind_of(DocumentType::SalesChangeOrder),
        subject,
        subject_version,
...
fn start_bindings_from_graph(
    ...
    for node in &graph.nodes {
        ...
        let failure = match sales_change_order_object_readable(organization_id, assignee) {
            Ok(true) => None,
...
pub(super) struct SalesChangeStartPersistInput {
    /// 已进入审批中的变更单。
    pub change_order: SalesChangeOrder,
    /// 已锁定的工作副本。
    pub working_copy: entities::sales_order::SalesOrderWorkingCopy,
    /// 冻结提交头。
    pub submission: SalesChangeSubmission,
    /// 冻结提交行。
...
pub(super) async fn persist_sales_change_start(
    db: &Database,
    input: SalesChangeStartPersistInput,
) -> Result<()> {
    let db = db.clone();
    let client = db.client().clone();
    let SalesChangeStartPersistInput {
        mut change_order,
        mut working_copy,
        submission,
        submission_lines,
    ...
    client
        .with_transaction(move |session| {
            Box::pin(async move {
                ...
                db.sales_review()
                    .submit_sales_change(&mut change_order, &submission, &submission_lines, session)
                    .await?;
...
async fn persist_runtime_writes(
    ...
    let first = writes
        .created_executions
        .first()
        .ok_or_else(|| Error::Internal("启动计划缺少入口执行，不得提交销售变更单".to_string()))?;
    db.bpm_workflow()
    ...
    for intent in &writes.create_tasks {
        ...
        let item = WorkItem::new_document_approval(
            ...
            DocumentApprovalWorkItemData {
                ...
                business_object_type: DocumentType::SalesChangeOrder.as_str().to_string(),
