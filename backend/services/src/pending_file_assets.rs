...
use entities::{
    file_asset::{FileAsset, SensitivityClass},
    ids::FileAssetId,
    AuditLog,
};
...
/// multipart 业务命令中临时文件引用的固定前缀。
pub(crate) const PENDING_FILE_REFERENCE_PREFIX: &str = "pending-file:";

/// 已完成实体构造、等待随业务聚合一起持久化的文件资产集合。
pub(crate) struct PendingFileAssets {
    assets: Vec<FileAsset>,
    audits: Vec<AuditLog>,
    reference_to_id: HashMap<String, FileAssetId>,
...
impl PendingFileAssets {
    ...
    pub(crate) fn prepare(requests: Vec<PendingFileAssetRequest>, actor: &AuditActor) -> Result<Self> {
        ...
        let mut reference_to_id = HashMap::with_capacity(requests.len());
        let mut sensitivity_by_id = HashMap::with_capacity(requests.len());
        for request in requests {
            let reference = request.reference.trim().to_string();
            if !reference.starts_with(PENDING_FILE_REFERENCE_PREFIX) {
                return Err(Error::ValidationError("临时文件引用格式无效".to_string()));
            }
            if reference_to_id.contains_key(&reference) {
                return Err(Error::ValidationError("临时文件引用不能重复".to_string()));
            }
            ...
            sensitivity_by_id.insert(asset.base.id.clone(), sensitivity);
            reference_to_id.insert(reference, asset_id);
            assets.push(asset);
            audits.push(audit);
        }
        Ok(Self {
    ...
    pub(crate) fn resolve_id(&self, id: &mut FileAssetId, used: &mut HashSet<String>) -> Result<bool> {
        ...
        let resolved = self
            ...
            .get(&reference)
            .cloned()
            .ok_or_else(|| Error::ValidationError("业务命令引用了未上传的文件".to_string()))?;
    ...
    pub(crate) fn ensure_all_used(&self, used: &HashSet<String>) -> Result<()> {
        if self
            .reference_to_id
            .keys()
            .any(|reference| !used.contains(reference))
        {
            return Err(Error::ValidationError(
                "存在未被业务命令引用的上传文件".to_string(),
            ));
        }
        Ok(())
    }
    ...
    pub(crate) async fn persist(&self, db: &Database, session: &mut ClientSession) -> Result<()> {
        for asset in &self.assets {
            db.file_assets().create(asset, session).await?;
        }
