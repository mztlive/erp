...
/// 证明撤回命令种类是统一 `CANCEL_APPROVAL`。
///
/// # 参数
/// * `blocked_port` - 是否受阻取消
///
/// # 返回
/// 业务撤回返回 `CancelApproval`，受阻取消返回 `CancelBlocked`。
#[cfg(test)]
pub fn cancel_approval_command_kind(blocked_port: bool) -> ApprovalCommandKind {
    if blocked_port {
...
mod tests {
    use super::{
        build_sales_order_cancel_input, cancel_approval_command_kind, ensure_instance_cancellable,
        LoadedCancelRuntime,
    };
    ...
    use bpm::model::{
        ApprovalNodeDefinition, ApprovalNodeExecution, ApprovalProcessDefinition, ApprovalProcessInstance,
        ApprovalTransitionDefinition, NewNodeExecution, ParticipantId, ProcessKind, SubjectRef, Timestamp,
    };
    use entities::common::time::Instant;
    ...
    fn approved_instance_cannot_cancel() {
        ...
        instance.complete_approved(at(20)).expect("通过");
        let error = ensure_instance_cancellable(&instance).unwrap_err();
        assert!(error.to_string().contains("已最终通过"));
    }
    ...
    fn cancel_builds_unified_input_and_prepare_cancel() {
        ...
        assert_eq!(
            cancel_approval_command_kind(input.blocked_port),
            ApprovalCommandKind::CancelApproval
        );
