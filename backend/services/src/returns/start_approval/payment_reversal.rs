...
/// # 错误
/// 幂等键非法或仓储失败时返回错误。
pub async fn load_payment_reversal_start_receipt(
    db: &Database,
    subject: &SubjectRef,
    subject_version: u32,
    idempotency_key: &str,
...
/// # 关键业务约束
/// 审批人取自已发布节点，不接受客户端选择。
pub struct PaymentReversalStartInput<'a> {
    /// 绑定定义图。
...
pub fn build_payment_reversal_start_input(
...
pub struct PaymentReversalStartPersistInput {
...
pub async fn persist_payment_reversal_start(
    db: &Database,
