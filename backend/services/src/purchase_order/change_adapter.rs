...
pub fn purchase_change_order_subject_ref(business_object_id: &str) -> Result<SubjectRef> {
    entities::approval_integration::subject_ref_for(DocumentType::PurchaseChangeOrder, business_object_id)
...
pub fn ensure_final_approve_effective(order: &PurchaseChangeOrder) -> Result<()> {
    if order.stable.status != PurchaseChangeOrderStatus::InApproval {
        return Err(Error::ConflictError(
            "只有审批中的采购变更单可以由最终通过动作生效".to_string(),
        ));
    }
    Ok(())
}
...
pub fn execute_purchase_change_domain_action(
    ...
    match action {
        ApprovalDomainAction::PurchaseChangeOrderApplyEffectiveChange => {
            ensure_final_approve_effective(order)
        }
        ApprovalDomainAction::PurchaseChangeOrderCancelApproval => {
            cancel_purchase_change_to_draft(order, updated_by)
        }
