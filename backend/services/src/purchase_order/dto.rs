...
//! - `purchase_no` 在创建时分配正式编号（实体层 `PurchaseOrder::new` 必填），
//!   前端 mock 约定首次提交时才分配；
//! - 表单类写操作（创建/保存/提交/审核）统一返回稳定业务结果，不再返回
//!   `FormalActionResponse` 信封（由 HTTP 统一信封承载）。

use entities::purchase_order::{
    FulfillmentResponsibility, ProgressStatus, PurchaseLineType, PurchaseOrderRevision, PurchaseOrderStatus,
    PurchaseReviewStatus, PurchaseType,
};
...
/// # 返回
/// 返回响应视图。
pub(crate) fn revision_line_to_view(
...
/// # 返回
/// 返回响应视图。
pub(crate) fn submission_line_to_view(
...
/// # 返回
/// 返回汇总视图。
pub(crate) fn revision_totals(revision: &PurchaseOrderRevision) -> TotalsView {
    TotalsView {
