...
pub(super) struct SupplierPaymentCancelPersistInput {
    /// 已执行 `cancel_action` 的付款单。
...
pub(super) async fn persist_supplier_payment_cancel(
    db: &Database,
    input: SupplierPaymentCancelPersistInput,
) -> Result<()> {
    let SupplierPaymentCancelPersistInput {
        mut payment,
        prepared,
        open_tasks,
        actor_id,
...
mod tests {
    ...
    fn instance(status: ApprovalProcessInstanceStatus) -> ApprovalProcessInstance {
        let mut item = ApprovalProcessInstance::start_running(bpm::model::NewProcessInstance {
            ...
            process_kind: ProcessKind::SupplierPayment,
            subject: SubjectRef::new("supplier_payment", "sp-1").unwrap(),
            subject_version: 1,
