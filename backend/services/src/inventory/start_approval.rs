...
use database::{
    AccessControlExt, ApprovalIntegrationExt, BpmExt, InventoryExt, NoTransaction, Transactional, WorkItemExt,
};
...
fn start_bindings_from_graph(
    ...
    for node in &graph.nodes {
        ...
        let failure = match stock_adjustment_object_readable(organization_id, assignee) {
            Ok(true) => None,
