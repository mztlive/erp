...
impl From<erp_sales::Error> for Error {
    ...
    fn from(error: erp_sales::Error) -> Self {
        match error {
            erp_sales::Error::Internal(message) => Self::Internal(message),
            erp_sales::Error::NotFound(message) => Self::NotFound(message),
            erp_sales::Error::ValidationError(message) => Self::ValidationError(message),
            erp_sales::Error::BusinessLogicError(message) => Self::BusinessLogicError(message),
            erp_sales::Error::ConflictError(message) => Self::ConflictError(message),
            erp_sales::Error::ReceiptDuplicate(error) => Self::ReceiptDuplicate(error),
            erp_sales::Error::TransientTransaction(error) => Self::TransientTransaction(error),
            erp_sales::Error::Forbidden(message) => Self::Forbidden(message),
            erp_sales::Error::Unauthenticated(message) => Self::Unauthenticated(message),
            erp_sales::Error::Logic(error) => Self::Logic(error),
            erp_sales::Error::OutcomeUnknown(error) => Self::OutcomeUnknown(error),
            erp_sales::Error::RepositoryError(error) => Self::RepositoryError(error),
        }
    }
}
...
impl From<erp_integration::Error> for Error {
    /// 将集成领域错误映射为服务错误。
    fn from(error: erp_integration::Error) -> Self {
        match error {
            erp_integration::Error::Internal(message) => Self::Internal(message),
            erp_integration::Error::NotFound(message) => Self::NotFound(message),
            erp_integration::Error::ValidationError(message) => Self::ValidationError(message),
            erp_integration::Error::BusinessLogicError(message) => Self::BusinessLogicError(message),
            erp_integration::Error::ConflictError(message) => Self::ConflictError(message),
            erp_integration::Error::ReceiptDuplicate(error) => Self::ReceiptDuplicate(error),
            erp_integration::Error::TransientTransaction(error) => Self::TransientTransaction(error),
            erp_integration::Error::Forbidden(message) => Self::Forbidden(message),
            erp_integration::Error::Unauthenticated(message) => Self::Unauthenticated(message),
            erp_integration::Error::Logic(error) => Self::Logic(error),
            erp_integration::Error::OutcomeUnknown(error) => Self::OutcomeUnknown(error),
            erp_integration::Error::RepositoryError(error) => Self::RepositoryError(error),
        }
    }
}
...
impl From<erp_supply::Error> for Error {
    /// 将供应链领域错误映射为服务错误。
    fn from(error: erp_supply::Error) -> Self {
        match error {
            erp_supply::Error::Internal(message) => Self::Internal(message),
            erp_supply::Error::NotFound(message) => Self::NotFound(message),
            erp_supply::Error::ValidationError(message) => Self::ValidationError(message),
            erp_supply::Error::BusinessLogicError(message) => Self::BusinessLogicError(message),
            erp_supply::Error::ConflictError(message) => Self::ConflictError(message),
            erp_supply::Error::ReceiptDuplicate(error) => Self::ReceiptDuplicate(error),
            erp_supply::Error::TransientTransaction(error) => Self::TransientTransaction(error),
            erp_supply::Error::Forbidden(message) => Self::Forbidden(message),
            erp_supply::Error::Unauthenticated(message) => Self::Unauthenticated(message),
            erp_supply::Error::Logic(error) => Self::Logic(error),
            erp_supply::Error::OutcomeUnknown(error) => Self::OutcomeUnknown(error),
            erp_supply::Error::RepositoryError(error) => Self::RepositoryError(error),
        }
    }
}
...
impl From<erp_fulfillment::Error> for Error {
    ...
    fn from(error: erp_fulfillment::Error) -> Self {
        match error {
            erp_fulfillment::Error::Internal(message) => Self::Internal(message),
            erp_fulfillment::Error::NotFound(message) => Self::NotFound(message),
            erp_fulfillment::Error::ValidationError(message) => Self::ValidationError(message),
            erp_fulfillment::Error::BusinessLogicError(message) => Self::BusinessLogicError(message),
            erp_fulfillment::Error::ConflictError(message) => Self::ConflictError(message),
            erp_fulfillment::Error::ReceiptDuplicate(error) => Self::ReceiptDuplicate(error),
            erp_fulfillment::Error::TransientTransaction(error) => Self::TransientTransaction(error),
            erp_fulfillment::Error::Forbidden(message) => Self::Forbidden(message),
            erp_fulfillment::Error::Unauthenticated(message) => Self::Unauthenticated(message),
            erp_fulfillment::Error::Logic(error) => Self::Logic(error),
            erp_fulfillment::Error::OutcomeUnknown(error) => Self::OutcomeUnknown(error),
            erp_fulfillment::Error::RepositoryError(error) => Self::RepositoryError(error),
        }
    }
}
