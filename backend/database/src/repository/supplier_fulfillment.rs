...
impl<'a> Repository<'a, SupplierFulfillmentOrder> {
    ...
    /// 按「连接 + 供应商订单号」定位履约订单（供应商回调入口）。
    ///
    /// 非空 `(connection_id, external_order_no)` 由
    /// `uk_supplier_fulfillment_orders_connection_external` 部分唯一索引保证（§6.19：
    /// 回调必须据连接和外部单号唯一定位子订单，零条或多条命中均进入接口差异）。
    ///
    /// # 参数
    ...
    /// # 错误
    /// 当 MongoDB 查询失败时返回错误。
    pub async fn find_by_external_order_no(
        &self,
    ...
    /// 批量按来源商城订单查询履约订单（`$in` 一次取回，避免 N+1）。
    ///
    /// 供 `mall_order_fact` 联动与商城订单详情页按订单聚合子订单使用（§6.19）；
    /// 结果按实体 ID 排序保证确定性。
    ///
    /// # 参数
    /// * `mall_order_ids` - 来源商城订单 ID 集合
    /// * `executor` - 数据访问执行器，由 Service 决定是否位于事务中
    ///
    /// # 返回
    /// 返回命中集合对应的全部未删除履约订单。
    ...
    pub async fn find_by_mall_order_ids(
        &self,
        mall_order_ids: &[MallOrderId],
        executor: &mut dyn Executor,
    ) -> Result<Vec<SupplierFulfillmentOrder>> {
        if mall_order_ids.is_empty() {
            return Ok(Vec::new());
        }

        let mut orders = self
            .find_many(
                doc! { "mall_order_id": { "$in": ids_to_strings(mall_order_ids) } },
                executor,
            )
            .await?;
