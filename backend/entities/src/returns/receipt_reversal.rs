...
mod tests {
    ...
    fn update_applies_draft_changes_and_rejects_posted() {
        ...
        reversal.transition(ReceiptReversalStatus::PendingReview).unwrap();
        reversal.transition(ReceiptReversalStatus::Posted).unwrap();
        assert!(reversal.is_posted());
        assert!(reversal
            .update(ReceiptReversalUpdate {
    ...
    fn state_machine_forces_review_before_posting() {
        ...
        let mut reversal = ReceiptReversal::new(ReceiptReversalId::new("rr-1"), data()).unwrap();
        assert!(
            reversal.transition(ReceiptReversalStatus::Posted).is_err(),
            "草稿不得直接过账"
        );
        reversal.transition(ReceiptReversalStatus::PendingReview).unwrap();
        assert!(reversal.transition(ReceiptReversalStatus::Posted).is_ok());
    }

    #[test]
    fn status_serializes_with_stable_codes_and_labels() {
        assert_eq!(
            serde_json::to_string(&ReceiptReversalStatus::PendingReview).unwrap(),
            "\"pending_review\""
        );
