//! `PurchaseInvoiceAllocationPlan` 进项发票分配计划值对象（FIN-E03）。
//!
//! 与 [`PurchaseInvoiceAllocation`](crate::payable::PurchaseInvoiceAllocation) 实体的
//! 分工：本计划负责发票总额／税额三口径（gross / net / tax）与发票应付额精确
//! 守恒、输入顺序确定的连续序号、分配实体生成与按子账聚合的收票增量归位；
//! `PurchaseInvoiceAllocation::new` 继续守卫单行自身的金额正数与
//! `gross = net + tax` 恒等不变量。计划不生成 ID、不读取数据库——发票 ID 与
//! 分配行 ID 由 Service 注入；账户／供应商存在性、跨供应商一致性、可收票额度、
...
use crate::payable::{AllocationAction, PurchaseInvoiceAllocation, PurchaseInvoiceAllocationData};

/// 进项发票分配计划输入行（金额三元组由计划统一校验）。
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PurchaseInvoiceAllocationLine {
    /// 采购单或供应商结算单应付子账。
...
/// 进项发票分配计划（FIN-E03）。
///
/// 以发票应付额（gross / net / tax）与输入行一次完成全部校验并生成不可部分
/// 成功的计划：
/// - 至少一条分配；每行金额为正且满足 `gross = net + tax`（实体不变量）；
/// - 三口径合计与发票应付额精确守恒（少分、超分、税额口径漂移均拒绝）；
/// - 序号按输入顺序从 1 连续分配；相同输入必然产出相同计划（确定性）；
/// - 按子账聚合收票增量（首次出现顺序），同一账户只推进一次聚合值。
///
/// 任一步校验失败即整体失败，不产生部分计划；金额运算全部为精确定点运算，
/// 任一步溢出即失败。
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PurchaseInvoiceAllocationPlan {
    /// 本次构造的进项发票分配（输入顺序，序号从 1 连续）。
    allocations: Vec<PurchaseInvoiceAllocation>,
    /// 按子账聚合的本次收票增量（首次出现顺序，确定性）。
    account_deltas: Vec<(PayableAccountId, Amount)>,
}

impl PurchaseInvoiceAllocationPlan {
    /// 依据发票应付额与输入行构建进项发票分配计划。
    ///
    /// 集中完成总额／税额口径守恒校验（三口径与发票应付额精确相等）、序号
    /// 分配、分配实体生成与按子账聚合收票增量；任一输入行金额非正或恒等
    /// 不成立时由实体构造器拒绝，整体不产生部分计划。
    ///
    /// # 参数
    /// * `invoice_id` - 目标进项发票
    /// * `invoice_gross` - 发票含税应付额（守恒基准）
    /// * `invoice_net` - 发票不含税应付额（守恒基准）
    ...
    ///
    /// # 约束
    /// 纯内存确定性构造：不生成 ID、不读写数据库、不校验账户／供应商存在性
    /// 与可收票额度；存在性与额度由 Service 装载事实后校验，并发安全由
    ...
    pub fn new(
        ...
        invoice_net: Amount,
        invoice_tax: Amount,
        lines: &[PurchaseInvoiceAllocationLine],
        allocation_ids: &[PurchaseInvoiceAllocationId],
    ) -> Result<Self> {
        if lines.is_empty() {
        ...
        let mut account_deltas: Vec<(PayableAccountId, Amount)> = Vec::new();
        ...
        for (index, line) in lines.iter().enumerate() {
            ...
            allocations.push(PurchaseInvoiceAllocation::new(
            ...
            match account_delta_index.get(line.payable_account_id.as_ref()) {
                Some(&delta_index) => {
    ...
    pub fn new_allocations(&self) -> &[PurchaseInvoiceAllocation] {
        &self.allocations
    }
    ...
    ///
    /// # 约束
    /// 聚合只保证金额守恒；子账存在性、供应商一致性与可收票额度由
    /// Service 校验。
    pub fn account_invoicing_deltas(&self) -> &[(PayableAccountId, Amount)] {
        &self.account_deltas
    }
}
...
mod tests {
    ...
    fn build(
        ...
        invoice_tax: &str,
        lines: &[PurchaseInvoiceAllocationLine],
    ) -> Result<PurchaseInvoiceAllocationPlan> {
        PurchaseInvoiceAllocationPlan::new(
    ...
    fn empty_lines_and_id_mismatch_rejected() {
        ...
        let lines = [line("acct-1", "100.00", "94.00", "6.00")];
        let err = PurchaseInvoiceAllocationPlan::new(
            InvoiceId::new("invoice-1"),
