...
use crate::entity::facts::{
    PrepaymentRequirementFact as PaymentTermSnapshot, PurchaseAllocationFact as PurchaseLineSalesAllocation,
...
pub(crate) mod tests {
    ...
    pub(crate) fn receipt_data() -> PurchaseReceiptData {
        PurchaseReceiptData {
    ...
    fn revision_line(quantity: Quantity) -> PurchaseOrderRevisionLine {
        PurchaseOrderRevisionLine {
            id: PurchaseOrderRevisionLineId::new("po-line-1"),
    ...
    fn purchase_fulfillment_eligibility_checks_loaded_context() {
        ...
        assert!(
            PurchaseFulfillmentEligibility::ensure_order_fulfillable(PurchaseOrderStatus::Other,).is_err()
        );
    ...
    /// 当前采购多行按总数量派生完成度，不改成逐行收满判断。
    #[test]
    fn fulfillment_progress_preserves_total_quantity_contract() {
        let lines = [
            PurchaseOrderRevisionLine {
                id: PurchaseOrderRevisionLineId::new("line-a"),
                quantity: Some(Quantity::from_str("5").unwrap()),
            },
            PurchaseOrderRevisionLine {
                id: PurchaseOrderRevisionLineId::new("line-b"),
                quantity: Some(Quantity::from_str("5").unwrap()),
            },
        ];
        let mut received = HashMap::new();
