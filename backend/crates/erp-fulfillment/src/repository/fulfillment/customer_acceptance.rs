...
impl CustomerAcceptanceRepositoryExt for persistence_core::Repository<'_, CustomerAcceptance> {
    ...
    #[tracing::instrument(
        ...
        fields(
            ...
            db.collection.name = "customer_acceptances",
    ...
    async fn search_customer_acceptances(
        ...
        super::search_projected_page(
            &self.collection(),
            filter.to_doc(),
            sort_doc(filter.sort_by.as_deref(), filter.sort_ascending, CUSTOMER_ACCEPTANCE_SORT_FIELDS),
            filter.skip(),
            filter.limit(),
            customer_acceptance_projection(),
            executor,
        )
        .await
    }
