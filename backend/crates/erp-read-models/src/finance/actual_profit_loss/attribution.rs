...
use super::{calculation::OrderResult, dto::ProfitLossQuery};
    ...
    matches_group(order, query.attribution_group.as_deref())
        && query.attribution_user_ids.as_ref().is_none_or(|ids| {
            order
                ...
                .attribution_user_id
                .as_ref()
                .is_some_and(|id| ids.as_slice().contains(id))
        })
        && query.attribution_org_unit_ids.as_ref().is_none_or(|ids| {
            order
                .attribution_path
                .iter()
                .any(|node| ids.as_slice().contains(&node.id))
        })
}
