...
use super::query::{DEFAULT_TIMEZONE, parse_priorities};
...
fn priority_codes_are_strict_and_ordered() {
    ...
    assert_eq!(priorities, vec![WorkItemPriority::Urgent, WorkItemPriority::Normal]);
...
fn sales_invoice_execution_routes_to_w11() {
    let route =
        handler_route(WorkItemType::SalesInvoiceExecution, "receivable_account", "role-finance").unwrap();
...
fn w18_route_context_uses_only_fixed_owner_role_registry() {
    ...
    for (role, scope) in cases {
        let route =
            handler_route(WorkItemType::ImportBusinessConfirmation, "LEGACY_IMPORT_BATCH", role).unwrap();
        assert_eq!(
            route.route_context.and_then(|context| context.confirmation_scope).as_deref(),
            Some(scope)
        );
    }
    let unknown =
        handler_route(WorkItemType::ImportBusinessConfirmation, "LEGACY_IMPORT_BATCH", "role-unregistered");
    match unknown {
...
fn retired_and_unmapped_work_items_fail_closed() {
    ...
    let retired = handler_route(WorkItemType::PurchaseOrderReview, "purchase_order", "role-finance");
    match retired {
    ...
    let unknown_exception =
        handler_route(WorkItemType::BusinessException, "UNREGISTERED_SUBJECT", "role-operations");
...
fn procurement_creation_maps_to_purchase_workspace_and_family() {
    let route =
        handler_route(WorkItemType::ProcurementOrderCreation, "sales_order", "role-procurement").unwrap();
    assert_eq!(route.handler_key, "procurement_order_creation");
    assert_eq!(route.destination_workspace_id, "W08");
    assert!(route.route_context.is_none());
    assert_eq!(family_of(WorkItemType::ProcurementOrderCreation), WorkItemFamily::Procurement);
    assert!(WORK_ITEM_TYPES.contains(&WorkItemType::ProcurementOrderCreation));
    assert!(
        handler_route(WorkItemType::ProcurementOrderCreation, "purchase_order", "role-procurement").is_err()
    );
}
...
fn customer_acceptance_maps_to_w06_fulfillment_family() {
    let route =
        handler_route(WorkItemType::CustomerAcceptanceRegistration, "sales_order", "sales_order_owner")
            .unwrap();

    assert_eq!(route.handler_key, "customer_acceptance_registration");
    assert_eq!(route.destination_workspace_id, "W06");
    assert!(route.route_context.is_none());
    assert_eq!(family_of(WorkItemType::CustomerAcceptanceRegistration), WorkItemFamily::Fulfillment);
    assert_eq!(role_label("sales_order_owner"), "负责销售");
    assert!(WORK_ITEM_TYPES.contains(&WorkItemType::CustomerAcceptanceRegistration));
    assert!(
        handler_route(WorkItemType::CustomerAcceptanceRegistration, "delivery", "sales_order_owner").is_err()
    );
}
...
fn supplier_payment_execution_maps_to_w12_finance_family() {
    let route =
        handler_route(WorkItemType::SupplierPaymentExecution, "payable_account", "role-finance").unwrap();

    assert_eq!(route.handler_key, "supplier_payment_execution");
    assert_eq!(route.destination_workspace_id, "W12");
    assert!(route.route_context.is_none());
    assert_eq!(family_of(WorkItemType::SupplierPaymentExecution), WorkItemFamily::Finance);
    assert!(WORK_ITEM_TYPES.contains(&WorkItemType::SupplierPaymentExecution));
    assert!(
        handler_route(WorkItemType::SupplierPaymentExecution, "supplier_payment", "role-finance").is_err()
    );
}
...
fn document_approval_maps_to_signed_workspace_and_approval_family() {
    let stock =
        handler_route(WorkItemType::DocumentApproval, "stock_adjustment", "stock_adjustment_approver")
            .unwrap();
    assert_eq!(stock.handler_key, "document_approval");
    assert_eq!(stock.destination_workspace_id, "W10");
    assert_eq!(
        stock.route_context.and_then(|context| context.document_type).as_deref(),
    ...
    assert_eq!(family_of(WorkItemType::DocumentApproval), WorkItemFamily::Approval);
