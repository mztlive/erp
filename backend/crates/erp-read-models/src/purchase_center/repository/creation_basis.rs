...
pub async fn load_creation_basis_facts(
    ...
    Ok(CreationBasisFacts {
        revisions: revisions
            .into_iter()
            .map(|revision| (revision.base.id.clone(), offering_revision_fact(revision)))
            .collect(),
        ...
        suppliers: suppliers
            .into_iter()
            .map(|supplier| (supplier.base.id.clone(), supplier_role_fact(supplier)))
            .collect(),
        commercial_profiles: commercial_profiles
            .into_iter()
            .map(|revision| (revision.base.id.clone(), supplier_commercial_fact(revision)))
            .collect(),
        supplier_names,
        offerings: offerings.into_iter().map(offering_fact).collect(),
    })
}
