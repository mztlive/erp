...
pub async fn load_customer_acceptance_progress(
    ...
    let revision_lines = db
        ...
        .await?;
    let revision_line_ids: Vec<erp_core::ids::SalesOrderRevisionLineId> =
    ...
    let goods_service_lines = db
        ...
        .await?;
    let deliveries = db.fulfillment().list_acceptance_eligible_deliveries(sales_order_id, session).await?;
    let delivery_ids: Vec<erp_core::ids::DeliveryId> =
        deliveries.iter().map(|delivery| delivery.base.id.clone().into()).collect();
    let delivery_lines = db.fulfillment().delivery_lines_by_delivery_ids(&delivery_ids, session).await?;
    let sales_order_line_ids = so_line_ids(&revision_lines);
    let electronic =
        db.fulfillment().list_confirmed_electronic_deliveries(&sales_order_line_ids, session).await?;
    let service = db
        .fulfillment()
    ...
    let delivery_allocations = db
        ...
        .allocations_by_fulfillment_fact(
            ...
            &delivery_lines.iter().map(|line| line.base.id.clone()).collect::<Vec<_>>(),
            session,
        )
        .await?;
    let electronic_allocations = db
        ...
        .allocations_by_fulfillment_fact(
            ...
            &electronic.iter().map(|record| record.base.id.clone()).collect::<Vec<_>>(),
            session,
        )
        .await?;
    let service_allocations = db
        ...
        .allocations_by_fulfillment_fact(
            ...
            &service.iter().map(|record| record.base.id.clone()).collect::<Vec<_>>(),
            session,
        )
        .await?;
