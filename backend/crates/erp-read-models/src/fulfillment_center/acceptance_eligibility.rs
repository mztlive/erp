...
pub(crate) struct EligibilityGroupSources<'a> {
    /// 销售版本公共行。
    pub(crate) revision_lines: &'a [erp_sales::entity::sales_order::SalesOrderRevisionLine],
    /// 实物及服务行（数量/单位）。
    pub(crate) goods_service_lines:
        &'a [erp_sales::entity::sales_order::SalesOrderGoodsServiceLineRevision],
    /// 有效发货单。
    pub(crate) deliveries: &'a [Delivery],
    /// 发货行。
    pub(crate) delivery_lines: &'a [DeliveryLine],
    /// 已确认电子交付。
    pub(crate) electronic: &'a [ElectronicDelivery],
    /// 已确认服务履约。
    pub(crate) service: &'a [ServiceFulfillment],
    /// 发货事实的验收分配。
    pub(crate) delivery_allocations: &'a [AcceptanceFulfillmentAllocation],
    /// 电子交付事实的验收分配。
    pub(crate) electronic_allocations: &'a [AcceptanceFulfillmentAllocation],
    /// 服务履约事实的验收分配。
    pub(crate) service_allocations: &'a [AcceptanceFulfillmentAllocation],
}
...
pub(crate) fn build_line_eligibilities(
    ...
    use erp_fulfillment::entity::facts::{AcceptanceSalesLineFact, AcceptanceSalesQuantityFact};
    use erp_fulfillment::service::acceptance_eligibility::{self, EligibilitySources};
    let revision_lines = sources
        .revision_lines
        .iter()
        .map(|line| AcceptanceSalesLineFact {
            id: line.base.id.clone(),
            sales_order_line_id: line.sales_order_line_id.clone(),
        })
        .collect::<Vec<_>>();
    let goods_service_lines = sources
        .goods_service_lines
        .iter()
        .map(|line| AcceptanceSalesQuantityFact {
            revision_line_id: line.revision_line_id.clone(),
