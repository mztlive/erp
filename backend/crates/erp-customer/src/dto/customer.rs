...
pub struct CustomerSensitiveFieldView {
    /// 敏感字段类型。
    pub kind: super::party_snapshot::SensitiveFieldKind,
    /// 事实行 ID。
...
pub struct CustomerProfileDetailView {
    ...
    pub account: CustomerView,
    /// Party 状态。
    pub party_status: super::party_snapshot::PartyStatus,
    /// Party 乐观锁版本。
    pub party_version: u64,
    /// 统一社会信用代码。
    pub unified_credit_code: Option<String>,
    /// 当前名称修订。
    pub current_revision: super::party_snapshot::PartyRevisionView,
    /// 名称修订历史，按修订号倒序。
    pub revisions: Vec<super::party_snapshot::PartyRevisionView>,
    /// 归属历史。
    pub assignments: Vec<CustomerAssignmentView>,
    /// 当前有效联系人。
    pub contacts: Vec<super::party_snapshot::PartyContactView>,
    /// 当前有效地址。
    pub addresses: Vec<super::party_snapshot::PartyAddressView>,
    /// 当前有效税务资料。
    pub tax_profiles: Vec<super::party_snapshot::PartyTaxProfileView>,
    /// 当前有效银行账户摘要。
    pub bank_accounts: Vec<super::party_snapshot::PartyBankAccountView>,
    /// 敏感字段短时揭示入口。
