...
/// Work-item type snapshot for import confirmation views; wire code matches workflow.
#[derive(Debug, Clone, Copy, Serialize, PartialEq, Eq)]
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum ImportWorkItemType {
    /// 导入业务确认。
    ImportBusinessConfirmation,
}

/// Work-item status snapshot for import confirmation views; wire code matches workflow.
#[derive(Debug, Clone, Copy, Serialize, PartialEq, Eq)]
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum ImportWorkItemStatus {
    /// 当前仍需处理。
    Open,
    /// 已由强类型领域命令原子完成。
    Completed,
...
pub struct ImportBusinessConfirmationWorkItemView {
    ...
    /// 固定任务类型。
    pub work_item_type: ImportWorkItemType,
    /// 任务乐观锁版本。
    pub task_version: String,
    /// 任务冻结的主体版本。
    pub subject_version: String,
    /// 任务生命周期状态。
    pub status: ImportWorkItemStatus,
    /// 固定责任角色。
