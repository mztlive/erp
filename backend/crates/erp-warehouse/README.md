# erp-warehouse：仓库资料与 SKU 策略

管理仓库在哪里、处于什么状态、由谁处理履约，以及仓库对各 SKU 的处理策略和有效期。

仓库资料及策略是库存、收发货使用的基础配置，需要独立修订并保留历史身份。仓内实际数量由库存领域维护。

## 使用场景

- 修改仓库基础资料、启停和修订查询。
- 配置履约处理人，维护仓库 SKU 策略及有效期。

## 协作示例

更换某仓库的收发货处理人时修改本领域配置；查询该仓有多少货使用 [erp-inventory](../erp-inventory/README.md)，登记实际发货使用 [erp-fulfillment](../erp-fulfillment/README.md)。

## 负责的数据与能力

- 仓库稳定身份、资料修订和仓库 SKU 策略。
- 仓库履约处理人、有效期与处理职责相关规则。

## 依赖与协作边界

术语与统一分工见[总索引](../README.md#代码中常见名称的含义)。

- 本领域的数据结构、业务规则、服务和数据库仓储在此维护。常规、构建和测试依赖均不得指向其他业务领域或组合层。
- 库存余额、流水、预占和调整属于 erp-inventory。
- 处理人身份、附件指纹和审计使用本域 Port；仓库资料修订不得改写已冻结业务快照。
- 需要其他领域的能力时，由组合层或应用将本域接口接到实际提供方；公开入口见 [src/lib.rs](src/lib.rs)。

## 代码入口

| 入口 | 用途 |
| --- | --- |
| [src/service/warehouse/mod.rs](src/service/warehouse/mod.rs) | WarehouseService |
| [src/ports/mod.rs](src/ports/mod.rs) | 处理人身份、指纹和审计合同 |
| [src/entity/mod.rs](src/entity/mod.rs) | 本域实体、值对象和确定性规则 |
| [src/repository/mod.rs](src/repository/mod.rs) | 本域 MongoDB 仓储与集合访问器 |
| [src/indexes/mod.rs](src/indexes/mod.rs) | 公开索引注册入口 |

## 修改执行要求

1. 将无 I/O 的校验、状态迁移和不变式放入本域实体或值对象；Service 组织本域用例。
2. Repository 使用调用方传入的 `persistence_core::Executor`；跨集合原子写入由本域用例或 Process 控制事务。
3. 新增或调整集合查询时同步评估索引；组合根复用本域公开索引入口，保持既有逐集合注册顺序。
4. HTTP 请求和响应优先复用本域 DTO；扩展公开合同须同步检查 Process、ReadModel 和应用调用方。
5. 业务改动补充本域库单元测试，覆盖成功、失败、边界及相关幂等或版本冲突路径。

## 验证要求

以下命令在 `backend/` 目录执行：

```bash
cargo check -p erp-warehouse --locked
env -u ERP_TEST_MONGO_URI cargo test -p erp-warehouse --lib --locked
```

代码变更还须执行[统一质量门禁](../README.md#质量门禁)。测试范围限定为库单元测试，不执行集成测试或真实外部服务测试。
