...
//! `crate::entity::receivable::card_funds_review_decision`；本文件仅负责 DTO 枚举到实体
...
use crate::entity::receivable::{EvidenceAssetError, ReviewEvidenceAssetFact};

use crate::dto::receivable::{
...
pub fn validate_evidence_assets(
    ...
    validated
        ...
        .map_err(|error| match error {
            EvidenceAssetError::Missing(_) => Error::NotFound(error.to_string()),
            EvidenceAssetError::Unusable => Error::BusinessLogicError(error.to_string()),
        })
}
