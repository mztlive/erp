...
pub async fn settle_customer_receipt(
    ...
    let entries = db
        .receivable_entries()
        .find_entries_by_ids(&entry_ids, executor)
        .await?;
    ...
    let settlement = db
        .receivable_accounts()
        .apply_settlements_many(&settlement_deltas, actor_id, executor)
        .await?;
    ...
    db.receivable()
        .create_receipt_allocations_many(ledger.new_allocations(), executor)
        .await?;
    Ok(sales_order_ids)
}
