...
/// # 错误
/// 无。
pub fn missing_order_id(requested: &[SalesOrderId], existing: &[SalesOrderId]) -> Option<SalesOrderId> {
...
/// 财务成本准备结果；构建完成后才能开始包含审计的根事务。
pub struct PreparedCostEntry {
    /// 已校验的成本事实。
    pub entry: CostEntry,
    /// 已校验的成本分配行，保持输入顺序。
