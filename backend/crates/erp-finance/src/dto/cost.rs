...
use erp_core::ids::{CostEntryId, FileAssetId, SalesOrderId, SalesOrderLineId, SupplierAccountId};
use erp_core::money::{Amount, Rate};
use serde::{Deserialize, Serialize};
use validator::Validate;

use crate::Result;
use crate::entity::cost::{CostBasis, CostScope, CostStage, CostType};
...
impl CostEntryListParams {
    ...
    pub fn normalized(&self) -> Result<CostEntryListQuery> {
...
/// 归一化后的成本分配列表查询参数。
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CostAllocationListQuery {
    /// 成本事实筛选。
    pub cost_entry_id: Option<CostEntryId>,
    /// 经营归属销售单筛选。
    pub sales_order_id: Option<SalesOrderId>,
...
impl CostAllocationListParams {
    ...
    /// # 错误
    /// 排序字段不在白名单或排序方向非法时返回 `ValidationError`。
    pub fn normalized(&self) -> Result<CostAllocationListQuery> {
...
mod tests {
    ...
    use super::{CostAllocationListParams, CostEntryListParams, SortDir, normalize_sort};
    use crate::entity::cost::CostStage;
