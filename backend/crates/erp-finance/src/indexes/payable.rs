...
pub async fn ensure(db: &Database) -> Result<()> {
    ...
    create_indexes(db, PURCHASE_INVOICE_ALLOCATIONS, purchase_invoice_allocation_indexes()).await?;
    Ok(())
}
...
fn payable_account_indexes() -> Vec<IndexModel> {
    vec![
        unique_index("uk_payable_accounts_source", doc! { "source_type": 1, "source_document_id": 1 }),
        named_index("idx_payable_accounts_aging", doc! { "supplier_id": 1, "status": 1 }),
    ]
}
...
fn payable_entry_indexes() -> Vec<IndexModel> {
    vec![
        ...
        named_index("idx_payable_entries_account_due", doc! { "payable_account_id": 1, "due_date": 1 }),
        named_index(
            "idx_payable_entries_account_direction_due",
            doc! { "payable_account_id": 1, "direction": 1, "due_date": 1, "id": 1 },
        ),
        named_index("idx_payable_entries_source", doc! { "source_fact_type": 1, "source_document_id": 1 }),
    ]
}
...
fn payable_entry_offset_indexes() -> Vec<IndexModel> {
    vec![
        ...
        named_index("idx_payable_entry_offsets_increase", doc! { "increase_entry_id": 1 }),
    ]
}
...
fn payment_allocation_indexes() -> Vec<IndexModel> {
    vec![
        ...
        named_index("idx_payment_allocations_entry_time", doc! { "payable_entry_id": 1, "allocated_at": 1 }),
        named_index("idx_payment_allocations_reverse", doc! { "reverses_allocation_id": 1 }),
    ]
}
...
fn purchase_invoice_allocation_indexes() -> Vec<IndexModel> {
    vec![
        ...
        named_index("idx_purchase_invoice_allocations_account", doc! { "payable_account_id": 1 }),
        named_index(
        ...
        named_index("idx_purchase_invoice_allocations_reverse", doc! { "reverses_allocation_id": 1 }),
    ]
}
...
mod tests {
    ...
    fn payable_account_source_identity_is_unique() {
        ...
        let identity =
            indexes.iter().find(|index| name(index) == Some("uk_payable_accounts_source")).unwrap();
        assert_eq!(identity.keys, doc! { "source_type": 1, "source_document_id": 1 });
        assert_eq!(identity.options.as_ref().unwrap().unique, Some(true));
        assert!(indexes.iter().any(|index| name(index) == Some("idx_payable_accounts_aging")));
    }
    ...
    fn payable_entry_identity_covers_the_full_business_key() {
        ...
        let identity =
            indexes.iter().find(|index| name(index) == Some("uk_payable_entries_identity")).unwrap();
        assert_eq!(
            identity.keys,
            doc! {
                "payable_account_id": 1,
    ...
    fn payment_allocations_cover_unique_and_bidirectional_lookups() {
        ...
        assert!(
            supplier_payment_indexes().iter().any(|index| name(index) == Some("uk_supplier_payments_no"))
        );
        assert!(
            payable_entry_offset_indexes().iter().any(|index| index.keys == doc! { "increase_entry_id": 1 })
        );
    }
}
