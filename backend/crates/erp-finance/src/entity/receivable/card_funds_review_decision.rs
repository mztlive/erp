...
use super::ReviewEvidenceAssetFact;
...
/// Evidence lookup failures retain their application error category without parsing text.
#[derive(Debug, Clone, PartialEq, Eq, thiserror::Error)]
pub enum EvidenceAssetError {
    /// The first required evidence identity absent from the loaded facts.
    #[error("复核证据文件不存在: {0}")]
    Missing(String),
    /// The first evidence asset rejected by the provider's point-in-time usability rule.
    #[error("复核证据文件未通过安全检查、已销毁或已过期")]
    Unusable,
}
...
impl CardFundsReviewEvidence {
    ...
    ///
    /// # 约束
    /// 纯内存判断；组合层通过 `FileAsset::is_usable_at` 提供权威可用性；不执行 I/O。
    pub fn validate_assets(
        &self,
        assets: &[ReviewEvidenceAssetFact],
    ) -> std::result::Result<(), EvidenceAssetError> {
        let assets_by_id: HashMap<&str, &ReviewEvidenceAssetFact> =
            assets.iter().map(|asset| (asset.id.as_ref(), asset)).collect();
        for id in &self.document_ids {
            let asset = assets_by_id
                .get(id.as_ref())
                .ok_or_else(|| EvidenceAssetError::Missing(id.to_string()))?;
            if !asset.usable {
                return Err(EvidenceAssetError::Unusable);
            }
        }
        Ok(())
    }
...
mod tests {
    ...
    fn evidence_asset(id: &str, usable: bool) -> ReviewEvidenceAssetFact {
        ReviewEvidenceAssetFact {
            id: FileAssetId::new(id),
            usable,
        }
    }
    ...
    /// scan 状态、destroyed、expired 通过提供方 FileAsset 可用性判断后返回事实。
    #[test]
    fn scan_destroyed_expired() {
        ...
        for (case, usable) in [
            ...
            let result = evidence.validate_assets(&[evidence_asset("file-1", usable)]);
            assert_eq!(result.is_ok(), usable, "{case}");
            if let Err(error) = result {
                assert_eq!(error, super::EvidenceAssetError::Unusable);
                assert_eq!(error.to_string(), "复核证据文件未通过安全检查、已销毁或已过期");
            }
        }
    }
    ...
    fn first_error_missing_before_scan() {
        ...
        let err = evidence.validate_assets(&assets).unwrap_err();
        assert_eq!(
            err,
            super::EvidenceAssetError::Missing("file-missing".to_string())
        );
