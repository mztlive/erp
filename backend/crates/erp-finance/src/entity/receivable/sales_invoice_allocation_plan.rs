...
//!
//! 与 [`SalesInvoiceAllocation`](crate::entity::receivable::SalesInvoiceAllocation) 实体的
...
mod tests {
    ...
    fn ids(count: usize) -> Vec<SalesInvoiceAllocationId> {
        (0..count).map(|index| SalesInvoiceAllocationId::new(format!("alloc-{index}"))).collect()
    }
    ...
    fn same_account_lines_aggregate_once_with_conservation() {
        ...
        assert_eq!(
            ...
            &[
                (ReceivableAccountId::new("acct-1"), Amount::from_str("70.00").unwrap()),
                (ReceivableAccountId::new("acct-2"), Amount::from_str("30.00").unwrap()),
            ],
            "同一账户只推进聚合值且按首次出现顺序"
        );
