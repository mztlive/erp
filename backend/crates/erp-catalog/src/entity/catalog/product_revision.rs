...
impl ProductRevision {
    ...
    ///
    /// # 参数
    /// * `id` - 实体主键（`erp_core::ids::ProductRevisionId`）
    /// * `data` - 创建数据
    ///
    /// # 返回
    /// 返回新建的商品修订实体。
...
mod tests {
    ...
    fn new_rejects_zero_revision_no_and_reversed_window() {
        ...
        assert_eq!(
            ProductRevision::new(ProductRevisionId::new("rev-1"), zero_revision).unwrap_err().to_string(),
            "修订序号必须从 1 开始"
        );
        ...
        assert_eq!(
            ProductRevision::new(ProductRevisionId::new("rev-1"), reversed).unwrap_err().to_string(),
            "生效结束日必须晚于生效开始日"
        );
        ...
        assert_eq!(
            ProductRevision::new(ProductRevisionId::new("rev-1"), equal_window).unwrap_err().to_string(),
            "生效结束日必须晚于生效开始日"
        );
    }
