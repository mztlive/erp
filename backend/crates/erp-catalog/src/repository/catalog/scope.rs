//! 商品当前维护责任范围查询；输入必须由应用层完成资源动作授权。

use mongodb::bson::{Document, doc};

/// 一个已经通过同角色权限证明的商品责任范围。
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct CatalogScopeClause {
    /// 公司范围覆盖该资源动作的全部商品。
    pub company: bool,
    /// 当前维护人为该账号的商品。
...
pub struct CatalogReadScope {
    /// 同角色正向范围。
    pub roles: Vec<CatalogScopeClause>,
    /// 用户个人上限；缺省表示不附加个人限制。
    pub user_limit: Option<CatalogScopeClause>,
}

impl CatalogScopeClause {
...
impl CatalogReadScope {
    ...
    pub fn is_empty(&self) -> bool {
        self.roles.iter().all(CatalogScopeClause::is_empty)
            || self.user_limit.as_ref().is_some_and(CatalogScopeClause::is_empty)
    }

    /// 生成角色并集与个人上限交集；商品不允许历史参与。
    ///
    /// # 参数
    /// 无。
    ///
    ...
    pub fn document(&self) -> Document {
        let grants = self.roles.iter().map(CatalogScopeClause::document).collect::<Vec<_>>();
    ...
    /// 按与 [`CatalogReadScope::document`] 相同的责任事实判断单对象。
    ///
    /// # 参数
    /// * `owner` - 对象显式维护人
    /// * `org` - 对象业务组织
...
mod tests {
    ...
    fn empty_scope_is_false_document_and_rejects_objects() {
        let scope = CatalogReadScope::default();
    ...
    fn company_and_self_owned_keep_union_and_user_limit_intersection() {
        let scope = CatalogReadScope {
            roles: vec![
                CatalogScopeClause { company: true, ..CatalogScopeClause::default() },
                CatalogScopeClause { owner_user_id: Some("user-1".into()), ..CatalogScopeClause::default() },
            ],
            user_limit: Some(CatalogScopeClause {
                business_org_unit_ids: vec!["org-a".into()],
                ..CatalogScopeClause::default()
            }),
        };
