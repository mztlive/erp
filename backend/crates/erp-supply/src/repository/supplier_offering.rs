...
mod tests {
    ...
    #[test]
    fn code_candidates_preserve_original_exact_sku_field_overwrite() {
        for candidates in [vec![erp_core::ids::SkuId::new("candidate")], vec![]] {
            let filter = SupplierOfferingFilter {
                offering_ids: None,
                sku_id: Some(erp_core::ids::SkuId::new("exact")),
                supplier_id: None,
                status: None,
                source_type: None,
            ...
            .to_doc();
            assert_eq!(
                filter.get_document("sku_id").unwrap(),
                &doc! {"$in": candidates.iter().map(ToString::to_string).collect::<Vec<_>>()}
            );
    ...
    fn offering_filter_combines_source_and_candidate_ids() {
        let filter = SupplierOfferingFilter {
            ...
            supplier_id: None,
            status: None,
            source_type: Some(crate::entity::supplier_offering::OfferingSourceType::Excel),
            supplier_sku_code: None,
            keyword_sku_ids: None,
            sku_ids: None,
            page: 1,
    ...
    fn rows_without_current_pointer_yield_no_revision_entry() {
        let rows = [
            super::SupplierOfferingRow {
                ...
                source_type: crate::entity::supplier_offering::OfferingSourceType::Manual,
                source_connection_id: None,
                status: crate::entity::supplier_offering::OfferingStatus::Active,
