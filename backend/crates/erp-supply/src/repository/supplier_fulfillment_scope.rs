//! 供应商履约订单当前跟进责任范围查询；输入必须由应用层完成资源动作授权。

use mongodb::bson::{Document, doc};

/// 一个已经通过同角色权限证明的履约订单责任范围。
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct FulfillmentOrderScopeClause {
    /// 公司范围覆盖该资源动作的全部订单。
    pub company: bool,
    /// 当前跟进人为该账号的订单。
...
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct FulfillmentOrderReadScope {
    /// 同角色正向范围。
    pub roles: Vec<FulfillmentOrderScopeClause>,
    /// 用户个人上限；缺省表示不附加个人限制。
    pub user_limit: Option<FulfillmentOrderScopeClause>,
}

impl FulfillmentOrderScopeClause {
    ...
    fn document(&self) -> Document {
        ...
        if let Some(user) = &self.owner_user_id {
            conditions.push(doc! { "follow_up_user_id": user });
        }
...
impl FulfillmentOrderReadScope {
    ...
    pub fn is_empty(&self) -> bool {
        self.roles.iter().all(FulfillmentOrderScopeClause::is_empty)
            || self.user_limit.as_ref().is_some_and(FulfillmentOrderScopeClause::is_empty)
    }

    /// 生成角色并集与个人上限交集；本资源不按历史参与扩大写权。
    ///
    /// # 参数
    /// 无。
    ///
    ...
    pub fn document(&self) -> Document {
        let grants = self.roles.iter().map(FulfillmentOrderScopeClause::document).collect::<Vec<_>>();
    ...
    /// 按与 [`FulfillmentOrderReadScope::document`] 相同的责任事实判断单对象。
    ///
    /// # 参数
    /// * `owner` - 对象显式跟进人
    /// * `org` - 对象业务组织
...
mod tests {
    ...
    fn empty_scope_is_false_document_and_rejects_objects() {
        let scope = FulfillmentOrderReadScope::default();
    ...
    fn company_and_self_owned_keep_union_and_user_limit_intersection() {
        let scope = FulfillmentOrderReadScope {
            roles: vec![
                FulfillmentOrderScopeClause { company: true, ..FulfillmentOrderScopeClause::default() },
                FulfillmentOrderScopeClause {
                    owner_user_id: Some("user-1".into()),
                    ..FulfillmentOrderScopeClause::default()
                },
            ],
            user_limit: Some(FulfillmentOrderScopeClause {
        ...
        assert_eq!(
            ...
            doc! {
                "$and": [
                    { "$or": [ {}, { "$or": [ { "follow_up_user_id": "user-1" } ] } ] },
    ...
    #[test]
    fn handler_is_not_compiled_into_owner_scope() {
        let scope = FulfillmentOrderReadScope {
            roles: vec![FulfillmentOrderScopeClause {
                owner_user_id: Some("follow-1".into()),
                ..FulfillmentOrderScopeClause::default()
            }],
            user_limit: None,
        };
        assert!(scope.allows_object("follow-1", "org-a"));
        assert!(!scope.allows_object("handler-1", "org-a"));
