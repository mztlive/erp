...
impl PaymentTermSnapshot {
    ...
    /// * `resolve_term` - 供应商提供方的历史快照解析回调，在代码规范化后、门禁校验前调用
    ///
    /// # 返回
    /// 返回快照实例。
    ///
    ...
    pub fn new(
        ...
        let payment_term = resolve_term(&payment_term_code)?;
    ...
    /// * `resolve_term` - 供应商提供方的历史快照解析回调，先于交付日期校验调用
    ///
    /// # 返回
    /// 返回由冻结付款条件确定的计划付款日。
    ///
...
mod tests {
    ...
    fn payment_term_computes_prepay_and_postpay_due_dates() {
        ...
        let prepay = PaymentTermSnapshot::new(
            "PREPAY_30".to_string(),
            true,
            None,
            None,
        ...
        .unwrap();
        let cash = PaymentTermSnapshot::new(
        ...
        .unwrap();
        let postpay = PaymentTermSnapshot::new(
            "POSTPAY_NET15".to_string(),
            false,
            None,
            None,
            crate::entity::test_support::payment_term,
