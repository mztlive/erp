...
mod status;
pub use status::{ProgressStatus, PurchaseOrderStatus, PurchaseReviewStatus};
...
impl PurchaseOrder {
    ...
    /// * `resolve_term` - 供应商提供方的纯解析回调，在代码规范化后、依据校验前调用一次
    ///
    /// # 返回
    /// 返回新建的采购单实体。
    ///
    ...
    pub fn new(
        ...
        let payment_term_code = resolve_term(&payment_term_code)?.canonical_code;
    ...
    /// * `update` - 更新数据
    /// * `updated_by` - 本次更新执行人
    /// * `resolve_term` - 供应商提供方的纯解析回调，仅更新付款条件时在代码规范化后调用
    ///
    /// # 返回
    /// 更新成功返回 `Ok(())`。
    ///
    ...
    pub fn update(
        ...
        self.ensure_draft()?;
        self.apply_payment_term(update.payment_term_code, resolve_term)?;
    ...
    fn apply_payment_term(
        ...
        if let Some(payment_term_code) = payment_term_code {
            ...
            self.payment_term_code = resolve_term(&payment_term_code)?.canonical_code;
        }
        Ok(())
    }
}
...
mod tests {
    ...
    /// 解析提供方只在原付款条件校验位置运行，不能提前到采购字段校验前。
    #[test]
    fn payment_resolver_preserves_creation_validation_order() {
        use std::cell::Cell;
        let called = Cell::new(false);
        let mut data = order_data();
        data.purchase_no = "p".repeat(65);
        let result = PurchaseOrder::new(PurchaseOrderId::new("po-parse"), data, "admin-1", |_| {
            called.set(true);
            Err(erp_core::Error::from("provider failure"))
        });
        assert!(result.is_err());
        assert!(!called.get());

        let mut data = order_data();
        data.creation_basis_id.clear();
        let result = PurchaseOrder::new(PurchaseOrderId::new("po-parse"), data, "admin-1", |code| {
            called.set(true);
            assert_eq!(code, "NET-30");
            Err(erp_core::Error::from("provider failure"))
        });
        assert_eq!(result.unwrap_err().to_string(), "provider failure");
        assert!(called.get());
    }

    /// 未修改付款条件时不访问解析提供方；非草稿状态仍先拒绝更新。
    #[test]
    fn payment_resolver_skips_unchanged_term_and_non_draft_update() {
        let mut order = new_order();
        order
            .update(PurchaseOrderUpdate::default(), "admin-1", |_| {
                panic!("unchanged payment term must not invoke resolver")
            })
            .unwrap();
