//! 采购范围授权 Port：只接收已解析事实，不暴露身份域实体或 Service。
...
/// 身份域已解析的采购正向范围条款；不含身份域实体句柄。
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct PurchaseResolvedClause {
    /// 公司范围覆盖该资源动作的全部采购单。
    pub company: bool,
    /// 当前采购负责人为操作人的单据。
    pub self_owned: bool,
    /// 协作维度已由公共解析器保留；采购对象不按协作客户展开。
    pub collaborative: bool,
    /// 已由公共解析器展开的内部组织。
    pub org_unit_ids: Vec<String>,
}

impl PurchaseResolvedClause {
    /// 判断条款是否构成有效采购范围规则。
    ///
    /// # 参数
    /// 无。
    ...
    ///
    /// # 关键业务约束
    /// 协作维度单独存在不构成采购对象规则；空条款保持空集且不得补公司。
...
pub struct PurchaseResolvedScope {
    /// 已认证操作人。
    pub user_id: String,
    /// 固定为采购单资源。
    pub resource: String,
    ...
    pub action: String,
    /// 同角色正向范围。
    pub role_clauses: Vec<PurchaseResolvedClause>,
    /// 用户个人上限；缺省表示不附加个人限制。
    pub user_limit: Option<PurchaseResolvedClause>,
    /// RBAC 策略版本。
...
pub trait PurchaseDataScopePort: Send + Sync {
    ...
    ///
    /// # 关键业务约束
    /// 资源由本 Port 固定为采购单；不得把原始 `scope_type / scope_targets` 交给采购域再解析。
    async fn resolve(
        &self,
        actor: &AuditActor,
        action: &str,
        executor: &mut dyn Executor,
    ) -> Result<PurchaseResolvedScope>;

    /// 在调用方事务内证明同角色完整权限集合，范围仍绑定目标采购动作。
    ///
    /// # 参数
    /// * `actor` - 服务端已认证身份
    /// * `action` - 采购资源已注册动作
    ...
    ///
    /// # 错误
    /// 未装配、权限码非法、无动作权限或版本变化必须明确失败。
    ///
    /// # 关键业务约束
    /// 解析器必须包含目标动作自身；额外权限不得改写资源标识。
...
pub struct FailClosedPurchaseDataScopePort;

impl FailClosedPurchaseDataScopePort {
    /// 返回未接线的共享端口。
    ///
    /// # 参数
    /// 无。
    ...
    pub fn shared() -> Arc<dyn PurchaseDataScopePort> {
        Arc::new(Self)
    }
}
