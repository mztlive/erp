...
pub struct StockAdjustmentSnapshotFact {
    /// 调整单号。
...
impl StockAdjustmentApprovalSnapshot {
    ...
    ///
    /// # 返回
    /// 返回本域冻结快照事实；工作流 payload 由组合层 adapter 映射。
    ///
    /// # 错误
    ...
    pub fn build(
    ...
    ) -> Result<StockAdjustmentSnapshotFact> {
        if lines.is_empty() {
        ...
        Ok(StockAdjustmentSnapshotFact {
            ...
            total_quantity: sum_line_quantity(lines)?,
...
mod tests {
    ...
    fn snapshot_freezes_org_counterparty_and_totals() {
        ...
        assert_eq!(payload.total_quantity.to_string(), "3");
        assert_eq!(payload.line_count, 2);
        assert_eq!(payload.submitted_by, "user-1");
        assert_eq!(payload.warehouse_id.to_string(), "wh-1");
    }
