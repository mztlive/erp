...
use crate::dto::contract::{
    ContractListParams, ContractListQuery, ContractRevisionView, ContractView, PageView, SortDir,
};
...
impl ContractService {
    ...
    pub async fn contract_list(
        ...
        let owners_by_customer = self
            .assignments
            .owner_user_ids_by_customer(&customer_id_strings, BusinessDate::today())
            .await?;
        let owner_ids = owners_by_customer.values().cloned().collect::<Vec<_>>();
        let owner_names = self.accounts.names_by_ids(&owner_ids).await?;
