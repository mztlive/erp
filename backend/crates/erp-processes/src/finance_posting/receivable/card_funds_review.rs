...
use erp_finance::entity::receivable::{
    ...
    ReceivableAccount, ReceivableFundsReview, ReceivableFundsReviewChain, ReceivableFundsReviewData,
    ReviewResult, CARD_FUNDS_REVIEW_ACTION,
};
use erp_finance::repository::ReceivableExt;
use erp_read_models::finance::receivable::separation::validate_card_funds_reviewer_separation;
            ...
            .with_transaction(move |session| {
                Box::pin(async move {
                    ...
                    let review_type = entity_review_type(match validated_for_tx.review_type() {
                        erp_finance::entity::receivable::EntityCardFundsReviewType::Opening => {
                            CardFundsReviewType::Opening
                        }
                        erp_finance::entity::receivable::EntityCardFundsReviewType::SyncDelta => {
                            CardFundsReviewType::SyncDelta
                        }
                    });
                    let review_result = entity_review_result(match validated_for_tx.review_result() {
                        erp_finance::entity::receivable::EntityCardFundsReviewResult::Approved => {
                            CardFundsReviewResult::Approved
                        }
                        erp_finance::entity::receivable::EntityCardFundsReviewResult::Rejected => {
                    ...
                    let cache_update = match validated_for_tx.review_result() {
                        erp_finance::entity::receivable::EntityCardFundsReviewResult::Approved => {
                            erp_finance::entity::receivable::ReceivableAccountUpdate {
...
fn entity_review_type(review_type: CardFundsReviewType) -> erp_finance::entity::receivable::FundsReviewType {
    match review_type {
        CardFundsReviewType::Opening => erp_finance::entity::receivable::FundsReviewType::Opening,
        CardFundsReviewType::SyncDelta => erp_finance::entity::receivable::FundsReviewType::SyncDelta,
    }
}
...
mod card_funds_review_tests {
    ...
    use erp_finance::entity::receivable::CardFundsCommandReceipt;
    ...
    fn evidence_usability_uses_file_asset_point_in_time_check() {
        ...
        assert!(validate_evidence_assets(&validated, &[evidence_asset("file-1", true)], now).is_ok());
        let pending_error =
            validate_evidence_assets(&validated, &[evidence_asset("file-1", false)], now).unwrap_err();
        assert!(matches!(pending_error, Error::BusinessLogicError(message)
            if message == "复核证据文件未通过安全检查、已销毁或已过期"));
    ...
    fn old_helpers_are_deleted_and_new_vo_is_unique_entry() {
        let source = concat!(
            include_str!("mod.rs"),
            include_str!("card_funds_decision.rs"),
            include_str!("../../../../erp-finance/src/service/receivable/mapping.rs"),
            include_str!("../../../../erp-finance/src/service/receivable/customer_receipt_posting.rs"),
            include_str!("../../../../erp-finance/src/service/receivable/card_funds_register.rs"),
            include_str!("../../../../erp-read-models/src/finance/receivable/snapshot.rs"),
