...
use erp_workflow::entity::work_item::{WorkItem, matches_sales_invoice_identity};
...
pub(crate) async fn record_invoice_execution(
    ...
    let InvoiceExecutionInput { work_item_id, expected_task_version, party_id, account_ids, invoice_amount } =
        input;
    let mut task = db
        .work_items()
    ...
    if task.base.version != expected_task_version {
        return Err(Error::ConflictError("开票任务版本已变化，请刷新工作台任务后重试".to_string()));
    }
    let account = db
        .receivable_accounts()
    ...
    if !task.is_owned_by(actor.id()) {
        return Err(Error::Forbidden("当前账号不是开放开票任务的当前责任人".to_string()));
    }
    ...
    if &account.counterparty_party_id != party_id {
        return Err(Error::BusinessLogicError("发票往来主体与当前任务的应收子账不一致".to_string()));
    }
    if account_ids.iter().any(|account_id| account_id.as_ref() != task.business_object_id) {
        return Err(Error::BusinessLogicError("一次销项开票只能分配到当前任务绑定的应收子账".to_string()));
    }
    ...
    .await?;
    task.record_activity(actor.id(), Instant::now()).map_err(Error::Logic)?;
...
fn ensure_task_identity(task: &WorkItem, account: &ReceivableAccount) -> Result<()> {
    ...
    Err(Error::BusinessLogicError("销项开票任务责任身份与应收子账不一致，请联系管理员修复后重试".to_string()))
}
