//! `PurchaseChangeOrder` 审批业务 Adapter。
//!
...
use erp_procurement::entity::purchase_order::{
    PurchaseChangeOrder, PurchaseChangeSubmission, PurchaseChangeSubmissionLine,
};
#[cfg(test)]
use erp_procurement::service::purchase_order::change::state::start_purchase_change_approval;
use erp_procurement::service::purchase_order::change::state::{
    cancel_purchase_change_to_draft, ensure_final_approve_effective,
};
...
pub struct PurchaseChangeOrderAdapter {
    /// 单据类型。
...
pub fn purchase_change_order_adapter() -> Result<PurchaseChangeOrderAdapter> {
    let spec = adapter_spec_of(DocumentType::PurchaseChangeOrder)?;
...
/// # 错误
/// 字段与合同签署值不一致时返回错误。
fn adapter_from_spec(spec: ApprovalAdapterSpec) -> Result<PurchaseChangeOrderAdapter> {
    if spec.document_type != DocumentType::PurchaseChangeOrder
        || spec.process_kind != process_kind_of(DocumentType::PurchaseChangeOrder)
        || spec.subject_version_source != ApprovalSubjectVersionSource::EntityApprovalSubjectVersion
        || spec.on_approval_start != ApprovalDomainAction::PurchaseChangeOrderSubmitChange
        || spec.on_final_approve != ApprovalDomainAction::PurchaseChangeOrderApplyEffectiveChange
        || spec.cancel_action != ApprovalDomainAction::PurchaseChangeOrderCancelApproval
    ...
    Ok(PurchaseChangeOrderAdapter {
        document_type: spec.document_type,
        process_kind: spec.process_kind,
        subject_ref_builder: "subject_ref_for(PurchaseChangeOrder)",
        subject_version_source: spec.subject_version_source,
        subject_snapshot_builder: "build_purchase_change_snapshot",
...
/// # 错误
/// 主键为空或超长时返回校验错误。
pub fn purchase_change_order_subject_ref(business_object_id: &str) -> Result<SubjectRef> {
...
pub fn require_frozen_binding(
    ...
    binding.ok_or_else(|| Error::ConflictError("无有效审批绑定的采购变更单不得提交".to_string()))
}

/// 采购变更单调用统一 `start_approval` 的目标命令。
///
/// 字段与合同 §14.2 对齐；不得包含定义 ID 或审批人。
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize)]
pub struct PurchaseChangeOrderStartCommand {
    /// 业务对象种类。
...
pub fn purchase_change_start_command(
    change_order_id: &str,
    subject_version: u32,
    actor_id: &str,
    idempotency_key: &str,
...
pub fn start_approval_command_kind(
    _command: &PurchaseChangeOrderStartCommand,
...
/// # 错误
/// 动作不属于本类型或状态不允许时返回错误。
pub fn execute_purchase_change_domain_action(
...
/// 仓储影响确认与财务复核不得充当审批流程节点。
///
/// 供适配器测试与提交路径证明旧复核旁路已切断。
///
/// # 错误
/// 恒返回冲突。
#[cfg(test)]
...
/// # 错误
/// 组织或审批人为空时返回校验错误。
pub fn purchase_change_order_object_readable(organization_id: &str, assignee_user_id: &str) -> Result<bool> {
...
/// 按合同 §4.4.5 冻结采购变更单快照。
///
/// 对手方为供应商；金额与数量合计必填。`document_no` 取变更单主键。
///
/// # 参数
/// * `change_order` - 变更单
/// * `sales_order` - 来源销售单
...
pub fn build_purchase_change_snapshot(
    ...
    Ok(ApprovalSubjectSnapshotPayload {
        ...
        responsible_org_id: purchase_change_responsible_org_id(sales_order)?,
        submitted_by: submitted_by.to_string(),
        submitted_at,
        counterparty: Some(ApprovalSubjectCounterparty::Supplier {
            supplier_id: SupplierAccountId::new(submission.supplier_id.to_string()),
        }),
        total_amount: Some(submission.gross_amount),
        total_quantity: Some(sum_line_quantity(lines)?),
        line_count: u32::try_from(lines.len())
            .map_err(|_| Error::ValidationError("采购变更明细行数溢出".to_string()))?,
    })
}
...
mod tests {
        ...
        PurchaseChangeSubmission::new(
            ...
            PurchaseChangeSubmissionData {
                ...
                payment_term_snapshot: PaymentTermSnapshot::new(
                    ...
                    None,
                    None,
                    crate::procure_to_pay::adapters::payment_term::parse_snapshot,
                )
                .expect("付款条件合法"),
    ...
    fn one_line() -> PurchaseChangeSubmissionLine {
    ...
    fn adapter_declares_all_required_fields() {
        let adapter = purchase_change_order_adapter().expect("采购变更单必须可登记");
    ...
    fn start_command_omits_definition_and_assignee() {
        let command = purchase_change_start_command("pco-1", 1, "user-1", "key-1");
        ...
        assert_eq!(command.subject_kind, "purchase_change_order");
    ...
    /// 快照冻结供应商对手方、变更单号、金额与数量合计。
    #[test]
    fn snapshot_freezes_supplier_amount_and_quantity() {
        let payload = build_purchase_change_snapshot(
            &draft_order(),
            &sales_order(),
            &submission(),
            &[one_line()],
        ...
        .unwrap();
        assert_eq!(payload.document_no, "pco-1");
    ...
    fn object_read_fails_closed_on_empty_identity() {
        assert!(purchase_change_order_object_readable("party-1", "u1").unwrap());
        assert!(purchase_change_order_object_readable(" ", "u1").is_err());
        assert!(purchase_change_order_object_readable("party-1", "").is_err());
    }
