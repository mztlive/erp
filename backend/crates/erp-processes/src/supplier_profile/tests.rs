...
use erp_supplier::{
    SaveSupplierProfileRequest, SupplierProfileCommand, SupplierProfileCommandData, command_view,
};
...
fn command_replay_is_bound_to_supplier_and_fingerprint_stable() {
    ...
    let req1 = SaveSupplierProfileRequest {
        ...
        settlement_mode: erp_supplier::SettlementMode::Prepayment,
        reconciliation_cycle: erp_supplier::ReconciliationCycle::Monthly,
        payment_term_snapshot: "PREPAY_30".to_string(),
        business_category: None,
        invoice_type: erp_supplier::InvoiceType::VatSpecial,
    ...
    .unwrap();
    assert!(cmd_bare.ensure_replayable("update", Some("supplier-1"), &fp1).is_ok());
