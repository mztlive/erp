...
fn goods_service_submit_does_not_create_procurement_confirmation() {
    let source = concat!(
        ...
        include_str!("void.rs"),
        include_str!("../../../../erp-sales/src/service/sales_order/sellable.rs"),
    );
    let submit = source
...
fn voucher_create_binds_and_submit_starts_unified_approval() {
    ...
    assert!(source.contains("crate::order_to_cash::document_type_of_sales_business"));
    let submit = source
        .split("pub async fn submit_sales_order")
        .nth(1)
        .and_then(|body| body.split("pub async fn cancel_approval_submission").next())
    ...
    let create = source
        ...
        .expect("绑定命令");
    assert!(create.contains("crate::order_to_cash::document_type_of_sales_business"));
