...
/// 销售单启动收据回放入参。
pub(super) struct ReplaySalesOrderStartInput<'a> {
    /// 销售单据类型。
    pub(super) document_type: DocumentType,
    /// 审批主题引用。
    pub(super) subject: &'a SubjectRef,
    /// 主题版本。
    pub(super) subject_version: u32,
    /// 启动幂等键。
    pub(super) idempotency_key: &'a str,
    /// 冻结的审批定义绑定。
    pub(super) binding: &'a ApprovalDefinitionBinding,
    /// 启动人账号 ID。
...
pub(super) async fn replay_sales_order_start_with_executor(
    db: &Database,
    input: ReplaySalesOrderStartInput<'_>,
    executor: &mut dyn Executor,
    ...
    {
        return Err(Error::ConflictError("销售启动收据与冻结运行事实不一致".to_string()));
    }
...
pub(super) fn build_sales_order_start_input(input: SalesOrderStartInput<'_>) -> Result<StartExecutionInput> {
    ...
    if graph.definition.definition_version != binding.approval_definition_version {
        return Err(Error::ConflictError("销售单绑定定义版本与已加载定义不一致".to_string()));
    }
...
fn start_bindings_from_graph(
    ...
    if bindings.is_empty() {
        return Err(Error::ConflictError("审批定义没有节点，无法启动销售单审批".to_string()));
    }
    Ok(bindings)
}
...
pub(super) struct SalesOrderStartPersistInput {
    ...
    /// 已锁定的工作副本。
    pub working_copy: erp_sales::entity::sales_order::SalesOrderWorkingCopy,
    /// 冻结提交头。
    pub submission: erp_sales::entity::sales_order::SalesOrderSubmission,
    /// 冻结提交行。
    pub submission_lines: Vec<erp_sales::entity::sales_order::SalesOrderSubmissionLine>,
    /// 工作流动作。
...
/// 会在同一事务新建并直接锁定副本。
pub(super) use erp_sales::service::sales_order::lifecycle::SalesOrderWorkingCopyPersistPlan;
...
pub(super) async fn persist_sales_order_start(
    ...
    let submission_view = erp_sales::service::sales_order::mapper::submission_view(
        submission.clone(),
...
pub(super) async fn persist_runtime_writes(
    ...
    let SalesOrderRuntimeWriteInput { document_type, snapshot_payload, owner_role, organization_id, now } =
        input;
    let first = writes
        .created_executions
        .first()
    ...
    persist_open_tasks(db, writes, document_type, owner_role, organization_id, now, executor).await
}
