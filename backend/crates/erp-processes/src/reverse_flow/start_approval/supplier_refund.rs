...
/// # 错误
/// 幂等键非法或仓储失败时返回错误。
pub async fn load_supplier_refund_start_receipt(
    db: &Database,
    subject: &SubjectRef,
    subject_version: u32,
    idempotency_key: &str,
...
/// # 关键业务约束
/// 审批人取自已发布节点，不接受客户端选择。
pub struct SupplierRefundStartInput<'a> {
    /// 绑定定义图。
...
pub fn build_supplier_refund_start_input(input: SupplierRefundStartInput<'_>) -> Result<StartExecutionInput> {
    ...
    if graph.definition.definition_version != binding.approval_definition_version {
        return Err(Error::ConflictError("供应商退款单绑定定义版本与已加载定义不一致".to_string()));
    }
...
fn supplier_refund_start_bindings(
    ...
    if bindings.is_empty() {
        return Err(Error::ConflictError("审批定义没有节点，无法启动供应商退款审批".to_string()));
    }
    Ok(bindings)
}
...
pub struct SupplierRefundStartPersistInput {
