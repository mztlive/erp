...
pub use erp_returns::service::approval::{
    cancel_payment_reversal_to_draft, ensure_payment_reversal_final_approve_posting,
};
...
pub fn payment_reversal_start_command(
    ...
    PaymentReversalStartCommand {
        subject_kind: process_kind_of(DocumentType::PaymentReversal).as_str().to_string(),
...
pub fn execute_payment_reversal_domain_action(
    ...
    match action {
        ApprovalDomainAction::PaymentReversalPost => {
            ensure_payment_reversal_final_approve_posting(reversal).map_err(Error::from)
        },
        ApprovalDomainAction::PaymentReversalCancelApproval => {
            cancel_payment_reversal_to_draft(reversal).map_err(Error::from)
        },
        other => Err(Error::ValidationError(format!("动作 {} 不属于付款冲正单", other.as_str()))),
    }
}
...
pub fn payment_reversal_responsible_org_id(organization_id: &str) -> Result<String> {
    if organization_id.trim().is_empty() {
        return Err(Error::ValidationError("付款冲正单缺少往来主体，无法冻结责任组织".to_string()));
    }
...
mod payment_reversal_tests {
    ...
    use erp_returns::entity::returns::PaymentReversalData;
    ...
    fn adapter_declares_all_required_fields() {
        ...
        assert_eq!(
            payment_reversal_subject_ref("prr-1").expect("主体引用必须可构造").subject_kind(),
            "payment_reversal"
        );
        ...
        assert_eq!(adapter.subject_snapshot_builder, "build_payment_reversal_snapshot");
        assert_eq!(adapter.on_approval_start, ApprovalDomainAction::PaymentReversalSubmit);
        assert_eq!(adapter.on_final_approve, ApprovalDomainAction::PaymentReversalPost);
        assert_eq!(adapter.cancel_action, ApprovalDomainAction::PaymentReversalCancelApproval);
    ...
    fn snapshot_freezes_supplier_and_amount() {
        ...
        assert!(matches!(payload.counterparty, Some(ApprovalSubjectCounterparty::Supplier { .. })));
        assert!(
