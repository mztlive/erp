...
mod wire {
    ...
    fn four_funds_documents_keep_amount_time_option_and_bson_roundtrip() {
        ...
        .unwrap();
        assert_eq!(
            roundtrip(&customer).get_str("original_receipt_id").unwrap(),
            "cr-1"
        );
        assert_eq!(
            roundtrip(&supplier).get_str("original_payment_id").unwrap(),
            "sp-1"
        );
        assert_eq!(
            roundtrip(&receipt)
                .get_str("original_customer_receipt_id")
                .unwrap(),
            "cr-1"
        );
        assert_eq!(
            roundtrip(&payment)
                .get_str("original_supplier_payment_id")
                .unwrap(),
            "sp-1"
        );
    }
    ...
    fn status_codes<T: Serialize + DeserializeOwned + PartialEq + Debug>(values: [T; 4]) {
        for (value, expected) in values
            .into_iter()
            .zip(["draft", "IN_APPROVAL", "posted", "reversed"])
        {
