...
//!
//! 集中根资料创建时的 Supplier、能力、资质与评级稳定构造；
//! 主体实体由组合层按 [`SupplierPartySeed`] 构造，本模块只保留主体 ID 与快照字段。
...
///
/// # 约束
/// 一次性返回全部待写供应商域实体；主体实体由组合层按 [`SupplierPartySeed`] 构造。
/// 任一构造失败时不返回部分计划，调用方不得落库任何子集。
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SupplierCreationPlan {
    /// 组合层构造主体所需的快照。
    pub party_seed: SupplierPartySeed,
    /// 新供应商角色。
...
mod tests {
    ...
    fn creation_plan_builds_first_revisions_and_links() {
        ...
        assert_eq!(plan.party_seed.party_id.to_string(), "party-1");
