...
use super::supplier_capability::{
    CapabilityStatus, FULFILLMENT_NOTE_MAX_LEN, OWNER_USER_ID_MAX_LEN, SERVICE_REGION_MAX_LEN,
};
...
impl SupplierCapabilityRevision {
    ...
    ///
    /// # 参数
    /// * `id` - 实体主键（`erp_core::ids::SupplierCapabilityRevisionId`）
    /// * `data` - 创建数据
    ///
    /// # 返回
    /// 返回新建的修订实体。
