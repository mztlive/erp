...
pub trait WorkItemRepositoryFulfillmentExt {
    /// 查询同一责任键下全部开放采购履约任务。
    ///
    /// # 参数
    ...
    async fn list_open_fulfillment_by_responsibility_key(
        &self,
    ...
    /// # 错误
    /// MongoDB 查询或反序列化失败时返回错误。
    async fn list_open_procurement_owned_by(
        &self,
        owner_user_id: &str,
    ...
    /// # 错误
    /// MongoDB 查询或反序列化失败时返回错误。
    async fn list_procurement_by_sales_order_newest_first(
        &self,
        sales_order_id: &str,
        executor: &mut dyn Executor,
    ) -> Result<Vec<WorkItem>>;
    ...
    async fn list_open_procurement_by_responsibility(
        &self,
...
impl WorkItemRepositoryFulfillmentExt for Repository<'_, WorkItem> {
    async fn list_open_fulfillment_by_responsibility_key(
        &self,
        responsibility_key: &str,
        executor: &mut dyn Executor,
