...
fn map_one(event: &BpmEvent) -> Option<NotificationIntent> {
    ...
    let (event_kind, dedup_key) = match event.kind {
        BpmEventKind::InstanceStarted => {
            (ApprovalNotificationEventKind::Started, format!("started:{instance_id}"))
        },
        BpmEventKind::NodeEntered => {
            (ApprovalNotificationEventKind::Entered, format!("entered:{}", execution_id?))
        },
        BpmEventKind::NodeApproved => {
            (ApprovalNotificationEventKind::NodeApproved, format!("approved:{}", execution_id?))
        },
        BpmEventKind::NodeRejected => {
            (ApprovalNotificationEventKind::NodeRejected, format!("rejected:{}", execution_id?))
        },
        BpmEventKind::InstanceBlocked => {
            (ApprovalNotificationEventKind::Blocked, format!("blocked:{}", execution_id?))
        },
        BpmEventKind::AssigneeRecovered => {
            (ApprovalNotificationEventKind::Resumed, format!("resumed:{}", execution_id?))
        },
        BpmEventKind::InstanceCancelled if event.blocker_code.is_some() => {
            (ApprovalNotificationEventKind::BlockedCancelled, format!("blocked_cancelled:{instance_id}"))
        },
        BpmEventKind::InstanceCancelled => {
            (ApprovalNotificationEventKind::Cancelled, format!("cancelled:{instance_id}:{}", event.round_no))
        },
        BpmEventKind::InstanceApproved => {
            (ApprovalNotificationEventKind::Completed, format!("completed:{instance_id}"))
        },
        BpmEventKind::RoundRestarted | BpmEventKind::ExecutionSuperseded => return None,
    };
    Some(NotificationIntent { dedup_key, event_kind })
}
...
mod tests {
    ...
    fn execution_outbox_dedup_keys_are_stable() {
        let started = BpmEvent::new(BpmEventKind::InstanceStarted, ApprovalProcessInstanceId::new("inst"), 1);
        let entered = BpmEvent::new(BpmEventKind::NodeEntered, ApprovalProcessInstanceId::new("inst"), 1)
            .with_execution(ApprovalNodeExecutionId::new("e1"));
        let resumed =
            BpmEvent::new(BpmEventKind::AssigneeRecovered, ApprovalProcessInstanceId::new("inst"), 1)
