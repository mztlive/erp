...
mod tests {
    ...
    fn file_asset_and_attachment_roundtrip_through_bson() {
        ...
        let roundtrip: FileAsset =
            mongodb::bson::deserialize_from_document(mongodb::bson::serialize_to_document(&asset).unwrap())
                .unwrap();
        assert_eq!(roundtrip, asset);

        let attachment = DocumentAttachment::new(
            DocumentAttachmentId::new("da-1"),
        ...
        .unwrap();
        let roundtrip: DocumentAttachment = mongodb::bson::deserialize_from_document(
            mongodb::bson::serialize_to_document(&attachment).unwrap(),
        )
        .unwrap();
