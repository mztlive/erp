...
function resetObjectTypes(includeCatalog) {
  return includeCatalog
    ? [...BUSINESS_OBJECT_TYPES, ...CATALOG_OBJECT_TYPES]
...
async function run() {
  ...
  for (const collectionName of PARTY_CHILD_COLLECTIONS) {
    line(
      `    ${collectionName}: ${await countByValues(collectionName, "party_id", deletablePartyIds)}`,
    );
  }
  ...
  if (!execute && !verifyOnly) {
    line();
    line(
      "预览完成：未执行任何写入。核对目标、集合摘要与范围后使用 --execute --confirm-db <db_name> --expect-summary <集合摘要>。",
    );
    return;
  }
  ...
  if (execute) {
    ...
    if (existing.has("work_items")) {
      const deleted = await targetDb
        .getCollection("work_items")
        .deleteMany(APPROVAL_WORK_ITEM_FILTER);
  ...
  if (execute) {
    ...
    deletedPartyMaps = await deleteByValues("external_identity_maps", "id", orphanedPartyMapIds);
    ...
    deletedMaps =
      deletedPartyMaps +
      (existing.has("external_identity_maps")
        ? (
...
run()
  ...
  .catch((error) => {
    ...
    if (code === "connection_failed") {
      ...
      console.error(
        "错误: MongoDB 重置失败（详细驱动错误已隐藏，避免泄露连接信息）。可在停写状态下幂等重跑。",
      );
    }
    quit(1);
  });
