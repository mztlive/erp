...
按 2026-09-07 用户指令，本轮必须先完成逻辑正确迁移并消除旧三层，停止编译指标测量。本节性能验收后置，不作为本轮阶段 17 的完成条件，也不得记录为已通过。

阶段 00 的固定测量脚本和原始记录必须保留；后续单独启动性能验收时，使用可核验的相同脚本、语义等价补丁和可比环境复测。
...
后续性能事项中，结构隔离通过但耗时指标未通过时不得声明性能达标；依据实测检查过程宏展开、泛型单态化、Build Script、Feature 传播和应用入口依赖。本轮逻辑迁移按第 18 节独立判定。
...
10. 已停止编译指标测量并记录性能验收后置；不得以此豁免第 3、4 项的实际依赖图检查，也不得声明性能阈值已通过。
