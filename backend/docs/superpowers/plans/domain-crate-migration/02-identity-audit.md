...
| 输入/输出提交 | 前序 `b399b662226064bd1fee36df026c461dd75066fd` / 实现提交 `543c4bea33ed667b2b99448933c94006b6de052b`（证据见 `.domain-migration-evidence/02/metadata.json`） |
...
| 证据 | 必填结果 | 初始状态 |
| --- | --- | --- |
| 输入基线 | 前序本地门禁通过 `b399b662226064bd1fee36df026c461dd75066fd`；分支 `chore/domain-crate-02-identity-audit`；source-map phase=02 行 46；owned types 7 | 已采集 `.domain-migration-evidence/02/input.json` |
| 文件与符号 | 相对输入 398 路径变化（83 add / 55 delete / 260 modify）；7 owned EntityRepository 迁入 erp-identity/erp-audit；历史 tests/ 字节不变 | 已采集 `.domain-migration-evidence/02/files.tsv` |
| 依赖 | 18 个成员；无 kind=test；erp-identity 与 erp-audit 双向均无直接依赖，也不回到旧三层 | 已采集 `.domain-migration-evidence/02/boundary.log` |
| 旧实现清零 | 旧 `services::iam/auth/audit` 生产路径删除；无 façade；历史 tests/ 档案未改 | 已采集：见 `boundary.log` / `files.tsv` |
| 测试 | `env -u ERP_TEST_MONGO_URI cargo test --workspace --lib --locked`；3264 passed / 0 failed / 71 ignored；exit 0 | 已执行 `.domain-migration-evidence/02/unit-tests.log` |
| 协议与数据 | 369 条管理路由；21 个 ErrorCode；368 条索引；权限生成物与阶段 01 哈希相等 | 已采集 `.domain-migration-evidence/02/contract-comparison.json` |
| 事务合同 | Executor/NoTransaction、snapshot+majority、run_audited 与 IdentityAuditPort 同一 Executor；真实数据库运行未验证 | 已采集 `.domain-migration-evidence/02/transaction-contract.json` |
| 公共门禁 | fmt/check/clippy/test/bpm/service/domain/permissions/git-diff-check 全部 exit 0 | 已执行 `.domain-migration-evidence/02/quality-gates.log` |
| 编译收益 | 适用场景原始样本、Fresh/Dirty、timings、中位数与改善率；不适用须写明 | 本阶段不适用；阈值在阶段 17 判定 |
| 阶段提交 | 实现提交 `543c4bea33ed667b2b99448933c94006b6de052b`；证据目录 `.domain-migration-evidence/02/`；禁止标记已验收 | 已写入 `.domain-migration-evidence/02/metadata.json` |
