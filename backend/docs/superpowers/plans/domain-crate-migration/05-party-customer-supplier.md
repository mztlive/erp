...
完成主体、客户、供应商纵向拆分，并交付客户日常修改的第一次编译收益复测。

## 3. 前置条件

- [阶段 04](04-support.md) 已验收，证据与提交完整。
- 先阅读 [README](README.md)、公共执行合同及本阶段完整内容。
- 对照 [source-map.tsv](source-map.tsv) 的 phase=05 行、[source-symbols.tsv](source-symbols.tsv) 和本节之后的符号拆分表核验当前输入；当前清单默认归属文件数为 92，其源文件哈希只是编制快照，不是迁移已完成证明。
...
三个独立领域，跨 Party/客户/供应商角色维护由组合层持有事务；客户中心只属于 read-models。

范围内文件由本阶段符号表、phase=05 的全量文件清单、必要调用方和已登记的注册文件构成。范围外包括新业务、数据回填、依赖版本升级、生产配置/凭据、发布推送，以及既有集成测试源码。

## 6. 源路径、符号、目标与调用方

| 当前源文件（相对 backend） | 主要符号/选择范围 | 目标文件 | 执行动作 |
| --- | --- | --- | --- |
| `entities/src/party/entity.rs` | `Party` | `crates/erp-party/src/entity/party/entity.rs` | 主体及地址/联系人/税务/账户/修订整体迁移 |
| `services/src/customer/profile.rs` | `CustomerProfileService` | `crates/erp-processes/src/customer_profile/mod.rs` | 跨 Party/Customer/审计的根用例迁出 |
| `services/src/customer/profile/create.rs` | `create` | `crates/erp-processes/src/customer_profile/create.rs` | 根事务编排；客户角色写入委托 erp-customer |
| `services/src/customer/mod.rs` | `CustomerService` | `crates/erp-customer/src/service/customer/mod.rs` | 只保留客户账户、分配、单域规则 |
| `services/src/supplier/profile/mod.rs` | `SupplierProfileService` | `crates/erp-processes/src/supplier_profile/mod.rs` | 主体、资质与附件组合 |
| `services/src/party/sensitive.rs` | `SensitiveDataCodec` | `crates/erp-party/src/service/party/sensitive.rs` | 能力由消费方 Port 使用，不把密钥/完整银行实体放公共层 |
| `database/src/repository/party/record.rs` | `Party` | `crates/erp-party/src/repository/party/record.rs` | 使用阶段 01 拥有仓储；当前文件为 record.rs |
| `database/src/repository/customer.rs` | `CustomerAccount` | `crates/erp-customer/src/repository/customer.rs` | 与 CustomerProfileCommand、CustomerAssignment 所属方法迁移 |
...
本阶段还必须迁移 repository-types.tsv 中 owner_phase=05 的全部拥有仓储类型：其 prepare_at 文件由阶段 01 创建，迁到 final_type_definition，并同步 impl_sources 中的所有专用方法。本阶段开始时这些准备文件属于前序输出，不因编制时尚不存在而遗漏。

本阶段必须更新的入口调用方：

- `apps/web-api/src/app_state.rs`。
- `apps/web-api/src/core/handler/customer/mod.rs`。
- `apps/web-api/src/core/handler/party/mod.rs`。
- `apps/web-api/src/core/handler/supplier/mod.rs`。
- `apps/web-api/src/core/routes/customer.rs`。
- `apps/web-api/src/core/routes/party.rs`。
- `apps/web-api/src/core/routes/supplier.rs`。
...
party/customer/supplier 三者互不依赖；processes 的 customer_profile/supplier_profile 组合它们；读模型可依赖各公开事实及尚未迁移的旧供应方。
...
```bash
cargo check -p erp-party -p erp-customer -p erp-supplier -p web-api -p cli --locked
```

## 10. 测试与质量门禁

- 客户：主体与角色原子维护、OWNER和分配、同请求重放、版本冲突、不存在/无权访问。
- 供应商：资质选择、角色复用、付款条件、敏感字段、附件确认失败的调用轨迹。
- 序列化：客户/供应商资料请求响应与持久化字段黄金结果不变。
- 编译：Customer 变更的 check/build 中位数、Dirty/Fresh 和反向依赖闭包均有原始证据。
...
erp-customer 引入 Party 实体/Repository；客户资料用例拆成多笔事务；供应商资质事实来源发生变化；使用目录改动数替代编译实测。
