...
当前用户要求与 `backend/AGENTS.md` 优先；设计契约采用 2026-09-07 修订口径；具体阶段的符号拆分表优先于全量文件清单的默认路径。差异必须写入阶段证据，不得靠猜测选择路径。

按 2026-09-07 用户指令，必须先完成逻辑正确迁移，消除旧 `services`、`entities`、`database` 的生产实现、注册和活动依赖。编译指标测量与性能阈值验收后置，不作为本轮完成条件；编译正确性、严格 lint、行为测试和依赖边界门禁仍必须通过。
...
必须同时核销路径/符号、记录契约比较、完成本轮质量门禁和阶段提交，再由阶段执行者记录“本地门禁通过”。连续阶段以前序完整代码及证据提交为输入；执行者不得自行登记人工“已验收”。性能后置状态必须单独记录，不得填为性能通过。
